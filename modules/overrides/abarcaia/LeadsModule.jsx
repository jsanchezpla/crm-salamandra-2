...
// Lee tanto claves en español (del form) como en inglés (seed/API interna)
function getExperiencia(cf) {
  return cf?.experiencia ?? cf?.experience ?? null;
}
function getZona(cf) {
  return cf?.zona ?? cf?.zone ?? null;
}
...
export default function AbarcaIALeadsModule() {
  ...
  const fetchLeads = useCallback(() => {
    ...
    fetch(`/api/leads?${params}`)
      ...
      .then((data) => {
        if (data.ok) {
          // Excluir referidos (tienen source = referido_abarcaia)
            ...
            <>
              ...
              <div className="hidden lg:block rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      {["Nombre", "Teléfono", "Email", "Experiencia", "Zona", "Estado", "Recibido"].map((h) => (
                        <th key={h} className="text-left py-3 px-4 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {leads.map((lead) => {
                      const exp = getExperiencia(lead.customFields);
                      const zone = getZona(lead.customFields);
                      ...
                      return (
                        ...
                        >
                          <td className="py-3 px-4 font-medium text-gray-900">{lead.name || lead.title || "—"}</td>
                          <td className="py-3 px-4">
                            <a href={`tel:${lead.phone}`} onClick={(e) => e.stopPropagation()} className="text-gray-500 hover:text-[var(--color-primary)] transition-colors">
                              {lead.phone || "—"}
                            </a>
                          </td>
                          ...
                          <td className="py-3 px-4">
                            {exp ? (
                              <span className="text-[11px] px-2 py-0.5 rounded-full font-medium bg-blue-50 text-blue-700">{exp}</span>
                            ) : (
                              <span className="text-gray-300">—</span>
                            )}
                          </td>
                          <td className="py-3 px-4 text-gray-500 text-sm">{zone || "—"}</td>
                          <td className="py-3 px-4">
                    ...
                    {leads.length === 0 && (
                      <tr>
                        <td colSpan={7} className="py-16 text-center text-gray-400 text-sm">
                          {search || activeStage !== "all" ? "Sin resultados para ese filtro" : "Todavía no hay leads."}
                        </td>
                      </tr>
                    )}
                  </tbody>
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="min-w-0 mr-3">
                          <div className="font-semibold text-gray-900 truncate">{lead.name || lead.title || "—"}</div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {exp && <span className="text-[11px] px-2 py-0.5 rounded-full font-medium bg-blue-50 text-blue-700">{exp}</span>}
                        {zone && <span className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 font-medium">{zone}</span>}
                      </div>
                      <div className="flex items-center justify-between mt-3">
                        <span className="text-xs text-gray-400">{formatDate(lead.createdAt)}</span>
                        {lead.phone && (
                          <a href={`tel:${lead.phone}`} onClick={(e) => e.stopPropagation()} className="text-xs text-[var(--color-primary)] font-medium">
                            {lead.phone}
                          </a>
                        )}
                      </div>
...
function LeadDetailPanel({ lead, open, saving, onClose, onStageChange, onNotesChange }) {
  ...
  const exp = getExperiencia(lead.customFields);
  const zone = getZona(lead.customFields);
  const fechaEnvio = lead.customFields?.fecha_envio;
    ...
    >
      <div className="px-5 pt-5 pb-4 border-b border-gray-100 flex items-start justify-between gap-4 border-t-[3px] border-t-[var(--color-primary)]">
        <div className="min-w-0">
          <h2 className="text-gray-900 font-semibold text-base truncate">{lead.name || lead.title || "Sin nombre"}</h2>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
        ...
        {/* Perfil */}
        <div>
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-3">Perfil</p>
          <div className="space-y-2.5">
            ...
            {fechaEnvio && (
              <div className="flex items-start gap-3">
                <span className="text-gray-400 w-24 shrink-0 text-xs mt-0.5">Envío form</span>
                <span className="text-gray-600 text-xs">{new Date(fechaEnvio).toLocaleString("es-ES")}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          />
          {notesDirty && (
            <button onClick={saveNotes} className="mt-2 text-xs text-[var(--color-primary)] hover:opacity-80 transition-opacity font-medium">
              Guardar notas
            </button>
          )}
        </div>
...
function InfoRow({ label, value, href }) {
  return (
    <div className="flex items-center gap-3">
      ...
      {value ? (
        <a href={href} className="text-gray-700 text-xs hover:text-[var(--color-primary)] transition-colors">{value}</a>
      ) : (
