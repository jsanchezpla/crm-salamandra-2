...
const CSV_HEADER_MAP = {
  nombre: "name", name: "name", candidato: "name",
  ...
  estado: "stage", stage: "stage", fase: "stage",
...
const STAGE_MAP = {
  nuevo: "new", new: "new", "nuevo lead": "new",
  contactado: "contacted", contacted: "contacted",
  "en seguimiento": "qualified", seguimiento: "qualified", qualified: "qualified",
  convertido: "won", won: "won",
  descartado: "lost", lost: "lost",
};
...
function ImportModal({ onClose, onImported }) {
  ...
  const PREVIEW_COLS = ["name", "email", "phone", "empresa", "experience", "zone", "stage"];
  const PREVIEW_LABELS = { name: "Nombre", email: "Email", phone: "Teléfono", empresa: "Empresa", experience: "Experiencia", zone: "Zona", stage: "Estado" };
    ...
    >
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-5 pt-5 pb-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-gray-900 font-semibold text-base">Importar leads desde CSV</h2>
            <p className="text-gray-400 text-xs mt-0.5">
              Cabeceras: nombre, email, teléfono, empresa, experiencia, zona, estado, notas
            </p>
          </div>
          ...
          {result ? (
            <div className="py-6 text-center">
              {result.error ? (
                ...
                <>
                  ...
                  {result.skipped > 0 && <p className="text-gray-400 text-sm mt-1">{result.skipped} filas omitidas (sin datos)</p>}
                  {result.errors?.length > 0 && (
                    <p className="text-red-500 text-sm mt-1">
                      {result.errors.length} errores en filas: {result.errors.map((e) => e.row).join(", ")}
                    </p>
                  )}
                </>
              )}
            ...
            <>
              ...
              <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-4">
                {[
                  { key: "file", label: "Subir archivo", icon: "M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" },
                  { key: "paste", label: "Pegar texto", icon: "M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z" },
                ].map(({ key, label, icon }) => (
                  <button
                    key={key}
                    onClick={() => { setTab(key); setParsed(null); setCsvText(""); }}
              ...
              {tab === "file" && !parsed && (
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer ${
                    dragOver ? "border-[var(--color-primary)] bg-green-50" : "border-gray-200 hover:border-gray-300 bg-gray-50"
                  }`}
                ...
                >
                  ...
                  <p className="text-gray-500 text-sm mb-1">Arrastra tu archivo CSV o haz clic para seleccionar</p>
                  <p className="text-gray-300 text-xs mt-3">Separadores aceptados: coma (,) o punto y coma (;) · UTF-8</p>
                  <input ref={fileInputRef} type="file" accept=".csv,text/csv" onChange={(e) => handleFileRead(e.target.files[0])} className="hidden" />
                </div>
              )}
              ...
              {tab === "paste" && !parsed && (
                <div>
                  <textarea
                    ...
                    placeholder={`Pega aquí el CSV. Ejemplo:\n\nnombre,email,telefono,empresa,experiencia,zona\nJuan García,juan@qec.com,600123456,Quality Energy Consulting,experienced,Madrid\nMaría López,maria@qec.com,611987654,Made of Energy,other_sector,Barcelona`}
                    rows={10}
                  ...
                  {parsed.error ? (
                    ...
                    <>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => { setParsed(null); setCsvText(""); }} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">
                          Limpiar
                        </button>
                      </div>
                          ...
                          <div className="overflow-x-auto max-h-48">
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                  {previewCols.map((c) => (
                                    <th key={c} className="text-left py-2 px-3 text-[10px] font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">
                                      {PREVIEW_LABELS[c] ?? c}
                                    </th>
                                  ))}
                                </tr>
                              ...
                              <tbody>
                                {parsed.rows.slice(0, 5).map((row, i) => (
                                  <tr key={i} className="border-b border-gray-100">
                                    {previewCols.map((c) => (
                                      <td key={c} className="py-2 px-3 text-gray-600 max-w-[140px] truncate">
        ...
        {!result && (
          <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
            <button onClick={handleClose} className="text-sm text-gray-500 hover:text-gray-700 transition-colors font-medium">
              Cancelar
            </button>
            <button
              onClick={handleImport}
              disabled={!parsed || !!parsed.error || !parsed.rows?.length || importing}
            ...
            >
              {importing ? (
                ...
                <>Importar {parsed?.rows?.length ? `${parsed.rows.length} leads` : ""}</>
              )}
            </button>
          </div>
        )}
        ...
        <div className="pt-2 border-t border-gray-100">
          {confirmDelete ? (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3">
              <p className="text-red-700 text-xs font-medium mb-3">¿Eliminar este lead? Esta acción no se puede deshacer.</p>
