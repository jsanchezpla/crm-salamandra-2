...
| `productos.md` | `productos`, `productos_avanzado`, `orders` | `auditorias.md` | `auditorias` |
...
- **Personalización por módulo** (`master.tenant_modules`), en orden de coste
  ...
  `featureFlags` (interruptores; hoy el código lee SEIS: `training.formacionAbierta`,
  `nutricion.autoAsignarEnAlta`, `citas.autoConfirmPublicBookings`,
  `citas.sinHorarioPropio`, `pacientes.altaPorPaciente` y
  `clients.correoSoloOficina` —cada uno con su lector en `lib/` y su script
  para encenderlo—),
...
| `pacientes` / `clinica` | Fichas de paciente / sesiones (audio→Whisper→Claude), informes, coordinaciones (`/clinica/coordinaciones`), estadísticas del centro (`/clinica/estadisticas`, solo dirección) y Diagnósticos (`/clinica/diagnosticos`, 12/09/2026: el expediente de un producto de horas —simple 10 h, completo 20 h— con la barra contada desde las citas y un bono SIN tope al seguir; `docs/decisions/2026-09-12-el-expediente-de-diagnostico.md`) | El cliente es la familia que paga; el paciente, el hijo. Alta con pacientes en la misma transacción. |
...
| — | **Correo** (`/correo`): un mensaje a muchos, eligiendo remitente | Sin `moduleKey`: se ve con `clients` **o** `outreach` — y desde el 10/09/2026 un centro puede dejarlo en manos de OFICINA (dirección o quien lleve `billing`) con el interruptor `clients.correoSoloOficina`, que leen igual el menú, la pantalla y los siete endpoints (`lib/correo/quienEscribe.js`, `scripts/correo-solo-oficina.js`; puesto en `aumenta`, que le abrió Clientes a sus terapeutas). Manda UNO POR DESTINATARIO (nadie ve la lista de los demás). Remitentes en `settings.integrations.remitentes` (`lib/email/remitentes.js`), con caída al `resendFromEmail` de siempre. El dry-run se cuenta como «simulado», nunca como enviado. Desde el 26/08/2026 habla el idioma del centro (`vocabulario.js`, nada de «Contratantes» en una clínica), con `pacientes` lista tutores+pacientes y filtra por profesional/terapia, y tiene listas guardadas, plantillas ilimitadas, adjuntos (imagen/PDF) y pies de firma por persona (tablas `correo_*`, `lib/correo/`). |
| `support` | Helpdesk hacia SUS clientes: nº correlativo, SLA, portal público | Correo ENTRANTE aún sin dar de alta en Resend. |
| `mailing` | Email marketing (06/09/2026): campañas y newsletters a quien ya es cliente y ha marcado la casilla «novedades» (`lib/clients/comunicaciones.js`) + correos sueltos con consentimiento; editor por bloques, segmentos, envío por lotes reanudable, bajas de un clic, supresión, cuota y métricas de clic. Sprint 2 (misma tarde): redacción con IA (rellena bloques, nunca HTML, prohibido inventar), secuencias por eventos (alta, cumpleaños, sin cita; una campaña automática por periodo, no barren el histórico), pestaña «Mailing» en la ficha, A/B de asunto y ritmo por hora | **Sale por Amazon SES (BYOK, una cuenta por cliente), NUNCA por Resend**: el marketing no comparte cuenta con los recordatorios (`docs/decisions/2026-09-06-mailing-por-ses-y-no-por-resend.md`). Sin SDK de AWS (`lib/mailing/sigv4.js`). No requiere `clients` (sin él, solo correos sueltos). Temporizador `crm-mailing.timer` cada minuto. Pantalla `/mailing`. |
| `auditorias` | Auditoría MENSUAL de desempeño por profesional (09/09/2026, AV-0100 de Aumenta): cuatro áreas de criterios apto/no apto/no aplica con observaciones, cierre con fortalezas, acción acordada y plazo, histórico por persona y lo que quedó pendiente del mes anterior delante | Requiere `team`, **no** `team_avanzado`: lo compra cualquier centro con varias personas y quien dirija. No se solapa con Desempeño/Productividad, que son NÚMEROS automáticos; esto es a mano y cualitativo. **Un «no apto» NO decide el resultado** y no hay puntuaciones (se pidió así). Material laboral: dirección escribe todas, cada persona ve las suyas ya CERRADAS. Reglas en `lib/team/auditoriaDesempeno.js` con su prueba. |
...
- **Auditoría** (`lib/utils/auditoria.js` o el helper del módulo): SIEMPRE lo
  ...
  y **una excepción puntual**: `scripts/limpiar-auditoria-citas.js` (Jorge,
  14/09/2026) reescribió en sitio las ~1.679 filas de citas anteriores al
  13/09 que llevaban nombre, contacto, notas y token de cancelar —mismo
  resumen que escribe hoy la app, sin borrar filas—; no abre la puerta a
  retocar otras.
