...
export function validarAdjuntos(brutos) {
  ...
  for (const bruto of brutos) {
    ...
    // Claves EN EL FORMATO DEL SDK de Resend (`resend@4.8.0`: `contentType`,
    // no `content_type`): el SDK traduce él mismo a la API y DESCARTA en
    // silencio las claves que no reconoce. La primera versión de este fichero
    // usaba los nombres de la API cruda y la imagen de la firma llegó como
    // adjunto suelto con el `cid:` roto (probado el 26/08/2026 contra el
    // buzón real de la agencia).
    adjuntos.push({ filename: nombre, content: base64, contentType: tipo });
  }
...
export function componerContenido({ cuerpo, firma }) {
  ...
  // `inlineContentId` es el nombre que entiende el SDK de Resend (4.8.0): es
  // lo que marca el adjunto como INCRUSTADO y casa con el `cid:firma` del
  // HTML. Con cualquier otro nombre el SDK lo descarta sin avisar y la imagen
  // llega como adjunto suelto con un hueco roto en el correo.
  const adjuntosFirma = firma.imagen
    ? [
        {
          filename: firma.imagen.nombre || "firma.png",
          content: firma.imagen.base64,
          contentType: firma.imagen.tipo,
          inlineContentId: "firma",
        },
      ]
    : [];
