...
import { conPaciente } from "../clinica/pacienteEnAvisos.js";
  ...
  try {
    ...
    const reportItems = overdueRows
      ...
      .map((j) => {
        ...
        return { entityId: j.id, title: "Informe vencido", body: `${name} · vencía el ${fmtDate(isoDate(j.dueDate))}` };
      });
    ...
    if (Incidencia) {
      // `soloPendientes` (04/09/2026): dar una por vista la calla; una
      // actualización le borra el visto y la campana vuelve a sonar.
      const deMio = await whereIncidenciasDe(models, teamMemberId, { soloPendientes: true });
      const inc = await Incidencia.findAll({
        where: { ...deMio, status: "pending" },
        // El paciente delante del título (07/09/2026, AV-0052 de Aumenta): a
        // quien recibe la incidencia le llega por aquí antes que por la lista,
        // y sin el nombre del niño «Incidencia asignada · Faltas» no dice nada.
        include: Patient
          ? [{ model: Patient, as: "patient", attributes: ["firstName", "lastName"], required: false }]
          : [],
        order: [["createdAt", "ASC"]],
        limit: 100,
      });
      const incItems = inc.map((r) => ({
        entityId: r.id,
        title: "Incidencia asignada",
        body: conPaciente(r.title, r.patient),
      }));
...
export function notificationLink(entityType, entityId = null, type = null) {
  // Los avisos de la cuenta de IA no llevan entidad: se reconocen por el tipo.
  if (!entityType && type === "ai_cuenta") return "/configuracion?zona=conexiones";
  switch (entityType) {
    case "ClinicalReport":
    ...
    case "Incidencia":
      ...
      // notificaciones ya guardadas también apuntan al sitio nuevo. Con id
      // (02/09/2026) abre ESA ficha, fresca del servidor: el aviso de un
      // comentario tiene que llevar al hilo, no a la lista.
      return entityId ? `/equipo/incidencias?incidencia=${entityId}` : "/equipo/incidencias";
    case "BookingChangeRequest":
      return "/citas";
    // Los que antes solo se marcaban como leídos (15/09/2026, Rodrigo: «si
    // pincho en una notificación, que me lleve a su contenido»).
    case "Booking":
      // Cita nueva, cancelada, falta, retención caducada, cobro sin cita:
      // `/citas?cita=` abre su ficha y salta a su día (CitasModule).
      return entityId ? `/citas?cita=${entityId}` : "/citas";
    case "Client":
      return entityId ? `/clientes/${entityId}` : "/clientes";
    case "Lead":
      return "/leads";
    case "FormSubmission":
      // Formularios recibidos y admisiones: la pantalla «Leads Comerciales».
      return "/formularios";
    case "AiPermission":
      return "/configuracion?zona=modulos";
    case "User":
      // «Ha olvidado su contraseña»: se restablece desde la ficha del miembro.
      return "/equipo";
    case "Document":
    ...
    case "BuzonAviso":
      /*
       * Lo que NOSOTROS le hemos contestado desde el panel, y lleva el id
       * (18/09/2026, AV-0212 de Aumenta). Hasta hoy iba a `/ayuda` a secas,
       * con el argumento de que allí solo están los suyos y no hay nada que
       * buscar. Con dos avisos es verdad; Olga lleva más de cuarenta, y lo
       * escribió tal cual: «al pinchar sobre ese resumen, no puedo acceder al
       * mensaje original completo». Con `?aviso=` la pantalla abre ese hilo
       * directamente, que es lo que Rodrigo pidió el 15/09 para todas las
       * demás: «si pincho en una notificación, que me lleve a su contenido».
       */
      return entityId ? `/ayuda?aviso=${entityId}` : "/ayuda";
    case "IaTope":
      // Tope mensual de gasto de IA (14/09/2026, `lib/ai/frenoDeGasto.js`): la
      // campana del 80 % y del 100 % lleva a donde se sube el tope.
      return "/configuracion?zona=conexiones";
    default:
      return null;
  }
}
...
export function serializeNotification(n) {
  ...
  return {
    ...
    link: notificationLink(j.entityType, j.entityId, j.type),
