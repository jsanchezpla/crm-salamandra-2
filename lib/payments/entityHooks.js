...
import { enlaceCancelacion } from "../citas/cancelacion.js";
...
export async function onEntityPaid(ctx, paymentSession, t) {
  switch (paymentSession.entityType) {
    ...
    // La tienda (25/08/2026). El `switch` ya estaba preparado para esto: el
    // checkout nació genérico («entityType: "booking" | "order" | …») y lo
    // único que faltaba era el caso.
    case "order":
      return await pedidoPagado(ctx, paymentSession, t);
    default:
...
/**
 * Un pedido de la tienda que YA está cobrado.
 *
 * Hace las dos cosas que definen la venta y ninguna antes de tiempo:
 *   · pasa el pedido de `draft` a `confirmed`;
 *   · descuenta el almacén con un movimiento de tipo `pedido` por línea.
 *
 * El stock se mueve AQUÍ y no al crear el pedido a propósito: un carrito
 * abandonado no puede apartar mercancía. Ver `lib/tienda/pedidoDesdeTienda.js`.
 *
 * Idempotente por partida doble — el webhook de Stripe reintenta si la primera
 * respuesta tarda: si el pedido ya no está en `draft` no se vuelve a tocar, y
 * `descontarStock` comprueba además si ya hay movimientos suyos. Cobrar una vez
 * y descontar dos dejaría el stock en negativo sin que nadie entienda por qué.
 */
async function pedidoPagado(ctx, paymentSession, t) {
  const { Order, OrderLine } = ctx.tenantModels;

  const pedido = await Order.findByPk(paymentSession.entityId, { transaction: t });
  if (!pedido) return { outcome: "pedido no encontrado" };
  if (pedido.status !== "draft") return { outcome: `el pedido ya estaba en ${pedido.status}` };

  const lineas = await OrderLine.findAll({ where: { orderId: pedido.id }, transaction: t });
  if (!lineas.length) return { outcome: "pedido sin líneas: no se confirma" };

  await pedido.update({ status: "confirmed" }, { transaction: t });

  // `import()` aquí dentro y no arriba: `lib/tienda/` solo existe para quien
  // tiene el módulo, y cargarlo en cada webhook de citas sería trabajo de más.
  const { descontarStock } = await import("../tienda/pedidoDesdeTienda.js");
  const { movimientos, repetido } = await descontarStock(ctx.tenantModels, pedido, lineas);

  return {
    outcome: repetido
      ? `pedido ${pedido.id} confirmado; el stock ya estaba descontado`
      : `pedido ${pedido.id} confirmado y ${movimientos} línea(s) descontadas del almacén`,
  };
}
...
async function citaConTarjetaRetenida(ctx, paymentSession, t, caducaEn) {
  ...
  const postCommit = async () => {
    ...
    if (!esCorreoTransaccional("bookingReceived")) return;
...
async function citaPagada(ctx, paymentSession, t) {
  ...
  const postCommit = async () => {
    ...
    const tpl = bookingConfirmedTemplate({
      ...
      // null si el centro no deja anular a la familia (08/08/2026). Este es el
      // quinto sitio que reparte el enlace y el más fácil de olvidar: sale del
      // webhook de Stripe, no de una pantalla.
      cancelUrl: enlaceCancelacion(ctx.tenant, { slug: ctx.slug, token: cita.cancellationToken }),
