...
/**
 * Pregunta a Stripe en qué estado está una retención, SIN tocarla.
 *
 * ── POR QUÉ HACE FALTA, HABIENDO YA `capturarPago` (regla #2) ────────────────
 * Hasta hoy el único sitio que leía el PaymentIntent era `capturarPago`, y para
 * enterarse de algo había que llamarlo — es decir, cobrar. Eso sirve cuando lo
 * que quieres es cobrar, no cuando lo que necesitas es DECIDIR. El caso es
 * «pedirle otra tarjeta al paciente»: antes de crear una retención nueva hay que
 * saber si la vieja sigue viva, porque si lo está el paciente acabaría con DOS a
 * la vez sobre la misma cita —el doble del importe bloqueado en su tarjeta hasta
 * que la primera caduque sola— y el CRM perdería el rastro de la primera, porque
 * `paymentSessionId` se pisa con la nueva.
 *
 * No mueve dinero, así que no lleva `assertNotDemoPaidCall`: es una lectura.
 *
 * ⚠️ `sePudoPreguntar: false` NO significa «no hay retención»: significa que no
 * lo sabemos. Quien llame tiene que tratarlo como el caso PELIGROSO y nunca como
 * vía libre. Es el mismo criterio que hace ancha la lista de `PUEDE_HABER_DINERO`:
 * preguntar de más es barato; darlo por perdido, no.
 *
 * @returns {Promise<{viva: boolean, estado: string|null, sePudoPreguntar: boolean}>}
 */
export async function leerEstadoAutorizacion(ctx, paymentSession) {
  const pi = paymentSession?.stripePaymentIntentId;
  // Sin PaymentIntent no hay nada que pueda quedarse vivo, y eso sí lo sabemos.
  if (!pi) return { viva: false, estado: null, sePudoPreguntar: true };

  try {
    // `getStripe` va DENTRO del try. Monta el SDK con la clave del tenant y
    // puede lanzar si está mal formada; fuera, esa excepción se escapaba de una
    // función que promete no lanzar nunca y acababa saliendo como un 500 en la
    // cara de la profesional, en vez del aviso que hay más abajo.
    const stripe = await getStripe(ctx);
    if (!stripe) return { viva: false, estado: null, sePudoPreguntar: false };

    const actual = await stripe.paymentIntents.retrieve(pi);
    // `requires_capture` es el ÚNICO estado con dinero apartado esperando. Un
    // `canceled` está muerto y un `succeeded` ya se cobró: ni uno ni otro se
    // pueden duplicar, así que ninguno estorba para pedir otra tarjeta.
    return {
      viva: actual.status === "requires_capture",
      estado: actual.status,
      sePudoPreguntar: true,
    };
  } catch (err) {
    // Que Stripe NO LO ENCUENTRE no es «no lo sé»: es que ahí no hay ninguna
    // retención que pueda duplicarse, y por tanto nada que estorbe.
    //
    // Pasa de verdad al rotar las claves de un tenant o al pasar de modo prueba
    // a producción: el PaymentIntent viejo era de otra cuenta y deja de
    // resolver. Metiéndolo en el saco de «no se pudo preguntar», el botón se
    // quedaba bloqueado PARA SIEMPRE en esa cita — y encima diciendo «inténtalo
    // dentro de un momento», que es mentira: reintentar no lo arregla nunca.
    if (err?.code === "resource_missing" || err?.statusCode === 404) {
      return { viva: false, estado: "inexistente", sePudoPreguntar: true };
    }
    process.stderr.write(`[pagos] no se pudo leer la retención ${pi}: ${err.message}\n`);
    return { viva: false, estado: null, sePudoPreguntar: false };
  }
}
