...
export async function createCheckoutSession(ctx, opts) {
  const {
    ...
    /**
     * Métodos de pago a los que se ACOTA esta compra. `null` (lo normal) deja
     * los que el centro tenga activados en su panel de Stripe.
     *
     * Se acota en un solo caso: el pago fraccionado, que va a TARJETA. Es lo
     * único domiciliable — ni Bizum ni las transferencias admiten cargos
     * recurrentes—, así que ofrecer otra cosa sería prometer una domiciliación
     * que después no se puede hacer.
     */
    paymentMethodTypes = null,
    /**
     * Cobro RECURRENTE: `{ intervalo: 'month', iterations: N }` (05/08/2026).
     *
     * Convierte el checkout en `mode: 'subscription'`: se cobra `amount` ahora
     * —la primera cuota— y Stripe repite el cargo cada intervalo. El TOPE de N
     * cuotas no se pone aquí: Checkout no sabe de topes. Lo pone el webhook al
     * confirmarse el primer pago, colgando un subscription schedule con
     * `end_behavior: 'cancel'`.
     *
     * ⚠️ Por qué no se crea el calendario directamente, que sería más limpio:
     * la primera factura de un subscription schedule NO se finaliza al momento
     * —nace en borrador y Stripe la cierra ~1 h después (documentado)—, así que
     * la paciente terminaría la reserva sin que nadie sepa en una hora si ha
     * pagado, y el hueco se habría soltado. Con Checkout, el primer cobro y su
     * autenticación (SCA) pasan ahí mismo.
     */
    recurring = null,
  } = opts ?? {};
    ...
    session = await stripe.checkout.sessions.create(
      {
        ...
        // nuevo no requiere tocar este código. Solo se acota cuando quien llama
        // lo pide expresamente (ver `paymentMethodTypes`).
        ...
        line_items: [
          {
            ...
            price_data: {
              ...
              // En recurrente, el precio TIENE que declarar su intervalo o
              // Stripe rechaza la sesión.
              ...(recurring ? { recurring: { interval: recurring.intervalo || "month" } } : {}),
            },
          },
        ],
        // La suscripción lleva encima de qué bono es: el webhook de las cuotas
        // siguientes solo recibe la factura, y sin esto no sabría a quién
        // apuntarle el cobro ni cuántas cuotas van.
        ...(recurring
          ? {
              subscription_data: {
                metadata: {
                  ...
                  entityType,
                  entityId,
                  cuotas: String(recurring.iterations ?? ""),
                },
              },
            }
          : {}),
