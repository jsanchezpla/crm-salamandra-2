 ...
 *
 * Los errores salen de `errorTypes.js` y no de `errors.js` (mismo criterio que
 * `autorizacion.js` e `isDemo.js`): esto es librería de servidor, y desde el
 * 20/08/2026 lo importa `lib/citas/reembolsoCita.js`, que sí llega a scripts y
 * pruebas de línea de comandos. `errors.js` arrastra `next/server`, que Node no
 * sabe resolver fuera del empaquetador de Next.
 */
...
export async function refundPayment(ctx, paymentSession, opts = {}) {
  // La demo da sesión de ADMIN a visitantes anónimos. Era la única función que
  // movía dinero sin este candado: `autorizarPago`, `capturarPago` y
  // `liberarAutorizacion` sí lo tienen. Devolver dinero también es mover dinero.
  assertNotDemoPaidCall(ctx, "La devolución");
