 ...
 *
 * ── CUANDO ANTHROPIC RECHAZA LA LLAMADA (13/09/2026) ───────────────────────
 * Antes de relanzar el error se pasa por `lib/ai/trasFalloDeIa.js`, que avisa
 * a los admins del tenant si el fallo es de su CUENTA (sin saldo, clave,
 * permiso, límite). Hasta hoy ese aviso lo ponía cada ruta en su catch y solo
 * se acordaban 7 de 20. El error que sube es el mismo de siempre. La rama de
 * OpenAI no se envuelve aquí: avisa su propio cliente (`lib/ai/openai.js`).
 */
...
export async function completeConParada({
  ...
  let msg;
  try {
    msg = stream ? await client.messages.stream(params).finalMessage() : await client.messages.create(params);
  } catch (e) {
