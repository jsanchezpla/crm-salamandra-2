 ...
 *
 * Los que no están en el catálogo se añaden aquí a mano. Que `analytics` no
 * esté es un fallo conocido —no se puede vender desde el alta, está en el
 * backlog—; `provisioning` no está a propósito: es nuestro panel, no un módulo
 * que se venda.
 */
 ...
 *
 * Solo en los tipos donde la integración se queda sin la otra punta: una
 * conversión sin sitio a donde ir, un enlace sin la ficha que apuntar, una
 * cascada que no arrastra nada. Un `gating` o una `agregacion` sin destino no
 * es un problema — simplemente esa condición no aplica y esa cifra no sale.
 *
 * Se usa para avisar, nunca para dar error: un cliente puede querer leads sin
 * fichas de cliente y estar en su derecho.
 */
const SE_NOTA_SIN_DESTINO = new Set(["conversion", "enlace", "cascada"]);

export function necesitaDestino(tipo) {
  return SE_NOTA_SIN_DESTINO.has(tipo);
}
...
export const INTEGRACIONES = [
  ...
  },
  {
    ...
    donde: [
      ...
      "components/projects/EmployeeProjectsSection.jsx:22",
    ],
    automatico: false,
    nota:
  ...
  },
  {
    ...
    queHace:
      "El alta manual de una cita ofrece un buscador con las fichas marcadas como paciente de Nutrición o de Clínica, y al elegir una rellena sola el correo y el teléfono. Hay una opción para ver todas las fichas, y si el cliente no usa esas marcas se ofrecen todas antes que un desplegable vacío.",
    donde: [
      "app/api/citas/clientes/route.js:25",
      "app/api/citas/clientes/route.js:49",
      "app/api/citas/clientes/route.js:73",
  ...
  },
  {
    ...
    donde: [
      ...
      "app/api/team/[id]/billing-summary/route.js:15",
      "app/api/team/[id]/billing-summary/route.js:24",
      "components/billing/EmployeeBillingSection.jsx:36",
      "lib/billing/billingSummary.js:234",
    ],
    automatico: true,
    nota:
      "El sueldo y el coste previsto se borran de la respuesta si quien mira no es dirección, en los dos sitios. El resumen de la ficha se abre con tener Equipo o Facturación, no hacen falta los dos.",
  },
  {
    desde: "billing",
    hacia: "team",
  ...
  },
  {
    ...
    donde: [
      ...
      "modules/overrides/nutri-laura/ClientDetailModule.jsx:365",
      "modules/overrides/nutri-laura/ClientPlansPanel.jsx:62",
    ],
    automatico: true,
    nota:
  ...
  // ── referidos ─────────────────────────────────────────────────────────
  {
    ...
    tipo: "conversion",
    titulo:
      "El formulario público de referidos crea un lead en el embudo",
    queHace:
      "El formulario de recomendaciones no guarda en ninguna tabla propia: crea un lead normal en la primera etapa, con el nombre completo como título y, dentro, el origen «referido» y el código de quien recomienda.",
    donde: [
      "app/api/public/referidos/route.js:24",
      "app/api/public/referidos/route.js:60",
      "app/api/public/referidos/route.js:66",
    ],
    automatico: true,
    nota:
      "El endpoint solo exige el módulo Leads, nunca el de Referidos: cualquier cliente con Leads y el formulario pegado en su web puede recibirlos. Y la etiqueta de origen lleva escrito a fuego el nombre de un cliente concreto.",
  },
  {
    desde: "referidos",
    hacia: "leads",
    tipo: "compartido",
    titulo:
      "La pantalla de Referidos es el embudo de Leads filtrado por origen",
    queHace:
      "Referidos no tiene tabla propia: su pantalla lee y escribe leads filtrando por la etiqueta de origen, y solo deja cambiar la etapa y las notas, a dirección. El lead que se toca ahí es el mismo que se ve en Leads, y en el menú Referidos cuelga del grupo Leads.",
    donde: [
      "app/api/referidos/route.js:6",
      "app/api/referidos/route.js:17",
      "app/api/referidos/[id]/route.js:8",
      "app/api/referidos/[id]/route.js:16",
      "components/layout/Sidebar.jsx:87",
    ],
    automatico: true,
    nota:
      "Sus endpoints exigen el módulo Leads y nunca el de Referidos: un cliente con Referidos y sin Leads recibiría un «no disponible» en su propio módulo.",
  },
  ...
  // ── inventory ─────────────────────────────────────────────────────────
  {
    ...
    donde: [
      ...
      "app/api/proveedores/[id]/route.js:85",
    ...
    automatico: true,
    nota:
      "La mitad de Gastos está sin cablear: la pantalla de gastos no pide proveedor y el servidor ni lo acepta, así que el «total pagado» de la ficha del proveedor sale cero para todos. Como la baja del proveedor solo mira gastos, hoy cualquier proveedor se borra de verdad y sus entregas se quedan sin él.",
  },
  {
    desde: "inventory",
    hacia: "orders",
  ...
  // ── training ──────────────────────────────────────────────────────────
  {
    ...
    tipo: "compartido",
    titulo:
      "Formación y Cuestionarios comparten los intentos y la conexión con la web",
    queHace:
      "Los dos módulos viven sobre la misma tabla de intentos de test y sobre la misma entrada de datos desde la plataforma de cursos: un aviso automático cuando alguien termina un test y un botón de sincronizar para ponerse al día. Las pantallas de consulta se abren teniendo cualquiera de los dos módulos.",
    donde: [
      "app/api/cuestionarios/route.js:5",
      "app/api/cuestionarios/sync/route.js:44",
      "app/api/webhooks/tutorlms/quiz-attempt/route.js:21",
      "app/api/training/quiz-attempts/stats/route.js:28",
      "lib/provisioning/catalogo.js:68",
    ],
    automatico: true,
    nota:
      "En el alta de clientes, Cuestionarios arrastra Formación. El aviso automático de la plataforma sí exige Formación en solitario.",
  },
];
