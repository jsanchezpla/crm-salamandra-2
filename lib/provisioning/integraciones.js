...
export const INTEGRACIONES = [
  ...
  },
  {
    ...
    queHace:
      "Una tarea del calendario puede tener responsable, elegido de un desplegable con el equipo, y ese responsable viaja a la tarjeta y al «reorganizar la semana con IA».",
    donde: [
      "models/tenant/CalendarTask.model.js:72",
      "lib/calendar/calendarEvent.js:59",
      "lib/calendar/calendarEvent.js:83",
      "app/api/calendar/tasks/route.js:28",
      "app/api/calendar/reorganize/route.js:33",
      "app/(dashboard)/calendario/page.jsx:93",
    ],
    automatico: false,
    nota:
      "Si el cliente no tiene Equipo, el responsable se ignora y la tarea se crea igual; si se da de baja a la persona, la tarea se queda sin responsable pero no se borra. OJO, cable a medio poner (comprobado el 28/08/2026): filtrar el calendario por persona lo sabe hacer el SERVIDOR (`?teamMemberId`, y también `?clientId`), pero la pantalla no lo pide nunca —solo manda fechas y el interruptor de Proyectos— ni tiene ningún control para elegirlo. Lo mismo con la IA: acepta `teamMemberId` y unas preferencias en texto, y la pantalla le manda solo la semana. Aquí decía que el filtro existía; no en la parte que ve el usuario.",
  },
  {
    desde: "team",
    hacia: "projects",
  ...
  },
  {
    ...
    donde: [
      ...
      "app/api/clinica/coordinations/route.js:103",
  ...
  },
  {
    ...
    queHace:
      "Al cerrar la caja, el servidor hace por su cuenta la cuenta entera de lo que debería haber (el fondo contado en el arqueo anterior, los días que quedaron sin cerrar, los cobros en efectivo del día y las entradas y salidas apuntadas), la compara con lo contado, guarda la diferencia y la firma con la persona que ha entrado. Si no cuadra, exige escribir el motivo antes de cerrar; y si el cajón se quedara a cero, hay que confirmarlo.",
    donde: [
      "app/api/arqueo/cierres/route.js:140",
      "app/api/arqueo/cierres/route.js:323",
      "app/api/arqueo/cierres/route.js:334",
      "lib/db/tenantDb.js:524",
      "app/(dashboard)/facturacion/arqueo/page.jsx:423",
    ],
    automatico: true,
    nota:
  ...
  // ── support ───────────────────────────────────────────────────────────
  {
    ...
    donde: [
      "app/api/public/c/[tenantSlug]/soporte/route.js:147",
      "app/api/public/c/[tenantSlug]/soporte/route.js:166",
  ...
  },
  {
    ...
    queHace:
      "La tarea puede ir asociada a una ficha de cliente y su nombre se ve en el evento. Si se borra la ficha, la tarea sigue viva y se queda sin cliente; si llega un cliente que no existe, avisa en vez de romper. (Filtrar el calendario POR cliente lo sabe hacer el servidor, pero la pantalla no lo pide: ver la nota del enlace Equipo → Calendario.)",
    donde: [
      "models/tenant/CalendarTask.model.js:60",
