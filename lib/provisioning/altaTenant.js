...
import { normalizarCorreo, revisarCorreoCuenta } from "../auth/correoCuenta.js";
import { correoLibre } from "../auth/correoCuentaDb.js";
...
export async function altaTenant({
  ...
  //
  // ⚠️ ESTO NO ES EL CORREO DEL CLIENTE, por mucho que lo parezca: es el nombre
  // de usuario con el que entra, y puede no ser ni un correo. A quién se le
  // ESCRIBE va aparte, en `contacto` (lib/provisioning/contactoCliente.js).
  ...
  /*
   * EL CORREO DEL ADMINISTRADOR, OBLIGATORIO (26/08/2026, Jorge: «que el crear
   * una nueva cuenta te pida que rellenes el correo que usará esa cuenta»).
   *
   * No es el `adminEmail` de aquí arriba, que es el LOGIN y puede no llevar
   * arroba; tampoco es el `contacto` del cliente, que es de la empresa y puede
   * ser otra persona. Este es el buzón DE ESA CUENTA: a donde se le manda el
   * enlace si pierde la contraseña, y un segundo identificador para entrar.
   *
   * Se cae con 422 antes de crear nada. Es el caso que más duele de todos: 11
   * clientes tienen UN SOLO administrador, así que si ese pierde su contraseña
   * y no hay a dónde escribirle, el cliente entero se queda parado hasta que
   * uno de nosotros entre por SSH.
   */
  const correoAdmin = normalizarCorreo(adminCorreo);
  const malCorreo = revisarCorreoCuenta(correoAdmin);
  if (malCorreo) return { error: malCorreo, status: 422 };

  const contactoNorm = normalizarContacto(contacto, {});
  if (contactoNorm.error) return { error: contactoNorm.error, status: 422 };
  ...
  const correoOcupado = await correoLibre(User, correoAdmin);
  if (correoOcupado) return { error: correoOcupado, status: 409 };
  ...
  // El acento es el tercer color de la marca y el isotipo la versión cuadrada
  // del logo (28/08/2026): los dos son opcionales y los estrena el PDF del
  // informe clínico, pero se aceptan desde el alta para no tener que volver.
  const accent = colorValido(brand.accentColor);
  const ruta = (v) => (typeof v === "string" && v.trim() ? v.trim().slice(0, 500) : null);
  const logoUrl = ruta(brand.logoUrl);
  const isotipoUrl = ruta(brand.isotipoUrl);
  if (primary || secondary || accent || logoUrl || isotipoUrl) {
    settings.brand = {
  ...
  if (contactoNorm.contacto) settings.contacto = contactoNorm.contacto;
  ...
  try {
    ...
    await User.create(
      { email: login, emailContacto: correoAdmin, passwordHash, role: "admin", tenantId: tenant.id, moduleAccess: ["all"] },
      { validate: false }
    );
    ...
    return { ok: true, slug, tenantId: tenant.id, adminEmail: login, adminCorreo: correoAdmin, password, modulos: seleccion, fiscalGuardado, hechos, avisos };
  } catch (err) {
    /*
