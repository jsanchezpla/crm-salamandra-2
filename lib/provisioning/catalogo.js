...
export const CATALOGO = [
  {
    ...
    modulos: [
      ...
      // Auditorías (09/09/2026, AV-0100). Exige `team` y NO `team_avanzado`: lo
      // compra cualquier centro con varias personas y alguien que dirige, tenga
      // o no las pantallas de gestión clínica. Y no se solapa con Desempeño, que
      // son números automáticos: esto es una valoración a mano, con histórico.
      { key: "auditorias", nombre: "Auditorías de desempeño", desc: "La revisión mensual de cada profesional, a mano: cuatro áreas de criterios apto / no apto / no aplica, fortalezas, acción acordada y plazo, con histórico y lo que quedó pendiente del mes anterior delante.", requiere: ["team"], avisa: "Es material laboral: solo dirección escribe, y cada persona ve las suyas una vez cerradas." },
  ...
  },
  {
    ...
    modulos: [
      ...
      // BANCO (29/08/2026). El extracto REAL de la cuenta, traído por PSD2 en
      // solo lectura, casado con los cobros y gastos de Facturación: desde un
      // cobro se salta al movimiento del banco con un botón. Es SUBMÓDULO de
      // Facturación (29/08/2026, Rodrigo) con la convención de la casa —clave
      // con el prefijo del padre, como `clients_avanzado` o `team_avanzado`—:
      // se vende como extra encima de `billing`, nunca suelto.
      { key: "billing_banco", nombre: "Facturación — Banco", desc: "Submódulo de Facturación: el extracto de tu banco dentro del CRM — movimientos reales (solo lectura, PSD2), conciliación con cobros y gastos, y salto del cobro al movimiento.", requiere: ["billing"], avisa: "Necesita las credenciales gratuitas de GoCardless Bank Account Data del propio cliente, y el consentimiento del banco se renueva cada 90 días." },
    ],
  },
  {
    /*
     * PRODUCTOS (03/09/2026, Rodrigo): «agrupar Inventario, Pedidos y Tienda en
     * un gran módulo llamado Productos». Hasta hoy Pedidos e Inventario se
     * vendían en «Dinero» y la Tienda en «Captación y web», y el cliente que
     * quería vender algo tenía que entender tres módulos antes de poder poner
     * un precio.
     *
     * Dos niveles, con la convención de la casa (`<padre>_avanzado`):
     *   · `productos`          → el catálogo: cada producto con su valor. Es lo
     *                            único que hace falta para saber qué se vende
     *                            y a cuánto. Sin dependencias: se vende solo.
     *   · `productos_avanzado` → las estadísticas de venta en esa misma
     *                            pantalla y la puerta a Inventario, Pedidos y
     *                            Tienda, que cuelgan de él en el menú.
     *
     * Inventario, Pedidos y Tienda siguen siendo claves propias —cada una
     * tiene sus tablas, sus endpoints y su pantalla— y por eso se marcan aparte
     * (lo que entra en la lista entra en la factura del cliente; ver
     * `dependencias.js`): el avanzado es la puerta, no el paquete.
     *
     * Los productos y su valor son de `productos`: la Tienda decide qué se
     * publica y con qué foto, pero el precio se pone en Productos.
     */
    grupo: "Productos",
    modulos: [
      { key: "productos", nombre: "Productos básico", desc: "El catálogo de lo que vendes: cada producto con su referencia, en qué se cuenta y su valor (precio de venta y de compra)." },
      { key: "productos_avanzado", nombre: "Productos avanzado", desc: "Estadísticas de venta sobre el catálogo —qué se vende, cuánto, cuándo y por dónde— y la puerta a Inventario, Pedidos y Tienda, que se marcan aparte.", requiere: ["productos"] },
      { key: "inventory", nombre: "Inventario", desc: "El almacén de los productos del catálogo: entradas de mercancía, movimientos de stock y avisos de mínimo; proveedores compartidos con Gastos.", requiere: ["productos", "productos_avanzado"] },
      ...
      { key: "orders", nombre: "Pedidos", desc: "Pedidos de cliente con líneas y estados; sus líneas son productos del catálogo.", requiere: ["clients", "billing", "productos_avanzado"] },
      // TIENDA (25/08/2026). Escaparate público de lo que ya hay en el
      // catálogo. Exige el trío: el stock vive en `inventory`, los pedidos
      // caen en `orders` y el comprador se convierte en ficha de `clients`.
      // Vender sin ninguno de los tres sería un formulario de pago sin nada
      // detrás. Desde el 03/09/2026 cuelga de Productos avanzado.
      { key: "tienda", nombre: "Tienda online", desc: "Escaparate público de tus productos: catálogo, carrito y pago con tarjeta. Decide qué se publica y con qué foto; el precio es el de Productos. Los pedidos entran en Pedidos y descuentan stock de Inventario.", requiere: ["inventory", "orders", "clients", "productos", "productos_avanzado"], avisa: "Necesita las claves de Stripe del propio cliente y una dirección donde incrustar la tienda." },
    ],
  },
  {
    grupo: "Salud",
  ...
  },
  {
    ...
    modulos: [
      ...
      // MAILING (06/09/2026). Email marketing a quien ya es cliente y ha dicho
      // que sí (casilla de novedades de la ficha) más correos sueltos con su
      // consentimiento. Sale por Amazon SES —cuenta del cliente, céntimos por
      // envío— y no por Resend, para que una campaña con quejas no arrastre a
      // los recordatorios de cita. No exige `clients`: sin fichas funciona con
      // los correos sueltos; con fichas, la lista se cuelga de ellas.
      { key: "mailing", nombre: "Mailing", desc: "Campañas y newsletters a clientes y suscriptores: editor por bloques, segmentos con datos del CRM, bajas de un clic y métricas de clic.", avisa: "Necesita una cuenta de Amazon SES del propio cliente (Configuración → Conexiones) y sacarla del modo de pruebas." },
      ...
      //
      // La TIENDA estuvo aquí del 25/08 al 03/09/2026; ahora se vende dentro
      // del grupo «Productos», del que cuelga en el menú.
