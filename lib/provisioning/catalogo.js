...
export const CATALOGO = [
  {
    ...
    modulos: [
      ...
      // Requiere Citas y Clientes desde el 10/08/2026, y CIERRA UNA PUERTA
      // COMERCIAL a propósito: lo que se vende no es «guardar un PDF», es «que
      // la familia lo firme», y el área privada donde se firma es de Citas —
      // la familia se identifica por su ficha. Vendido suelto queda un contrato
      // que nadie puede firmar. Si algún día se quiere vender el archivador a
      // secas, es otro producto y va con su propia clave.
      { key: "documents", nombre: "Documentos básico", desc: "Solo el Contrato de Prestación de Servicios del centro: subirlo, verlo y que la familia lo firme en su área privada.", requiere: ["citas", "clients"] },
  ...
  },
  {
    ...
    modulos: [
      ...
      /*
       * REFERIDOS SE RETIRÓ DEL CATÁLOGO EL 12/08/2026 (Jorge). Estaba aquí con
       * su casilla y su letra pequeña —«hoy está hecho a medida de un cliente;
       * requiere ajuste»—, o sea que se podía marcar en un alta y venderle a
       * alguien algo que no le iba a funcionar: no tiene tabla propia (su
       * pantalla lee y escribe `leads` filtrando por origen), sus endpoints
       * exigen `leads` y NUNCA `referidos`, y su formulario público está escrito
       * a la medida de abarcaia.
       *
       * ── QUITARLO DE AQUÍ NO SE LO APAGA A NADIE, Y CONVIENE SABER POR QUÉ ──
       * El editor de clientes solo desactiva lo que está en `CLAVES_VALIDAS`
       * (ver `cicloVida.js`), y `validarSeleccion()` filtra por esa misma lista
       * antes de nada. Un módulo fuera del catálogo queda INTOCABLE desde el
       * panel — exactamente el trato que ya recibe `provisioning`. Abarcaia lo
       * conserva encendido; simplemente deja de tener casilla.
       *
       * Su entrada en `dependencias.js` se queda: `discrepanciasConCatalogo()`
       * se salta los módulos que no están aquí, así que no chilla, y el día que
       * se vuelva a vender ya está escrito lo que necesita.
       *
       * Comprobado en producción ese día: solo `abarcaia` lo tiene activo (y
