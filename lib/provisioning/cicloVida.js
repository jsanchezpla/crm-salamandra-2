...
export async function editarTenant(slug, cambios = {}) {
  ...
  if (cambios.brand && typeof cambios.brand === "object") {
    ...
    let tocado = false;
    // `accentColor` es opcional y solo lo lee el PDF del informe clínico
    // (28/08/2026): es el filete bajo cada titular y la mancha cálida de la
    // portada. Sin él, el documento sale a un solo color, que es sobrio; con él
    // se puede usar el tercer color de la marca, que muchos logos tienen y los
    // dos campos de siempre no recogen.
    for (const campo of ["primaryColor", "secondaryColor", "accentColor"]) {
