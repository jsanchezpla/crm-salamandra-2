...
export const DEPENDENCIAS = [
  ...
  // Auditorías de desempeño (09/09/2026, AV-0100). Se apunta aquí a la vez que
  // en el catálogo: lo que le pasó a Fichaje —estar en el catálogo y no en la
  // matriz durante un mes— lo caza `_smoke-provisioning-dependencias.mjs`, y no
  // tiene sentido volver a estrenarlo.
  {
    modulo: "auditorias",
    paraFuncionar: "Sí, total",
    resumen:
      "`auditorias_desempeno.team_member_id` es NOT NULL y toda auditoría es de alguien de la plantilla → sin Equipo no hay a quién auditar.",
    necesita: [
      {
        claves: ["team"],
        nivel: "obligatorio",
        porque:
          "Una auditoría se abre eligiendo a una persona del equipo: la columna es NOT NULL con FK a `team_members` y la migración se salta el schema que no tenga esa tabla. Exige `team`, NO `team_avanzado`: lo compra cualquier centro con varias personas y alguien que dirige, tenga o no las pantallas de gestión clínica.",
        donde: [
          "models/tenant/AuditoriaDesempeno.model.js:24",
          "scripts/migrate-auditorias-module.js:99",
          "lib/team/auditoriasStore.js:26",
        ],
      },
    ],
  },
  ...
  },
  {
    modulo: "mailing",
    paraFuncionar: "No",
    resumen: "Manda igual a los correos sueltos. Sin Clientes no hay lista de fichas ni segmentos por módulo, estado o cita.",
    necesita: [
      {
        claves: ["clients"],
        nivel: "parcial",
        porque:
          "La lista se cuelga de las fichas con la casilla de novedades; sin Clientes solo quedan los correos sueltos importados o añadidos a mano.",
        donde: ["lib/mailing/audiencia.js:104"],
      },
      {
        claves: ["citas"],
        nivel: "parcial",
        porque: "El filtro de segmento «última cita hace más/menos de N días» no tiene de dónde leer: se ignora.",
        donde: ["lib/mailing/audiencia.js:73"],
      },
    ],
  },
