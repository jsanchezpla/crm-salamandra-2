...
import { defineBillingConcept } from "../../models/tenant/BillingConcept.model.js";
import { defineCuota } from "../../models/tenant/Cuota.model.js";
...
import { defineCashMovement } from "../../models/tenant/CashMovement.model.js";
...
import { defineCalendarTaskAttendee } from "../../models/tenant/CalendarTaskAttendee.model.js";
import { defineCalendarTaskOwner } from "../../models/tenant/CalendarTaskOwner.model.js";
import { defineCalendarCategory } from "../../models/tenant/CalendarCategory.model.js";
import { defineGoogleCalendarConnection } from "../../models/tenant/GoogleCalendarConnection.model.js";
...
// El registro de sesión de un taller (01/09/2026): UNO para todo el grupo.
import { defineTallerSesion } from "../../models/tenant/TallerSesion.model.js";
/*
 ...
 * actividad», impartidos por varios terapeutas y con lista de asistencia.
 */
import { defineTallerGrupo } from "../../models/tenant/TallerGrupo.model.js";
import { defineTallerGrupoTerapeuta } from "../../models/tenant/TallerGrupoTerapeuta.model.js";
import { defineTallerCitaTerapeuta } from "../../models/tenant/TallerCitaTerapeuta.model.js";
import { defineTallerAsistencia } from "../../models/tenant/TallerAsistencia.model.js";
...
import { defineAuditoriaDesempeno } from "../../models/tenant/AuditoriaDesempeno.model.js";
...
import { defineDiagnostico } from "../../models/tenant/Diagnostico.model.js";
...
import { defineDocumentRead } from "../../models/tenant/DocumentRead.model.js";
import { defineDocumentFolderMember } from "../../models/tenant/DocumentFolderMember.model.js";
import { defineClientModuleAssignment } from "../../models/tenant/ClientModuleAssignment.model.js";
// Módulo Mailing (06/09/2026): email marketing por Amazon SES, separado del
// correo transaccional de Resend a propósito (docs/modules/mailing.md).
import { defineMailingContact } from "../../models/tenant/MailingContact.model.js";
import { defineMailingSegment } from "../../models/tenant/MailingSegment.model.js";
import { defineMailingCampaign } from "../../models/tenant/MailingCampaign.model.js";
import { defineMailingSend } from "../../models/tenant/MailingSend.model.js";
import { defineMailingSuppression } from "../../models/tenant/MailingSuppression.model.js";
import { defineMailingTemplate } from "../../models/tenant/MailingTemplate.model.js";
import { defineMailingEvent } from "../../models/tenant/MailingEvent.model.js";
import { defineMailingSequence } from "../../models/tenant/MailingSequence.model.js";
import { engancharReactivacion } from "../clients/reactivarPorActividad.js";
...
function initTenantDb(slug) {
  ...
  // A quién afecta cada evento (29/08/2026) + la cuenta de Google de cada
  // miembro. Registrados para TODOS los tenants, como el resto: sus tablas las
  // crea en todos los schemas migrate-calendar-google (CORE), y quién puede
  // USARLAS lo deciden los endpoints con hasModule, no la existencia de la tabla.
  const CalendarTaskAttendee = defineCalendarTaskAttendee(sequelize);
  const GoogleCalendarConnection = defineGoogleCalendarConnection(sequelize);
  // Los responsables (varios por evento) y el catálogo de categorías
  // (01/09/2026). Mismo criterio: registrados para TODOS, sus tablas las crea
  // migrate-calendar-categorias (CORE), y quién puede usarlas lo deciden los
  // endpoints con hasModule.
  const CalendarTaskOwner = defineCalendarTaskOwner(sequelize);
  const CalendarCategory = defineCalendarCategory(sequelize);
  ...
  const TallerGrupo = defineTallerGrupo(sequelize);
  const TallerGrupoTerapeuta = defineTallerGrupoTerapeuta(sequelize);
  const TallerCitaTerapeuta = defineTallerCitaTerapeuta(sequelize);
  const TallerAsistencia = defineTallerAsistencia(sequelize);
  const TallerInscripcion = defineTallerInscripcion(sequelize);
  const TallerSesion = defineTallerSesion(sequelize);
  // El expediente de DIAGNÓSTICO de un paciente (12/09/2026): producto de
  // horas, terapeuta asignado y estado; las horas se cuentan desde las citas.
  const Diagnostico = defineDiagnostico(sequelize);
  ...
  const AuditoriaDesempeno = defineAuditoriaDesempeno(sequelize);
  ...
  // Acuse de lectura: a quién se le ha pedido leer cada documento (01/09/2026).
  const DocumentRead = defineDocumentRead(sequelize);
  // Y con quién está compartida cada carpeta (01/09/2026).
  const DocumentFolderMember = defineDocumentFolderMember(sequelize);

  // ── Asignación de clientes a módulos (nutricion/clinica) ────────────────────
  const ClientModuleAssignment = defineClientModuleAssignment(sequelize);

  // ── Módulo Mailing (06/09/2026) ─────────────────────────────────────────────
  // Registrados para TODOS los tenants, como Outreach: sus tablas solo existen
  // donde el módulo está activo (migrate-mailing-sprint-1, en MODULES.mailing)
  // y quién puede usarlas lo deciden los endpoints con hasModule("mailing").
  const MailingContact = defineMailingContact(sequelize);
  const MailingSegment = defineMailingSegment(sequelize);
  const MailingCampaign = defineMailingCampaign(sequelize);
  const MailingSend = defineMailingSend(sequelize);
  const MailingSuppression = defineMailingSuppression(sequelize);
  const MailingTemplate = defineMailingTemplate(sequelize);
  const MailingEvent = defineMailingEvent(sequelize);
  // Sprint 2 (06/09/2026): secuencias por eventos del CRM.
  const MailingSequence = defineMailingSequence(sequelize);
  ...
  const BillingConcept = defineBillingConcept(sequelize);
  // La cuota asignada: que paga cada familia todos los meses (01/09/2026).
  const Cuota = defineCuota(sequelize);
  ...
  // Lo que entra y sale del cajon y NO es un cobro (01/09/2026).
  const CashMovement = defineCashMovement(sequelize);
  ...
  // ── Asociaciones Mailing ─────────────────────────────────────────────────────
  // Campaña → segmento (SET NULL: borrar un segmento no borra sus campañas) y
  // campaña → envíos / evento → envío (CASCADE: sin campaña no hay histórico
  // que guardar; la supresión, que es lo que de verdad importa, va sin FK).
  MailingSegment.hasMany(MailingCampaign, { foreignKey: "segmentId", as: "campaigns", onDelete: "SET NULL" });
  MailingCampaign.belongsTo(MailingSegment, { foreignKey: "segmentId", as: "segment" });
  MailingCampaign.hasMany(MailingSend, { foreignKey: "campaignId", as: "sends", onDelete: "CASCADE" });
  MailingSend.belongsTo(MailingCampaign, { foreignKey: "campaignId", as: "campaign" });
  MailingSend.hasMany(MailingEvent, { foreignKey: "sendId", as: "events", onDelete: "CASCADE" });
  MailingEvent.belongsTo(MailingSend, { foreignKey: "sendId", as: "send" });
  // Secuencia → sus campañas automáticas (una por periodo). CASCADE: borrar la
  // secuencia se lleva su histórico; la supresión, que es lo que importa, no.
  MailingSequence.hasMany(MailingCampaign, { foreignKey: "sequenceId", as: "campaigns", onDelete: "CASCADE" });
  MailingCampaign.belongsTo(MailingSequence, { foreignKey: "sequenceId", as: "sequence" });
  ...
  /*
   * Lectura pedida de un documento (01/09/2026). CASCADE en las dos patas y a
   * propósito: si el documento se borra, no queda nada que leer; si la persona
   * deja el centro, su ficha se lleva sus lecturas. Lo pendiente de alguien que
   * ya no está es ruido en la portada de nadie.
   */
  Document.hasMany(DocumentRead, { foreignKey: "documentId", as: "lecturas", onDelete: "CASCADE" });
  DocumentRead.belongsTo(Document, { foreignKey: "documentId", as: "document" });

  /*
   * Con quién está compartida una carpeta (01/09/2026). CASCADE por las dos
   * patas, igual que las lecturas: si la carpeta se borra no hay nada que
   * compartir, y quien deja el centro deja de estar en ninguna lista.
   */
  DocumentFolder.hasMany(DocumentFolderMember, { foreignKey: "folderId", as: "miembros", onDelete: "CASCADE" });
  DocumentFolderMember.belongsTo(DocumentFolder, { foreignKey: "folderId", as: "folder" });
  ...
  // Documentos → sesión clínica de cuya preparación salieron (02/09/2026).
  ClinicSession.hasMany(Document, { foreignKey: "clinicSessionId", as: "adjuntosDePreparacion" });
  Document.belongsTo(ClinicSession, { foreignKey: "clinicSessionId", as: "clinicSession" });
  ...
  // ── Cuotas asignadas (01/09/2026) ──────────────────────────────────────────
  // Quien paga la cuota, de que paciente es, y de que cuota nacio cada cobro
  // (payments.cuota_id: es lo que evita generar dos veces el mismo mes).
  Client.hasMany(Cuota, { foreignKey: "clientId", as: "cuotas" });
  Cuota.belongsTo(Client, { foreignKey: "clientId", as: "client" });
  // Y quien la paga cuando no es la familia (07/09/2026): otra ficha, la
  // fundación o la empresa. Solo se materializa en queries con el alias.
  Cuota.belongsTo(Client, { foreignKey: "payerClientId", as: "payer" });
  Cuota.hasMany(Payment, { foreignKey: "cuotaId", as: "cobros" });
  Payment.belongsTo(Cuota, { foreignKey: "cuotaId", as: "cuota" });
  ...
  // Entradas y salidas del cajon: cuelgan de su caja y saben quien las apunto.
  CashPoint.hasMany(CashMovement, { foreignKey: "cashPointId", as: "movements" });
  CashMovement.belongsTo(CashPoint, { foreignKey: "cashPointId", as: "cashPoint" });
  TeamMember.hasMany(CashMovement, { foreignKey: "createdById", as: "cashMovements" });
  CashMovement.belongsTo(TeamMember, { foreignKey: "createdById", as: "createdBy" });
  ...
  /*
   * El documento aparejado a un tramo de la agenda (01/09/2026): el acta que se
   * ve al abrir la Reunión de equipo del miércoles. `SET NULL` en la migración
   * —no CASCADE—: borrar el bloqueo quita la pareja, nunca el documento.
   */
  TeamBlock.hasMany(Document, { foreignKey: "teamBlockId", as: "documentos" });
  Document.belongsTo(TeamBlock, { foreignKey: "teamBlockId", as: "teamBlock" });

  // Y a quién se le pidió leerlo. La ficha de equipo manda (ver el modelo).
  TeamMember.hasMany(DocumentRead, { foreignKey: "teamMemberId", as: "lecturasPedidas", onDelete: "CASCADE" });
  DocumentRead.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "teamMember" });

  TeamMember.hasMany(DocumentFolderMember, { foreignKey: "teamMemberId", as: "carpetasCompartidas", onDelete: "CASCADE" });
  DocumentFolderMember.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "teamMember" });
  ...
  // Quién da las sesiones del bono (18/09/2026, AV-0183). `constraints: false`
  // como el resto de lo que cruza módulos: hay schemas con bonos y sin equipo.
  SessionPack.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "teamMember", constraints: false });
  ...
  // A quién afecta el evento (29/08/2026), patrón TaskAssignee. `onDelete`
  // EXPLÍCITO por la lección del 26/08 (ver la nota grande más abajo): borrar el
  // evento o al miembro se lleva sus filas de convocatoria, que sin ellos no
  // significan nada.
  CalendarTask.belongsToMany(TeamMember, {
    through: CalendarTaskAttendee,
    foreignKey: "taskId",
    otherKey: "teamMemberId",
    as: "attendees",
  });
  TeamMember.belongsToMany(CalendarTask, {
    through: CalendarTaskAttendee,
    foreignKey: "teamMemberId",
    otherKey: "taskId",
    as: "attendedCalendarTasks",
  });
  // Los responsables del evento (01/09/2026), misma forma que los convocados y
  // por el mismo motivo: sin el evento o sin la persona, la fila no significa nada.
  CalendarTask.belongsToMany(TeamMember, {
    through: CalendarTaskOwner,
    foreignKey: "taskId",
    otherKey: "teamMemberId",
    as: "owners",
  });
  TeamMember.belongsToMany(CalendarTask, {
    ...
    foreignKey: "teamMemberId",
    otherKey: "taskId",
    as: "ownedCalendarTasks",
  });
  CalendarTask.hasMany(CalendarTaskOwner, { foreignKey: "taskId", as: "ownerLinks", onDelete: "CASCADE" });
  CalendarTaskOwner.belongsTo(CalendarTask, { foreignKey: "taskId", as: "task", onDelete: "CASCADE" });
  TeamMember.hasMany(CalendarTaskOwner, { foreignKey: "teamMemberId", as: "calendarOwnerships", onDelete: "CASCADE" });
  CalendarTaskOwner.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "teamMember", onDelete: "CASCADE" });

  // La categoría del evento. SET NULL y no CASCADE: borrar una categoría no
  // puede llevarse por delante las reuniones que estaban clasificadas con ella
  // —quedan sin categoría, que es exactamente lo que ha pasado.
  CalendarCategory.hasMany(CalendarTask, { foreignKey: "categoryId", as: "calendarTasks", onDelete: "SET NULL" });
  CalendarTask.belongsTo(CalendarCategory, { foreignKey: "categoryId", as: "category", onDelete: "SET NULL" });

  CalendarTask.hasMany(CalendarTaskAttendee, { foreignKey: "taskId", as: "attendeeLinks", onDelete: "CASCADE" });
  CalendarTaskAttendee.belongsTo(CalendarTask, { foreignKey: "taskId", as: "task", onDelete: "CASCADE" });
  TeamMember.hasMany(CalendarTaskAttendee, { foreignKey: "teamMemberId", as: "calendarAttendances", onDelete: "CASCADE" });
  CalendarTaskAttendee.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "teamMember", onDelete: "CASCADE" });

  // Su Google conectado: una conexión por persona. CASCADE: sin el miembro, los
  // tokens no abren nada que nos importe conservar.
  TeamMember.hasOne(GoogleCalendarConnection, { foreignKey: "teamMemberId", as: "googleCalendar", onDelete: "CASCADE" });
  GoogleCalendarConnection.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "teamMember", onDelete: "CASCADE" });
  ...
  // La cuota del mes tambien apunta a su paciente, con la misma condicion:
  // solo se materializa en queries que incluyan el alias (01/09/2026).
  Patient.hasMany(Cuota, { foreignKey: "patientId", as: "cuotas" });
  Cuota.belongsTo(Patient, { foreignKey: "patientId", as: "patient" });
  // Y el cobro tambien (03/09/2026): payments.patient_id ya existia y se
  // rellena desde la cuota; el alias es lo que deja BUSCAR un cobro por el
  // nombre del paciente para dar con la familia que paga. Misma condicion.
  Patient.hasMany(Payment, { foreignKey: "patientId", as: "cobros" });
  ...
  // Quién imparte el taller. Se conserva como el responsable de la ACTIVIDAD;
  // quien da cada grupo son varios y viven en `taller_grupo_terapeutas`.
  ...
  /*
   * ── Los GRUPOS de cada actividad (01/09/2026) ────────────────────────────
   * Retirar la actividad se lleva sus grupos (CASCADE), igual que ya se llevaba
   * sus inscripciones. La inscripción cuelga ADEMÁS del grupo, y ese es hoy el
   * enlace que manda: `tallerId` se queda para poder preguntar por la actividad
   * entera sin pasar por los grupos.
   */
  Taller.hasMany(TallerGrupo, { foreignKey: "tallerId", as: "grupos", onDelete: "CASCADE" });
  TallerGrupo.belongsTo(Taller, { foreignKey: "tallerId", as: "taller" });

  TallerGrupo.hasMany(TallerInscripcion, { foreignKey: "grupoId", as: "inscripciones", onDelete: "CASCADE" });
  TallerInscripcion.belongsTo(TallerGrupo, { foreignKey: "grupoId", as: "grupo" });

  TallerGrupo.hasMany(TallerGrupoTerapeuta, { foreignKey: "grupoId", as: "terapeutas", onDelete: "CASCADE" });
  TallerGrupoTerapeuta.belongsTo(TallerGrupo, { foreignKey: "grupoId", as: "grupo" });
  TeamMember.hasMany(TallerGrupoTerapeuta, { foreignKey: "teamMemberId", as: "gruposDeTaller" });
  TallerGrupoTerapeuta.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "profesional" });

  /*
   * ── Lo que cuelga de una CITA de taller (01/09/2026) ─────────────────────
   * Quién la dio y quién fue. Las dos con CASCADE sobre la cita: borrada la
   * cita, ni la lista de asistencia ni los impartidores significan nada.
   *
   * OJO con la asimetría deliberada: `taller_sesiones` (el registro) NO lleva
   * asociación con la cita ni FK dura, por lo mismo que `clinic_sessions` —
   * borrar una cita del calendario no puede llevarse por delante lo que se
   * escribió de esa tarde—. La lista de asistencia sí se va, porque fuera de su
   * cita no dice nada de nadie.
   */
  Booking.hasMany(TallerAsistencia, { foreignKey: "bookingId", as: "asistentes", onDelete: "CASCADE" });
  TallerAsistencia.belongsTo(Booking, { foreignKey: "bookingId", as: "cita" });
  Patient.hasMany(TallerAsistencia, { foreignKey: "patientId", as: "asistenciasTaller", onDelete: "CASCADE" });
  TallerAsistencia.belongsTo(Patient, { foreignKey: "patientId", as: "patient" });
  TallerGrupo.hasMany(TallerAsistencia, { foreignKey: "grupoId", as: "asistencias" });
  TallerAsistencia.belongsTo(TallerGrupo, { foreignKey: "grupoId", as: "grupo" });

  Booking.hasMany(TallerCitaTerapeuta, { foreignKey: "bookingId", as: "impartidores", onDelete: "CASCADE" });
  TallerCitaTerapeuta.belongsTo(Booking, { foreignKey: "bookingId", as: "cita" });
  TeamMember.hasMany(TallerCitaTerapeuta, { foreignKey: "teamMemberId", as: "talleresImpartidos" });
  TallerCitaTerapeuta.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "profesional" });

  /*
   * El grupo que se da en una cita. SIN FK dura (por eso `constraints: false`):
   * `taller_grupos` es del módulo Clínica y `bookings` del de Citas, así que
   * hay schemas con la una y sin la otra. La asociación existe solo para poder
   * pedir el grupo con un include.
   */
  TallerGrupo.hasMany(Booking, { foreignKey: "tallerGrupoId", as: "citas", constraints: false });
  Booking.belongsTo(TallerGrupo, { foreignKey: "tallerGrupoId", as: "tallerGrupo", constraints: false });
  TallerGrupo.hasMany(EventType, { foreignKey: "tallerGrupoId", as: "tiposDeCita", constraints: false });
  EventType.belongsTo(TallerGrupo, { foreignKey: "tallerGrupoId", as: "tallerGrupo", constraints: false });

  /*
   * La CUOTA de un tipo de cita (04/09/2026): qué concepto del catálogo cubre
   * las citas de ese tipo, para que el alta traiga el dinero puesto
   * (`lib/citas/dineroDeLaCita.js`). `constraints: false` por lo mismo que
   * arriba: hay schemas con `event_types` y sin módulo de facturación, donde
   * `billing_concepts` ni existe y una FK de verdad no se podría crear. Quien
   * pida el include tiene que mirar antes que el centro tenga `billing`.
   */
  BillingConcept.hasMany(EventType, { foreignKey: "conceptId", as: "tiposDeCita", constraints: false });
  EventType.belongsTo(BillingConcept, { foreignKey: "conceptId", as: "concepto", constraints: false });

  /*
   * Las SESIONES del taller (01/09/2026). Una fila por tarde de taller, con el
   * registro común del grupo; borrar el taller se las lleva (CASCADE), que es
   * lo mismo que ya pasa con sus inscripciones.
   *
   * La sesión de CADA paciente apunta aquí por `tallerSesionId`, pero SIN
   * asociación declarada y sin FK: borrar la sesión de un taller no puede
   * llevarse por delante la historia clínica de ocho pacientes. El puntero se
   * queda colgando y esa sesión vuelve a leerse como una sesión normal — la
   * misma decisión que `deliveredDocumentId` (29/08/2026).
   */
  Taller.hasMany(TallerSesion, { foreignKey: "tallerId", as: "sesiones", onDelete: "CASCADE" });
  TallerSesion.belongsTo(Taller, { foreignKey: "tallerId", as: "taller" });

  /*
   * ── DIAGNÓSTICOS (12/09/2026) ─────────────────────────────────────────────
   * El expediente cuelga del paciente; la familia y el terapeuta son fotos
   * para pintar la lista sin saltos. Todo `constraints: false`: `diagnosticos`
   * es de Clínica y apunta a tablas de Citas y Facturación que no están en
   * todos los schemas (el mismo motivo que `TallerGrupo` ↔ `Booking`), y
   * ninguna FK real se crea desde aquí. Las citas del expediente son lo que
   * cuenta su barra de horas (`lib/clinica/diagnostico.js`, `horasDe`).
   */
  Patient.hasMany(Diagnostico, { foreignKey: "patientId", as: "diagnosticos", constraints: false });
  Diagnostico.belongsTo(Patient, { foreignKey: "patientId", as: "patient", constraints: false });
  Diagnostico.belongsTo(Client, { foreignKey: "clientId", as: "client", constraints: false });
  Diagnostico.belongsTo(TeamMember, { foreignKey: "therapistId", as: "therapist", constraints: false });
  Diagnostico.hasMany(Booking, { foreignKey: "diagnosticoId", as: "citas", constraints: false });
  Booking.belongsTo(Diagnostico, { foreignKey: "diagnosticoId", as: "diagnostico", constraints: false });
  Diagnostico.belongsTo(SessionPack, { foreignKey: "packId", as: "pack", constraints: false });
  SessionPack.belongsTo(Diagnostico, { foreignKey: "diagnosticoId", as: "diagnostico", constraints: false });
  Diagnostico.hasMany(ClinicSession, { foreignKey: "diagnosticoId", as: "sesiones", constraints: false });
  ClinicSession.belongsTo(Diagnostico, { foreignKey: "diagnosticoId", as: "diagnostico", constraints: false });

  TallerGrupo.hasMany(TallerSesion, { foreignKey: "grupoId", as: "sesiones", onDelete: "CASCADE" });
  TallerSesion.belongsTo(TallerGrupo, { foreignKey: "grupoId", as: "grupo" });

  TeamMember.hasMany(TallerSesion, { foreignKey: "teamMemberId", as: "sesionesDeTaller" });
  TallerSesion.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "profesional" });
  ...
  // Auditoría de desempeño (módulo `auditorias`, 09/09/2026): dos enlaces al
  // equipo, el auditado y quien audita. FKs lógicas nullable, igual que arriba;
  // la integridad física la fija su migración.
  TeamMember.hasMany(AuditoriaDesempeno, { foreignKey: "teamMemberId", as: "auditorias" });
  AuditoriaDesempeno.belongsTo(TeamMember, { foreignKey: "teamMemberId", as: "auditado" });
  TeamMember.hasMany(AuditoriaDesempeno, { foreignKey: "auditorId", as: "auditoriasHechas" });
  AuditoriaDesempeno.belongsTo(TeamMember, { foreignKey: "auditorId", as: "auditor" });
  ...
  // Cobro completado o cuota viva → el paciente y su familia vuelven a Activo
  // si estaban En pausa o de Baja (15/09/2026, lib/clients/reactivarPorActividad.js).
  engancharReactivacion(models);
