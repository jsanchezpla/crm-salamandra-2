...
import { defineTableroAdjunto } from "../../models/master/TableroAdjunto.model.js";
...
import { defineCalendarioGlobalVinculo } from "../../models/master/CalendarioGlobalVinculo.model.js";
import { defineAiUso } from "../../models/master/AiUso.model.js";
...
function initMasterDb() {
  ...
  // Y las capturas (24/08/2026), que tampoco se asocian a nada: cuelgan de la
  // FICHA de la tarea —`<!--id:…-->`, escrita dentro del propio texto— y esa
  // ficha no es la clave de ninguna tabla. Es la única forma de que un fichero
  // no se quede huérfano cuando alguien reescribe un título, que es lo que ya le
  // pasa al tick. Quien las caduca es `scripts/podar-tablero-adjuntos.js`.
  const TableroAdjunto = defineTableroAdjunto(sequelize);
  ...
  // Calendario global (03/09/2026): qué calendarios de cliente ve cada cuenta
  // de Salamandra desde calendar.salamandrasolutions.com. Sin asociaciones ni
  // FK a propósito: la baja de un cliente no debe tropezar con una fila de
  // aquí, y `vinculosDe` ya ignora los tenants que no están.
  const CalendarioGlobalVinculo = defineCalendarioGlobalVinculo(sequelize);
  // Contabilidad de la IA (11/09/2026): una fila por llamada de pago, con
  // tokens y coste estimado. Sin FK a propósito, como el Buzón: la baja de un
  // cliente no tiene que tropezar con su historial de gasto.
  const AiUso = defineAiUso(sequelize);
