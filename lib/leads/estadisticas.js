...
const GANADOS = new Set(["won", "closed_yes", "paciente"]);
const PERDIDOS = new Set(["lost", "closed_no"]);
...
function embudo(leads) {
  ...
  for (const l of leads) {
    ...
    if (GANADOS.has(l.stage)) ganados++;
    else if (PERDIDOS.has(l.stage)) perdidos++;
...
export async function calcularEstadisticas(ctx, rango) {
  ...
  return {
    ...
    profesionales: { ...embudo(leads), origenes: origenes(leads) },
