...
export async function calcularEstadisticas(ctx, rango) {
  ...
  return {
    ...
    profesionales: {
      ...
      // ganado (17/08/2026). Le pasa a aumenta (y le pasaba a demo y sandbox
      // hasta el 18/08, cuando pasaron al embudo por defecto), que termina en
      ...
      // `ctx.hasModule` va como segundo argumento desde el 24/08/2026: el embudo
      // de `booking` se decide por módulo, y sin pasarlo esta pantalla le diría
      // a una agencia de management que su embudo no puede ganar a nadie.
      ...(tieneEtapaGanada(ctx.tenant?.slug, ctx.hasModule) ? {} : { ganados: null, conversion: null }),
    },
