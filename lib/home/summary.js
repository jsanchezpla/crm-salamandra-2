...
async function buildAgenda(models, { soloDe = null } = {}) {
  ...
  // igual que en app/api/citas/bookings/route.js. Un fallo de resolución no puede
  // destapar la agenda del equipo.
  //
  // «Lo suyo» incluye las citas SIN profesional asignado, que no son de nadie
  // (19/08/2026, Jorge: «tan solo que no vea las de Laura, el resto que sí las
  // vea»). La regla y su porqué, en `soloLoSuyo`.
  const mias = soloDe ? { teamMemberId: soloLoSuyo(soloDe) } : {};
  const now = new Date();
...
// (won/lost/closed_yes/closed_no), la terminal del embudo nutricional de
// nutri_laura ("paciente" = convertido a paciente activo) y las dos de booking
// ("fecha_confirmada" y "actuacion_realizada": el bolo está cerrado o ya se
// tocó), para no inflar el contador de leads abiertos con conversiones ya
// cerradas. Ver lib/leads/stages.js.
//
...
// vigila que no se desincronicen es `scripts/_smoke-leads-etapas.mjs` («el
// embudo y la portada cuentan lo mismo como cerrado»), que es justo quien cazó
// el olvido al añadir booking el 24/08/2026.
const CLOSED_STAGES = [
  "won",
  "lost",
  "closed_yes",
...
export async function buildHomeSummary(ctx) {
  const { hasModule, tenantHasModule, tenantModels, user, tenant } = ctx;
  ...
  if (hasModule("citas") && tenantHasModule("team") && !veTodaLaAgenda({ tenant, role: user.role })) {
