 ...
 *                   derecha operativa (bandeja, semana, tareas) en vez de
 *                   gráficas. Ver «SIN FACTURACIÓN NO HAY GRÁFICAS» abajo.
 *
...
// Primer día del mes siguiente al de hoy (fecha civil de Madrid).
function primerDiaMesSiguiente() {
  const d = new Date(fechaMadrid().slice(0, 7) + "-01T00:00:00Z");
  d.setUTCMonth(d.getUTCMonth() + 1);
  return d.toISOString().slice(0, 10);
}

/**
 * Ingresos por servicio del mes en curso (Rodrigo, 29/08/2026): citas no
 * anuladas × el precio del TIPO de cita, donde esté relleno — la opción que se
 * eligió frente a etiquetar facturas. Es valor de agenda, no caja: lo cobrado
 * de verdad ya lo dice Facturación. Un tenant que deja los precios vacíos a
 * propósito (Aumenta, decisión de venta) no suma nada y la vista no se enseña.
 * El precio de un pack es del PACK entero: se reparte entre sus sesiones para
 * no contar N veces el total.
 */
async function vistaIngresosServicio(models) {
  const { Booking, EventType } = models;
  const desde = fechaMadrid().slice(0, 7) + "-01";
  const hasta = primerDiaMesSiguiente();
  const filas = await Booking.findAll({
    attributes: ["eventTypeId", [fn("COUNT", col("id")), "n"]],
    where: {
      eventTypeId: { [Op.ne]: null },
      [Op.and]: [
        literal(`(scheduled_at AT TIME ZONE 'Europe/Madrid') >= '${desde}'`),
        literal(`(scheduled_at AT TIME ZONE 'Europe/Madrid') < '${hasta}'`),
      ],
      status: { [Op.notIn]: ["cancelled", "no_show"] },
    },
    group: ["eventTypeId"],
  ...
  const tipos = await EventType.findAll({
    where: { id: filas.map((f) => f.eventTypeId) },
    attributes: ["id", "name", "price", "sessionsCount"],
    raw: true,
  });
  const porId = new Map(tipos.map((t) => [t.id, t]));
  const datos = filas
    .map((f) => {
      const t = porId.get(f.eventTypeId);
      if (!t || !Number(t.price)) return null; // sin precio no hay ingreso que pintar
      const porSesion = Number(t.price) / Math.max(1, Number(t.sessionsCount) || 1) / 100;
      return {
        etiqueta: (t.name || "—").toLowerCase(),
        tooltip: t.name || "—",
        valor: round2(porSesion * Number(f.n || 0)),
        resalte: false,
      };
    })
    .filter(Boolean)
    ...
    .slice(0, 6);
  if (datos.length) datos[0].resalte = true;
  return vista("ingresos-servicio", "Ingresos por servicio · este mes", "eur", datos);
}
 ...
 * tiene nadie, la vista entera no se enseña.
 *
...
async function vistaOcupacion(models) {
  ...
  return vista("ocupacion", "Ocupación del equipo · este mes", "pct", datos);
}

// ─── «Mi trabajo»: la mitad derecha de quien no ve gráficas ─────────────────
// Bandeja (informes + incidencias asignadas), la semana que viene y las tareas
// propias. Cada caja con el MISMO gating que su pantalla de origen y dentro de
...
async function buildTrabajo(models, { hasModule, conCitas, conEquipo, miId, miFiltro }) {
  ...
  const [bandeja, proximas, tareas] = await Promise.all([
      ...
      ? safeBlock(async () => {
          ...
          const [informes, vencidos, incRows, incTotal] = await Promise.all([
            ...
            models.Incidencia.findAll({
              where: { assignedToId: miId, status: { [Op.ne]: "resolved" } },
  ...
  if (!bandeja && !proximas && !tareas) return null;
  return { bandeja, proximas, tareas };
}
...
export async function buildPortada(ctx) {
  ...
  // tareas (`trabajo`). La condición cruza tenant Y usuario a propósito: en un
