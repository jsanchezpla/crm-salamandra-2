...
export async function buildPortada(ctx) {
  ...
  // Incidencias abiertas del centro (31/08/2026, Rodrigo): mismos gates que
  ...
  if ((hasModule("clinica") || hasModule("pacientes")) && hasModule("team_avanzado")) {
    const n = await safeBlock(() =>
      tenantModels.Incidencia.count({ where: { status: { [Op.ne]: "resolved" } } })
    );
    if (n > 0)
      pendiente.push({
        key: "incidencias",
