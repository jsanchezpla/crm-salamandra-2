...
import { nombrePaciente } from "../clinica/pacienteEnAvisos.js";
...
import { ventanaDeLaSemana, citasSinRegistro } from "../clinica/loMio.js";
import { cargarFestivos } from "../citas/festivos.js";
import { includeDelPaciente, nombreDeLaCita } from "../citas/nombreEnLaAgenda.js";
import { avisaDelPendienteDeTodos } from "../clinica/coordinadoras.js";
...
// Rango del mes en curso para las finanzas: el MES ENTERO en fecha de Madrid.
// Cortaba en hoy ([día 1, hoy]) y eso descuadraba la portada con el Panel
// operativo de Facturación, que lee el mes completo: un cobro apuntado más
// adelante (la cuota del día 15 registrada el día 4) salía en una pantalla y
// no en la otra. El día de hoy sigue viajando aparte: es lo que decide qué
// factura está vencida, y eso no es del mes sino de hoy.
function monthRange() {
  const today = fechaMadrid();
  const [aa, mm] = today.split("-").map(Number);
  // Date.UTC con día 0 del mes SIGUIENTE = último día de este mes.
  const finDeMes = new Date(Date.UTC(aa, mm, 0)).toISOString().slice(0, 10);
  return { from: today.slice(0, 7) + "-01", to: finDeMes, today };
}

// ─── Finanzas (billing): las tres cifras de la portada ──────────────────────
// Tres magnitudes operativas visibles para cualquier usuario con billing:
//   - Facturado del mes (base imponible).
//   - Cobrado del mes (lo que ha entrado).
...
// histórica, no acotada al mes) se leería como contradictorio.
//
// ── COBRADO SON LOS COBROS, NO LAS FACTURAS (04/09/2026, Rodrigo) ──────────
// «Cobrado» salía de `getKpisForPeriod`: el `paid_amount` de las facturas
// EMITIDAS en el mes. En Aumenta se cobra la cuota primero y se factura al
// cierre («Facturar el mes»), así que en el mes vivo no hay ni una factura: la
// portada enseñaba 0 € facturado, 0 € cobrado y 0 € vencido —tres ceros
// clavados todo el mes— mientras el Panel operativo contaba 14.701 € en 84
// cobros. Es el mismo arreglo que se le hizo al panel el 03/09 y por el mismo
// motivo: lo cobrado no depende de lo facturado. Aquí «Cobrado» son los cobros
// COMPLETADOS con fecha de cobro dentro del mes, tengan factura o no, y el pie
// dice cuántos son y no un % sobre lo facturado (ese % se iba por encima de 100
// o daba 0 en cuanto se cobraba antes de facturar).
//
// ── Y LO VE QUIEN TIENE FACTURACIÓN, NO SOLO EL ADMIN (04/09/2026, Rodrigo) ─
// La cifra estaba reservada a admin por sensible, y el efecto era el contrario
// del buscado: en Aumenta las dos únicas personas que llevan el dinero —Olga y
// Rosa— no son admin, así que veían dos ceros donde había 14.701 € cobrados. El
// gate sigue siendo el módulo: quien no está adherido a Facturación no ve
// ninguna de las tres cifras. Quien lo está, las ve todas.
//
// Los no-admin corren 3 queries (facturado del mes + cobros + vencido); las
...
// Los cobros completados del periodo, por fecha de cobro. Mismo corte que el
// Panel operativo y que el listado de Cobros. Tolera que la tabla no exista
// (schema parcial): devuelve null y la portada pierde esa cifra, no las tres.
async function cobrosDelPeriodo(models, { from, to }) {
  ...
  try {
    const filas = await Payment.findAll({
      where: {
        status: "completed",
        // El mes en hora de MADRID (06/09/2026): el literal `'AAAA-MM-DD 00:00:00'`
        // se comparaba en UTC —la sesión de Sequelize va a +00:00— y en verano el
        // mes empezaba a las 02:00; Caja corta a medianoche de Madrid.
        [Op.and]: [
          literal(`(paid_at AT TIME ZONE 'Europe/Madrid') >= '${from}'`),
          literal(`(paid_at AT TIME ZONE 'Europe/Madrid') < '${sumaDias(to, 1)}'`),
        ],
      },
      attributes: [
        [fn("COUNT", col("id")), "n"],
    ...
    return { count: Number(filas[0]?.n || 0), amount: round2(Number(filas[0]?.sum || 0)) };
  } catch (err) {
    if (isMissingRelation(err)) return null;
    throw err;
  }
...
async function buildFinance(models, { from, to, today, admin }) {
  ...
  // Facturado del mes y cobros del mes, en paralelo. El admin lo pide con las
  // cifras canónicas del Resumen (getKpisForPeriod, que además trae costes y
  // márgenes); el resto con una sola agregación, que es lo que se pinta.
  const [facturado, cobros] = await Promise.all([
    admin
      ? getKpisForPeriod({ tenantModels: models, from, to }).then((k) => ({
          billed: k.income.billedBase,
          invoices: k.income.invoiceCount,
        }))
      : Invoice.findAll({
          ...
          billed: round2(Number(rows[0]?.billed || 0)),
          invoices: Number(rows[0]?.n || 0),
        })),
    cobrosDelPeriodo(models, { from, to }),
  ]);

  return {
    month: facturado,
    overdue,
    collected: cobros ? cobros.amount : null,
    collectedCount: cobros ? cobros.count : 0,
  };
}
...
async function buildAgendaHoy(models, { conEquipo, veTodo, miFiltro, tenantHasModule }) {
  const { Booking, EventType, TeamMember, Patient } = models;
  ...
  const lista = async (where, conProfesional) => {
    const filas = await Booking.findAll({
      ...
      include: [
        ...
        ...includeDelPaciente({ Patient, tenantHasModule }),
        ...(conProfesional
    ...
    return filas.map((b) => ({
      id: b.id,
      // El PACIENTE cuando la cita tiene uno, como en la rejilla de la agenda
      // (AV-0208, 18/09/2026: «en inicio en mi agenda salen clientes no mis
      // pacientes del día»). El arreglo de AV-0088 se quedó en el calendario y
      // la portada siguió diciendo el nombre de quien paga.
      clientName: nombreDeLaCita(b),
...
async function buildTrabajo(models, { hasModule, tenantHasModule, conCitas, conEquipo, miId, miFiltro }) {
  ...
  const [bandeja, proximas, lecturas, tareas] = await Promise.all([
    ...
    (hasModule("clinica") || hasModule("pacientes")) && hasModule("team_avanzado") && tengoFicha
      ? safeBlock(async () => {
          ...
          // `soloPendientes` (04/09/2026): las dadas por vistas no cuentan
          // en «Mi trabajo» — ver lib/clinica/vistoIncidencia.js.
          const misIncidencias = await whereIncidenciasDe(models, miId, { soloPendientes: true });
          const [informes, vencidos, incRows, incTotal] = await Promise.all([
            ...
            models.Incidencia.findAll({
              ...
              // Con el paciente (07/09/2026, AV-0052 de Aumenta): aquí es donde
              // la responsable ve la incidencia antes que en la lista.
              include: models.Patient
                ? [{ model: models.Patient, as: "patient", attributes: ["firstName", "lastName"], required: false }]
                : [],
          ...
          return {
            ...
            incidencias: incRows.map((r) => ({
              id: r.id,
              titulo: r.title,
              paciente: nombrePaciente(r.patient) || null,
              estado: r.status,
    ...
    conCitas && conEquipo && tengoFicha
      ? safeBlock(async () => {
          ...
          const filas = await models.Booking.findAll({
            ...
            include: [
              { model: models.EventType, as: "eventType", attributes: ["name"], required: false },
              ...includeDelPaciente({ Patient: models.Patient, tenantHasModule }),
            ],
            order: [["scheduledAt", "ASC"]],
            limit: 8,
          });
          ...
          return filas.map((b) => ({
            id: b.id,
            // El paciente, igual que en «Mi agenda» (AV-0208).
            clientName: nombreDeLaCita(b),
            scheduledAt: b.scheduledAt,
            status: b.status,
            tipo: b.eventType?.name || null,
...
async function buildSoporte(models) {
  ...
  return tarjeta("soporte", "Soporte", "/soporte", {
    ...
    lista: ultimos.map((t) => ({
      ...
      // /soporte no tiene página por id: el ticket se abre con el deep-link
      // ?ticket= que ya leen la campana y los correos (SupportModule.jsx).
      href: `/soporte?ticket=${t.id}`,
    })),
...
export async function buildPortada(ctx) {
  ...
  const [finance, agenda, vFact, vIngresos, vSemana, vProf, vOcupacion, vEmbudo, vAltas, vSesiones, vMatriculas, trabajo] =
    await Promise.all([
      hasModule("billing")
        ...
        : null,
      conCitas ? safeBlock(() => buildAgendaHoy(tenantModels, { conEquipo, veTodo, miFiltro, tenantHasModule })) : null,
      ...
      sinGraficas
        ? safeBlock(() => buildTrabajo(tenantModels, { hasModule, tenantHasModule, conCitas, conEquipo, miId, miFiltro }))
        : null,
    ]);
  ...
  /*
   * Informes vencidos: SOLO LOS MÍOS salvo para quien DIRIGE
   * (02/09/2026, AV-0022 de Aumenta; afinado el 18/09/2026 con AV-0191).
   * Nació contando los de todo el centro para todo el mundo, igual que las
   * incidencias de abajo, y por el mismo motivo se corrige: un aviso solo es
   * un aviso si es para ti. Aquel día las coordinadoras se quedaron DENTRO
   * del total, y una de ellas escribió pidiendo justo lo contrario: coordinar
   * es poder MIRAR el trabajo de otra (la Bandeja), no que te avisen de él.
   * Ahora la pregunta es la de al lado, `avisaDelPendienteDeTodos`, la misma
   * que ya usaban las incidencias; sin ficha de equipo no hay informes
   * propios y la tarjeta no sale.
   */
  if (hasModule("clinica")) {
    const todoElCentro = avisaDelPendienteDeTodos({ role: user.role });
  ...
  /*
   * Registros sin escribir (15/09/2026, AV-0138 de Aumenta): «que salgan
   * registros pendientes directamente en inicio al igual que salen las
   * incidencias». Los MÍOS y de esta semana, con la misma regla que Equipo →
   * Bandeja (`citasSinRegistro`, lib/clinica/loMio.js) y sus mismos gates:
   * clínica o pacientes + equipo avanzado + ficha propia. Cuenta los sin
   * empezar y los a medias; el detalle, en la Bandeja.
   */
  if ((hasModule("clinica") || hasModule("pacientes")) && hasModule("team_avanzado") && miId !== NADIE && tenantModels.ClinicSession) {
    const n = await safeBlock(async () => {
      const ventana = ventanaDeLaSemana(new Date());
      const citas = await tenantModels.Booking.findAll({
        where: {
          teamMemberId: miId,
        ...
        attributes: ["id", "patientId", "teamMemberId", "scheduledAt", "status"],
        raw: true,
        limit: 600,
      });
      if (!citas.length) return 0;
      const sesiones = await tenantModels.ClinicSession.findAll({
        where: {
          patientId: { [Op.in]: [...new Set(citas.map((c) => c.patientId))] },
      ...
      // Las citas enlazadas desde un registro que siguen existiendo (AV-0140).
      const enlazadas = [...new Set(sesiones.map((s) => s.bookingId).filter(Boolean))];
      const vivas = enlazadas.length
        ? await tenantModels.Booking.findAll({ where: { id: { [Op.in]: enlazadas } }, attributes: ["id"], raw: true })
        : [];
      // Los días que el centro cerró (16/09/2026, AV-0165): la portada tiene
      // que contar EXACTAMENTE lo que lista la Bandeja, o el número de arriba
      // manda a una lista donde falta uno y nadie sabe cuál.
      const diasCerrados = await cargarFestivos(tenantModels, { desde: ventana.desdeDia, hasta: ventana.hastaDia });
      const r = citasSinRegistro(citas, sesiones, { ahora: new Date(), citasQueExisten: new Set(vivas.map((b) => String(b.id))), diasCerrados });
      return r.sinEmpezar.length + r.aMedias.length;
    });
    if (n > 0)
      pendiente.push({
        key: "registros",
        count: n,
        titulo: n === 1 ? "Registro sin escribir" : "Registros sin escribir",
        modulo: "Clínica",
        href: "/equipo/bandeja",
        tono: "rojo",
      });
  ...
  if ((hasModule("clinica") || hasModule("pacientes")) && hasModule("team_avanzado") && (admin || miId !== NADIE)) {
    const n = await safeBlock(async () => {
      const dequien = admin ? {} : await whereIncidenciasDe(tenantModels, miId, { soloPendientes: true });
