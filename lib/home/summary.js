...
// que es una magnitud sensible reservada a admin.
//
// ── COBRADO SON LOS COBROS, NO LAS FACTURAS (04/09/2026, Rodrigo) ──────────
// La tercera cifra, «Cobrado» (solo admin), salía de `getKpisForPeriod`: el
// `paid_amount` de las facturas EMITIDAS en el mes. En Aumenta se cobra la
// cuota primero y se factura al cierre («Facturar el mes»), así que en el mes
// vivo no hay ni una factura: la portada enseñaba 0 € facturado, 0 € cobrado y
// 0 € vencido —tres ceros clavados todo el mes— mientras el Panel operativo
// contaba 14.701 € en 84 cobros. Es el mismo arreglo que se le hizo al panel
// el 03/09 y por el mismo motivo: lo cobrado no depende de lo facturado. Aquí
// «Cobrado» son los cobros COMPLETADOS con fecha de cobro dentro del mes,
// tengan factura o no, y el pie dice cuántos son y no un % sobre lo facturado
// (ese % se iba por encima de 100 o daba 0 en cuanto se cobraba antes de
// facturar). Las dos pantallas leen ya lo mismo.
//
...
async function buildFinance(models, { from, to, today, admin }) {
  ...
  // Admin: facturado en base imponible con las cifras canónicas del Resumen
  // (getKpisForPeriod) y cobrado desde los COBROS del mes (ver cabecera).
  const [kpis, cobros] = await Promise.all([
    getKpisForPeriod({ tenantModels: models, from, to }),
