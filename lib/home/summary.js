...
export async function buildPortada(ctx) {
  ...
  // MIS incidencias abiertas (01/09/2026, Rodrigo). Nació el 31/08 contando
  // las de TODO el centro y avisaba a cada uno de incidencias que no le tocan:
  // un aviso solo es un aviso si es para ti. Ahora solo las suyas, con la
  // MISMA regla de la pivote que ya usan la campana, Equipo → Bandeja y Mi
  // trabajo (lib/clinica/incidenciasDe.js) — quien no tenga ficha de equipo no
  // tiene incidencias propias y no ve la tarjeta. El listado de
  // /equipo/incidencias sigue enseñándolas todas: ahí se va a mirar, aquí a
  // que te avisen. Mismos gates que /api/clinica/incidencias (clínica o
  // pacientes + equipo avanzado). Ojo: Pendiente corta en 6 tarjetas — esta
  // entra detrás de informes y facturas.
  if ((hasModule("clinica") || hasModule("pacientes")) && hasModule("team_avanzado") && miId !== NADIE) {
    const n = await safeBlock(async () => {
      const mias = await whereIncidenciasDe(tenantModels, miId);
      return tenantModels.Incidencia.count({ where: { ...mias, status: { [Op.ne]: "resolved" } } });
    });
    if (n > 0)
      pendiente.push({
        key: "incidencias",
