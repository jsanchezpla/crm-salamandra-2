 ...
 *  - TENANT-WIDE: todos los widgets muestran datos de todo el negocio (hoy
 *    Booking/CalendarTask no tienen FK a usuario, así que "lo mío" es inviable
...
// ─── Bloque: Agenda (citas) ─────────────────────────────────────────────────
async function buildAgenda(models) {
  ...
  const [todayCount, upcoming] = await Promise.all([
    ...
    Booking.findAll({
      where: { scheduledAt: { [Op.gte]: now }, status: { [Op.notIn]: ["cancelled", "no_show"] } },
...
export async function buildHomeSummary(ctx) {
  const { hasModule, tenantModels, user } = ctx;
  ...
  add("agenda", hasModule("citas"), () => buildAgenda(tenantModels));
  ...
  add("leads", hasModule("leads") || hasModule("sales"), () => buildLeads(tenantModels));
  ...
  const blocks = {};
  for (const r of results) if (r.data) blocks[r.key] = r.data;
  return { blocks, admin };
}
