...
/**
 * Clave con la que se busca a una persona: sin espacios y en minúsculas.
 *
 * 21/08/2026 — `UUID_RE` lleva `/i` a propósito, pero las búsquedas contra el
 * equipo eran exactas (`teamById.has(id)`). El modelo copia los ids del prompt
 * y no garantiza la caja: el mismo uuid en mayúsculas pasaba la regex y luego
 * no casaba con nadie. En `assigneeIds` de un `createTask` el fallo es MUDO
 * —la tarea se crea sin asignar y no hay warning—, así que se busca por
 * minúsculas y se guarda SIEMPRE el id del equipo, no el que escribió el
 * modelo: lo que acaba en la FK tiene que ser el valor de la base.
 *
 * Sólo para ids de PERSONA. Los de fase, tarea y columna se siguen buscando
 * exactos: allí un id que no case ya sale con su aviso («la fase indicada no
 * existe en el proyecto»), que es visible y se puede corregir.
 */
function clavePersona(valor) {
  return typeof valor === "string" ? valor.trim().toLowerCase() : "";
}

function nombresDeEquipo(ids, teamById) {
  const nombres = ids.map((id) => teamById.get(clavePersona(id))?.name).filter(Boolean);
...
export function normalizeOperations(raw, snapshot) {
  ...
  // Los dos índices de personas, por `clavePersona` (ver arriba). Son listas
  // DISTINTAS y no intercambiables: `team` es la plantilla ACTIVA del tenant
  // (`loadProjectSnapshot` la carga con `status: "active"`) y `members` son los
  // miembros del proyecto, den de alta o no. Quien causa baja sale de `team` y
  // sigue en `members`.
  // La clave vacía se descarta, o un `teamMemberId` ausente casaría con quien
  // tuviera un id que no es texto.
  const indexar = (filas, sacaId) => {
    const m = new Map();
    for (const fila of filas ?? []) {
      const clave = clavePersona(sacaId(fila ?? {}));
      if (clave) m.set(clave, fila);
    }
    return m;
  };
  const teamById = indexar(snapshot.team, (m) => m.id);
  const memberById = indexar(snapshot.members, (m) => m.teamMemberId);
  ...
  const filtraAsignados = (value) => {
    ...
    for (const rawId of value) {
      ...
      // El id que se guarda es el del equipo, no el que escribió el modelo.
      const canonico = teamById.get(clavePersona(id))?.id;
      if (!canonico || out.includes(canonico)) continue;
      out.push(canonico);
    }
    return out;
  };
  ...
  const ponAsignados = (sujeto, changes, c) => {
    ...
    const desconocidos = c.assigneeIds.filter(
      (id) => typeof id !== "string" || !teamById.has(clavePersona(id))
    ).length;
  ...
  for (const rawOp of list) {
    ...
    switch (rawOp.op) {
      ...
      case "addMember": {
        const tm = teamById.get(clavePersona(rawOp.teamMemberId));
        if (!tm) {
          descarta(rawOp, "la persona indicada no existe en el equipo");
          break;
        }
        if (memberById.has(clavePersona(tm.id))) {
      ...
      case "removeMember": {
        // Se comprueba contra `members` (el proyecto) y NO contra `team` (la
        // plantilla activa). Hasta el 21/08/2026 se exigían las dos, y eso
        // dejaba fuera justo el caso corriente: alguien causa baja en el
        // equipo, `loadProjectSnapshot` deja de traerlo en `team` (carga con
        // `status: "active"`) y ya no se le podía sacar de los proyectos en los
        // que sigue siendo miembro — con el agravante de que el motivo, «la
        // persona indicada no es miembro del proyecto», era falso: sí lo era, y
        // el propio snapshot lo llevaba en `members`.
        //
        // Quien no está en `members` sigue descartándose con ese mismo motivo,
        // que ahí sí es cierto.
        const miembro = memberById.get(clavePersona(rawOp.teamMemberId));
        if (!miembro) {
          descarta(rawOp, "la persona indicada no es miembro del proyecto");
          break;
        }
        // El nombre lo trae `members`; sólo se cae al equipo si allí viene
        // vacío (una fila cargada sin el include de TeamMember).
        const tm = teamById.get(clavePersona(miembro.teamMemberId));
        operations.push({
          op: "removeMember",
          teamMemberId: miembro.teamMemberId,
          description: `Quitar a ${miembro.name || tm?.name || "una persona"} del proyecto`,
        });
        break;
      }
