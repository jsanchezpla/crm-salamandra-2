 ...
 */
export function buildProjectSnapshot({ project, phases = [], columns = [], tasks = [], members = [], teamMembers = [] }) {
...
function describirCambios(changes, { teamById } = {}) {
  ...
  if (changes.priority !== undefined) partes.push(`prioridad → ${priorityMeta(changes.priority).label}`);
  if (changes.status !== undefined) partes.push(`estado → ${changes.status}`);
  if (changes.startDate !== undefined) partes.push(`fecha de inicio → ${changes.startDate ?? "sin fecha"}`);
  if (changes.dueDate !== undefined) partes.push(`fecha límite → ${changes.dueDate ?? "sin fecha"}`);
  if (changes.endDate !== undefined) partes.push(`fecha de fin → ${changes.endDate ?? "sin fecha"}`);
  if (changes.estimatedHours !== undefined) partes.push(`horas estimadas → ${changes.estimatedHours ?? "sin estimar"}`);
  ...
  if (changes.assigneeIds !== undefined) {
    ...
    partes.push(changes.assigneeIds.length === 0 ? "sin asignados" : `asignados → ${nombres || `${changes.assigneeIds.length} personas`}`);
  }
  return partes.join(", ");
}
 ...
 * canónica + un campo `description` legible para la vista previa.
 */
  ...
  for (const rawOp of list) {
    ...
    switch (rawOp.op) {
      case "updateProject": {
        ...
        if (c.description !== undefined) changes.description = toText(c.description) || null;
        if (c.priority !== undefined && PRIORITY_VALUES.includes(c.priority)) changes.priority = c.priority;
        if (c.status !== undefined && STATUS_VALUES.includes(c.status)) changes.status = c.status;
        if (c.startDate !== undefined) changes.startDate = toDateOnly(c.startDate);
        if (c.dueDate !== undefined) changes.dueDate = toDateOnly(c.dueDate);
      ...
      case "createPhase": {
        ...
        operations.push({
          ...
          name,
          phaseDescription:
            toText("phaseDescription" in rawOp ? rawOp.phaseDescription : rawOp.description) || null,
          startDate,
          endDate,
      ...
      case "updatePhase": {
        ...
        if (c.description !== undefined) changes.description = toText(c.description) || null;
        if (c.startDate !== undefined) changes.startDate = toDateOnly(c.startDate);
        if (c.endDate !== undefined) changes.endDate = toDateOnly(c.endDate);
        if (Object.keys(changes).length === 0) {
          descarta(rawOp, "no incluye ningún cambio válido");
          break;
        }
      ...
      case "createTask": {
        ...
        const checklist = Array.isArray(rawOp.checklist)
          ? rawOp.checklist.map((s) => (typeof s === "string" ? s.trim() : "")).filter(Boolean).slice(0, MAX_CHECKLIST)
          : [];
      ...
      case "updateTask": {
        ...
        if (c.priority !== undefined && PRIORITY_VALUES.includes(c.priority)) changes.priority = c.priority;
        if (c.dueDate !== undefined) changes.dueDate = toDateOnly(c.dueDate);
        if (c.estimatedHours !== undefined) changes.estimatedHours = toHours(c.estimatedHours);
        ...
        if (c.assigneeIds !== undefined) changes.assigneeIds = filtraAsignados(c.assigneeIds);
        if (Object.keys(changes).length === 0) {
          descarta(rawOp, "no incluye ningún cambio válido");
          break;
        }
        ...
        if (changes.boardColumnId !== undefined) {
          detalle = detalle.replace("cambio de columna", `columna → «${columnById.get(changes.boardColumnId).name}»`);
        }
        if (changes.phaseId) {
          detalle = detalle.replace("cambio de fase", `fase → «${phaseById.get(changes.phaseId).name}»`);
        }
        operations.push({
          op: "updateTask",
          taskId: task.id,
