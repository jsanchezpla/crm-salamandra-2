...
/**
 * El JSON que hay dentro de lo que ha escrito el modelo.
 *
 * Se le pide «SOLO un JSON, sin markdown» y casi siempre obedece, pero «casi»
 * no es «siempre»: una respuesta que empieza con «Aquí tienes el plan:» y
 * sigue con una valla ```json era JSON perfectamente válido que el parser
 * tiraba a la basura, y el usuario veía «La IA no ha devuelto un plan
 * válido» sin que la IA hubiera hecho nada mal (01/09/2026).
 *
 * Tres intentos, de menos a más tolerante: el texto tal cual, lo que haya
 * dentro de la primera valla de código, y el primer objeto `{...}` con las
 * llaves equilibradas — contando llaves y saltando lo que va entre comillas,
 * porque una `}` dentro de una descripción no cierra nada.
 */
export function extraerJson(text) {
  ...
  const enValla = intentar(valla?.[1]?.trim());
  if (enValla !== null) return enValla;

  return intentar(primerObjeto(t));
}

/** El primer `{...}` con las llaves equilibradas, o "" si no hay ninguno. */
function primerObjeto(t) {
  ...
  for (let i = desde; i < t.length; i++) {
    ...
    if (c === '"') enCadena = true;
      ...
      nivel--;
      if (nivel === 0) return t.slice(desde, i + 1);
    }
  }
  return "";
}
...
export function normalizePlan(raw, { teamMembers = [] } = {}) {
  ...
  if (typeof raw === "string") {
    obj = extraerJson(raw);
    if (obj === null) throw new ValidationError(INVALID_PLAN_MSG);
  }
