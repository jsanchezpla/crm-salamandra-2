...
export async function puedeReservarValoracionInicial(tenantModels, email) {
  ...
  try {
    ...
    /*
     * CUALQUIER cita gasta la valoración, no solo otra valoración
     * (06/08/2026, Rodrigo: «una vez pidas valoración inicial o te decantes
     * directamente por pedir cualquier otro tipo de cita, la valoración inicial
     * dejará de estar disponible»).
     *
     * Es coherente con lo que la valoración ES: una consulta para conocerse
     * antes de decidir si se empieza. Quien ya ha reservado un acompañamiento
     * —con sus documentos firmados, porque sin eso no llega— ya decidió; darle
     * después una primera visita gratuita no tiene sentido, y era un hueco
     * abierto: bastaba pedir primero un seguimiento para conservarla.
     *
     * Sigue contando el estado y no la existencia: una cita CANCELADA no gasta
     * nada (ver la cabecera). Por eso el filtro de `status` es el mismo.
     */
    const citas = await Booking.count({
      where: {
    ...
    if (citas === 0) return { puede: true, motivo: "sin_valoracion" };

    // Se distingue para poder decirle la verdad: no es lo mismo «ya hiciste tu
    // primera visita» que «ya eres paciente».
    ...
    return { puede: false, motivo: yaTuvo > 0 ? "ya_la_tuvo" : "ya_tiene_citas" };
  } catch (err) {
...
export function mensajeValoracionUsada(nombreCentro = null, motivo = "ya_la_tuvo") {
  const quien = nombreCentro ? ` de ${nombreCentro}` : "";
  // Dos motivos distintos y dos frases distintas: a quien ya es paciente
  // decirle «ya has hecho tu primera visita» cuando nunca la hizo le suena a
  // error del sistema, y escribe para preguntar.
  if (motivo === "ya_tiene_citas") {
    return {
      codigo: "VALORACION_YA_REALIZADA",
      titulo: "Esto es para la primera visita",
      texto:
        `La valoración inicial es la consulta para conoceros antes de empezar, y tú ya tienes cita ` +
        `con el equipo${quien}. Elige cualquiera de los otros tipos.`,
    };
  }
  return {
    codigo: "VALORACION_YA_REALIZADA",
