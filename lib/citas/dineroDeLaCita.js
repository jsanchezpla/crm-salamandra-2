...
/**
 * Los tres modos que elige quien apunta la cita, y un cuarto que solo escribe
 * el servidor: `bono` (07/09/2026, AV-0055 de Aumenta) — la sesión sale de un
 * bono de sesiones ya pagado (`lib/citas/packs.js`, `cobroDeBono`), así que
 * no hay nada que cobrar ni nada que decidir. No se acepta desde el navegador:
 * ahí lo que se manda es `packId`, y es el servidor quien comprueba que ese
 * bono es de esa familia y le quedan sesiones.
 */
export const MODOS_COBRO = Object.freeze(["cuota", "libre", "sin_coste", "bono"]);
...
export function normalizarCobro(bruto, { concepto = null, exigido = false, porDefecto = null } = {}) {
  ...
  // El bono no se «elige» como modo: se manda el bono (`packId`) y lo pone el
  // servidor. Aceptarlo aquí dejaría marcar cualquier cita como ya pagada.
  if (modo === "bono") return { error: "Para que la cita salga de un bono, elige el bono en la cita" };
...
export function resumenCobro(booking) {
  ...
  // `cobroTexto` ya dice «Bono «X» · sesión 3 de 5» (packs.js, cobroDeBono).
  if (b.cobroModo === "bono") return t || "Bono de sesiones";
