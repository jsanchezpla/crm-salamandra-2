 ...
 *   serie). Desde el 13/09/2026 la huella identifica la cita por `clientId`/
 *   `patientId`/`tallerGrupoId` y SIN el nombre, porque master no duplica datos
 *   personales (`resumenDeCita.js`); una reserva pública de alguien sin ficha
 *   queda sin «de quién».
 */

import { Op } from "sequelize";
import { huellaDeCita } from "./resumenDeCita.js";
...
// La huella de la cita borrada vive en `resumenDeCita.js` desde el 13/09/2026
// (sin el nombre); se re-exporta para quien la importaba de aquí.
export { huellaDeCita };
