 ...
 *
 * (13/09/2026, regla #2: `buildCandidates` descuenta también los bloqueos de la
 * agenda y busca hueco con la duración de la cita; antes proponía horas dentro
 * de un bloqueo. `chooseSlots` no cambia.)
 *
 * (13/09/2026, regla #2: si la IA falla, `chooseSlots` devuelve los tres huecos
 * repartidos sin IA con `avisoIA`, el motivo legible. Antes el error subía a la
 * ruta, que lo tapaba con huecos sin decir nada.)
 */
import { generateSlotsForDay, getMadridDayOfWeek, getMadridParts } from "./slots.js";
import { bloqueosComoOcupacion } from "./choqueConBloqueos.js";
...
/**
 * Genera candidatos VÁLIDOS: horario propio de cada profesional − sus citas −
 * festivos − bloqueos de la agenda (los del centro y los suyos), con la rejilla
 * del TIPO y la duración de la CITA. `members` = [{ id, name,
 * hours:[{dayOfWeek,startTime,endTime}], bookings:[{scheduledAt,duration}] }].
 * `centerAvailabilities` es el fallback (Availability global) para profesionales
 * sin horario propio configurado.
 *
 * (13/09/2026) `bloqueos` son filas de `team_blocks` ({ teamMemberId, startAt,
 * endAt }) de todo el equipo: a cada profesional le cuentan las suyas y las del
 * centro, como una cita más que ocupa su tramo (por qué ocupación y no resta:
 * `choqueConBloqueos.js`). `duracion` son los minutos de la cita que se
 * reprograma; `null` = los del tipo. Sin `bloqueos` ni `duracion` sale lo mismo
 * que antes.
 *
 * Devuelve [{ slotId, datetime, teamMemberId, teamMemberName, label }].
 */
export function buildCandidates({ eventType, duracion = null, members, horizonDays = 14, now = new Date(), centerAvailabilities = [], blockedDates = null, bloqueos = [] }) {
  const out = [];
  // La ocupación de cada profesional se calcula UNA vez, no por día.
  const ocupacion = new Map(
    members.map((m) => [m.id, [...(m.bookings || []), ...bloqueosComoOcupacion(bloqueos, m.id)]]),
  );
  for (let i = 1; i <= horizonDays; i++) {
    ...
    for (const m of members) {
      ...
      const slots = generateSlotsForDay({
        ...
        date,
        existingBookings: ocupacion.get(m.id),
        now,
        blockedDates,
 ...
 * y `avisoIA` cuando la IA ha fallado y los tres salen sin ella.
 */
export async function chooseSlots({ candidates, context, apiKey, model, forceFake = false }) {
  ...
  let raw;
  try {
    raw = await complete({ system, user, model, maxTokens: 700, apiKey });
  } catch (e) {
    // El aviso a dirección, si es la cuenta, ya ha salido del cliente central.
    console.error("[citas:suggest-slots] la IA no ha respondido:", e?.name, e?.status, e?.message);
    return {
      model: "sin-ia",
      suggestions: spread(candidates, 3).map((c) => ({ slotId: c.slotId, reason: `Hueco libre: ${c.label}.` })),
      avisoIA: avisoSinIa(e, "Mientras tanto, estos son tres huecos libres elegidos sin IA."),
    };
  }
  let parsed;
  try {
