 ...
 *
 * (13/09/2026, regla #2: `generateSlotsForDay` acepta `duracionCita`, opcional.
 * Lo usa solo «Proponer 3 horarios» para buscar hueco a una cita que no dura lo
 * que su tipo: la rejilla sigue siendo la del tipo y solo la longitud es la de
 * la cita. Sin él —widget público, alta— sale lo mismo que antes.)
 */
...
/**
 * Una duración solo vale si es un entero ≥ 1. Con 0, null o negativa el bucle
 * `m += duration` no avanzaría y el generador NO TERMINARÍA NUNCA (colgaría el
 * proceso de Node entero); con texto (`"abc"`, `"60"`) no sale ni un hueco. Lo
 * frena también el modelo EventType (INTEGER, min 1, max 480), pero una lib no
 * puede colgar el proceso por un dato: `generateSlotsForDay` devuelve [] y
 * `dayHasAnySlot` false, y así los dos dicen lo mismo (antes, con 0 y un hueco
 * libre, el mes pintaba en verde un día cuyo detalle colgaría).
 * (19/08/2026, lo sacó `_smoke-citas-slots.mjs`.)
 */
function _duracionValida(duration) {
...
export function generateSlotsForDay({
  ...
  duracionCita = null, // minutos de consulta de UNA cita concreta; null = los del tipo
}) {
  ...
  // Sin una duración válida no hay huecos (y no se entra en el bucle): ver
  // `_duracionValida`.
  if (!_duracionValida(duration)) return [];
  ...
  for (const av of availabilities) {
    ...
    const contactoTipo = duracionDeContacto(eventType);
    /*
     * La REJILLA es del tipo; la LONGITUD puede ser la de una cita concreta
     * (13/09/2026, «Proponer 3 horarios»). Una cita ya dada no siempre dura lo
     * que su tipo —un diagnóstico de 180 minutos con un tipo de 60; en Aumenta,
     * 152 citas futuras—. Si el paso fuera la duración de la cita, la agenda se
     * correría (una de 45 recibiría 9:00, 9:45, 10:30); por eso el bloque sigue
     * avanzando de `duration` en `duration` —la cabecera: quien pone 60 quiere
     * que la siguiente empiece 60 después— y solo cambia lo que se comprueba
     * libre y lo que tiene que caber antes del cierre:
     *
     *     tipo 60, cita 180, horario 9-14   → 9:00, 10:00, 11:00
     *     tipo 60, cita 45                  → 9:00, 10:00, … (nada a las 9:45)
     *
     * `largo` es lo que ocupa el bloque desde que arranca: con la cita del tipo
     * es `duration` (idéntico a antes); con una más larga, el previo + la cita +
     * el descanso posterior del tipo. Sin `duracionCita` (el widget público, el
     * alta), todo sale exactamente igual que siempre.
     */
    const propia = Number.isInteger(duracionCita) && duracionCita > 0 ? duracionCita : null;
    const contacto = propia ?? contactoTipo;
    const largo = Math.max(duration, desfase + contacto + (duration - desfase - contactoTipo));
    for (let m = startMin; m + largo <= endMin; m += duration) {
      ...
      // del tenant — el calendario es único). «Proponer 3 horarios» mete aquí
      // también los bloqueos de la agenda, como si fueran citas
      // (lib/citas/choqueConBloqueos.js `bloqueosComoOcupacion`).
      let overlaps = false;
...
export function dayHasAnySlot({
  ...
  const duration = eventType.duration;
  // Misma defensa que el generador: sin duración válida, false (no true «al
  // primer hueco libre», que pintaría el mes en verde). Ver `_duracionValida`.
  if (!_duracionValida(duration)) return false;
  const minNoticeMs = (eventType.minNoticeHours ?? 0) * 60 * 60 * 1000;
  const noticeBoundary = new Date(now.getTime() + minNoticeMs);
