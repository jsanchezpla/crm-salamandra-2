 ...
 * NO HAY HUECOS. Ese día no se le ofrece nada.
 *
 * ⚠️ ESTO CAMBIÓ EL 07/08/2026 Y ANTES ERA AL REVÉS. Nació devolviendo la
 * agenda del centro entera «por si acaso»: casi nadie tenía su horario metido y
 * daba miedo dejar a una paciente sin poder pedir cita. El resultado real fue
 * peor, y lo vio Rodrigo: en nutri_laura solo UNA de las dos nutricionistas
 * tenía horario, así que la paciente asignada a la otra veía —y podía
 * reservar— las tardes en las que solo trabaja su compañera. Le estábamos
 * dando cita con quien no le corresponde.
 *
 * Entre «no le ofrezco nada» y «le ofrezco la hora de otra», lo primero se
 * arregla rellenando un horario y lo segundo se arregla llamando a la paciente
 * para deshacer una cita que nunca debió existir.
 *
 * Quien no tiene profesional asignada sigue viendo la agenda del centro: esto
 * solo aplica a quien SÍ la tiene. Y la pantalla de asignar avisa cuando la
 * elegida no tiene horario, para que no se descubra tarde.
 */
 ...
 *
 * Sin horario suyo ese día devuelve `[]`: ver la cabecera.
 */
export function recortarAlHorario(disponibilidades, horasProfesional, dayOfWeek) {
  ...
  // Sin horario suyo ESE DÍA no hay huecos: ver la cabecera. Un día que ella no
  // trabaja es un día sin citas, aunque el centro esté abierto.
