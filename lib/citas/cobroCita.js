...
/**
 * ¿Estorba algo para pedirle OTRA tarjeta al paciente?
 *
 * ── POR QUÉ NO VALE `tieneRetencionPendiente` ────────────────────────────────
 * Esa lista es ancha A PROPÓSITO y así se queda: sus otros cuatro consumidores
 * —confirmar la cita, pasarla a confirmada o completada, y el reembolso— quieren
 * exactamente eso, «hay que preguntarle a Stripe qué pasa con este dinero».
 *
 * Pero este botón no necesita saber si HAY QUE PREGUNTAR: necesita la respuesta.
 * Usando la lista ancha, toda cita `failed` quedaba cortada con un «ya tiene
 * dinero reservado, confírmala para cobrarlo» que era un callejón sin salida —de
 * las tres salidas del dinero perdido (reintentar, pedir otra tarjeta, rechazar)
 * desaparecía la del medio, justo la que el aviso de la pantalla recomienda—. Y
 * el propio endpoint tenía escrito, catorce líneas más abajo, el camino de la
 * tarjeta rechazada con su palabra propia («rechazada»), inalcanzable.
 *
 * Así que aquí se distingue, y solo aquí:
 *   · `authorized` / `capturing` → sí estorba, y no es un caso de dinero
 *     perdido: hay una retención sana. Lo que toca es confirmar y cobrar.
 *   · `failed` → se le PREGUNTA a Stripe. Un rechazo del banco no garantiza que
 *     la retención haya muerto; si sigue viva y creáramos otra, el paciente se
 *     quedaría con dos importes bloqueados a la vez.
 *   · lo demás → no estorba nada.
 *
 * ⚠️ Si no se puede preguntar, ESTORBA. «No lo sé» nunca es vía libre aquí: el
 * precio de equivocarse lo paga el paciente en su cuenta corriente.
 *
 * El CRM no suelta el dinero por su cuenta (decisión de Rodrigo, 13/08/2026):
 * dice lo que pasa y decide la profesional. Es la misma política por la que en
 * `reembolsoCita.js` se borró el código de devoluciones automáticas.
 *
 * @returns {Promise<{estorba: boolean, mensaje: string|null}>}
 */
export async function estorbaParaPedirOtraTarjeta(ctx, booking) {
  const estado = booking?.paymentStatus;

  if (estado === "authorized" || estado === "capturing") {
    return {
      estorba: true,
      mensaje: "Esta cita ya tiene dinero reservado en la tarjeta del paciente. Confírmala para cobrarlo.",
    };
  }

  if (estado !== "failed") return { estorba: false, mensaje: null };
  ...
  // Sin cobro que mirar no hay retención que pueda duplicarse.
  if (!ps) return { estorba: false, mensaje: null };

  const retencion = await leerEstadoAutorizacion(ctx, ps);

  if (!retencion.sePudoPreguntar) {
    return {
      estorba: true,
      mensaje:
        "No se ha podido comprobar si el cobro anterior sigue vivo. Inténtalo dentro de un momento: " +
        "pedirle otra tarjeta sin saberlo podría dejarle el importe bloqueado dos veces.",
    };
  }
  ...
  if (retencion.viva) {
    return {
      estorba: true,
      mensaje:
        "El paciente todavía tiene el importe retenido del intento anterior. Suéltalo antes de pedirle " +
        "otra tarjeta: recházala, o confírmala sin cobrar si le vas a cobrar en consulta. Si no, se " +
        "quedaría con dos importes bloqueados a la vez.",
    };
  }

  return { estorba: false, mensaje: null };
}
