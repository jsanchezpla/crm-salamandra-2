...
import { camposQueFaltan } from "../clients/datosFicha.js";
...
export async function estadoContrato(tenantModels, client, guardian) {
  ...
  return {
    ...
    // Los datos de la FICHA que faltan para poder firmar. Se piden en su propia
    // pantalla, ANTES del contrato: así se guardan donde tienen que estar (la
    // ficha) y la fecha de nacimiento se sabe antes de decidir si además hace
    // falta el consentimiento del tutor.
    datosPendientes: siguiente ? camposQueFaltan(camposDe(siguiente), client) : [],
