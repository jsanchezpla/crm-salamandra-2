...
/**
 * Plantillas estructuradas activas del centro, EN EL ORDEN EN QUE SE FIRMAN.
 *
 * ── EL CONSENTIMIENTO PARENTAL VA PRIMERO (06/08/2026, Rodrigo) ─────────────
 * `onlyMinors DESC`. Antes iba detrás del contrato y el orden se notaba en dos
 * sitios, los dos mal:
 *
 *   · El contrato le pedía a la menor SU DNI —que muchas no tienen— antes de
 *     que nadie hubiera declarado el del tutor. Poniendo delante el
 *     consentimiento parental, cuando llega el contrato el DNI de quien
 *     autoriza ya está dado y la casilla desaparece.
 *   · Y el orden lógico es ese: primero autoriza quien tiene la patria
 *     potestad, y luego se firma lo que ha autorizado. Al revés, la menor
 *     firmaba un contrato que todavía no estaba consentido por nadie.
 *
 * Para quien es mayor de edad no cambia nada: el consentimiento parental solo
 * entra en la lista si su fecha de nacimiento dice que es menor (`documentosPara`
 * en lib/clients/contratoFirma.js), así que sigue viendo un solo documento.
 *
 * La fecha de nacimiento se pregunta ANTES que todo esto, en «Completa tus
 * datos», que es lo que hace posible saberlo a tiempo.
 */
...
/**
 * Qué le falta a la FICHA de esta persona, mirando todos sus documentos, y en
 * qué momento se le pide.
 *
 * ── POR QUÉ ESTO ES UNA FUNCIÓN Y NO DOS TROZOS DE CÓDIGO (06/08/2026) ──────
 * La pantalla («Completa tus datos») y el endpoint que guarda lo que rellena
 * calculaban esto por su cuenta y con universos distintos: la pantalla sobre
 * TODAS las plantillas y el servidor solo sobre la que tocaba firmar. Resultado:
 * el servidor exigía campos que la pantalla no había preguntado y el botón
 * «Continuar» no se dejaba pulsar nunca, quejándose de un DNI que no estaba en
 * ninguna casilla.
 *
 * ── EL DEDUPLICADO, Y POR QUÉ IMPORTA CUÁL GANA ─────────────────────────────
 * El contrato y el consentimiento parental apuntan los dos a huecos de la misma
 * ficha (`cliente.name`, `cliente.birthDate`, `cliente.taxId`). Preguntar dos
 * veces por el mismo hueco no tiene sentido, pero elegir mal cuál de las dos
 * definiciones manda tampoco es inocuo: en el consentimiento parental los datos
 * de la paciente son una COPIA que se enseña ya rellena, con sus propias reglas
 * —ahí su DNI es opcional y su fecha de nacimiento no es previa—, mientras que
 * en SU contrato son los de verdad.
 *
 * Cuando el consentimiento parental pasó a ir primero, esa copia empezó a ganar
 * el deduplicado por orden de aparición y se llevó por delante dos cosas: la
 * fecha de nacimiento dejó de pedirse ANTES de firmar (que es lo único que
 * decide si hace falta el consentimiento del tutor) y el DNI dejó de ser un
 * hueco. Por eso ahora **manda la definición del documento que NO es solo para
 * menores**, y el orden de las plantillas ya no cambia nada.
 */
export function huecosDeFicha(plantillas, client) {
  const porFicha = new Map();
  for (const plantilla of plantillas ?? []) {
    const soloMenores = !!(plantilla?.onlyMinors ?? plantilla?.get?.("onlyMinors"));
    for (const campo of camposDe(plantilla)) {
      if (!campo.ficha) continue;
      const anterior = porFicha.get(campo.ficha);
      if (!anterior || (anterior.soloMenores && !soloMenores)) {
        porFicha.set(campo.ficha, { campo, soloMenores });
      }
    }
  }
  const todos = [...porFicha.values()].map((v) => v.campo);
  const huecos = camposQueFaltan(todos, client);
  const { previos, posteriores } = separarPorMomento(huecos);
  return { todos, huecos, previos, posteriores };
}
...
export async function estadoContrato(tenantModels, client, guardian) {
  ...
  /*
   * Quién ha entrado: si la ficha tiene tutores FIRMANTES, el del email de la
   * sesión; si no, el titular (que es con cuyo correo se entra).
   *
   * ── EL CALLEJÓN SIN SALIDA QUE CIERRA EL `??` DEL MEDIO (06/08/2026) ───────
   * Al firmar el consentimiento parental, la familia declara al tutor y este se
   * guarda en la ficha —a propósito con `signer: false`, para que las firmas ya
   * hechas a nombre del titular sigan valiendo—. Pero el tutor casi siempre trae
   * el MISMO correo que la ficha: es la madre rellenándolo desde la cuenta de su
   * hija, o la cuenta de la familia.
   *
   * Y entonces `resolvePortalAccess` devolvía a esa persona como «tutor», su id
   * no estaba entre los firmantes (porque no firma), y `firmante` se quedaba a
   * NULL. Con eso, `requiereFirma` es false —así que la pantalla del contrato ya
   * no sale— pero `bloqueado` sigue siendo true: la paciente se queda mirando
   * «tienes el contrato pendiente de firmar» sin ningún botón para firmarlo, y
   * sin forma de volver atrás. Encerrada, y sin que nadie pueda sacarla salvo a
   * mano desde el CRM.
   *
   * Entrar con el correo de la ficha te identifica como su titular. Que además
   * aparezcas apuntado como tutor no te puede quitar eso.
   */
  const firmante = guardian
    ? firmantes.find((f) => f.id.toLowerCase() === String(guardian.id).toLowerCase()) ??
      firmantes.find((f) => f.titular) ??
      null
  ...
  return {
    ...
    ...(() => {
      const { previos, posteriores } = huecosDeFicha(plantillas, client);
