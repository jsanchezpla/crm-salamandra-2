...
/**
 * DE QUIÉN ES EL BLOQUEO QUE SE ABRE DESDE EL CALENDARIO (18/09/2026, Aumenta:
 * «si se está en el horario de una terapeuta que se ponga automáticamente como
 * la dueña en el desplegable Quién»).
 *
 * El bloqueo rápido nacía siempre a nombre de QUIEN MIRA. En Aumenta la agenda
 * la coloca administración, que no atiende a nadie: cada tramo que cerraban
 * mirando la agenda de una terapeuta se guardaba en la suya propia —un hueco
 * que seguía ofreciéndose y un bloqueo en una agenda vacía—, salvo que se
 * acordaran de bajar el desplegable.
 *
 * La pista es el FILTRO: si en pantalla hay exactamente una profesional, el
 * hueco pulsado es suyo. Con dos o más, o con «Sin asignar» de por medio, no
 * hay una respuesta cierta y no se adivina: se deja a quien mira, como hasta
 * hoy. Es una sugerencia, no una valla — el desplegable sigue ahí y el servidor
 * impone sus permisos igual.
 *
 * @param {string[]|null} visibleTmIds El filtro del calendario (`null` = todos)
 * @returns {string|null} El id de la ficha de equipo, o `null` si no se deduce
 */
export function duenoSugeridoDelBloqueo(visibleTmIds) {
  if (!Array.isArray(visibleTmIds) || visibleTmIds.length !== 1) return null;
  const solo = visibleTmIds[0];
  return typeof solo === "string" && UUID_RE.test(solo) ? solo : null;
}
