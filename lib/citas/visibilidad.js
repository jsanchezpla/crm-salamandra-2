...
import { trocearFiltroDeProfesionales } from "./filtros.js";
...
/**
 * El `where.teamMemberId` de quien FILTRA por profesional en el calendario.
 *
 * Ojo con la diferencia, que es la que se confundió hasta el 25/08/2026:
 *
 *   `soloLoSuyo`           → «qué PUEDE ver esta persona» (un permiso)
 *   `filtroDeProfesionales` → «qué HA PEDIDO ver» (una elección)
 *
 * Un permiso puede ser generoso con las citas sin profesional —no son de nadie,
 * no destapan a un paciente ajeno— y por eso `soloLoSuyo` las deja pasar. Una
 * elección no: si has marcado a Araceli, lo que quieres son las de Araceli. El
 * porqué, con los números de producción, en `lib/citas/filtros.js`.
 *
 * @param {string|null} csv  el `teamMemberIds` de la query
 * @returns {object|null}    el fragmento de where, o null si no hay que filtrar
 */
export function filtroDeProfesionales(csv) {
  const filtro = trocearFiltroDeProfesionales(csv);
  if (!filtro) return null;

  const { ids, incluirSinAsignar } = filtro;
  // Solo «Sin asignar»: la cola de reparto, y nada más.
  if (ids.length === 0) return incluirSinAsignar ? { [Op.is]: null } : null;
  // Personas y además las que no son de nadie.
  if (incluirSinAsignar) return { [Op.or]: [{ [Op.in]: ids }, { [Op.is]: null }] };
  // Personas y SOLO personas: lo que se pidió y nada más.
  return { [Op.in]: ids };
}
