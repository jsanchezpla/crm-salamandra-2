 ...
 *
 * La IP se toma con `ipDeConfianza` (la que añade nuestro proxy), no leyendo
 * `X-Forwarded-For` tal cual: esa cabecera la puede rellenar quien llama y una
 * prueba que el interesado puede escribir no prueba nada. Se guarda además la
 * cadena completa que llegó, por si alguna vez hay que enseñar el contexto.
 */
export function pruebaDeConsentimiento({ importeCentimos, ip = null, cadenaProxies = null }) {
  return {
