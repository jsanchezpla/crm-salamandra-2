...
/**
 * ⚠️ TERCERA VEZ QUE ESTA LISTA MUERDE. Sequelize trae SOLO los atributos
 * pedidos, y lo que falte se lee como `undefined` sin dar ningún error: la
 * pantalla no se rompe, simplemente miente.
 *   · `contractDocumentId` → el portal no encontraba el contrato de la familia.
 *   · `communicationPrefs` (01/08) → enseñaba los valores por defecto, daba
 *     igual lo que la familia hubiera marcado.
 *   · `taxId` / `birthDate` / `phone` / `customFields` (04/08) → «Completa tus
 *     datos» volvía a pedir el DNI y la fecha de nacimiento RECIÉN guardados, y
 *     con la fecha invisible el consentimiento parental no aparecía nunca.
 *
 * Los cuatro últimos son los que puede pedir el contrato del área privada
 * (`lib/clients/datosFicha.js`). **Si una plantilla apunta a una columna nueva
 * del cliente, hay que añadirla AQUÍ**, o el portal la verá vacía para siempre.
 */
const ATRIBUTOS = [
  "id",
  "name",
