/**
 * lib/citas/incidenciaPorFalta.js — marcar una falta ABRE UNA INCIDENCIA y
 * avisa a quien lleva la administración del centro (01/09/2026, Rodrigo).
 *
 * ── SE AVISA, PERO YA NO SE LE ASIGNA (18/09/2026, Rodrigo, AV-0197) ────────
 * Olga: «sigo viendo incidencias en las que no estoy etiquetada». No era un
 * fallo del filtro —la bandeja abre en «las mías» desde el 01/09— sino esto:
 * ella es la destinataria de las faltas del centro, así que el CRM se las
 * asignaba solas. Medido el 18/09: de las 128 incidencias en las que figuraba
 * como responsable, **41 se las había puesto el propio CRM**; un tercio de su
 * bandeja era trabajo que ella no se había repartido.
 *
 * Las faltas ya tienen su sitio: desde el 03/09 (AV-0038) viven en la pestaña
 * «Faltas», separadas de las incidencias de siempre. Así que la incidencia
 * nace **sin responsable** y se queda ahí, que es donde se gestiona. Lo que NO
 * cambia es el aviso: a la lista del centro le sigue llegando su campanazo —
 * quitarlo sería que nadie se entere de una falta.
 *
 * La lista `settings.citas.incidenciaPorFalta` sigue siendo las dos cosas de
 * antes menos una: el INTERRUPTOR (vacía = no se abre ninguna incidencia) y a
 * QUIÉN SE AVISA. Ya no dice de quién es la incidencia.
 *
 * ⚠️ Quien esté en esa lista tiene que poder VER las faltas sin ser
 * responsable: hoy se cumple porque las ve dirección (`alcanceIncidencias.js`)
 * y porque quien marcó la falta la registró. Si algún día se pone en la lista a
 * alguien que no es dirección, habrá que darle alcance por la lista, o dejará
 * de ver lo que se le avisa. Comprobado el 18/09/2026: el único centro con la
 * función encendida es Aumenta, y su destinataria es `admin`.
 *
 * ── EL ENCARGO ──────────────────────────────────────────────────────────────
...
import { faltaDesdeBooking } from "../clinica/faltas.js";
...
/**
 * A quién se AVISA, leído del cliente. Lista de ids de miembros del equipo, sin
 * duplicados y sin basura. Vacía = la función está apagada para este centro,
 * que es el estado de fábrica. Desde el 18/09/2026 no es «de quién es la
 * incidencia»: las faltas nacen sin responsable (ver la cabecera).
 */
 ...
 *
 * @returns {Promise<{id: string, avisados: string[]}|null>} `null` si no
...
export async function abrirIncidenciaPorFalta({
  ...
  try {
    ...
    // Solo los que EXISTEN hoy: a una persona que se fue del centro no se le
    // puede avisar, y si en la lista no queda nadie vivo no se abre nada — la
    // falta se quedaría sin que nadie lo supiera, que es lo que esto evita.
    const vivos = (
    ...
    const incidencia = await Incidencia.create({
      ...
      // SIN responsable a propósito (18/09/2026): la falta vive en su pestaña,
      // no en la bandeja de nadie. Ver la cabecera.
      assignedToId: null,
      reportedById: reportedById || null,
      comments: [],
      // La falta con su ciclo (03/09/2026, AV-0038): es lo que la manda a la
      // pestaña «Faltas» en vez de mezclarla con las incidencias de siempre.
      falta: faltaDesdeBooking(booking),
    });

    // Ni pivote: la incidencia no se le reparte a nadie. Si alguien de
    // administración quiere hacerse cargo de una, se pone de responsable a mano
    // desde su ficha, y entonces sí sale en su bandeja — porque lo ha decidido
    // una persona.
    ...
    return { id: incidencia.id, avisados: ordenados };
  } catch (err) {
