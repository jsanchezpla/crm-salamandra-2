...
export function detalleDeBloqueo(b) {
  const partes = [];
  /*
   * De quién es el hueco, cuando se ha guardado para alguien (08/09/2026).
   * Va PRIMERO porque es lo que se busca al abrir una reserva de plaza, y va
   * AQUÍ y no en el título por la misma regla de siempre: en el calendario,
   * solo la categoría; el detalle, al pulsar.
   *
   * Antes este nombre estaba escrito dentro del rótulo, así que lo veía
   * cualquiera que viera el bloqueo. Ahora llega solo a quien puede verlo — lo
   * decide el servidor (`pacientesDeBloqueos`), y a quien no, `patientName`
   * le llega vacío y esta línea no aparece.
   */
  const paciente = texto(b?.patientName);
