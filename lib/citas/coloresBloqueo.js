 ...
 * ── Quién gana ─────────────────────────────────────────────────────────────
 *
 * La CATEGORÍA, si el bloqueo tiene una (01/09/2026); si no, el de la persona;
 * si no, el general del centro; si tampoco, el negro de siempre. Un bloqueo del
 * CENTRO (sin `teamMemberId`, un cierre con hora) no tiene persona de la que
 * heredar y usa siempre el general.
 *
 * ── Por qué la categoría manda sobre la persona (01/09/2026, Rodrigo) ──────
 *
 * Porque para eso se pidió: «color personalizable desde Admin **para que a
 * todo el equipo le salga igual**». El color de la persona contesta a «¿de
 * quién es esta ausencia?» y el de la categoría a «¿qué es esto?»; en cuanto
 * hay categoría, la segunda pregunta es la que importa —una reunión de equipo
 * es la misma reunión en la fila de las seis que van—. Si ganara el color
 * personal, la misma reunión saldría de seis colores distintos y la categoría
 * no serviría para nada.
 *
 * Un bloqueo SIN categoría se pinta exactamente como antes del 01/09/2026: los
 * centros que no usen categorías no notan el cambio.
 */
 ...
 *
 * Se llama con un objeto y no con tres sueltos porque el orden de los
 * argumentos ya no coincide con el orden de prioridad —la categoría llegó la
 * última y manda la primera—, y `colorDeBloqueo(a, b, c)` se leería al revés
 * de lo que hace.
 *
 * @param categoria  el color de la categoría del bloqueo (o null)
 * @param persona    `blockColor` del miembro dueño del bloqueo (o null)
 * @param centro     el del centro, de `settings.citas.colorBloqueos` (o null)
 */
export function colorDeBloqueo({ categoria = null, persona = null, centro = null } = {}) {
  return categoria || persona || centro || COLOR_BLOQUEO_POR_DEFECTO;
}
