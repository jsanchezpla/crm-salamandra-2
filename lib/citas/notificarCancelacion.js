 ...
 *
 * ESTE CORREO ESTUVO ESCRITO DOS VECES (06/08 → 21/08/2026). El panel de la
 * cita —la vía de cancelación más usada del CRM— guardaba su propia copia
 * dentro de `app/api/citas/bookings/[id]/route.js` (`sendCancellationEmail`).
 * Guardas, cuenta de Resend y manejo de errores coincidían; el CUERPO no: el
 * `esBono` de aquí abajo —«tu programa sigue activo»— se añadió el 06/08 solo
 * en esta copia, así que cancelar desde el panel la sesión de un bono mandaba
 * un correo que solo hablaba de esa cita a quien acababa de pagar diez
 * sesiones. Juntarlas cambiaba lo que recibe esa familia, así que esperó a
 * decidirse: el 20/08/2026 Jorge dijo que sí, que el párrafo del bono también
 * tiene que salir desde el panel. Desde entonces esta función es la única, y
 * todos los caminos de cancelación mandan el MISMO correo con la MISMA
 * etiqueta de log (`citas:cancelada`; el panel escribía `citas:cancelled`, y
 * buscar por una en el VPS dejaba fuera la mitad de las cancelaciones).
 */
