 ...
 * razón por la que `festivos.js` vive aquí. Desde el 13/09/2026 también
 * «Proponer 3 horarios», que lee los bloqueos con `cargarAusencias` pero los
 * trata como ocupación y no los resta: ver `choqueConBloqueos.js`.)
 *
 ...
 * @param profesionalIds varias personas a la vez (13/09/2026, regla #2: «Proponer
 *                       3 horarios» en ámbito «todo el centro» mira la agenda de
 *                       todo el equipo y así no hace una consulta por persona).
 *                       Si viene con algún id, manda sobre `profesionalId`; los
 *                       del centro salen siempre. Quien la usa tiene que repartir
 *                       luego cada fila a su persona (`bloqueosQueAplican`).
 */
export async function cargarAusencias(tenantModels, { desde, hasta, profesionalId = null, profesionalIds = null } = {}) {
  ...
  try {
    ...
    // es la suya.
    const varios = Array.isArray(profesionalIds) ? [...new Set(profesionalIds.filter(Boolean))] : [];
    where[Op.or] = varios.length
      ? [{ teamMemberId: null }, { teamMemberId: { [Op.in]: varios } }]
      : profesionalId
