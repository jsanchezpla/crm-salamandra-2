...
const CAMPOS_CITA = [
  "amount",
  // El importe al que va atada la cita (04/09/2026, Aumenta). El MODO y el
  // TEXTO se quedan —quien apunta la cita tiene que ver si lleva cuota, cobro
  // libre o va sin coste, y el nombre del concepto es el mismo que ya lee en el
  // tipo de cita—; la CIFRA no, que es la tarifa del centro y es justo lo que
  // Laura no quiere que vea su equipo.
  "cobroImporte",
  "paymentStatus",
...
export function tipoSinDinero(tipo) {
  ...
  // El concepto que cubre las citas de este tipo (04/09/2026) viaja con el tipo
  // para que el alta pueda enseñar «Cuota: Logopedia 45x1» sin pedirle nada al
  // módulo de facturación. El NOMBRE se queda; el precio del catálogo es tarifa
  // y se va, como el del tipo.
  if (limpio.concepto && typeof limpio.concepto === "object") {
    const { unitPrice: _fuera, ...resto } = limpio.concepto;
    limpio.concepto = resto;
  }
  return limpio;
}
...
/**
 * UN tipo de cita, listo para salir según quién lo pida.
 *
 * Faltaba, y su ausencia costó dinero de verdad (28/08/2026). El detalle de un
 * tipo llamaba a `tipoSinDinero` A SECAS, sin mirar el rol: el comentario decía
 * «sin la tarifa si no es dirección» pero el `if` nunca se escribió. Así que el
 * formulario de Tipos de cita se abría con «Precio (€)» VACÍO también para un
 * administrador, y al guardar —aunque solo se cambiara el color— ese vacío
 * viajaba como «sin precio» y **borraba la tarifa**. Sin aviso y sin fallo.
 *
 * De los 71 tipos de todo el CRM, los únicos con precio son 3 de los 8 de Laura,
 * que es justo quien cobra las citas por la web con tarjeta: borrarle el precio
 * no deja un campo en blanco, deja su reserva pública sin cobrar.
 *
 * ⚠️ NO hace falta además tapar lo que ENTRA por el PATCH, y se comprobó antes
 * de escribirlo: ese endpoint ya exige `admin`/`superadmin`, exactamente el
 * mismo conjunto que `puedeVerDinero`. O sea que quien puede guardar un tipo
 * puede ver su tarifa siempre, y un filtro de entrada sería código que no puede
 * ejecutarse nunca. Si algún día se abre la edición a más roles, ese filtro SÍ
 * hará falta: sin él, quien no ve el precio lo borraría al guardar el color.
 */
export function tipoSegunRol(tipo, rol) {
  return puedeVerDinero(rol) ? tipo : tipoSinDinero(tipo);
}
