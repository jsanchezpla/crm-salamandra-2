...
import { cancelacionBloqueada } from "./cancelacion.js";
...
function queLePasaAlDinero(row, now, bonos = null) {
  ...
  switch (row.paymentStatus) {
    ...
    case "paid":
      /*
       * ⚠️ NO SE DEVUELVE NUNCA (07/08/2026, Rodrigo). Antes se miraba si
       * quedaban 24 h y se prometía la devolución íntegra; hoy la política es
       * que el CRM no mueve dinero y que lo que se cancela es la SESIÓN, no la
       * compra. Lo pagado sigue pagado y se le da otra fecha hablando con el
       * centro; si en algún caso hay que devolver algo, lo hace la consulta a
       * mano (`lib/citas/politicaReembolso.js`).
       *
       * El aviso deja de prometer y pasa a decir qué hacer, que es lo que la
       * paciente necesita saber ANTES de pulsar cancelar.
       */
      return { tipo: "no_se_devuelve", importe };
    case "refunded":
...
/**
 * @param tenant Opcional. Solo se usa para saber si el centro deja anular a la
 *              familia (`lib/citas/cancelacion.js`). Va como cuarto parámetro y
 *              no dentro de `row` porque es una decisión del CENTRO, no de la
 *              cita: sin él, `cancellable` se comporta como siempre.
 */
export function serializeClientBooking(row, now = new Date(), bonos = null, tenant = null) {
  ...
  const dejaAnular = !cancelacionBloqueada(tenant);
  return {
    id: row.id,
    ...
    // El botón de anular. Además de que la cita esté viva y sea futura, el
    // centro tiene que permitirlo: Aumenta gestiona sus citas en el centro y su
    // área privada es de consulta (08/08/2026). Es SOLO la pantalla — el
    // endpoint que cancela lo comprueba por su cuenta, porque esconder un botón
    // no cierra una puerta.
    cancellable: dejaAnular && isActive(row.status) && isFuture,
 ...
 */
export function splitBookings(rows, now = new Date(), bonos = null, tenant = null) {
  const upcoming = [];
  const history = [];
  for (const row of rows) {
    const scheduled = new Date(row.scheduledAt);
    const isFuture = scheduled.getTime() > now.getTime();
    const item = serializeClientBooking(row, now, bonos, tenant);
