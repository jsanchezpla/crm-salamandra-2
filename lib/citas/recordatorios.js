...
import { enlaceCancelacion } from "./cancelacion.js";
 ...
 *
 * Delega en `enlaceCancelacion` para que la decisión de si el centro deja
 * anular viva en un solo sitio: aquí antes se construía a mano y el
 * recordatorio se habría quedado repartiendo enlaces que ya no funcionan.
 */
function cancelUrl(tenant, baseUrl, slug, token) {
  if (!baseUrl) return null;
  return enlaceCancelacion(tenant, { baseUrl, slug, token });
}
  ...
  for (const cita of citas) {
    ...
    try {
      ...
      const tpl = bookingReminderTemplate({
        ...
        cancelUrl: cancelUrl(tenant, baseUrl, slug, cita.cancellationToken),
      });
