 ...
 *
 * (Fichero nuevo en /lib, regla #2: lo comparten los endpoints que disparan
 * IA de pago —eran 7 al nacer, 21 el 14/09/2026— y el panel de permisos de
 * Configuración → IA.)
 *
 ...
 *
 * ── EL TOPE MENSUAL DE GASTO (14/09/2026, regla #2) ────────────────────────
 * El 10/09/2026 Aumenta se quedó sin saldo a media tarde y nada avisó antes.
 * Desde hoy, si dirección ha fijado un tope (`settings.integrations.
 * iaTopeMensual`, `lib/ai/topeDeGasto.js`), `vetoAi` lo mira ANTES de la
 * puerta de los admins y del candado (`lib/ai/frenoDeGasto.js`):
 *   · sin tope no hay consulta ni cambia nada;
 *   · al 80 % y al 100 %, campana `ia_tope` a los admins, pulse quien pulse;
 *   · al 100 %, quien no es admin recibe un 429 con `motivo: "tope_ia"` y la
 *     frase de hasta cuándo, y queda la auditoría `ai.tope_frenada`; el admin
 *     sigue pasando;
 *   · va antes del candado para no gastar un permiso de un solo uso ni crear
 *     una solicitud que no serviría de nada;
 *   · ante un fallo de la consulta, deja pasar (el tope no es un permiso).
 * El freno va AQUÍ y no en los clientes centrales porque necesita los ajustes
 * y el rol, y porque un 429 lanzado desde un cliente central se tomaría por el
 * «límite de uso» del proveedor y avisaría como un fallo de la cuenta.
 */

import { getMasterModels } from "../db/masterDb.js";
import { error, errorConDatos } from "../utils/apiResponse.js";
import { marcarAccion } from "./usoDeIA.js";
import { avisarTramoDelTope, comprobarTopeDeGasto } from "./frenoDeGasto.js";
import { mensajeDeTopeAlcanzado, MOTIVO_TOPE } from "./topeDeGasto.js";
...
/*
 * El rastro de cada llamada FRENADA por el tope (14/09/2026), simétrico a
 * `ai.uso`: los logs del contenedor se pierden en cada despliegue. Solo la
 * acción, sin datos personales. No va a `master.ai_uso`: no es una llamada.
 */
async function auditaFrenada(ctx, userId, accion) {
  try {
    const { AuditLog } = getMasterModels();
    await AuditLog.create({
      tenantId: ctx.tenant.id,
      userId: userId || null,
      action: "ai.tope_frenada",
      entity: "Ai",
      entityId: null,
      before: null,
      after: { accion },
...
/**
 * Devuelve null si se puede usar la IA, o una respuesta 403 (candado) o 429
 * (tope de gasto) lista para devolver si no. `accion` es la etiqueta legible
 * que verá el admin ("transcribir una sesión", "análisis de lead"...).
 */
export async function vetoAi(ctx, request, accion) {
  try {
    const userId = ctx.user?.id || request.headers.get("x-user-id");
    // Para la contabilidad de la IA (`lib/ai/usoDeIA.js`): qué se va a hacer.
    marcarAccion(accion);
    const esAdmin = ADMIN_ROLES.has(ctx.user?.role);

    // Tope mensual de gasto (14/09/2026, ver cabecera). ANTES de la puerta de
    // los admins, para que la campana salga pulse quien pulse. Sin tope no hay
    // consulta. `comprobarTopeDeGasto` y `avisarTramoDelTope` nunca lanzan ni
    // esperan más de 500 ms cada una.
    const tope = await comprobarTopeDeGasto(ctx, { esAdmin });
    if (tope.avisarTramo) await avisarTramoDelTope(ctx, tope.estado, tope.avisarTramo);
    if (!tope.permitir) {
      await auditaFrenada(ctx, userId, accion);
      console.info(`[ai:tope] frenada tenant=${ctx.slug || ctx.tenant?.slug} accion=${accion}`);
      return errorConDatos(mensajeDeTopeAlcanzado(new Date()), 429, { motivo: MOTIVO_TOPE });
    }
