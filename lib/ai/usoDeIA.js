...
import { costeAnthropicUsd, costeWhisperUsd, desgloseDeUsage } from "./precios.js";
...
/**
 * Guarda una llamada. `usage` es el bloque de Anthropic; `segundosAudio` el de
...
export async function registrarUso(
  ...
  try {
    ...
    const costeUsd = cacheado
      ? 0
      : proveedor === "openai"
        ? costeWhisperUsd(segundos)
        : costeAnthropicUsd(modelo, usage);
    await crear({
