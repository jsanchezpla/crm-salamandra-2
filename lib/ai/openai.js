 ...
 * los dos.
 *
 * Y desde el 13/09/2026 (regla #2) el aviso a dirección cuando falla la CUENTA
 * sale de aquí: `llamar` pasa cada error por `lib/ai/trasFalloDeIa.js` antes de
 * relanzarlo, sin que la ruta tenga que acordarse.
 *
...
import { trasFalloDeIa } from "./trasFalloDeIa.js";
...
async function llamarSinAviso({ apiKey, body, timeoutMs }) {
...
/** La llamada de verdad, y si falla, el aviso de `trasFalloDeIa` antes de relanzar. */
async function llamar({ apiKey, body, timeoutMs }) {
  const t0 = Date.now();
  try {
    return await llamarSinAviso({ apiKey, body, timeoutMs });
  } catch (e) {
    await trasFalloDeIa(e, { proveedor: "openai", modelo: body?.model ?? null, ms: Date.now() - t0 });
    throw e;
  }
}
