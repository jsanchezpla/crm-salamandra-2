 ...
 * nuevo). Sin captura se devuelve null y los emails salen sin reply-to especial
 * (el portal sigue cubriendo la respuesta del cliente).
 *
 * ⚠️ PIDE LAS DOS VARIABLES, NO SOLO EL DOMINIO (14/08/2026). Hasta hoy bastaba
 * con `RESEND_INBOUND_DOMAIN`, y esa media configuración es el peor estado
 * posible: con solo el dominio puesto, la configuración de Soporte le enseña a
 * cada cliente su dirección de captura y le dice, con todas las letras, que
 * reenvíe ahí TODO su buzón de soporte — mientras el webhook, que necesita
 * `RESEND_WEBHOOK_SECRET`, contesta 503 a cada entrega.
 *
 * O sea: el cliente monta el reenvío, sus pacientes escriben, y el correo se
 * pierde entero sin que salte nada. No hay rebote (Resend acepta el correo, es
 * NUESTRO endpoint el que lo rechaza después) ni aviso. Es la diferencia entre
 * una puerta cerrada —que se ve— y una puerta pintada en la pared.
 *
 * Con la comprobación de las dos, faltar una deja la vía de correo simplemente
 * ausente: la UI esconde el bloque y ofrece el portal, que es lo que hoy
 * funciona. Ver `app/api/webhooks/resend-inbound/route.js`.
 */
export function captureAddress(slug) {
  const domain = (process.env.RESEND_INBOUND_DOMAIN || "").trim();
  const secret = (process.env.RESEND_WEBHOOK_SECRET || "").trim();
  if (!domain || !secret || !slug) return null;
...
export async function emailClient({ ctx, ticket, kind, replyBody, baseUrl, settings = null }) {
  ...
  // BYOK: el ticket sale de la cuenta de Resend del propio negocio. El
  // replyTo del ticket manda sobre el del tenant (es el buzón del hilo).
  ...
  const res = await sendEmail({
    ...
    html: tpl.html,
    text: tpl.text,
    replyTo: replyTo || cfgResend.replyTo || undefined,
...
export async function emailTeam({ ctx, ticket, kind, to, preview, baseUrl }) {
  ...
  const cfgEquipo = getTenantResendConfig(ctx);
  await sendEmail({
    to: recipients,
    subject: tpl.subject,
    html: tpl.html,
    text: tpl.text,
    from: cfgEquipo.fromEmail || undefined,
    replyTo: cfgEquipo.replyTo || undefined,
    apiKey: cfgEquipo.apiKey || undefined,
  });
}
