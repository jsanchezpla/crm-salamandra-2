 ...
 *
 * `lanzar` (12/09/2026): con `true` el error SÍ sale. Lo pide el calendario
 * global, que junta muchos clientes y tiene que poder decir «este no
 * responde»; con [] un cliente caído parecía un cliente sin nada esta semana.
 * Sin la opción, lo de siempre: el feed del calendario del tenant
 * (/api/calendar/tasks) no la pasa y no cambia.
 */
export async function fetchProjectEvents({ tenantModels, hasModule, start, end, lanzar = false }) {
  try {
    ...
    if (Milestone) {
      ...
      for (const m of milestones) {
        ...
        events.push({
          ...
          backgroundColor: color,
          borderColor: color,
          // Un hito ya resuelto (completado / no cumplido) no se arrastra: mover
          // su fecha dejaría el estado obsoleto. Solo los 'pending' se reprograman.
    ...
    if (lanzar) throw err; // el global decide qué hacer con él (ver arriba)
