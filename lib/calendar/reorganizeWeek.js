 ...
 *
 * (13/09/2026, regla #2: si la IA falla, las tres propuestas deterministas
 * salen con `avisoIA`, el motivo legible. Antes salían «Sin IA» sin decir por
 * qué, y una cuenta sin saldo pasaba por una semana sin clave.)
 */
...
// Índices de días LABORABLES (lunes-viernes). La IA nunca propone mover trabajo
// a sábado/domingo (bug 2026-07-27): solo se reparte dentro de la semana laboral.
const WORKDAY_IDX = [0, 1, 2, 3, 4];
...
/** Estrategia 1 — equilibrar: mueve tareas non-high del día más cargado a un día
 * LABORABLE más libre (nunca a fin de semana). */
function strategyBalance(tasks, weekDates) {
  const byDay = loadByDay(tasks, weekDates);
  const load = weekDates.map((d, i) => ({ d, i, n: byDay[d].length }));
  const moves = [];
  const moved = new Set();
  for (let i = 0; i < tasks.length; i++) {
    const max = [...load].sort((a, b) => b.n - a.n)[0]; // día más cargado (cualquiera)
    // Destino: el día LABORABLE menos cargado (nunca sábado/domingo).
    const min = load.filter((x) => WORKDAY_IDX.includes(x.i)).sort((a, b) => a.n - b.n)[0];
    if (!max || !min || max.d === min.d || max.n - min.n <= 1) break;
...
/** Estrategia 2 — prioridades al principio: alta pronto, baja/media hacia el
 * final de la SEMANA LABORAL (nunca fin de semana). */
function strategyPriorityFirst(tasks, weekDates) {
  const zones = { high: [0, 1], medium: [2, 3], low: [3, 4] };
  const byDay = loadByDay(tasks, weekDates);
...
function strategyMinimal(tasks, weekDates) {
  ...
  const empties = weekDates
    .map((d, i) => ({ d, i, n: byDay[d].length }))
    .filter((x) => x.d !== peak.d && WORKDAY_IDX.includes(x.i)) // solo días laborables
    .sort((a, b) => a.n - b.n);
  const moves = [];
  let ei = 0;
 ...
 * Si la IA ha fallado, además `avisoIA` con el motivo (las 3 van sin IA).
 */
export async function reorganizeWeek({ tasks, weekDates, apiKey, model, preferences = "", forceFake = false }) {
  if (!tasks.length) return { model: "none", proposals: emptyProposals() };

  // forceFake = demo pública: se muestra la IA en modo simulado, sin llamar a la API real.
  if (reorgFakeEnabled() || forceFake) {
  ...
  try {
    ...
    console.error("[calendar:reorganize] la IA no ha respondido:", e?.name, e?.status, e?.message);
    return {
      model: "sin-ia",
      proposals: deterministicProposals(tasks, weekDates),
      avisoIA: avisoSinIa(e, "Las tres propuestas de abajo están calculadas sin IA."),
    };
  }
  let parsed;
  try {
