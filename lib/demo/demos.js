...
export const DEMOS = [
  ...
  },
  {
    ...
    // Pedidos cuelga de Productos desde el 03/09/2026, así que van los dos
    // niveles con él: sin ellos el alta no se sostiene (`validarSeleccion`) y
    // el menú no tendría por dónde llegar a los pedidos.
    modulos: [
      "clients", "leads", "formularios", "team",
      "projects", "billing", "productos", "productos_avanzado", "orders", "calendar", "support",
      "outreach", "analytics",
    ],
  },
];
...
/**
 * Slugs de tenant cuyo registro de auditoría es de JUGUETE y se poda a 7 días
 * (`scripts/podar-audit-logs.js`). Son todas las demos de la lista más su
 * `_golden` por si alguna foto dorada llegara a darse de alta como tenant (hoy
 * son solo schemas, pero la poda vieja ya contaba con `demo_golden`).
 *
 * (Añadido en /lib, regla #2, 14/09/2026: la poda tenía `('demo', 'demo_golden')`
 * a mano y las demos por oficio se guardaban 3 años como un cliente real.)
 */
export function slugsDemoParaPoda() {
  return DEMO_SLUGS.flatMap((slug) => [slug, `${slug}_golden`]);
}
