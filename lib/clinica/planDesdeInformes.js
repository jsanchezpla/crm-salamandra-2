/**
 * lib/clinica/planDesdeInformes.js — el Plan trae el motivo de consulta y los
 * objetivos de los informes que el paciente ya tiene subidos (15/09/2026,
 * AV-0103 de Silvia; los objetivos, 16/09/2026, AV-0163 de Raquel).
 *
...
import { MAX_TEXTO_OBJETIVO, normalizarObjetivos } from "./objetivosDelPlan.js";
...
/** De un informe salen los suyos, no los de toda la carpeta: el Plan admite 40. */
export const MAX_OBJETIVOS_POR_INFORME = 25;
...
function lineasYPies(texto) {
  ...
  return { lineas, clave, veces };
}

/** Las líneas que cuelgan de un encabezado, hasta el siguiente. */
function cuerpoDelApartado(lineas, clave, veces, desde) {
  const cuerpo = [];
  for (let j = desde + 1; j < lineas.length; j++) {
    const l = lineas[j];
    if (!l) continue;
    /*
     * El pie que sale en TODAS las páginas se mira antes que el encabezado: los
     * informes de logopedia llevan uno en mayúsculas («USC/WPS Nº 1648») que
     * pasaba por título y cortaba el apartado al cambiar de página. Si tiene
     * forma de título hacen falta TRES apariciones, porque un título que además
     * está en el índice sale dos: ese sí cierra el apartado, y es como se
     * encuentra el final de «Motivo» en los informes de valoración.
     */
    const repetida = (veces.get(clave(l)) ?? 0) > 1 && l.length < 120;
    if (repetida && (!esEncabezado(l) || (veces.get(clave(l)) ?? 0) > 2)) continue;
...
function apartado(texto, re, { max }) {
  const { lineas, clave, veces } = lineasYPies(texto);
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    cuerpo.push(...cuerpoDelApartado(lineas, clave, veces, i));
...
/*
 * ── LOS OBJETIVOS, UNO POR LÍNEA (16/09/2026, AV-0163 de Raquel) ────────────
 * «Sacar los objetivos del informe subido del año pasado, al igual que se ha
 * hecho con el motivo de consulta». Mismo apartado y mismo copiado literal,
 * pero lo que sale no es un texto: son OBJETIVOS sueltos, que en el Plan son
 * una lista con su terapeuta (AV-0061).
 *
 * En la muestra de producción (60 informes, 48 con texto) 40 traen un apartado
 * «OBJETIVOS» —casi siempre cerrado por «EVOLUCIÓN»— con entre 4 y 38 viñetas,
 * cada una partida en dos o tres renglones por la maqueta. Se vuelven a juntar
 * con la misma regla que el motivo: viñeta o punto final abren objetivo nuevo.
 */
const OBJETIVOS_RE = /^(\d+\s*[.)]\s*)?objetivos?\b.{0,60}$/i;
const VINETA_SUELTA = /^[•●▪◦■\-–o*]\s*/;
/** Un objetivo de una palabra no es un objetivo; tres lo dudoso ya lo corrige quien lo lea. */
const MIN_TEXTO_OBJETIVO = 15;

/**
 * Los objetivos escritos en el apartado «Objetivos» del informe, tal cual,
 * limpios de viñeta y recortados a lo que admite el campo.
 */
export function objetivosDelInforme(texto, { max = MAX_OBJETIVOS_POR_INFORME } = {}) {
  const { lineas, clave, veces } = lineasYPies(texto);
  for (let i = 0; i < lineas.length; i++) {
    const l = lineas[i];
    if (!esEncabezado(l) || !OBJETIVOS_RE.test(sinTildes(l))) continue;
    const cuerpo = cuerpoDelApartado(lineas, clave, veces, i);
    const sueltos = unirLineas(cuerpo)
      .split("\n")
      .map((t) => t.replace(VINETA_SUELTA, "").trim())
      .filter((t) => t.length >= MIN_TEXTO_OBJETIVO);
    if (!sueltos.length) continue;
    // Un párrafo entero no es un objetivo y en el campo entra cortado a
    // media frase: se queda fuera y se dice cuántos, para ir al informe.
    const caben = sueltos.filter((t) => t.length <= MAX_TEXTO_OBJETIVO);
    return { textos: caben.slice(0, max), noCaben: sueltos.length - caben.length };
  }
  return { textos: [], noCaben: 0 };
}

/** El motivo de consulta, los objetivos y, si lo tiene con su nombre, el diagnóstico. */
export function leerInforme(texto) {
  ...
  const objetivos = objetivosDelInforme(texto);
  return {
    motivo: motivo.texto,
    motivoDemasiadoLargo: motivo.demasiadoLargo,
    diagnostico: diagnostico.texto,
    objetivos: objetivos.textos,
    objetivosQueNoCaben: objetivos.noCaben,
  };
}
 ...
 *
 * Devuelve lo que se rellena —motivos por terapeuta, objetivos por terapeuta,
 * el motivo general y el diagnóstico—, de qué fichero sale cada cosa y el
 * recuento de lo que no se pudo leer, para decírselo a quien pulsa.
 */
export function rellenoDesdeInformes(plan = {}, terapeutas = [], informes = []) {
  ...
  const cuenta = { leidos: 0, cifrados: 0, sinTexto: 0, sinMotivo: 0, objetivosQueNoCaben: 0 };

  // Quién tiene YA objetivos escritos en el plan: a esa no se le traen otros
  // veinte de un informe viejo. El `null` de la clave son los sin atribuir.
  const yaTieneObjetivos = new Set(normalizarObjetivos(plan.objectives).map((o) => String(o.terapeutaId)));
  ...
  for (const inf of informes) {
    ...
    if (!leido.motivo) cuenta.sinMotivo++;
    ...
    // Los objetivos van a la terapeuta del informe, como el motivo; si no hay
    // ninguna con esa especialidad, quedan sin atribuir, que es una forma que
    // el Plan ya entiende (AV-0061) y que se arregla con el botón «mío».
    if (leido.objetivos?.length) {
      for (const id of suyas.length ? suyas.map((t) => t.id) : [null]) {
        if (yaTieneObjetivos.has(String(id))) continue; // los suyos no se tocan
        if (objetivos.some((o) => o.terapeutaId === id)) continue; // ya los trajo uno más nuevo
        objetivos.push({ terapeutaId: id, textos: leido.objetivos, terapia, ...origen });
        cuenta.objetivosQueNoCaben += leido.objetivosQueNoCaben ?? 0;
      }
    }
  }

  return { motivos, objetivos, general, diagnostico, cuenta };
}
