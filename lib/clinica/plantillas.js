 ...
 * una con la misma clave. Hoy, solo la entrevista inicial, para el registro.
 */
export const PLANTILLAS_EXTRA = Object.freeze({
  registro: Object.freeze([PLANTILLA_ENTREVISTA]),
});

/** La pista de un apartado no es un texto libre sin tope: 400 bastan para los subpuntos. */
const MAX_PISTA = 400;
...
export function plantillasDe(tenant, doc) {
  ...
  // las del centro, y solo si el centro no ha guardado la suya con esa clave.
  for (const extra of PLANTILLAS_EXTRA[doc] ?? []) {
    if (!lista.some((p) => p.key === extra.key)) {
