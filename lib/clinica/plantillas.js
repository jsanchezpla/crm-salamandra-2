...
/**
 * Los apartados de una SESIÓN DE DIAGNÓSTICO (12/09/2026, segunda entrega del
 * Diagnóstico; Isa por Rodrigo): el registro de cada hora del tramo `horas` de
 * un expediente —una sesión de pruebas—, que después se UNEN con la IA en el
 * informe de valoración diagnóstica (`PLANTILLA_DIAGNOSTICO`).
 *
 * Son exactamente los cinco que Aumenta aceptó de la propuesta, en su orden, y
 * ninguno más: se les propuso también un apartado de «notas internas» y
 * contestaron que no («ese informe es el que se entrega al paciente; todo lo
 * demás genial»). Lo interno sigue siendo el bloque aparte del registro
 * (`internalNotes`), que no viaja ni al informe ni al material de la IA.
 *
 * Las claves son nuevas y camelCase a propósito: ninguna es de fábrica
 * (`CAMPOS_SESION`), así que todo cae en el JSONB `contentSections` y no pisa
 * las columnas de siempre; y ninguna es reservada. La entrevista inicial del
 * mismo expediente NO usa esta plantilla: sigue con `entrevista_inicial`.
 */
export const APARTADOS_SESION_DIAGNOSTICO_BASE = Object.freeze([
  { key: "pruebaAdministrada", label: "Prueba administrada", tipo: "texto", pista: "Nombre de la prueba y, si son varias, cada una en su línea." },
  { key: "queEvalua", label: "Qué evalúa", tipo: "texto", pista: "La función o el área que mide la prueba." },
  { key: "observacionesDurantePrueba", label: "Observaciones durante la prueba", tipo: "texto", pista: "Actitud ante la tarea, atención, conducta, regulación, relación con el evaluador." },
  { key: "puntuaciones", label: "Puntuaciones", tipo: "texto", pista: "Puntuaciones directas, típicas y percentiles tal como se anotan; las tablas van en el informe." },
  { key: "interpretacion", label: "Interpretación", tipo: "texto", pista: "Qué dicen esas puntuaciones y qué hipótesis sostienen o descartan." },
]);

export const PLANTILLA_SESION_DIAGNOSTICO = Object.freeze({
  key: "sesion_diagnostico",
  name: "Sesión de diagnóstico",
  apartados: APARTADOS_SESION_DIAGNOSTICO_BASE,
});
 ...
 * una con la misma clave. Hoy, la entrevista inicial, el registro de taller y
 * la sesión de diagnóstico, para el registro.
 */
export const PLANTILLAS_EXTRA = Object.freeze({
  registro: Object.freeze([PLANTILLA_ENTREVISTA, PLANTILLA_TALLER, PLANTILLA_SESION_DIAGNOSTICO]),
