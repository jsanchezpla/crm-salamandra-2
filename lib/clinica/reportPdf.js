 ...
 * El cuerpo del informe es la redacción de la profesional (a mano o con IA);
 * de las sesiones, la portada del PDF dice CUÁNDO fueron — el contenido
 * literal solo viaja si se pide el anexo.
 */
function fechasDeSesiones(sesiones) {
 ...
 * (`prepText`): es material interno del equipo, y este PDF lo recibe la
 * familia.
 */
function anexoRegistros(doc, F, sesiones) {
  doc.addPage();
  doc.font(F.bold).fontSize(16).fillColor(INK).text("Anexo · Registros de sesión");
  doc.moveDown(0.3);
  ...
  sesiones.forEach((s, i) => {
    ...
    doc.font(F.bold).fontSize(12).fillColor(INK).text(`Sesión del ${fmtFecha(s.sessionDate)}`);
    doc.moveDown(0.4);

    const bloque = (label, contenido, esLista) => {
      const c = esLista ? lista(contenido) : (texto(contenido) ? [texto(contenido)] : []);
      if (!c.length) return;
      doc.font(F.medium).fontSize(9).fillColor(MUTED).text(label.toUpperCase());
 ...
 * @param sourceSessions  las sesiones en las que se basa el informe (fecha y
 *   registro), ya cargadas por quien llama. La portada solo imprime sus FECHAS
 *   (periodo + lista); el contenido literal, únicamente en el anexo opcional.
 */
export async function buildReportPdfBuffer({ report, patientName, therapistName, tenantName, brand, tenant = null, patientSpecialties = [], sourceSessions = [] }) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      fichaDatos(doc, F, [
        ...
        // De las sesiones, aquí solo las FECHAS (26/08/2026, Rodrigo): el
        // cuerpo es la redacción de la profesional; lo literal, en el anexo.
        ["Periodo", fechas ? fechas.periodo : ""],
        ["Basado en", fechas ? fechas.basadoEn : ""],
        [
      ...
      if (anexar) anexoRegistros(doc, F, sourceSessions);
