...
/** Última hoja: protección de datos, y el isotipo cerrando el documento. */
function hojaLegal(doc, F, { centro, marca, isotipo }) {
  const hayTexto = Boolean(centro.proteccionDatos);
  // Sin texto legal y sin isotipo no hay nada que poner: una página en blanco
  // al final de un informe parece un fallo de impresión.
  ...
  /*
   * El isotipo cierra el documento (28/08/2026, Jorge). Va anclado al PIE de la
   * página y no a `doc.y`: así queda a la misma altura salga el texto legal
   * largo o corto, que es lo que hace que parezca puesto a propósito y no
  ...
  if (isotipo) {
    try {
      const { width: W, height: H } = doc.page;
      const anchoIso = 58;
      doc.image(isotipo, (W - anchoIso) / 2, H - doc.page.margins.bottom - anchoIso - 26, { width: anchoIso });
    } catch {
...
export async function buildReportPdfBuffer({
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      hojaLegal(doc, F, { centro, marca, isotipo });
