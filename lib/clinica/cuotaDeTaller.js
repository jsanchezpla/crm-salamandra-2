...
import { sincronizarCobroDelMes } from "../billing/cobroDeCuota.js";

const hoyISO = () => madridToday();
...
export async function asegurarCuotaDeTaller({ tenantModels, taller, grupo, patientId, clientId }) {
  ...
  try {
    ...
    const cuota = await Cuota.create({
      ...
      amount: null,
      // Cómo y qué día se cobra: lo mismo que ya paga esa familia por sus otras
      // cuotas (revisión del 06/09/2026: nacía sin método y no entraba en
      // «solo las de banco» ni tenía día de cobro).
      method: vivas.find((c) => c.method)?.method ?? null,
      dayOfMonth: vivas.find((c) => c.dayOfMonth)?.dayOfMonth ?? null,
      startDate: hoyISO(),
      active: true,
      notes: `Taller: ${taller?.name ?? ""}${grupo?.name ? ` · ${grupo.name}` : ""}`.trim(),
    });
    // Y su cobro del mes en curso, como cualquier cuota nueva (revisión del
    // 06/09/2026): apuntar al niño ya deja el cobro en Cobros, sin esperar a
    // «Generar el mes». Best-effort: nunca lanza.
    await sincronizarCobroDelMes({ tenantModels, cuotaId: cuota.id });
...
export async function cerrarCuotaDeTaller({ tenantModels, cuotaId }) {
  ...
  try {
    ...
    // El cobro pendiente del mes se prorratea hasta hoy o se retira, como al
    // dar de baja una cuota desde Facturación (revisión del 06/09/2026).
    await sincronizarCobroDelMes({ tenantModels, cuotaId: cuota.id });
