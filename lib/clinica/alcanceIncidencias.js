...
/**
 * Dirección borra cualquiera; el resto, la que registró o de la que es
 * responsable. Pura: `esResponsable` lo calcula quien llama (la pivote hay
 * que leerla), y sin él vale el espejo `assignedToId` de la fila.
 *
 * ── POR QUÉ TAMBIÉN LA RESPONSABLE (03/09/2026, AV-0039) ────────────────────
 * Con la regla del 02/09 (solo dirección o quien registró) Olga, que lleva
 * la administración de Aumenta con rol `user`, no podía quitar las
 * incidencias que abre sola la agenda al marcar una falta: las registra la
 * terapeuta y se le ASIGNAN a ella. Quien gestiona una incidencia tiene que
 * poder cerrarla del todo cuando no procede; y ya podía verla por ser
 * responsable, así que ver y borrar vuelven a ir de la mano.
 */
export function puedeBorrarIncidencia({ esAdmin, row, teamMemberId, esResponsable = false }) {
