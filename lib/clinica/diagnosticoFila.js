...
import { fasesDeCobro, cobroDeProductoEntre, importeDeLaEntrevista, avisoDeOrden } from "./cobrosDelDiagnostico.js";
 ...
 *   · borrar          — el abierto por error (18/09/2026, AV-0202): mientras no
 *                       haya salido de sí mismo (sin bono, sin cobro y sin
 *                       informe). Aquí solo se decide si el BOTÓN se pinta; el
 *                       permiso de quién lo pulsa lo comprueba el endpoint con
 *                       `motivoParaNoBorrarDiagnostico` (`alcanceDiagnostico.js`),
 *                       porque esta función es pura y no sabe quién mira.
 */
export function accionesDe({ status, puedeDecidir = false, packId = null, entrevistaPaymentId = null, informeId = null } = {}) {
  ...
  return {
    ...
    borrar: !packId && !entrevistaPaymentId && !informeId,
  };
}
...
/** Un cobro (la entrevista, el producto), resumido para la fila: `{ id, importe, status }` o null. */
function cobroResumido(cobro) {
...
export function filaDeExpediente({
  ...
  const entrevistaCobro = cobroResumido(cobroEntrevista);
  // Las dos fases del dinero (18/09/2026): qué se puede mandar a cobro hoy y
  // por cuánto. La MISMA función que vuelve a comprobar el endpoint.
  const cobroProducto = cobroDeProductoEntre(e, cobros);
  const fases = fasesDeCobro({
    expediente: e,
    cobroEntrevista,
    cobroProducto,
    importeEntrevista: importeDeLaEntrevista(importeEntrevista === null ? null : { importeEuros: importeEntrevista }),
    cobroDelProducto: cobroAlSeguir,
    puedeDecidir,
  });
  ...
  const fila = {
    ...
    dinero: {
      ...
      entrevista: entrevistaCobro,
      producto: cobroResumido(cobroProducto),
    },
    cobroAlSeguir: cobroAlSeguirDe(cobroAlSeguir),
    // Las dos fases y sus botones: `{ entrevista, producto }` con importe,
    // estado del cobro, `puedeGenerar` y el motivo cuando no.
    cobros: fases,
    // Lo que hay que decir antes de «Seguir» si la entrevista no se ha
    // mandado a cobro: el producto nacerá entero.
    avisoAlSeguir: avisoDeOrden({ cobroEntrevista, cobroDelProducto: cobroAlSeguir }),
    notes: e.notes ?? null,
    ...
    acciones: accionesDe({
      ...
      puedeDecidir,
      packId: e.packId,
      entrevistaPaymentId: e.entrevistaPaymentId,
