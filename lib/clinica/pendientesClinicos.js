...
export function loQueFaltaAlPlan(plan) {
  ...
  if (vacio(plan.consultationReasons)) falta.push("motivo de consulta");
 ...
 */
export function pendientesPorProfesional({ citas = [], planes = [], altaDelPaciente = new Map(), conEntrevista = new Set(), ahora = new Date() }) {
  ...
  for (const c of citas ?? []) {
    ...
    if (!Number.isNaN(t) && t >= limiteNuevo && !conEntrevista.has(pac)) {
