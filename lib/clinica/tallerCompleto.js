...
import { lineaDeBloque, moldeDeBloques, normalizarPropuesta, reglasDelReparto } from "./registroCompleto.js";
import { nucleoClinico } from "./estiloClinico.js";
 ...
 */
export function partesDelPromptDeTaller(bloques, { centro = null } = {}) {
  const entero = promptDeTaller(bloques, { centro });
  const nucleo = nucleoClinico({ centro });
  return {
    systemCacheado: nucleo,
    // Lo de siempre, quitándole el núcleo: así el prompt que ve el modelo es
    // exactamente el mismo texto, solo que partido por donde se cachea.
    system: entero.split(nucleo).join("").replace(/\n{3,}/g, "\n\n").trim(),
  };
}

export function promptDeTaller(bloques, { centro = null } = {}) {
  ...
  return [
    ...
    // `reglasDelReparto` y no la constante `REGLAS` (09/09/2026): aquella se
    // evalúa al cargar el módulo, así que el perfil del centro no le llegaría
    // nunca y el taller seguiría escribiendo como antes, sin un solo error.
    reglasDelReparto({ centro }),
    REGLAS_TALLER,
  ].join("\n\n");
}
