...
import { rangoPedido, fechaISO, rangoDe } from "../utils/rangoFechas.js";
...
// Una cita confirmada que ya terminó se cuenta como atendida aunque nadie la
// marcara: la regla y su porqué, en un solo sitio.
import { estadoEfectivo } from "../citas/asistencia.js";
...
// `rangoPedido`, `fechaISO` y `rangoDe` nacieron aquí y desde el 03/09/2026
// viven en `lib/utils/rangoFechas.js`, porque las estadísticas de venta de
// Productos leen el periodo exactamente igual. Se re-exportan para que la
// pantalla, el Excel y el PDF sigan importándolas de aquí.
export { rangoPedido, fechaISO, rangoDe };
...
async function agenda(models, { inicio, fin }) {
  ...
  const citas = await Booking.findAll({
    ...
    // `duration` entra desde el 01/09/2026: hace falta para saber si la cita ya
    // TERMINÓ y darla por asistida (`lib/citas/asistencia.js`).
    attributes: ["id", "status", "teamMemberId", "scheduledAt", "duration", "noShowJustified"],
  });
  const equipo = TeamMember ? await TeamMember.findAll({ attributes: ["id", "displayName"] }) : [];
  const nombres = new Map(equipo.map((t) => [String(t.id), t.displayName]));

  /*
   * Una cita confirmada que ya pasó cuenta como atendida aunque nadie la
   * marcara (01/09/2026, Rodrigo). Antes solo contaban las `completed`, que en
   * Aumenta son las poquísimas que alguien recordó pulsar: «atendidas» salía
   * por los suelos y la tasa de ausencias, disparada, porque se mide contra
   * atendidas + faltas. El porqué entero, en `lib/citas/asistencia.js`.
   *
   * Se calcula UNA vez por cita y se reutiliza: `estadoEfectivo` mira el reloj,
   * y llamarlo dos veces para la misma cita podría dar respuestas distintas si
   * el minuto cambia en medio del recuento.
   */
  const ahora = new Date();
  const efectivo = new Map(citas.map((c) => [c.id, estadoEfectivo(c, ahora)]));
  const estadoDe = (c) => efectivo.get(c.id);

  const porEstado = cuenta(citas, estadoDe);
  const faltas = citas.filter((c) => estadoDe(c) === "no_show");
  ...
  for (const c of citas) {
    ...
    acc.citas++;
    if (estadoDe(c) === "completed") acc.atendidas++;
    if (estadoDe(c) === "no_show") acc.faltas++;
