...
export function rangoPedido(request) {
  ...
  const rango = rangoDe(sp.get("desde") || primeroDeMes, sp.get("hasta") || hoy.toISOString().slice(0, 10));
...
async function actividadClinica(models, { inicio, fin }) {
  ...
  const enRangoFecha = { [Op.between]: [inicio.toISOString().slice(0, 10), fin.toISOString().slice(0, 10)] };
  ...
  const [pacientes, sesiones, informes, equipo] = await Promise.all([
    ...
    ClinicSession
      ? ClinicSession.findAll({ where: { sessionDate: enRango }, attributes: ["id", "therapistId", "patientId", "sessionDate"] })
      : [],
    ClinicalReport
...
export async function calcularEstadisticas(models, rango) {
  ...
  return {
    desde: rango.inicio.toISOString().slice(0, 10),
    hasta: rango.fin.toISOString().slice(0, 10),
    clinica,
    agenda: citas,
    captacion: entrada,
  };
