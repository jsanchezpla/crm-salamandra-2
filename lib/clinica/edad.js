...
export function edadDe(paciente, hoy = new Date()) {
  ...
  // `Number(null)` es 0, y un paciente sin edad escrita NO tiene cero años:
  // sin dato, null (visto en local el 03/09/2026: la ficha decía «0 años»).
  if (paciente?.age == null || paciente.age === "") return null;
  const n = Number(paciente.age);
  return Number.isInteger(n) && n >= 0 && n <= 120 ? n : null;
}

/**
 * Qué se GUARDA en la columna `age` al salvar una ficha (18/09/2026, AV-0178
 * de Aumenta).
 *
 * Con fecha de nacimiento, `null`: la fecha ya manda al leer (`edadDe`), y
 * dejar las dos guardadas es garantizar que una de ellas mienta el próximo
 * cumpleaños —en producción había una ficha así—. Sin fecha, la casilla es el
 * único dato que hay y se respeta; vacía o ilegible, `null`, porque un paciente
 * sin edad escrita no tiene cero años.
 */
export function edadParaGuardar({ birthDate, age } = {}) {
