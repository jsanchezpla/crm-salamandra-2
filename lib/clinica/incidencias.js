...
/**
 * De una lista de ids de equipo, los que EXISTEN de verdad, en el orden en que
 * venían del formulario.
 *
 * Se sacó de `sincronizarResponsables` el 18/09/2026 porque el alta necesita la
 * misma comprobación ANTES de crear la fila: `assigned_to_id` y `reported_by_id`
 * tienen clave ajena contra `team_members`, así que un id de una ficha borrada
 * —o de otro tenant— reventaba el INSERT y salía «Error interno del servidor»
 * en vez de crear la incidencia sin responsable. La pivote ya se filtraba; la
 * fila, no.
 */
export async function responsablesQueExisten(ids, tenantModels) {
  if (!ids?.length || !tenantModels?.TeamMember) return [];
  const existentes = (await tenantModels.TeamMember.findAll({ where: { id: ids }, attributes: ["id"] })).map((t) => t.id);
  return ids.filter((id) => existentes.includes(id)); // se respeta el orden del formulario
}
...
export async function sincronizarResponsables(incidencia, ids, tenantModels) {
  const ordenados = await responsablesQueExisten(ids, tenantModels);
