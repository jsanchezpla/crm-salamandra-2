 ...
 *   · `duration`         — minutos, múltiplo de 15 entre 15 y 480. SOLO se
...
export const MENSAJES = Object.freeze({
  ...
  duracionInvalida: "duration inválida: minutos en múltiplos de 15, entre 15 y 480",
  ...
  bonoNoActivo:
    "El bono de este diagnóstico está anulado: reactívalo en Facturación → Bonos (cuadro «Agotados y anulados») y vuelve a apuntar la hora",
  bonoPerdido:
    "El bono de este diagnóstico ya no está: pregunta en Facturación → Bonos antes de apuntarle más horas",
 ...
 *
 * El bono de un diagnóstico no lo elige quien apunta la cita: lo puso el
 * expediente al «Seguir». Así que las frases de `elegirPack`, que están
 * escritas para alguien que acaba de elegir un bono de una lista —«Ese bono
 * está anulado o agotado»—, mandan a esa persona a buscar algo que no sabe que
 * existe. El 17/09/2026 en Aumenta se anularon desde Facturación los dos bonos
 * de diagnóstico que había, y desde entonces apuntar una hora a esos dos
 * expedientes se rechaza con esa frase, que no dice ni que sea del diagnóstico
 * ni dónde se arregla.
 *
 * Se traduce por CÓDIGO (`elegirPack` lo devuelve al lado del texto) y no
 * reconociendo la frase por su letra: un día se reescribe el texto y la
 * traducción dejaría de saltar en silencio. Lo que no tiene frase propia
 * —«otra familia», «otro tipo de cita»— se deja tal cual: ahí el problema es
 * del expediente, no del bono, y la frase original ya lo describe.
 */
export function mensajeDelBonoDelExpediente({ error, codigo } = {}) {
  if (codigo === "no_activo") return MENSAJES.bonoNoActivo;
  if (codigo === "no_existe") return MENSAJES.bonoPerdido;
  return error;
}
