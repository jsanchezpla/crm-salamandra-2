 ...
 */
export function mensajeDeInforme({ transcription, escrito = null, bloques = [], pruebas = null, tipo = "", delPlan = "" }) {
  const partes = [`LO QUE HA DICTADO O APUNTADO LA PROFESIONAL:\n\n${texto(transcription)}`];
  // El plan del paciente, con el motivo y los objetivos de cada terapia
  // (15/09/2026, AV-0143): ya viene redactado por `planParaElInforme`.
  if (typeof delPlan === "string" && delPlan.trim()) partes.push(delPlan.trim());
