...
function tablaDePuntuaciones(doc, F, filas, marca) {
  ...
  const anchoNum = Math.min(88, (ancho * 0.5) / Math.max(fijas, 1));
  ...
  const cabecera = () => {
    let cx = x;
    doc.save();
    doc.rect(x, doc.y, ancho, altoCab).fill(marca.tinteFuerte);
    doc.restore();
    const yc = doc.y + 5;
    doc.font(F.bold).fontSize(8.4).fillColor("#FFFFFF");
    columnas.forEach((c, i) => {
      doc.text(c.label, cx + relleno, yc, { width: anchos[i] - relleno * 2, lineBreak: false });
      cx += anchos[i];
    });
    doc.y = yc + altoCab - 5;
    doc.x = x;
