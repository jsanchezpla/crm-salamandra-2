...
import { columnasConDatos } from "./pruebasDiagnosticas.js";
...
export function apartadoNumerado(doc, F, ap, marca) {
  ...
  if (Array.isArray(ap.pruebas)) {
    // «Resultados de la evaluación» del informe de diagnóstico (05/09/2026).
    bloqueDePruebas(doc, F, ap.pruebas, marca);
  } else if (ap.lista) {
...
/**
 * LAS PRUEBAS DEL INFORME DE DIAGNÓSTICO (05/09/2026, AV-0045 de Aumenta,
 * Isabel Alberca: «cada prueba debería abrir automáticamente una estructura
 * con tres subapartados: breve descripción, resultados con tablas de
 * puntuaciones e interpretación clínica»).
 *
 * Cada prueba es un subtítulo dentro del apartado «Resultados de la
 * evaluación»: nombre en negrita, qué evalúa en cursiva, la tabla de
 * puntuaciones —solo con las columnas que alguna fila rellena— y la
 * interpretación. La tabla es la misma para todas las pruebas a propósito
 * (`pruebasDiagnosticas.js` explica por qué): un baremo por prueba sería
 * setenta y cinco tablas que nadie mantiene.
 */
export function bloqueDePruebas(doc, F, pruebas, marca) {
  const x = doc.page.margins.left;
  const ancho = util(doc);
  for (const p of pruebas) {
    asegurarHueco(doc, 90);
    doc.font(F.bold).fontSize(11).fillColor(marca.oscuro);
    doc.text(p.nombre, x, doc.y, { width: ancho });
    if (texto(p.descripcion)) {
      doc.font(F.italic).fontSize(9.6).fillColor(marca.suave);
      doc.text(p.descripcion, x, doc.y + 1, { width: ancho, align: "justify", lineGap: 1.8 });
    }
    doc.moveDown(0.4);
    if (p.resultados?.length) tablaDePuntuaciones(doc, F, p.resultados, marca);
    if (texto(p.interpretacion)) {
      doc.moveDown(0.2);
      doc.font(F.regular).fontSize(10.2).fillColor(marca.tinta);
      doc.text(p.interpretacion, x, doc.y, { width: ancho, align: "justify", lineGap: 2.6, paragraphGap: 8 });
    }
    doc.moveDown(0.9);
    doc.x = x;
  }
...
/**
 * La tabla de puntuaciones de una prueba: cabecera tintada, una fila por
 * escala, y solo las columnas con datos. Se dibuja a mano con rectángulos y
 * texto porque pdfkit no trae tablas; si una fila no cabe, salta de página
 * repitiendo la cabecera.
 */
function tablaDePuntuaciones(doc, F, filas, marca) {
  const columnas = columnasConDatos(filas);
  if (!columnas.length) return;
  const x = doc.page.margins.left;
  const ancho = util(doc);
  // La primera columna (la escala) es la que lleva texto largo: se lleva lo
  // que dejan las demás, que son números.
  const fijas = columnas.length - 1;
  const relleno = 4;
  // Las columnas numéricas miden lo que su rótulo más largo (revisión del
  // 06/09/2026): con 88 pt de tope, «Clasificación» se partía por la mitad.
  // La escala se queda con el resto, y nunca menos del 38 % de la tabla.
  doc.font(F.bold).fontSize(8.4);
  const masAncho = Math.max(0, ...columnas.slice(1).map((c) => doc.widthOfString(c.label))) + relleno * 2;
  const anchoNum = Math.min(Math.max(56, Math.ceil(masAncho)), (ancho * 0.62) / Math.max(fijas, 1));
  const anchos = columnas.map((c, i) => (i === 0 ? ancho - anchoNum * fijas : anchoNum));
  // La cabecera se mide antes de pintarla (revisión del 06/09/2026): con las
  // cinco columnas, «Clasificación» y «P. típica / escalar» no cabían en una
  // línea y salían cortados («Clasificaci»); ahora el rótulo puede ir en dos
  // líneas y la banda crece con él.
  doc.font(F.bold).fontSize(8.4);
  const altoRotulos = Math.max(
    ...columnas.map((c, i) => doc.heightOfString(c.label, { width: anchos[i] - relleno * 2 }))
  );
  const altoCab = Math.max(18, Math.ceil(altoRotulos) + 8);

  const cabecera = () => {
    // Si la banda cae en el filo de la página, pdfkit abre página en cada
    // rótulo y deja hojas en blanco: se pide sitio para ella y una fila.
    asegurarHueco(doc, altoCab + 30);
    let cx = x;
    const y0 = doc.y;
    doc.save();
    doc.rect(x, y0, ancho, altoCab).fill(marca.tinteFuerte);
    doc.restore();
    doc.font(F.bold).fontSize(8.4).fillColor("#FFFFFF");
    columnas.forEach((c, i) => {
      doc.text(c.label, cx + relleno, y0 + 4, { width: anchos[i] - relleno * 2, lineGap: 0 });
      cx += anchos[i];
    });
    doc.y = y0 + altoCab;
    doc.x = x;
  ...
  cabecera();
  doc.font(F.regular).fontSize(9.2).fillColor(marca.tinta);
  for (const fila of filas) {
    // Alto de la fila: lo que necesite la celda más alta.
    const altos = columnas.map((c, i) => doc.heightOfString(texto(fila[c.key]) || " ", { width: anchos[i] - relleno * 2 }));
    const alto = Math.max(...altos) + relleno * 2;
    ...
    let cx = x;
    columnas.forEach((c, i) => {
      doc.text(texto(fila[c.key]), cx + relleno, y0 + relleno, { width: anchos[i] - relleno * 2 });
      cx += anchos[i];
    });
    filete(doc, y0 + alto, marca.suave, 0.4, x, x + ancho);
    doc.y = y0 + alto;
    doc.x = x;
  }
  doc.moveDown(0.5);
