...
import { esApartadoDeSintesis, estiloClinico, nucleoClinico, restoClinico, haySintesis, lineaDePaciente } from "./estiloClinico.js";
 ...
 */
export function reglasDelReparto({ sintesis = false, contexto = "", centro = null } = {}) {
  return [estiloClinico({ sintesis, contexto, centro }), REGLAS_REPARTO].join("\n\n");
}

/** Lo de las reglas que cambia en cada llamada. El núcleo va aparte. */
function reglasVariables({ sintesis = false, contexto = "" } = {}) {
  return [restoClinico({ sintesis, contexto }), REGLAS_REPARTO].filter(Boolean).join("\n\n");
}
...
/**
 * ── LAS DOS MITADES, PARA PODER CACHEAR (09/09/2026) ───────────────────────
 *
 * `systemCacheado` es lo que NO cambia entre llamadas del mismo centro —la voz,
 * el saber clínico, las reglas, lo que el centro dice de sí mismo—; `system` es
 * lo de ESTE documento —sus apartados, el molde de la respuesta, quién es el
 * paciente—. Van en ese orden porque una caché de prompt es un PREFIJO: lo
 * cacheado tiene que ir delante y ser byte a byte igual.
 *
 * `promptDeRegistro` sigue devolviendo la cadena entera, que es lo que esperan las
 * pruebas y cualquiera que solo quiera leer el prompt. Las dos formas salen de
 * la MISMA función para que no puedan separarse.
 */
export function partesDelPromptDeRegistro(bloques, { paciente = null, centro = null } = {}) {
  ...
  return {
    ...
    system: [
      ...
      reglasVariables({ sintesis, contexto: lineaDePaciente(paciente) }),
      // Lo que NO cabe en ningún apartado deja de tirarse (04/09/2026): el
      // modelo puede proponer apartados nuevos, y la profesional los acepta o no.
      INSTRUCCION_NUEVOS,
    ]
      .filter(Boolean)
      .join("\n\n"),
...
export function promptDeRegistro(bloques, opciones = {}) {
  const { systemCacheado, system } = partesDelPromptDeRegistro(bloques, opciones);
  return [systemCacheado, system].filter(Boolean).join("\n\n");
}
 ...
 * común: a partir de ahí el prompt empieza a costar dinero de verdad.
 *
 * Eran 20.000 caracteres (unas 3.500 palabras). Aumenta pidió más para las
 * entrevistas iniciales (AV-0053, 07/09/2026: «necesitamos entre 25.000 y
 * 30.000»), y la cifra que la caja enseña son CARACTERES, no palabras: 30.000
 * son unas 5.000 palabras, que es lo que una entrevista larga trae escrita.
 */
export const MAX_NOTAS = 30_000;

/**
 * Tope de la transcripción que la pantalla devuelve para una segunda pasada.
 * Mucho más alto que `MAX_NOTAS` a propósito: nadie ESCRIBE 30.000 caracteres,
