...
import { CLAVE_PRUEBAS, CLAVE_ANTES_DE_RESULTADOS, TITULO_RESULTADOS, normalizarPruebas } from "./pruebasDiagnosticas.js";
...
export function apartadosDelInforme(report, tenant = null) {
  ...
  for (const s of definicion) {
    ...
    salen.push({ key: s.key, label: sinNumeroDelante(s.label), lista: Boolean(s.lista), parrafos });
  }

  /*
   * LAS PRUEBAS CON SUS PUNTUACIONES (05/09/2026, AV-0045 de Aumenta): no son
   * un apartado de la plantilla sino `contentSections.pruebas`, y se imprimen
   * como el apartado «Resultados de la evaluación», numerado como uno más y en
   * el índice, justo detrás de «Pruebas administradas» —si ese apartado se
   * imprime— o al final. Cada prueba lleva qué evalúa, su tabla y su
   * interpretación (`documentoPdf.js`, `bloqueDePruebas`). En la beca no aplica.
   */
  const pruebas = beca ? [] : normalizarPruebas(cs[CLAVE_PRUEBAS]);
  if (pruebas.length) {
    const bloque = { key: `__${CLAVE_PRUEBAS}`, label: TITULO_RESULTADOS, lista: false, parrafos: [], pruebas };
    const tras = salen.findIndex((s) => s.key === CLAVE_ANTES_DE_RESULTADOS);
    salen.splice(tras >= 0 ? tras + 1 : salen.length, 0, bloque);
  }
  salen.forEach((s, i) => { s.n = i + 1; });
