 ...
 */
export async function structureTaller({ transcription, apartados, asistentes, etiquetaNota, escrito, centro = null, apiKey, model }) {
  ...
  const { texto: raw, parada } = await completeConParada({
    ...
    ...partesDelPromptDeTaller(bloques, { centro }),
    user: mensajeDeTaller({ transcription, escrito, bloques }),
    model,
    maxTokens: MAX_TOKENS,
    apiKey,
    // Por streaming, como el registro normal: un taller con diez asistentes
    // pide una respuesta larga, y sin stream la petición se queda muda hasta
    // que Claude termina — a veces más de los 120 s del timeout.
    stream: true,
  });
