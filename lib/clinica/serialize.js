...
export function serializePatient(p, extra = {}) {
  ...
  return {
    ...
    client: clientJson
      ? { id: clientJson.id, name: clientJson.name, separated: clientJson.separated ?? null }
      : null,
...
export function serializePerformance(m, { therapist, history = [], teamAverage = null } = {}) {
  const j = m.toJSON ? m.toJSON() : m;
  const areas = PERFORMANCE_AREAS.map((a) => {
    const score = j[`${a.key}Score`] ?? null;
    const level = scoreToSemaforo(score);
    return {
      key: a.key, n: a.n, name: a.name, weight: a.weight, icon: a.icon, score, level,
  ...
  return {
    ...
    totalLevel: scoreToSemaforo(j.totalScore),
    ...
    areas,
    proposedIncentive: num(j.proposedIncentive),
...
// Fila de ranking (panel de Dirección).
export function serializeRankingRow(m, { therapist } = {}) {
  const j = m.toJSON ? m.toJSON() : m;
  const areas = {};
  for (const a of PERFORMANCE_AREAS) areas[a.key] = j[`${a.key}Score`] ?? null;
  return {
    id: j.id,
    therapistId: j.therapistId,
    therapist: therapist ? serializeTherapist(therapist) : null,
    ...
    areas,
    complementsLabel: complementsLabel(j),
    proposedIncentive: num(j.proposedIncentive) ?? 0,
    approvedIncentive: num(j.approvedIncentive),
    approved: j.approvedIncentive != null,
