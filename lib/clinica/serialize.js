...
import { tutoresDeLaFamilia } from "../clients/guardians.js";
import { datosDeLaFamilia } from "../clients/familiaEnLaFicha.js";
import { contactoDeFicha } from "../clients/contactoDeFicha.js";
import { edadDe } from "./edad.js";

// ── Labels (enum → etiqueta ES) ─────────────────────────────────────────────
// `discharged` se rotula «Baja» desde el 15/09/2026 (Rodrigo): Activo / En pausa
// / Baja, los mismos tres que la ficha de la familia (lib/clients/estadoPorActividad.js).
export const PATIENT_STATUS_LABEL = { active: "Activo", paused: "En pausa", discharged: "Baja" };
...
// "asesoramiento" se añadió el 04/09/2026 (Aumenta por Rodrigo): el informe de
// las sesiones de asesoramiento, que el centro ya cobra como tales. Se compone
// con las plantillas del centro como los demás —la beca es la única excepción,
// y lo es porque sus apartados los manda la convocatoria—; su valor en BD lo
// añade scripts/migrate-informe-asesoramiento.js.
// "diagnostico" (05/09/2026, AV-0045 de Aumenta): la valoración diagnóstica,
// con su plantilla de fábrica y el bloque de pruebas con puntuaciones
// (lib/clinica/pruebasDiagnosticas.js); su valor en BD lo añade
// scripts/migrate-informe-diagnostico.js.
export const REPORT_TYPES = ["evolution", "admission", "discharge", "referral", "beca", "asesoramiento", "diagnostico"];
/**
 * Los tipos que se pueden CREAR (03/09/2026, Rodrigo: «los informes con IA de
 * Entrevista Inicial los has clasificado como informes, pero tienen que tener
 * la estructura de los registros de sesión»). La entrevista inicial es un
 * REGISTRO DE SESIÓN con su plantilla de 15 apartados (`plantillas.js`,
 * `PLANTILLA_ENTREVISTA`), que se rellena desde el bloc de notas o el audio
 * con IA como cualquier registro; ofrecerla también como tipo de informe hacía
 * que se escribiera por el camino equivocado. `admission` sigue en
 * `REPORT_TYPES` porque hay informes ya escritos con ese tipo (en producción,
 * uno en Aumenta y los de las demos) y se siguen leyendo e imprimiendo.
 */
export const REPORT_TYPES_NUEVOS = REPORT_TYPES.filter((t) => t !== "admission");
...
/**
 * Cómo se NOMBRA el documento cuando el rótulo va SOLO: la portada del PDF y
 * las dos cabeceras de la pantalla («Informe de alta», no «Informe alta»).
 *
 * No vale `REPORT_TYPE_LABEL`, que está escrita para caber en un chip de la
 * columna «Tipo»: ahí «Evolutivo» se entiende porque tiene la columna al lado,
 * pero suelto, en la portada de un documento que la familia lleva al colegio,
 * «Evolutivo» no es el nombre de nada. Vivía en `reportPdf.js`, que es donde
 * nació; se mudó aquí el 04/09/2026 con el informe de asesoramiento, porque
 * las dos pantallas que escribían «Informe {tipo en minúsculas}» decían
 * «Informe asesoramiento» —y ya venían diciendo «Informe alta» y «Informe
 * beca»—. Una sola lista, tres lectores; `reportPdf.js` la importa de aquí y
 * no puede importarse al revés (arrastra pdfkit al navegador).
 *
 * Un tipo que no esté aquí cae a «Informe», que siempre es cierto.
 *
 * `admission` sigue aquí por los informes que ya existen con ese tipo: desde el
 * 03/09/2026 no se crean más (la entrevista inicial es un REGISTRO DE SESIÓN
 * con su plantilla, ver `plantillas.js`), pero los escritos antes se imprimen
 * con su nombre.
 */
export const REPORT_TYPE_DOCUMENTO = {
  ...
  asesoramiento: "Informe de asesoramiento",
  diagnostico: "Informe de valoración diagnóstica",
};

/** El nombre del documento, con su caída a «Informe». */
export function nombreDelInforme(reportType) {
  return REPORT_TYPE_DOCUMENTO[reportType] ?? "Informe";
}
...
//
// ── SI NO HAY MÉTODOS DE CONTACTO, MANDA LA FICHA (11/09/2026, AV-0124) ────
// Arantxa (Aumenta): «al meterme en la ficha de ciertos pacientes no me sale ni
// el correo ni el número de contacto». Las familias que vinieron de Organízate
// —casi todas— tienen el teléfono y el correo en `clients.phone` y
// `clients.email` (y a veces solo en un tutor), pero ni una fila en
// `client_contact_methods`: esa tabla solo la rellenan el alta y la edición
// desde Clientes. Esto solo miraba la tabla, así que decía «Sin contactos
// registrados» con el número guardado. La lista de pacientes y el alta de cita
// ya caían a la ficha y al tutor (`contactoDeFicha`); aquí faltaba.
//
// Los métodos de contacto siguen mandando cuando existen: son los que
// administración etiqueta a propósito. Solo si no hay ninguno se enseña lo de
// la ficha, y se dice de dónde sale (etiqueta «ficha» o «tutor»).
export function payerContactsOf(client) {
  const c0 = client?.toJSON ? client.toJSON() : client;
  const cms = c0?.contactMethods;
  const filas = Array.isArray(cms)
    ? cms.map((cm) => {
        const c = cm.toJSON ? cm.toJSON() : cm;
        return { id: c.id, kind: c.kind, value: c.value, label: c.label ?? null, isPrimary: !!c.isPrimary };
      })
    : [];
  if (filas.length || !c0) return filas;
  const { email, phone, emailDeTutor, phoneDeTutor } = contactoDeFicha(c0);
  const deFicha = [];
  if (phone) deFicha.push({ id: "ficha-phone", kind: "phone", value: phone, label: phoneDeTutor ? "tutor" : "ficha", isPrimary: false });
  if (email) deFicha.push({ id: "ficha-email", kind: "email", value: email, label: emailDeTutor ? "tutor" : "ficha", isPrimary: false });
  return deFicha;
}
...
export function serializePatient(p, extra = {}) {
  ...
  return {
    ...
    // La edad que se ENSEÑA (03/09/2026, AV-0034): calculada desde la fecha
    // de nacimiento cuando la hay, y si no la escrita a mano. `age` sigue
    // viajando tal cual para el formulario.
    edad: edadDe(j),
    ...
    client: clientJson
      ? {
          ...
          // Padres/tutores de la familia (sprint Aumenta 2026-07-28); llegan
          // solo si el include del endpoint pide el atributo guardians. Desde
          // el 02/09/2026 van RECORTADOS —nombre, parentesco, teléfono y
          // correo, sin DNI ni quién firma— porque la ficha del paciente los
          // enseña a todo el equipo (`tutoresParaFicha`, lib/clients/guardians.js).
          //
          // Desde el 18/09/2026 la lista la arma `tutoresDeLaFamilia`, que mete
          // dentro al TITULAR de la ficha: era el único progenitor que no
          // salía, y 104 familias de Aumenta con pacientes aparecían «sin
          // padres ni tutores apuntados» teniendo su teléfono delante. Por eso
          // el include trae también `taxId` y `customFields`: el primero para no
          // duplicar al titular que ya está dentro de `guardians` (814 familias),
          // el segundo por su `parentescoTitular`. Ninguno de los dos sale de aquí.
          guardians: tutoresDeLaFamilia(clientJson),
          // Los datos de trabajo de la familia (18/09/2026, AV-181): domicilio,
          // localidad y motivo de consulta. Quién decide qué entra y qué no
          // —y por qué el dinero no— está en `lib/clients/familiaEnLaFicha.js`.
          // Las 13 terapeutas de Aumenta no tienen Clientes en su menú: esta es
          // la única pantalla donde pueden verlos.
          familia: datosDeLaFamilia(clientJson),
          // ¿Puede esta persona abrir la ficha de la familia? Lo decide el
          // endpoint con `hasModule("clients")` —del USUARIO, no del centro—;
          // sin él el nombre se pinta sin enlace, en vez de llevar a un 403.
          puedeAbrirse: extra.puedeAbrirLaFicha === true,
        }
      : null,
...
export function serializeSession(s) {
  ...
  return {
    ...
    // De qué sesión de TALLER sale este registro (01/09/2026): null en una
    // sesión normal. La ficha lo usa para marcarlo — el registro común lo
    // escribe quien da el taller, así que verlo aquí sin saber de dónde sale
    // parecería una sesión individual que nadie recuerda haber dado.
    tallerSesionId: j.tallerSesionId ?? null,
    // De qué CITA sale este registro (01/09/2026): null en las 22.045 de
    // siempre y en toda sesión escrita desde la ficha. Lo necesita la pantalla
    // para no abrir una sesión nueva cada vez que se entra a la misma cita
    // (`sesionDeLaCita` en lib/clinica/prepararSesion.js), y el modal de la
    // cita para decir «Seguir con la sesión» en vez de «Preparar sesión».
    bookingId: j.bookingId ?? null,
    // De qué DIAGNÓSTICO es este registro y cómo se titula su entrada en el
    // expediente (12/09/2026, segunda entrega). Null en las 22.045 de siempre;
    // el título por defecto («Sesión de diagnóstico 3») lo pone el expediente,
    // que es quien sabe el número: aquí viaja lo escrito o null.
    diagnosticoId: j.diagnosticoId ?? null,
    titulo: j.titulo ?? null,
    // Cuándo se tocó por última vez. Solo desempata cuando dos sesiones
    // apuntan a la misma cita; sin esto habría que desempatar por la fecha de
    // la SESIÓN, que en ese caso es la misma en las dos.
    updatedAt: isoDate(j.updatedAt),
    ...
    contentSections:
      j.contentSections && typeof j.contentSections === "object" && !Array.isArray(j.contentSections)
        ? j.contentSections
        : {},
    // Notas internas (29/08/2026): solo para el equipo. Este serializador lo
    // consume el dashboard, donde ya hay sesión de un profesional del centro;
    // al portal de la familia no llega ninguna sesión (no hay endpoint que las
    // sirva) y al PDF del informe tampoco — `sesionesDelInforme.js` pide sus
    // columnas por nombre y esta no está en la lista, a propósito.
    internalNotes: j.internalNotes ?? "",
    // Enviado al área privada de la familia (29/08/2026). La pantalla necesita
    // las dos: el id para saber si el botón dice «Enviar» o «Volver a enviar»,
    // y la fecha para poder decir desde cuándo lo tiene la familia.
