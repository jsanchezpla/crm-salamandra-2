 ...
 *
 * 13/09/2026 (motivo, regla #2): recibe `pruebas` (`contentSections.pruebas`)
 * para que el mensaje de usuario lleve la tabla de puntuaciones y para
 * descartar un apartado nuevo «Resultados de la evaluación», que el PDF ya
 * imprime; el prompt sigue construyéndose en `informeMaterial.js`.
 */
...
import { bloquesDelInforme, bloquesOcupados, mensajeDeInforme, partesDelPromptDeInforme } from "./informeMaterial.js";
 ...
 * @param {Array}  [args.pruebas]      `contentSections.pruebas` (la tabla de
 *                                     puntuaciones del diagnóstico, 13/09/2026):
 *                                     viaja en su bloque del mensaje; en la
 *                                     beca no viaja.
 ...
 */
export async function structureInforme({ transcription, apartados, escrito, pruebas = null, delPlan = "", paciente, tipo, centro = null, apiKey, model }) {
  const bloques = bloquesDelInforme(apartados);

  const { texto: raw, parada } = await completeConParada({
    // El núcleo del prompt —la voz, el saber clínico, las reglas— viaja
    // aparte para que Anthropic lo cachee: son 29.437 tokens idénticos en
    // cada llamada, y sin caché costarían +58,74 $/mes de la cuenta del centro.
    ...partesDelPromptDeInforme(bloques, { tipo, contexto: lineaDePaciente(paciente), centro }),
    user: mensajeDeInforme({ transcription, escrito, bloques, pruebas, tipo, delPlan }),
    model,
  ...
  return {
    ...
    // Con tabla, un apartado nuevo titulado «Resultados de la evaluación» se
    // descarta: el PDF ya la imprime con ese título.
    nuevos: apartadosPropuestos(objeto, bloquesOcupados(bloques, { pruebas, tipo })),
    bloques,
    incidencia,
  };
}
