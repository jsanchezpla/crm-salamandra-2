 ...
 */
export function motivoParaNoAvisar({ email, correos } = {}) {
  // Desde el 06/09/2026 (Rodrigo) el aviso va también a los tutores cuando la
  // ficha no tiene correo: lo que se mira es la lista resuelta
  // (`correosParaAvisar`, lib/clients/contactoDeFicha.js); `email` a secas
  // sigue valiendo para quien la traiga ya resuelta.
  const lista = Array.isArray(correos) ? correos.filter((c) => texto(c)) : texto(email) ? [email] : [];
  if (!lista.length) {
    return "La familia no tiene correo ni en su ficha ni en sus tutores, así que el registro se publica en su área privada pero no se avisa por correo.";
  }
  return null;
}
