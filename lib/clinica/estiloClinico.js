...
import { saberClinico } from "./saberClinico.js";
...
export function estiloClinico({ sintesis = false, contexto = "", centro = null } = {}) {
  return [nucleoClinico({ centro }), restoClinico({ sintesis, contexto })].filter(Boolean).join("\n\n");
}

/**
 * ── EL NÚCLEO, QUE ES LO QUE SE CACHEA (09/09/2026) ────────────────────────
 *
 * Todo lo que NO cambia entre una llamada y la siguiente del mismo centro:
 * quién escribe · lo que dice el centro de sí mismo · el saber clínico · la
 * frontera · las marcas · lo prohibido (con lo que añada el centro) · la forma.
 *
 * Va PRIMERO y entero a propósito. Una caché de prompt es un PREFIJO: basta con
 * que delante haya una línea que cambie —la edad del paciente, los apartados de
 * esta plantilla— para que no quede nada que cachear. Hasta hoy la línea del
 * paciente iba metida entre la voz y la frontera, y en el prompt del registro
 * el estilo entero iba DESPUÉS de la lista de apartados: las dos cosas
 * partían el prefijo por el medio.
 *
 * Con el saber clínico dentro esto son ~18.000 tokens. A las ~1.000 llamadas
 * al mes de Aumenta, cachearlo es la diferencia entre +20 $/mes y +1,40 $/mes
 * en SU factura de Anthropic. Y como es byte a byte el mismo en los cuatro
 * prompts clínicos —registro, taller, informe y objetivos—, los cuatro
 * comparten la misma entrada de caché.
 *
 * ⚠️ SI SE AÑADE ALGO AQUÍ, QUE NO DEPENDA DEL CASO. Una sola pieza que varíe
 * con el paciente convierte cada llamada en una escritura de caché, que cuesta
 * el doble que una entrada normal: se pagaría por cachear y no se acertaría
 * nunca.
 */
export function nucleoClinico({ centro = null } = {}) {
  return [
    VOZ,
...
/**
 * Lo que cambia en cada llamada: quién es el paciente y si este documento tiene
 * apartados que se elaboran. Va DETRÁS del núcleo, siempre.
 */
export function restoClinico({ sintesis = false, contexto = "" } = {}) {
  return [contexto, sintesis ? SINTESIS : ""].filter(Boolean).join("\n\n");
}
