...
import { estiloClinico, lineaDePaciente, contextoDelPaciente, nucleoClinico, restoClinico } from "./estiloClinico.js";
export { edadDe };
/*
 ...
 */
export function promptObjetivos({ ideas, plan = {}, paciente = {}, centro = null, motivoDeSuTerapia = "" }) {
  // El núcleo va aparte para que se cachee: 29.437 tokens idénticos en cada
  // llamada, compartidos con los otros tres prompts clínicos (09/09/2026).
  const systemCacheado = nucleoClinico({ centro });
  const system = [
    restoClinico({ contexto: lineaDePaciente(paciente) }),
  ...
  const user = [
    ...
    // El de la terapia de quien pide los objetivos (15/09/2026, AV-0143).
    motivoDeSuTerapia ? `Motivo de consulta en la terapia de esta profesional: ${texto(motivoDeSuTerapia, 4000)}` : null,
  ...
  return { system, systemCacheado, user };
}
