 ...
 *
 * ── DE UNA OPCIÓN A VARIAS (18/09/2026, AV-198 de Aumenta) ────────────────
 * «Que el filtro sea múltiple y no solo de una opción»: `category`,
 * `reportedById` y `assignedToId` admiten VARIOS valores, separados por comas
 * (`?category=terapeutica,laboral`) o repitiendo el parámetro. Uno solo sigue
 * valiendo tal cual, así que lo de siempre —la campana, el Excel, un enlace
 * guardado— no cambia. Quien los lee es `listaDeParametro`, en un solo sitio,
 * para que la lista y su Excel no puedan entender cosas distintas.
 */
...
/**
 * Los valores de un parámetro que puede venir repetido (`?x=a&x=b`) o con comas
 * (`?x=a,b`), sin repetidos y sin huecos. Con `validar` se queda SOLO con los
 * que valen: un id inventado en la URL no cuela una condición, igual que cuando
 * el filtro era de uno. Devuelve `[]` cuando no hay nada por lo que filtrar.
 */
export function listaDeParametro(sp, clave, validar) {
  const crudos = sp.getAll(clave).flatMap((v) => String(v).split(","));
  const limpios = crudos.map((v) => v.trim()).filter(Boolean);
  const buenos = validar ? limpios.filter(validar) : limpios;
  return [...new Set(buenos)];
}

/** El valor a secas con uno, `{ [Op.in]: [...] }` con varios. */
function igualOEntre(valores) {
  return valores.length === 1 ? valores[0] : { [Op.in]: valores };
}
...
export async function whereDeIncidencias({ request, sp, M, ctx }) {
   ...
   *
   * ── Y AHORA LAS DOS A LA VEZ (18/09/2026, AV-198 de Aumenta) ─────────────
   * Rosa, por el Buzón: «poder filtrar más de un tipo de incidencia (ej:
   * faltas, pendientes y en proceso)». Así que `status` admite varios y puede
   * venir CON `faltas=1`: entonces son dos ramas unidas por O —las de siempre
   * con esos estados, más las faltas—, no una condición imposible. Cada una
   * por separado sigue dando exactamente lo de antes.
   */
  const estados = listaDeParametro(sp, "status", isValidStatus);
  const conFaltas = sp.get("faltas") === "1";
  const deSiempre = { falta: null, ...(estados.length ? { status: igualOEntre(estados) } : {}) };
  const pestana = !conFaltas
    ? deSiempre
    : estados.length
      ? { [Op.or]: [deSiempre, { falta: { [Op.ne]: null } }] }
      : { falta: { [Op.ne]: null } };
  Object.assign(where, pestana);
  const categorias = listaDeParametro(sp, "category", isValidCategory);
  if (categorias.length) where.category = igualOEntre(categorias);
  ...
  const registradaPor = listaDeParametro(sp, "reportedById", (v) => UUID_RE.test(v));
  if (registradaPor.length) where.reportedById = igualOEntre(registradaPor);
  ...
  /*
   * ── «LAS MÍAS» SON LAS DOS COSAS (18/09/2026, AV-198 de Aumenta) ──────────
   * Rosa: «que aparecieran por defecto en cada usuario las que ha registrado y
   * debe resolver ese usuario». Hasta hoy `mine=1` filtraba SOLO por
   * responsable, así que a dirección no le salían las que ella misma había
   * apuntado para otra persona —que son justo las que quiere seguir—. Ahora es
   * la MISMA regla que el alcance de quien no es dirección
   * (`whereIncidenciasVisibles`: la registré yo O soy responsable), escrita una
   * sola vez para que las dos no puedan divergir.
   *
   * Sin ficha de equipo —dirección, o quien entra con un usuario que no está en
   * la plantilla— «las mías» no significa nada, y en su día esto ponía un id
   * imposible que devolvía CERO incidencias: la pantalla se abría vacía y
   * parecía que no había ninguna. En ese caso no se filtra.
   *
   * Entra como AND y aparte del filtro de responsable, así que «las mías» + «de
   * Olga» son las que me atañen Y lleva Olga, que es lo que se espera al marcar
   * los dos.
   */
  /*
   * ── LAS FALTAS NO SON DE NADIE, ASÍ QUE NO LAS ESCONDE (18/09/2026) ───────
   * El mismo día entraron dos cambios correctos por separado que juntos
   * escondían las faltas justo a quien las gestiona: AV-0197 dejó de asignarle
   * la falta a nadie (`assignedToId: null`, y quien la registra es la terapeuta
   * que la marcó) y AV-198 dejó la pantalla abierta en «Solo las mías». Quien
   * lleva administración no es ninguna de las dos cosas en una falta, así que
   * abría Incidencias y veía CERO de 42 — y el contador de la marca «Faltas»
   * también decía 0, porque se cuenta con este mismo filtro puesto.
   *
   * Jorge, 18/09/2026: las faltas se salen de «las mías». No es una excepción
   * escondida: la casilla lo DICE, y la regla es la de la propia falta —no
   * tiene dueño, luego no puede quedar fuera de un filtro que va por dueño—.
   *
   * Ojo con lo que NO toca esto: el ALCANCE de quien no es dirección
   * (`whereIncidenciasVisibles` unas líneas más arriba) se queda igual. Ese es
   * el freno de privacidad de AV-0018 —una terapeuta no ve las incidencias de
   * otra—, y sacarle las faltas enseñaría a todo el equipo a qué paciente le
   * falló la cita de quién. Esto de aquí es solo la casilla de dirección.
   */
  if (sp.get("mine") === "1" && esAdmin && yoSoy) {
    (where[Op.and] ||= []).push({
      [Op.or]: [await whereIncidenciasVisibles(M, yoSoy), { falta: { [Op.ne]: null } }],
    });
  }

  const responsables = listaDeParametro(sp, "assignedToId", (v) => UUID_RE.test(v));
  if (responsables.length) {
    ...
    //
    // Con VARIAS personas es O, no Y (18/09/2026, AV-198): «las de Olga y las
    // de Rosa» es lo que se quiere ver en una lista de trabajo; exigir que las
    // dos lleven la MISMA incidencia devolvería casi siempre nada.
    if (M.IncidenciaAssignee) {
      const enlaces = await M.IncidenciaAssignee.findAll({
        where: { teamMemberId: { [Op.in]: responsables } },
        attributes: ["incidenciaId"],
      });
      where.id = { [Op.in]: [...new Set(enlaces.map((e) => e.incidenciaId))] };
    } else {
      where.assignedToId = igualOEntre(responsables); // tenant sin migrar
    }
  }

  /*
   * Los recuentos de las pestañas se cuentan con TODO menos la pestaña (si no,
   * «Pendientes» siempre diría lo mismo que la lista). Antes el endpoint
   * borraba `status` y `falta` del `where` a mano; ahora que la pestaña puede
   * ser un O de dos ramas, eso ya no vale y se devuelve aparte.
   */
  const sinPestana = { ...where };
  for (const k of Object.keys(pestana)) delete sinPestana[k];
  for (const s of Object.getOwnPropertySymbols(pestana)) delete sinPestana[s];

  return { where, whereSinPestana: sinPestana, yoSoy, misVistas, verVistas, esAdmin, conFaltas, estados };
}
