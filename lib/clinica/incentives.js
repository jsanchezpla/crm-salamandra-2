...
/**
 * Puntuación total (0-100): media ponderada de las áreas puntuadas usando sus
 * pesos. Las áreas sin valor NO cuentan (se renormaliza por el peso de las que
 * sí tienen). Devuelve null si no hay ninguna área puntuada.
 *
 * `scores` va cotejado por clave de área ("area1", "area2"…), no por columna.
 *
 * (Cambio 2026-07-29, desempeño por roles — regla #2: `areas` es un segundo
 * parámetro OPCIONAL con las áreas del rol del evaluado. Su default son las
 * PERFORMANCE_AREAS históricas, así que los call-sites existentes no cambian.)
 */
export function computeTotalScore(scores, areas = PERFORMANCE_AREAS) {
  let acc = 0;
  let w = 0;
