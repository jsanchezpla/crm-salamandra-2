...
import { buildMadridDate, getMadridParts } from "../citas/slots.js";
...
export async function citasDeTallerQueImparte({ tenantModels, teamMemberId, desde, hasta }) {
  ...
  // Una persona o varias (11/09/2026): el calendario con agenda compartida
  // filtra por las que se han marcado, y hay que ampliar por todas ellas.
  const ids = (Array.isArray(teamMemberId) ? teamMemberId : [teamMemberId]).filter(Boolean);
  if (!TallerCitaTerapeuta || !Booking || !ids.length) return [];
  try {
    const { Op } = await import("sequelize");
    const filas = await TallerCitaTerapeuta.findAll({
      where: { teamMemberId: ids.length === 1 ? ids[0] : { [Op.in]: ids } },
...
/** El día de Madrid en el que cae un instante: [desde, hasta) en UTC. */
export function limitesDelDiaMadrid(instante) {
  const d = instante instanceof Date ? instante : new Date(instante);
  if (Number.isNaN(d.getTime())) return null;
  const p = getMadridParts(d);
  const desde = buildMadridDate(p.year, p.month, p.day, 0, 0);
  const siguiente = new Date(Date.UTC(p.year, p.month - 1, p.day) + 24 * 60 * 60 * 1000);
  const hasta = buildMadridDate(siguiente.getUTCFullYear(), siguiente.getUTCMonth() + 1, siguiente.getUTCDate(), 0, 0);
  return { desde, hasta };
}

/**
 * El registro de sesión de UNA tarde de taller (11/09/2026, AV-0108 de
 * Aumenta).
 *
 * Laura Garrido: «me aparece en mi horario, pero si la sesión la registra
 * Daniela yo no puedo verla y me sale como no completada». Un grupo lo dan dos
 * terapeutas, pero la cita de cada martes es de UNA (`bookings.team_member_id`
 * solo admite una), y el registro colgaba de esa cita por `booking_id`: desde
 * la cita de la otra semana —o desde una cita duplicada de la misma tarde— no
 * se encontraba, y la pantalla ofrecía «Registrar la sesión» como si no hubiera
 * nada, con lo que el grupo acababa con dos registros de la misma tarde.
 *
 * Primero por la cita (lo rápido y lo de siempre); si no hay, por el GRUPO en
 * el MISMO DÍA de Madrid (el índice `taller_sesiones_grupo_fecha_idx` ya
 * existe). `deOtraCita` dice si vino por el segundo camino, para que la
 * pantalla pueda decir de quién es.
 */
export async function registroDeLaTarde({ tenantModels, booking }) {
  const { TallerSesion } = tenantModels;
  if (!TallerSesion || !booking?.id) return { sesion: null, deOtraCita: false };
  try {
    const propio = await TallerSesion.findOne({ where: { bookingId: booking.id } });
    if (propio) return { sesion: propio, deOtraCita: false };
    const limites = booking.tallerGrupoId ? limitesDelDiaMadrid(booking.scheduledAt) : null;
    if (!limites) return { sesion: null, deOtraCita: false };
    const { Op } = await import("sequelize");
    const delGrupo = await TallerSesion.findOne({
      where: { grupoId: booking.tallerGrupoId, sessionDate: { [Op.gte]: limites.desde, [Op.lt]: limites.hasta } },
      order: [["createdAt", "ASC"]],
    });
    return { sesion: delGrupo ?? null, deOtraCita: Boolean(delGrupo) };
  } catch (err) {
    if (tablaAusente(err)) return { sesion: null, deOtraCita: false };
    throw err;
  }
}
...
export async function detalleDeCitaDeTaller({ tenantModels, booking }) {
  ...
  try {
    ...
    // diga «Ver registro» en vez de «Registrar sesión». Desde el 11/09/2026 se
    // busca también por grupo y día: ver `registroDeLaTarde`.
    const { sesion, deOtraCita } = await registroDeLaTarde({ tenantModels, booking });
    ...
    return {
      ...
      sesion: sesion ? { id: sesion.id, status: sesion.status, deOtraCita } : null,
    };
  } catch (err) {
