...
/**
 * El TÍTULO de un registro (12/09/2026, segunda entrega del Diagnóstico):
 * «Sesión de diagnóstico 3», «Pruebas WISC-V». 160 caracteres, el mismo
 * número que la columna `clinic_sessions.titulo`.
 *
 * Vive aquí y no en `registroDeDiagnostico.js` —que lo re-exporta— porque el
 * título es un campo del REGISTRO (la columna es de `clinic_sessions`) y este
 * es el fichero que arma lo que se manda al alta; y porque
 * `registroDeDiagnostico.js` importa de aquí `colaDePreparacion`, y un
 * `import` en los dos sentidos es lo que la regla 16 pide evitar.
 */
export const MAX_TITULO = 160;

/** El título, recortado; `null` si no hay nada (que es lo que quita el que había). */
export function limpiarTitulo(valor) {
  const v = String(valor ?? "").replace(/\s+/g, " ").trim();
  return v ? v.slice(0, MAX_TITULO) : null;
}
 ...
 */
export function payloadDePreparacion({
  patientId,
  therapistId,
  fecha,
  prepText,
  ...
  const foto = clave
    ...
    : null;
  // De qué DIAGNÓSTICO es y cómo se titula (12/09/2026). Solo cuando vienen,
  // por lo mismo que la cita: un registro de siempre no es de ningún
  // expediente. El servidor, cuando hay cita, hereda el expediente de ELLA por
  // encima de esto (`bookings.diagnostico_id`); sin cita, comprueba que sea
  // del mismo paciente.
  const expediente = String(diagnosticoId ?? "").trim();
  ...
  return {
    ...
    ...(UUID_RE.test(expediente) ? { diagnosticoId: expediente } : {}),
    ...(tituloLimpio ? { titulo: tituloLimpio } : {}),
  };
}
 ...
 *
 * ── NI LA DE OTRA PROFESIONAL (08/09/2026, AV-0083 de Aumenta) ─────────────
 * Y tampoco se adopta la sesión suelta de otra terapeuta. La hora exacta parece
 * bastante desempate, pero no lo es en un centro donde un mismo niño tiene dos
 * citas a la vez con dos profesionales: hoy hay 38 parejas así, y 57 de los 268
 * pacientes de septiembre los ven dos o más. Adoptarla significaría abrir a
 * escribir ENCIMA de la nota clínica de otra, firmada por ella.
 *
 * Todavía no ha mordido —de las 210 sesiones atadas a una cita, ninguna es de
 * una terapeuta distinta a la de su cita—, y se cierra antes de que muerda.
 *
 * @param {Array} sesiones  las del paciente, tal como las devuelve la API
 * @param {{ bookingId?: string, scheduledAt?: string|Date, deLaTerapeuta?: string|null }} cita
 * @returns {{ sesion: object, via: "cita"|"fecha" }|null}
 */
export function sesionDeLaCita(sesiones, { bookingId = null, scheduledAt = null, deLaTerapeuta = null } = {}) {
  ...
  const suya = String(deLaTerapeuta ?? "").trim();
  const candidatas = lista.filter((s) => {
    ...
    // Ni la de otra profesional (08/09/2026): escribiría encima de su nota.
    if (suya && String(s?.therapistId ?? "") !== suya) return false;
 ...
 *
 * ── Y DE LA MISMA PROFESIONAL (08/09/2026, AV-0083 de Aumenta) ──────────────
 * Raquel: «me aparece la preparación de la sesión de Blanca en pacientes que
 * compartimos, pero no es la preparación para mi próxima sesión, sino la de
 * otra terapeuta». Y era literal: se cogía la sesión anterior del PACIENTE,
 * fuera de quien fuera. En Aumenta 134 de 544 pacientes con historia los ven
 * dos, tres o cuatro profesionales, y 98 de las 211 notas de «Próximas
 * sesiones» están en pacientes compartidos: casi la mitad se heredaban al
 * despacho de al lado.
 *
 * No es solo que estorbe. «Próximas sesiones» es el plan de ESA línea de
 * trabajo —logopedia no continúa lo de psicología—, y si la que lo hereda le da
 * a guardar, el plan de una acaba firmado como preparación de la otra.
 *
 * La regla: si sabemos quién prepara la sesión, solo hereda de las suyas. Si no
 * lo sabemos —una sesión sin profesional, o un centro que no las firma— se
 * queda como estaba, porque ahí no hay a quién confundir con quién. Y las
 * sesiones sin firmar nunca se heredan a alguien con nombre: en Aumenta son
 * 4.297 registros viejos importados, y atribuirlos sería adivinar.
 *
 * @param {Array} sesiones   las del paciente, como las devuelve la API
 * @param {{ antesDe: Date|string, excluirId?: string|null, deLaTerapeuta?: string|null }} opciones
 * @returns {{ texto: string, sesion: object }|null}
 */
export function proximasSesionesPendientes(sesiones, { antesDe, excluirId = null, deLaTerapeuta = null } = {}) {
  const lista = Array.isArray(sesiones) ? sesiones : [];
  ...
  for (const s of lista) {
    ...
    if (suya && String(s.therapistId ?? "") !== suya) continue;
