...
import { estiloClinico, lineaDePaciente, nucleoClinico } from "./estiloClinico.js";
...
const QUE_VA_EN = {
  ...
  evolution:
    "el cuerpo del informe, y va POR ASPECTOS TRABAJADOS, no sesión a sesión. Junta todo lo que se ha hecho sobre un mismo aspecto —aunque se viera en cuatro sesiones distintas y separadas en el tiempo— y cuéntalo de una vez: qué se trabajó, con qué tipo de tarea, cómo respondió y qué ha cambiado desde el principio. Nombra los procesos implicados y di qué sugiere lo observado. Un párrafo por aspecto; NUNCA un párrafo por sesión, ni frases que empiecen por una fecha.",
  achievements:
 ...
 */
function systemDelInforme({ contexto, hayQueProponer, centro = null }) {
  return [
    estiloClinico({ sintesis: hayQueProponer, contexto, centro }),
...
2. LAS FECHAS: no añadas NINGUNA que no esté en las anotaciones —ni una—. Conservar las que vienen depende del apartado: en la EVOLUCIÓN la fecha solo se escribe cuando marca un hito (un cambio claro, un alta, una prueba pasada), porque ahí se cuenta por aspectos trabajados y no por sesiones; en los demás apartados se conservan como vengan y en su orden. Quitar una fecha no es perder información: el hecho que la acompaña tiene que seguir estando (regla 1).
...
async function pedirPropuesta({ system, systemCacheado = null, user, model, apiKey, claves }) {
 ...
 */
export async function pulirInforme({ contentSections, paciente = null, centro = null, apiKey, model }) {
  ...
  const entero = systemDelInforme({
    contexto: lineaDePaciente(paciente),
    hayQueProponer: aProponer.length > 0,
    centro,
  });
  /*
   * Partido para que el núcleo se cachee (09/09/2026): son 29.437 tokens
   * idénticos en cada llamada. Y aquí importa el doble, porque pulir un informe
   * puede pedir DOS veces al modelo —la propuesta y, si no pasa
   * `verificarSinInventar`, la corrección—: la segunda es acierto de caché
   * seguro.
   */
  const systemCacheado = nucleoClinico({ centro });
  const system = entero.split(systemCacheado).join("").replace(/\n{3,}/g, "\n\n").trim();
  const user = mensajeDelInforme(entrada, aProponer);

  let { propuesta, incidencia } = await pedirPropuesta({ system, systemCacheado, user, model, apiKey, claves });
