 ...
 *     «Pruebas administradas» (`apartadosInforme.js`), y viaja a la IA del
 *     informe (dictado y «Unir») como texto, `pruebasParaLaIA` (13/09/2026).
 *
...
/* ═══ Lo que ve la IA (13/09/2026) ═════════════════════════════════════════ */
// (Regla #2: la forma de `contentSections.pruebas` vive aquí, así que su texto
// para la IA también; el fichero sigue sin imports.)

/**
 * Tope del texto de la tabla que viaja a la IA. `normalizarPruebas` acota cada
 * pieza, no el conjunto (~1,2 M de caracteres en el peor caso). Pasarse es un
 * 413 que dice qué acortar, nunca un recorte en silencio: recortar quitaría
 * puntuaciones que la IA no puede inventar. 120.000 caben con el material
 * máximo dentro del contexto; un diagnóstico completo con interpretaciones
 * largas ronda los 100.000.
 */
export const MAX_PRUEBAS_PARA_IA = 120_000;

/** Las columnas con su nombre completo: la IA no tiene por qué saber qué es «P. directa». */
const COLUMNAS_PARA_IA = Object.freeze({
  pd: "puntuación directa",
  pt: "puntuación típica o escalar",
  pc: "percentil",
  clasificacion: "clasificación",
});

/**
 * `contentSections.pruebas` → el texto que lee la IA del informe. "" si no hay
 * ninguna. Solo lo que consta: una columna en blanco no sale, y una prueba sin
 * filas lo dice («ninguna») en vez de callárselo.
 */
export function pruebasParaLaIA(bruto) {
  const pruebas = normalizarPruebas(bruto);
  if (!pruebas.length) return "";
  const nombreDeArea = new Map([...AREAS_PRUEBAS.map((a) => [a.key, a.nombre]), ["otras", "Otras pruebas"]]);
  const conValor = COLUMNAS_PUNTUACION.filter((c) => c.key !== "escala");
  return pruebas
    .map((p) => {
      const lineas = [`### Prueba: ${p.nombre}`];
      if (nombreDeArea.has(p.area)) lineas.push(`Área: ${nombreDeArea.get(p.area)}`);
      if (p.descripcion) lineas.push(`Qué evalúa: ${p.descripcion}`);
      if (p.resultados.length) {
        lineas.push("Puntuaciones anotadas:");
        for (const f of p.resultados) {
          const celdas = conValor
            .filter((c) => f[c.key])
            .map((c) => `${COLUMNAS_PARA_IA[c.key] ?? c.label.toLowerCase()}: ${f[c.key]}`);
          lineas.push(`- ${f.escala || "Sin escala anotada"} — ${celdas.length ? celdas.join("; ") : "sin puntuación anotada"}`);
        }
      } else {
        lineas.push("Puntuaciones anotadas: ninguna.");
      }
      if (p.interpretacion) lineas.push(`Interpretación escrita por la profesional: ${p.interpretacion}`);
      return lineas.join("\n");
    })
    .join("\n\n");
}
