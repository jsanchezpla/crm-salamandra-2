...
// documents/{slug}/{shared|uuid}/{uuid}.{pdf|docx|xlsx}
const STORAGE_PATH_RE = /^documents\/[a-z0-9_]+\/(shared|[0-9a-f-]{36})\/[0-9a-f-]{36}\.(pdf|docx|xlsx)$/i;
 ...
 */
export async function saveDocumentFile(tenantSlug, ownerSegment, documentId, buffer, mimeType) {
  if (!SLUG_RE.test(tenantSlug)) throw new Error(`Invalid tenant slug: ${tenantSlug}`);
  ...
  await fs.writeFile(path.join(dir, `${documentId}.${ext}`), buffer);

  return `documents/${tenantSlug}/${ownerSegment}/${documentId}.${ext}`;
}
