...
import { cuerpoDeFichero } from "../utils/cuerpoDeFichero.js";
 ...
 * Devuelve { stream, size }. `stream` es un `ReadableStream` web listo para
 * `new Response(stream, …)`, no el de Node: el porqué, en
 * `lib/utils/cuerpoDeFichero.js` (11/09/2026).
 */
export async function readDocumentStream(tenantSlug, storagePath) {
  ...
  return { stream: cuerpoDeFichero(abs), size: st.size };
}

/**
 * El archivo entero en memoria, para quien tiene que LEERLO y no servirlo
 * (15/09/2026: el Plan saca el motivo de consulta de los informes PDF). Con el
 * mismo blindaje de ruta que el stream; el tope de 25 MB por archivo lo acota.
 */
export async function readDocumentBuffer(tenantSlug, storagePath) {
  return fs.readFile(absFromStoragePath(tenantSlug, storagePath));
}
