 ...
 */
export async function guardarContrato({ tenantModels, tenantSlug, file, nombre, ownerUserId, ctx = null }) {
  ...
  /*
   * La cuota del CENTRO, no la de fábrica (08/09/2026, AV-0079 de Aumenta).
   *
   * Isabel: «al subir el contrato estándar escaneado me pone que la cuota de
   * almacenamiento está superada y no deja subirlo». Y era esta línea: cuando
   * la cuota pasó a ser por cliente (26/08/2026, `documents.quotaBytes`) se
   * cambiaron los diez sitios que suben ficheros y se quedó este.
   *
   * Aumenta tiene 10 GB dados y usaba 6,2 GB —3,9 libres—, pero el contrato se
   * medía contra el gigabyte de fábrica, así que le decía que no teniendo
   * sitio de sobra.
   *
   * `ctx` es opcional para no romper a quien llame sin él: sin contexto se cae
   * a la de fábrica, que es lo que hacía siempre.
   */
  const usage = await getTenantStorageUsage(tenantSlug);
  const cuota = ctx ? quotaBytesDe(ctx) : TENANT_QUOTA_BYTES;
  if (usage + realSize > cuota) return { error: "Cuota de almacenamiento superada", status: 507 };
