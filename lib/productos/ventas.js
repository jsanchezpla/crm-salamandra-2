 ...
 *
 * ── LAS VARIANTES (03/09/2026) ──────────────────────────────────────────────
 * Una camiseta con tallas S, M y L es UNA fila del ranking —para reponer hace
 * falta saber qué talla se vende, no solo qué camiseta—, así que cada fila
 * lleva `variantes`: lo vendido de cada talla u opción, por `variantId` (o por
 * el nombre copiado en la línea si la variante ya no existe), ordenado por
 * unidades. Una línea sin variante no crea ninguna: un producto sin tallas
 * tiene `variantes: []`, y una camiseta vendida «a secas» suma al producto
 * pero no a ninguna talla.
 */
...
export function agregarVentas(pedidos = [], { activos = [], costes = null, fuenteCoste = "ficha" } = {}) {
  ...
  for (const p of ventas) {
    ...
    for (const l of p.lines ?? []) {
      ...
      f.importe += lineTotal;
      f.pedidos.add(p.id);
      if (l.variantId || l.variantName) {
        const claveVariante = l.variantId || `nombre:${l.variantName}`;
        const v = f.variantes.get(claveVariante) ?? {
          variantId: l.variantId || null,
          nombre: l.variantName || "(sin nombre)",
          unidades: 0,
          importe: 0,
        };
        v.unidades += cantidad;
        v.importe += lineTotal;
        f.variantes.set(claveVariante, v);
      }
      if (coste !== null) {
  ...
  const ranking = [...porProducto.values()]
    .map((f) => ({
      ...
      variantes: [...f.variantes.values()]
        .map((v) => ({ ...v, unidades: redondea(v.unidades), importe: redondea(v.importe) }))
        .sort((a, b) => b.unidades - a.unidades || b.importe - a.importe || a.nombre.localeCompare(b.nombre, "es")),
    }))
