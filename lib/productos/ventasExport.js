...
export async function buildVentasXlsx(datos, { tenantName } = {}) {
  ...
  // Solo si algún producto se vendió por talla u opción: una hoja vacía en un
  // centro sin variantes es una pregunta sin respuesta.
  const porVariante = datos.porProducto.flatMap((f) =>
    (f.variantes ?? []).map((v) => ({ producto: f.nombre, variante: v.nombre, unidades: v.unidades, importe: v.importe }))
  );
  if (porVariante.length) {
    hoja(
      ...
      [
        ...
        { header: "Talla u opción", key: "variante", width: 22 },
        { header: "Unidades", key: "unidades", width: 11, style: { numFmt: NUM_UDS } },
        { header: "Vendido (€)", key: "importe", width: 14, style: { numFmt: NUM_EUR } },
      ],
      porVariante
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (datos.porProducto.length) {
        ...
        const conVariantes = datos.porProducto.filter((f) => (f.variantes ?? []).length > 0).slice(0, 10);
        if (conVariantes.length) {
          bloqueTitulo(doc, F, "Por talla u opción");
          tablita(
            doc,
            F,
            ["Producto · talla u opción", "Unidades", "Vendido"],
            conVariantes.flatMap((f) => f.variantes.map((v) => [`${f.nombre} · ${v.nombre}`, num(v.unidades), eur(v.importe)]))
          );
        }
      }
