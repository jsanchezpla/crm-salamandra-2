...
export const SLUGS = [
  ...
  "gm_alvar_alonso",
  // Alta del 23/08/2026 (reina de `tienda` y `booking`). Faltó aquí tres
  // semanas: «· aumenta, laura_ubeda» salía solo en el grupo de Aumenta.
  "laura_ubeda",
];
...
export function trocearTodo(texto) {
  ...
  lineas.forEach((linea, i) => {
    ...
    if (h3) {
      ...
      let titulo = bruto;
      let quien = null;
      // `quien` es lo que se ENSEÑA (sin comillas invertidas); `quienBruto` es
      // lo que se vuelve a escribir si alguien reescribe la tarea desde el
      // tablero, para que el documento no se despeine solo.
      let quienBruto = null;
      let quienes = [];
      if (corte > 0) {
        // La cola TAL CUAL está escrita, con sus comillas invertidas. Se guarda
        // aparte porque el editor la vuelve a escribir al reescribir una tarea:
        // sin esto, la primera vez que se toca una tarea desde el tablero se le
        // caen las comillas del slug (`aumenta` → aumenta) y el documento se va
        // despeinando solo, tarea a tarea. Pasó a la primera, el 24/08/2026, al
        // darle su ficha a la tarea de la agenda de Aumenta.
        const colaBruta = bruto.slice(corte + 1).trim();
        const cola = colaBruta.replace(/`/g, "").trim();
      ...
      tarea = { titulo, quien, quienBruto, quienes, cuerpo: [], linea: i + 1 };
      return;
    }
...
export function comprobar(texto, nombre) {
  ...
  /*
   * ── DOS TÍTULOS IGUALES: ERROR SOLO SI DE VERDAD SE PISAN (19/09/2026) ────
   * Esto era error siempre, porque hasta el 24/08/2026 el título ERA la
   * identidad de una tarea. Desde que existe la ficha ya no lo es, y la regla
   * vieja dejó atascados dos casos reales del Buzón el 19/09/2026: dos mensajes
   * con el mismo asunto caen en Resuelto bajo la misma fecha —la sección de
   * Resuelto es un día, no un cajón lógico— y el segundo no se podía enviar.
   *
   * Con las dos fichas puestas se distinguen para todo lo que decide algo: el
   * tablero las localiza por ficha y los adjuntos cuelgan de ella.
   *
   * Sin ficha sí se pisan de verdad —`localizar` por clave devuelve siempre la
   * primera, o sea que editar una tocaría la otra—, así que ahí sigue siendo
   * error.
   *
   * ── Y EN RESUELTO NI SIQUIERA SE AVISA (19/09/2026, Rodrigo) ──────────────
   * El aviso duró unas horas: en Resuelto la sección es un DÍA, dos mensajes
   * del mismo asunto cerrados la misma tarde son lo normal y no hay nada que
   * hacer al respecto. Pero el panel del Buzón pinta los avisos en rojo, en el
   * mismo sitio que un fallo, así que Rodrigo mandó el suyo, entró bien y leyó
   * que no se había podido. Un aviso que no se puede atender es ruido. En el
   * backlog se queda: allí dos tareas que se llaman igual comparten el tick y
   * el reparto, y eso se ve al priorizar.
   */
  for (const s of deVerdad) {
    const titulos = new Map();
    for (const t of s.tareas) {
      if (titulos.has(t.titulo)) {
        const otra = titulos.get(t.titulo);
        if (t.id && otra && otra !== t.id) {
          if (nombre === "backlog") {
            avisos.push(
              `Dos tareas con el mismo título en «${s.titulo}»: «${t.titulo}» (línea ${t.linea}). Se distinguen por su ficha, pero comparten el tick y el reparto del tablero.`
            );
          }
        } else {
          errores.push(
            `Dos tareas con el mismo título en «${s.titulo}»: «${t.titulo}» (línea ${t.linea}). Sin ficha propia se pisan: dale otro título a una, o tócala desde el tablero para que gane la suya.`
          );
        }
      } else {
        titulos.set(t.titulo, t.id ?? null);
      }
