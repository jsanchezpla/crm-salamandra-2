 ...
 *
 * EL REPARTO, EN UNA FRASE: el texto de cada tarea vive en `docs/backlog.md` y
 * `docs/resuelto.md`; lo que se cambia en caliente desde la pantalla —de quién
 * es y si ya está— vive en `master.tablero_estado` y se pinta ENCIMA.
 *
 * No es una preferencia: los dos ficheros viajan DENTRO de la imagen de Docker
 * (`Dockerfile:33`), así que lo que la pantalla escribiera en ellos se lo
 * llevaría el siguiente despliegue sin dar ningún error.
 */
...
export function repartirPorEstado(pendiente, resuelto, estados) {
  ...
  const limpiar = (secciones, fuente, mover, sePasa) =>
    (secciones ?? [])
      .map((s) => {
        const quedan = [];
        for (const t of (s.tareas ?? []).map((x) => conEstado(x, estados, fuente))) {
