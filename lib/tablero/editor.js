...
export function crearTarea(
  texto,
  { seccion, titulo, quien, cuerpo, permitirRepetido = false },
  ...
  /*
   * `permitirRepetido` lo pone quien YA ha desempatado por otro lado: el Buzón
   * mira antes si el backlog cita el `AV-####` del aviso, así que un título
   * repetido ahí solo quiere decir que dos clientes escribieron con el mismo
   * asunto —lo normal—, y pararlo dejaba el segundo aviso sin poder enviarse
   * (19/09/2026). Escribiendo a mano desde el tablero sí se para: allí el
   * título es lo único que hay para distinguirlas.
   */
  const yaEsta = permitirRepetido ? null : localizar(texto, { clave: claveDeTarea(titulo) });
  if (yaEsta) {
    throw new ErrorDeEdicion(
...
export function editarTarea(
  ...
  /*
   * Si NO se manda cliente, se vuelve a escribir la cola TAL CUAL estaba, con
   * sus comillas invertidas (`quienBruto`) y no la versión limpia que se enseña
   * en pantalla.
   *
   * Sin esto, la primera vez que se toca una tarea desde el tablero se le caen
   * las comillas del slug — «· `aumenta`» pasa a «· aumenta»— y el documento se
   * va despeinando tarea a tarea sin que nadie lo pida. Pasó a la primera, el
   * 24/08/2026, al darle su ficha a la tarea de la agenda de Aumenta para poder
   * colgarle una captura.
   */
  const nuevoQuien = quien === undefined ? (tarea.quienBruto ?? tarea.quien ?? "") : quien;
...
export function cerrarTarea(backlog, resuelto, { id = null, clave = null, comoSeArreglo, fecha }) {
  ...
  const bloque = bloqueDeTarea({
    titulo: tarea.titulo,
    // La cola tal cual estaba, con sus comillas: cerrar una tarea la copia a
    // Resuelto, y ahí tiene que leerse igual que se leía.
    quien: tarea.quienBruto ?? tarea.quien ?? "",
    cuerpo,
    id: tarea.id ?? null,
  });
...
/**
 * Apunta en Resuelto algo que nunca pasó por el backlog (15/09/2026): lo manda
 * el Buzón cuando un aviso no necesita tarea. Devuelve `{ texto, id }`.
 *
 * Mismo bloque y mismo sitio que `cerrarTarea`, con ficha nueva, para que las
 * capturas del aviso puedan colgar de ella. Lo que no puede repetirse dentro
 * del mismo día es el AVISO, no el título: se dice con `senal` (su `AV-####`).
 */
export function apuntarEnResuelto(
  resuelto,
  { titulo, quien, cuerpo, fecha, senal = null },
  aleatorio = Math.random
) {
  limpiarTarea({ titulo, quien, cuerpo });
  const seccion = fechaDeSeccion(fecha);
  const delDia = trocearTodo(resuelto).secciones.find((s) => !s.esManual && s.titulo === seccion);
  /*
   * ── QUÉ ES «LO MISMO» AQUÍ (19/09/2026) ───────────────────────────────────
   * Esto miraba el TÍTULO, y el título de un mensaje del Buzón es su asunto:
   * dos avisos distintos con el mismo asunto —que en el Buzón es lo normal, se
   * escriben desde la misma pantalla— y el segundo rebotaba sin poder enviarse
   * nunca. Lo que hay que parar no es «otra entrada que se llama igual», es
   * «este mismo aviso otra vez», y eso lo dice su referencia (`AV-####`), que
   * `entradaDeResuelto` escribe dentro del cuerpo.
   *
   * Sin señal se sigue mirando el título: quien apunta a mano no tiene otra
   * cosa con la que desempatar.
   */
  const marca = String(senal ?? "").trim();
  if (marca) {
    if (delDia?.tareas.some((t) => sueltoEn(t.cuerpo ?? "", marca))) {
      throw new ErrorDeEdicion(`Hoy ya hay en Resuelto una entrada de ${marca}.`);
    }
  } else if (delDia?.tareas.some((t) => claveDeTarea(t.titulo) === claveDeTarea(titulo))) {
    throw new ErrorDeEdicion(`Hoy ya hay en Resuelto una entrada con ese título: «${titulo}».`);
  }
  const id = nuevaFicha(resuelto, aleatorio);
  return { texto: pegarEnResuelto(resuelto, seccion, bloqueDeTarea({ titulo, quien, cuerpo, id })), id };
}
