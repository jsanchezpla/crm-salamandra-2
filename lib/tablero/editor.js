 ...
 * capturas del aviso puedan colgar de ella. El título no puede repetirse
 * dentro del mismo día: `comprobar` lo pararía al publicar.
 */
export function apuntarEnResuelto(resuelto, { titulo, quien, cuerpo, fecha }, aleatorio = Math.random) {
  limpiarTarea({ titulo, quien, cuerpo });
  const seccion = fechaDeSeccion(fecha);
  const delDia = trocearTodo(resuelto).secciones.find((s) => !s.esManual && s.titulo === seccion);
  if (delDia?.tareas.some((t) => claveDeTarea(t.titulo) === claveDeTarea(titulo))) {
