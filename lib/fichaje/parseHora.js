...
export function parseHoraDelDia(bruto) {
  ...
  // 3. Texto "H:MM", "HH:MM", "HH:MM:SS", con coma o punto como separador
  //    decimal si viene como "8,5".
  if (typeof v === "string") {
    const s = v.trim();
    if (!s) return fallo("vacío");

    const reloj = /^(\d{1,2})[:.h](\d{1,2})(?::(\d{1,2}))?$/i.exec(s);
    if (reloj) {
      const h = Number(reloj[1]);
      const m = Number(reloj[2]);
      if (h > 23) return fallo(`hora fuera de rango: ${s}`);
      if (m > 59) return fallo(`minutos fuera de rango: ${s}`);
      return exito(h * 60 + m);
    }
...
export function parseDuracion(bruto) {
  ...
  if (v instanceof Date) {
    ...
    if (total < 0) return fallo("la duración sale negativa (probablemente falta una hora en el Excel)");
    return exito(total);
  }
  ...
  if (typeof v === "string") {
    ...
    const reloj = /^(\d{1,3})[:.h](\d{1,2})(?::(\d{1,2}))?$/i.exec(s);
    if (reloj) {
      const m = Number(reloj[2]);
      if (m > 59) return fallo(`minutos fuera de rango: ${s}`);
      return exito(Number(reloj[1]) * 60 + m);
    }
