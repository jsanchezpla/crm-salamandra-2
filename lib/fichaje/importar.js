...
/**
 * La misma clave que el índice único `fichajes_import_unico`.
 *
 * Existe para poder cazar los duplicados DENTRO del propio fichero antes de
 * escribir. Sin esto, dos filas iguales en el Excel —la misma persona, el mismo
 * día y la misma hora de entrada, que es lo que pasa cuando alguien copia y
 * pega una fila— llegaban al INSERT, chocaban con el índice y tumbaban la
 * transacción entera con un 23505 de Postgres: el mes no entraba y el mensaje
 * no decía ni qué fila lo había roto.
 */
function claveUnica(f, teamMemberId) {
  return [teamMemberId, f.fecha, f.entrada ?? "", "trabajo"].join("|");
}

/**
 * Separa las filas que chocarían entre sí. Devuelve `{buenas, duplicadas}`; de
 * cada grupo repetido se queda la PRIMERA, que es la que un humano esperaría al
 * leer el fichero de arriba abajo.
 */
function partirDuplicados(filas, teamMemberIdDe) {
  const vistas = new Map();
  const buenas = [];
  const duplicadas = [];
  for (const f of filas) {
    const id = teamMemberIdDe(f);
    const k = claveUnica(f, id);
    const previa = vistas.get(k);
    if (previa) {
      duplicadas.push({ ...f, motivo: `repetida: ya está la misma persona y hora en ${previa.hoja} fila ${previa.fila}` });
    } else {
      vistas.set(k, f);
      buenas.push(f);
    }
  }
  return { buenas, duplicadas };
}
 ...
 */
export async function previsualizar({ workbook, periodo, slug, tenantModels, fileHash = null, mapeos = {} }) {
  ...
  const { resueltos, pendientes: sinAsignar } = resolverNombres(leido.nombres, personas);

  // Los nombres que la persona YA ha asignado en el modal, aplicados EN
  // MEMORIA (el alias no se guarda: eso es de `aplicar`, cuando de verdad se
  // importa). Sin esto, el primer volcado de un fichero cuyos nombres no casan
  // con NADIE se quedaba clavado: todas las filas salían bloqueadas, `listas`
  // era 0 y el botón no se encendía por mucho que se asignaran los nombres en
  // los desplegables (31/08/2026, el fichero del reloj de julio: 8 nombres,
  // 0 exactos). Un id que no sea del equipo activo se ignora: mapear a alguien
  // de fuera es exactamente lo que este módulo no hace.
  const idsDelEquipo = new Set(personas.map((p) => p.id));
  for (const [nombreExcel, teamMemberId] of Object.entries(mapeos || {})) {
    if (resueltos.has(nombreExcel) || !idsDelEquipo.has(teamMemberId)) continue;
    resueltos.set(nombreExcel, teamMemberId);
  }
  const pendientes = sinAsignar.filter((p) => !resueltos.has(p.nombre));
  ...
  // Duplicados DENTRO del fichero: se sacan de `listas` y se enseñan como
  // bloqueadas, con la fila con la que chocan. Que el índice único los pare es
  // la red de debajo; enterarse en el preview es lo que evita el susto.
  const { buenas, duplicadas } = partirDuplicados(listas, (f) => f.teamMemberId);
  listas.length = 0;
  listas.push(...buenas);
  bloqueadas.push(...duplicadas);
...
export async function aplicar({
  ...
  // Mismo filtro de duplicados que el preview, para que lo que se aplica sea
  // exactamente lo que se enseñó.
  const { buenas: listas, duplicadas } = partirDuplicados(
    leido.filas.filter((f) => !f.errores?.length && resueltos.get(f.nombreExcel)),
    (f) => resueltos.get(f.nombreExcel)
  );
  if (listas.length === 0) {
  ...
  const resultado = await tenantSequelize.transaction(async (t) => {
    ...
    // La baja va SIN condicionar a que haya lote anterior, y eso importa: si
    // por lo que sea quedan filas `origen='import'` de este mes sin un lote
    // vivo detrás —alguien borró la fila del lote, un seed antiguo, una
    // reparación a mano—, saltárselas haría que el INSERT de abajo chocara con
    // el índice único y la transacción entera se cayera con un 23505 que no
    // dice nada. Aquí no hay nada que perder: son filas de import, y las
    // correcciones a mano quedan fuera del `where` por su `origen`.
    const [reemplazadas] = await Fichaje.update(
      { deletedAt: new Date() },
      {
        where: {
          fecha: { [Op.between]: [rango.desde, rango.hasta] },
    ...
    return { loteId: lote.id, creadas: listas.length, reemplazadas, duplicadas: duplicadas.length };
  });
 ...
 *
 * ⚠️ SE LLEVA TAMBIÉN LAS FILAS DE ESE LOTE QUE ALGUIEN HUBIERA CORREGIDO, y
 * eso es distinto de lo que hace un re-volcado. Las dos reglas son a propósito:
 *
 *   · RE-VOLCAR es «aquí está el fichero bueno»: se van las filas del import y
 *     las correcciones a mano SOBREVIVEN, porque siguen siendo válidas.
 *   · REVERTIR es «este fichero entero no debería existir»: se va todo lo que
 *     nació de él, corregido o no. Una corrección sobre una fila que no debió
 *     entrar no vale nada — solo hereda el error con mejor letra.
 *
 * Por eso se DEVUELVE `correcciones`: quien pulse el botón tiene que poder ver
 * «esto se lleva por delante 3 correcciones que hiciste» antes de hacerlo. Que
 * la regla sea defendible no la hace evidente.
 */
export async function revertir({ importId, tenantModels, tenantSequelize }) {
  ...
  return await tenantSequelize.transaction(async (t) => {
    ...
    // Se cuentan ANTES de darlas de baja: después ya no casan con el `where`.
    const correcciones = await Fichaje.count({
      where: { importId, deletedAt: null, origen: { [Op.ne]: "import" } },
      transaction: t,
    });
    ...
    return { bajas, correcciones, periodo: lote.periodo };
  });
}
