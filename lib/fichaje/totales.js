...
/** Suma un campo del resumen por persona (no fichajes: eso ya viene sumado). */
 ...
 *
 * Los tramos dados de baja no suman, igual que en `avisosDelMes`: lo que se
 * borró no se paga. Los dos endpoints que llaman aquí ya los dejan fuera de la
 * consulta, pero una función que decide horas de nómina no puede fiarse de que
 * el tercero se acuerde de escribir el `where`.
 */
export function resumirPorPersona(fichajes, personas) {
  ...
  for (const f of fichajes) {
    ...
    // Las horas EXTRA apuntadas a mano (31/08/2026, Rodrigo) son trabajo — se
    // pagan y suman al mes — pero además se cuentan aparte, etiquetadas: la
    // diferencia con `extras` (que es un derivado contra lo previsto) es que
    // esto lo apuntó alguien a propósito y lleva su nota.
    const esExtra = f.tipo === "extra";
    if (f.tipo !== "trabajo" && !esExtra) continue;
    r.filas += 1;
    r.minutos += Number(f.minutos) || 0;
    r.minutosPrevistos += Number(f.minutosPrevistos) || 0;
    if (esExtra) r.extrasApuntadas += Number(f.minutos) || 0;
...
export function totalesDelMes(resumen) {
  return {
    ...
    extrasApuntadas: suma(resumen, "extrasApuntadas"),
...
export function avisosDelMes(fichajes, personas, { festivos = new Set(), ausencias = new Map() } = {}) {
  ...
  // Quien no tiene NADA en todo el mes. Va detrás de los avisos de fila y en
  // su propio tipo porque no es un dato malo, es un dato que falta — y esa es
  // justo la diferencia que hace que se mire.
  for (const p of personas) {
...
/**
 * El orden de la lista de avisos, exportado (31/08/2026): el endpoint junta
 * los de avisosDelMes con los de puntualidad (lib/fichaje/puntualidad.js) y
 * tiene que ordenarlos con LA MISMA regla, o la lista se lee a saltos.
 *
 * Un aviso sin fecha es de la persona, no de una fila, y por eso va detrás:
 * delante tienen que estar los días concretos que se pueden arreglar hoy. La
 * pantalla pinta esta lista tal cual, así que el orden es lo que se lee.
 */
export function ordenarAvisos(avisos) {
  const orden = { error: 0, revisar: 1 };
  const conFecha = (a) => (a.fecha ? 0 : 1);
  return avisos.sort(
    (a, b) =>
      orden[a.gravedad] - orden[b.gravedad] ||
      conFecha(a) - conFecha(b) ||
