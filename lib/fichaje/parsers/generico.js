...
export const meta = {
  ...
  descripcion:
    "Persona · Fecha · Entrada · Salida · Horas · Nota. Es el formato de la plantilla descargable.",
};
...
export async function parse(workbook, { periodo }) {
  ...
  for (const ws of workbook.worksheets) {
    ...
    for (let r = filaCab + 1; r <= ws.rowCount; r++) {
      ...
      // Fila vacía. La fecha se mira RECORTADA, igual que la persona de arriba:
      // hasta el 21/08/2026 se comparaba con `=== ""` a pelo, y un espacio
      // olvidado en la celda de Fecha —lo normal en las filas del final de
      // cualquier Excel— convertía una fila en blanco en tres errores del
      // preview que no existían («falta la persona», «falta la fecha», «no hay
      // horas…») y en una unidad más de `rowsError`. Que es justo lo que hace
      // desconfiar del lote entero.
      const fechaVacia =
        fechaBruta === null || fechaBruta === undefined || String(fechaBruta).trim() === "";
      if (!persona && fechaVacia) continue;
      ...
      const entrada =
        mapa.entrada !== undefined ? parseHoraDelDia(row.getCell(mapa.entrada)) : { ok: false };
      const salida =
        mapa.salida !== undefined ? parseHoraDelDia(row.getCell(mapa.salida)) : { ok: false };
      ...
      // nadie lo convertía en aviso: «17:00 → 08:30» (las dos celdas cambiadas)
      // entraba como 15h 30min sin que el preview dijera nada. Un turno de noche
      // es legítimo, así que la fila sigue entrando, pero avisada (lo sacó la
      // prueba, 19/08/2026).
      if (cruzaMedianoche) {
        avisos.push({
          nivel: "aviso",
          texto: `${persona || "(sin persona)"} · ${fecha || "(sin fecha)"} (hoja ${ws.name}, fila ${r}): la salida (${formatearHora(salida.valor)}) es anterior a la entrada (${formatearHora(entrada.valor)}). Se ha contado como jornada que cruza la medianoche (${formatearMinutos(minutos)}): revisa si están cambiadas.`,
        });
      }
      ...
      filas.push({
        ...
        nota:
          mapa.nota !== undefined
            ? String(valorDeCelda(row.getCell(mapa.nota)) ?? "").trim() || null
            : null,
        errores,
      });
    }
