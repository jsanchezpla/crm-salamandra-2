...
export const meta = {
  key: "aumenta",
  nombre: "Aumenta — hoja semanal o volcado del reloj",
  descripcion:
    "Dos formatos, se reconoce solo cuál es: el volcado mensual del reloj de fichar (hoja «Registro asistencia» con los marcajes en crudo) o la hoja semanal a mano de antes de julio de 2026 («02-6», «9-13»… con L/M/X/J/V).",
};
...
/**
 * ¿Esta celda tiene algo ESCRITO A MANO que el lector no ha sabido leer?
 * Devuelve ese texto, o `null` si la celda estaba vacía o si lo de dentro se
 * entiende.
 *
 * Mira SOLO los textos, y a propósito. Los números y las fechas de estas
 * columnas los escribe la fórmula del Excel, y cuando le falta una hora devuelve
 * un cero o una fecha anterior a la época (−956 minutos): eso es un día que no
 * se trabajó, no un error de nadie, y llevarlo al preview sería inundar de rojo
 * un fichero correcto. Las LETRAS, en cambio, las teclea la persona que rellena
 * la hoja, y un «8 horas» en la casilla del total es una jornada que hay que
 * preguntar, no una que se tira.
 */
function textoQueNoSeEntiende(celda, parsear) {
  const v = valorDeCelda(celda);
  if (typeof v !== "string") return null;
  const s = v.trim();
  if (!s) return null;
  if (parsear(v).ok) return null;
  // Recortado, como hace `parseHoraDelDia` con su propio motivo. Este texto
  // acaba dentro del `motivo` de la fila bloqueada del preview y viaja entero
  // en el JSON: una celda de Excel admite 32.767 caracteres y el preview manda
  // hasta 200 filas bloqueadas con tres celdas cada una, así que un texto
  // pegado por error en la columna del total podía convertir la respuesta en
  // varios megas para decir «esta celda no se entiende». Con 40 caracteres se
  // reconoce la celda, que es para lo único que sirve.
...
export async function parse(workbook, { periodo }) {
  // Desde julio de 2026 Aumenta sube el volcado del RELOJ tal cual (.xls con la
  // hoja «Registro asistencia»); antes rellenaban una hoja semanal a mano. El
  // mismo tenant, dos formatos según el mes que se re-importe: se distingue por
  // la hoja, que solo existe en el fichero del reloj.
  if (aumentaReloj.hojaDeMarcajes(workbook)) {
    return aumentaReloj.parse(workbook, { periodo });
  }
  ...
  for (const ws of workbook.worksheets) {
    ...
    for (let r = 2; r <= ws.rowCount; r++) {
      ...
      // A qué día del mes va esta fila, y si ese día EXISTE: la hoja «30-31» de
      // marzo tiene J y V, que saldrían los días 33 y 34.
      //
      // Se calcula una sola vez porque lo necesitan los dos usos de la fila —la
      // anotación y la jornada— y tenerlo escrito dos veces fue justo el fallo:
      // la jornada comprobaba que cupiera en el mes y la anotación no, así que
      // el preview y el resumen del lote (que se PERSISTE, `importar.js`)
      // guardaban días imposibles como «2026-03-33» (21/08/2026).
      const diaDelMes = dia ? rango.desde + dia.indice : null;
      const cabeEnElMes = diaDelMes !== null && diaDelMes <= diasDelMes;
      ...
      if (abreBloque) {
        ...
        // Una anotación de un día que no existe en el mes no se guarda. No hace
        // falta avisar aquí: esta misma fila sale unas líneas más abajo con su
        // aviso «Fila saltada», así que la persona se entera igual.
        if (cabeEnElMes) {
          anotaciones.push({
            nombreExcel: personaActual,
            fecha: fechaISO(year, month, diaDelMes),
            texto: nombre,
            hoja: ws.name,
            fila: r,
          });
      ...
      let previstoCruzaMedianoche = false;
      if (entradaPrev.ok && salidaPrev.ok) {
        const p = minutosEntre(entradaPrev.valor, salidaPrev.valor);
        minutosPrevistos = p.minutos;
        previstoCruzaMedianoche = p.cruzaMedianoche;
      } else {
      ...
      // Qué celdas de esta fila llevaban algo escrito que no se ha sabido leer.
      // Solo hace falta preguntárselo cuando no han salido minutos por ningún
      // lado: es ahí donde la fila está a punto de desaparecer.
      const sinNadaQueContar = minutos === null && !entrada.ok && !salida.ok;
      const ilegibles = sinNadaQueContar
        ? [
            ["la entrada", COL.entrada, parseHoraDelDia],
            ["la salida", COL.salida, parseHoraDelDia],
            ["el total de horas", COL.horasFichadas, parseDuracion],
          ]
            .map(([etiqueta, col, parsear]) => {
              const txt = textoQueNoSeEntiende(row.getCell(col), parsear);
              return txt === null ? null : `${etiqueta} dice «${txt}»`;
            })
            .filter(Boolean)
        : [];
      ...
      //
      // Pero solo si de verdad no había NADA escrito. Hasta el 21/08/2026 se
      // descartaba antes de mirarlo, y un «8 horas» tecleado en la casilla del
      // total —lo escribe la persona del centro que rellena la hoja— hacía
      // desaparecer la jornada entera del control horario sin un solo aviso; el
      // error «no se ha podido leer ninguna hora» de aquí abajo no podía saltar
      // nunca, porque esa fila ya se había ido. La cabecera de este fichero
      // promete lo contrario: «lo que ha entendido, lo que no, y por qué».
      if (sinNadaQueContar && ilegibles.length === 0) continue;
      ...
      if (minutos === null) {
        errores.push(
          entrada.ok || salida.ok
            ...
            : `no se ha podido leer ninguna hora (${ilegibles.join(", ")})`
        );
      }
      ...
      // Lo mismo en las columnas del HORARIO. Aquí el `cruzaMedianoche` que
      // devuelve `minutosEntre` se tiraba sin mirarlo, así que la misma errata
      // de dos celdas cambiadas (09:00 → 08:00) daba 23 h PREVISTAS calladas. No
      // es cosmético: las horas extra del mes se restan contra ese número
      // (`lib/fichaje/totales.js`) y se guarda en la fila del fichaje
      // (`minutosPrevistos`). La fila sigue entrando —un horario de noche es
      // legítimo—, pero avisada, como su gemela de arriba (21/08/2026).
      if (previstoCruzaMedianoche) {
        avisos.push({
          nivel: "aviso",
          texto: `${personaActual}: el ${diaLetra} de la hoja «${ws.name}» (fila ${r}) tiene en el HORARIO la salida prevista (${formatearHora(salidaPrev.valor)}) anterior a la entrada prevista (${formatearHora(entradaPrev.valor)}). Se ha contado un horario de ${formatearMinutos(minutosPrevistos)} cruzando la medianoche: revisa si están cambiadas.`,
        });
      }
