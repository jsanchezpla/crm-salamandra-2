...
import { ESTADOS, ESTADOS_ANTIGUOS, ORIGEN_SALAMANDRA, estadoActual, estadoTrasMensaje, referencia } from "./buzon.js";
...
/**
 * Un aviso que abrimos NOSOTROS para una persona concreta (15/09/2026).
 *
 * Nace como si ya le hubiéramos contestado: `respondidoAt` encendido y
 * `vistoClienteAt` vacío, que es lo que le pone el «Nueva respuesta», el punto
 * del menú y el bloque de la portada sin tocar ninguna de las dos parejas de
 * fechas. `clienteEscribioAt` se queda vacío —él no ha escrito nada— y así
 * nuestra campana no se enciende con lo que acabamos de mandar; si contesta,
 * `anadirMensaje` la enciende como siempre.
 */
export async function crearAvisoDeSalamandra({ tenant, destinatario, limpio, firmante }) {
  return escribiendo(async () => {
    const { BuzonAviso } = getMasterModels();
    ...
    return BuzonAviso.create({
      ...
      usuarioEmail: destinatario.email ?? null,
      usuarioNombre: destinatario.nombre ?? null,
      usuarioRol: destinatario.rol ?? null,
      tipo: "duda",
      asunto: limpio.asunto,
      cuerpo: limpio.cuerpo,
      bloquea: false,
      pantalla: null,
      contexto: { origen: ORIGEN_SALAMANDRA, firmante: firmante ?? "Salamandra" },
...
export async function listarDelTenant({ tenantId, tenantSlug }) {
  try {
    ...
    const avisos = await BuzonAviso.findAll({
      ...
      limit: 300, // 300 desde el 11/09/2026 (AV-0118): Aumenta pasó de 100 y los antiguos no salían ni buscando
    });
...
export async function listarParaSalamandra({ estado, tenantSlug, asignadoA, q, limit = 100 } = {}) {
  try {
    ...
    if (estado && estado !== "todos") {
      if (estado === "mensajes") {
        // Lo que le escribimos nosotros y aún no ha contestado (15/09/2026).
        Object.assign(where, whereMensajeSinRespuesta());
      } else if (estado === "activos") {
        /*
         * ── ACTIVOS = LO NUEVO **Y** LO QUE HA SEGUIDO (09/09/2026, Rodrigo) ──
         * «Hay conversaciones que han continuado y no lo registras.» Y era
         * verdad: mandar un aviso al Registro lo sacaba de esta pestaña para
         * siempre, así que cuando el cliente volvía a escribir —«sigue
         * pasando», «no era eso»— su mensaje caía en una lista que nadie mira.
         * En producción había NUEVE hilos así.
         *
         * El estado no cambia (dice si está en el Registro, y sigue estando),
         * pero vuelve a la pestaña de trabajo mientras la pelota esté en
         * nuestro tejado. Al contestarlo o abrirlo se sale solo.
         */
        where[Op.or] = [
          {
            estado: { [Op.in]: clavesQueCuentanComo("nuevo") },
            // Un mensaje nuestro sin contestar no es trabajo: espera en «Mensajes».
            [Op.and]: BuzonAviso.sequelize.literal(`NOT ${SQL_MENSAJE_SIN_RESPUESTA}`),
          },
          wherePendienteNuestro(),
        ];
      } else {
        // Un estado de hoy también trae las filas con su nombre viejo hasta que
        // `migrate-buzon-estados.js` las reescriba.
        where.estado = { [Op.in]: clavesQueCuentanComo(estado) };
      }
    }
    if (tenantSlug) where.tenantSlug = tenantSlug;
    if (asignadoA) where.asignadoA = asignadoA === "nadie" ? null : asignadoA;
    /*
     * Sin tildes y por palabras (10/09/2026, Rodrigo: «ni un solo buscador del
     * CRM debería obligar a poner tildes»). Y de paso deja de pisar el filtro
     * de la pestaña: esto asignaba `where[Op.or]` por segunda vez, así que con
     * «Activos» puesto el buscador se llevaba por delante el filtro de estado
     * —en silencio— y sacaba avisos ya cerrados.
     */
    await filtrarPorTexto(where, BuzonAviso, q || "", ["asunto", "cuerpo", "tenantNombre"]);
    ...
    /*
     * Activos = los nuevos MÁS los que han seguido después de mandarlos al
     * Registro. Una pestaña que cuenta una cosa y enseña otra es peor que no
     * contar nada, así que se cuenta con la misma condición con la que se
     * filtra (09/09/2026).
     */
    const siguenVivos = await BuzonAviso.count({
      where: {
        estado: { [Op.notIn]: clavesQueCuentanComo("nuevo") },
        ...wherePendienteNuestro(),
      },
    });
    const mensajes = await BuzonAviso.count({ where: whereMensajeSinRespuesta() });
    recuento.todos = ESTADOS.reduce((suma, e) => suma + recuento[e.key], 0);
    recuento.mensajes = mensajes;
    recuento.activos = recuento.nuevo - mensajes + siguenVivos;
    // Y aparte, para poder decirlo en la pestaña: cuántos de esos son hilos que
    // ya estaban en el Registro y el cliente ha vuelto a mover.
    recuento.hanSeguido = siguenVivos;
    ...
    return {
      avisos: [],
      recuento: { nuevo: 0, enviado: 0, cerrado: 0, activos: 0, mensajes: 0, todos: 0 },
      soloLectura: true,
    };
  }
}
...
function wherePendienteNuestro() {
  ...
  return {
    ...
    /*
     * Contra la última vez que hicimos algo —abrirlo o contestarlo—, no solo
     * contra `leido_at` (09/09/2026). Se puede contestar sin abrir, y entonces
     * `leido_at` se quedaba viejo y el aviso «nos esperaba» para siempre: 45
     * marcados de 100 cuando los que de verdad habían seguido eran 9.
     *
     * `-infinity` en vez de un NULL suelto para que las dos ausencias se
     * comporten igual: si no lo hemos abierto ni contestado, cualquier cosa que
     * escriba es más reciente y nos espera.
     */
    [Op.and]: BuzonAviso.sequelize.literal(
      "cliente_escribio_at > GREATEST(COALESCE(leido_at, '-infinity'::timestamptz), COALESCE(respondido_at, '-infinity'::timestamptz))"
    ),
  };
}

/**
 * «Un mensaje nuestro que aún no ha contestado», en SQL. Gemela de
 * `esperaSuRespuesta` en `lib/buzon/buzon.js` — las dos, a la vez.
 *
 * Sin columna nueva: el origen vive en `contexto` (JSONB), por lo que cuenta
 * `ORIGEN_SALAMANDRA`. Las columnas van sin tabla delante, como en
 * `wherePendienteNuestro`: ni mensajes ni adjuntos tienen otras con ese nombre.
 */
const SQL_MENSAJE_SIN_RESPUESTA =
  `(contexto->>'origen' = '${ORIGEN_SALAMANDRA}' AND cliente_escribio_at IS NULL)`;

function whereMensajeSinRespuesta() {
  const { BuzonAviso } = getMasterModels();
  return {
    estado: { [Op.in]: clavesQueCuentanComo("nuevo") },
    [Op.and]: BuzonAviso.sequelize.literal(SQL_MENSAJE_SIN_RESPUESTA),
  };
}
...
export async function cambiar(aviso, cambios) {
  return escribiendo(async () => {
    // `resueltoAt` ya no se escribe (02/09/2026: no hay «resuelto» en el
    // Buzón); se conserva en las filas viejas, y la poda la lee como fecha
    // de cierre de lo anterior a ese día.
    await aviso.update({ ...cambios });
    return aviso;
  });
}
...
/**
 * El aviso ya está escrito en Resuelto (15/09/2026). Misma ficha y misma fecha
 * que «enviado» —la tarea es la misma si venía del backlog, y si no, es la
 * entrada nueva—, con estado `cerrado`. Lo llama solo
 * `POST /api/admin/buzon/[id]/resuelto`, DESPUÉS de publicar.
 */
export async function marcarEnResuelto(aviso, { ficha }) {
  return escribiendo(async () => {
    await aviso.update({
      estado: "cerrado",
      registroFicha: ficha ?? aviso.registroFicha ?? null,
      registroEnviadoAt: new Date(),
    });
    return aviso;
  });
...
export async function anadirMensaje(aviso, { autorTipo, autorNombre, autorEmail, cuerpo, interno = false }) {
  return escribiendo(async () => {
    ...
    // Reabierto desde Resuelto (15/09/2026): la ficha vieja se guarda en
    // `contexto` y se suelta, porque su tarea ya está cerrada y los dos
    // botones se niegan a actuar sobre un aviso que tiene ficha.
    if (parche.estado === "nuevo" && estadoActual(aviso.estado) === "cerrado" && aviso.registroFicha) {
      const contexto = aviso.contexto ?? {};
      parche.contexto = {
        ...contexto,
        fichasAnteriores: [...(contexto.fichasAnteriores ?? []), aviso.registroFicha],
      };
      parche.registroFicha = null;
      parche.registroEnviadoAt = null;
    }
