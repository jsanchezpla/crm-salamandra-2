...
export async function listarParaSalamandra({ estado, tenantSlug, asignadoA, q, limit = 100 } = {}) {
  try {
    ...
    if (estado && estado !== "todos") {
      ...
      } else if (estado === "nuevo" || estado === "activos") {
        /*
         * «Nuevo» (hasta el 15/09/2026 por la tarde, «Activos»). Desde el 09/09
         * esta pestaña sumaba además lo enviado en lo que el cliente había
         * vuelto a escribir; ahora eso tiene estado propio, `activo`, y su
         * pestaña. `activos` se sigue aceptando por si alguien lo tiene en un
         * enlace.
         */
        where.estado = { [Op.in]: clavesQueCuentanComo("nuevo") };
        // Un mensaje nuestro sin contestar no es trabajo: espera en «Mensajes».
        where[Op.and] = BuzonAviso.sequelize.literal(`NOT ${SQL_MENSAJE_SIN_RESPUESTA}`);
      } else {
    ...
    // La pestaña «Nuevo» cuenta con la misma condición con la que filtra: sin
    // los mensajes nuestros, que tienen la suya.
    ...
    return {
      avisos: [],
      recuento: { nuevo: 0, activo: 0, enviado: 0, cerrado: 0, mensajes: 0, todos: 0 },
      soloLectura: true,
    };
  }
}
