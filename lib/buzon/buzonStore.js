...
export async function listarParaSalamandra({ estado, tenantSlug, asignadoA, q, limit = 100 } = {}) {
  try {
    ...
    if (estado && estado !== "todos") {
      // «Activos» = lo que aún no está en el Registro, o sea, nuevo (Rodrigo,
      // 02/09/2026: lo enviado cuenta como cerrado aquí). Es la pestaña por
      // defecto. Un estado de hoy también trae las filas con su nombre viejo
      // hasta que `migrate-buzon-estados.js` las reescriba.
      where.estado = { [Op.in]: clavesQueCuentanComo(estado === "activos" ? "nuevo" : estado) };
    }
    ...
    // Activos = nuevos: lo enviado al Registro ya no cuenta aquí.
    recuento.activos = recuento.nuevo;
