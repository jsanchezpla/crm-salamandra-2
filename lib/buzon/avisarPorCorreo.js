/**
 * lib/buzon/avisarPorCorreo.js — el correo del buzón.
 *
 * UNO, y en un solo sentido: cuando entra un aviso, nos llega a NOSOTROS. Al
 * cliente NO se le manda correo al contestarle (Jorge, 13/08/2026): se le avisa
 * dentro de su CRM, con la campana y el bloque de la portada. Es gente que
 * entra todos los días, y un correo por cada respuesta era ruido.
 *
 ...
 *
 * ── BEST-EFFORT, SIEMPRE ────────────────────────────────────────────────────
 * Esto no puede tumbar la petición. Un aviso guardado y sin correo se ve igual
 * en el buzón; un aviso perdido porque el correo falló no lo recupera nadie. Si
 * no sale, se dice en el log con todas las letras.
 */
...
import { avisoParaNosotros, seguimientoParaNosotros } from "../email/templates/buzon/avisoNuevo.js";
...
/**
 * Han vuelto a escribir en un hilo que ya existía: avisarnos igual
 * (09/09/2026, Rodrigo).
 *
 * Mismo camino que `avisarnos` —la misma cuenta, el mismo destinatario, el
 * mismo «esto no puede tumbar la petición»—, y por eso comparten fichero. Lo
 * único distinto es la plantilla y la etiqueta, para poder distinguirlos en
 * Resend cuando haya que mirar cuántos seguimientos llegan.
 */
export async function avisarnosDeSeguimiento({ aviso, mensaje }) {
  try {
    const emisor = await credencialesDeSalamandra("seguimiento");
    if (!emisor) return { ok: false, motivo: "emisor sin configurar" };

    const panel = baseDelPanel();
    const tpl = seguimientoParaNosotros({
      aviso,
      mensaje,
    ...
    const res = await sendEmail({
      ...
      tags: [{ name: "tipo", value: "buzon-seguimiento" }],
    });

    const { salio, motivo } = envioRealizado(res, "buzon:seguimiento");
    return { ok: salio, motivo };
  } catch (err) {
    process.stderr.write(`[buzon:seguimiento] fallo: ${err.message}\n`);
