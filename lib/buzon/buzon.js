...
/**
 * ¿Este fichero se puede ENSEÑAR en pantalla, y como qué? Devuelve el
 * Content-Type con el que servirlo en línea, o `null` si hay que obligar a
 * descargarlo.
 *
 * Vive AQUÍ y no en `buzonStorage.js` por dos motivos: lo necesitan a la vez el
 * endpoint (para decidir la cabecera) y el navegador (para saber si pinta el
 * botón «Ver»), y este fichero no importa nada — `buzonStorage` arrastra
 * `node:fs` y no puede acabar en el bundle del cliente.
 *
 ...
 *    fichero, o sea que lo elige él. La extensión de `ruta` la escribió
 *    `guardarFichero` pasando por `extFromFileName`. Fiarse del `mime` sería
 *    dejar que quien sube decida cómo se lo servimos al que mira.
 *
 ...
 *    ORIGEN. En el buzón eso sería peor que en ningún otro sitio, porque una de
 *    las dos pantallas es `admin.salamandrasolutions.com`, donde vive la sesión
 *    que toca la configuración de todos los clientes.
 *
 * Es lista BLANCA: lo que no esté aquí (HTML, ZIP, lo que sea) se descarga.
 */
const VISIBLES = {
  png: "image/png",
  jpg: "image/jpeg",
   ...
   *
   * No es una rareza teórica: Chrome en Windows guarda JPEGs como `.jfif` en
   * cuanto la web los sirve sin nombre —pasa con lo que se descarga de un chat,
   * que es justo de donde salen las capturas que nos mandan—. Pasó a la primera:
   * Jorge colgó una captura de Rodrigo en el Registro, el fichero se llamaba
   * `03743d5b-….jfif` con `mime: image/jpeg`, y como `jfif` no estaba en esta
   * lista se pintó como «fichero» en vez de como imagen.
   *
   * Aceptarlos aquí arregla además las que YA están guardadas, sin tocar ni una
   * fila: esta lista se consulta al servir, no al subir.
   */
  jfif: "image/jpeg",
  jpe: "image/jpeg",
  gif: "image/gif",
...
export function serializarAviso(fila, { para = "salamandra" } = {}) {
  ...
  const base = {
    ...
    sinLeer: tieneRespuestaSinVer(fila),
    mensajes,
    adjuntos,
  };
