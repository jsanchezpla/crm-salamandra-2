 ...
 * `estadoTrasMensaje`.
 *
 * ── DOS ESTADOS, Y EL SEGUNDO ES UNA ACCIÓN (Rodrigo, 02/09/2026) ──────────
 * Un aviso del Buzón existe para acabar en el Registro, que es donde se
 * arregla. Así que el camino es `nuevo` → `enviado` (al Registro), y ahí se
 * acaba el Buzón: lo enviado cuenta como cerrado aquí («Activos» son solo los
 * nuevos) y sigue vivo en su tarea del tablero. «Enviado» no es una etiqueta
 * que se ponga a mano: la pone el botón «Enviar al registro» de
 * `/admin/buzon`, que apunta la tarea de verdad
 * (`POST /api/admin/buzon/[id]/registro`, `lib/buzon/alRegistro.js`).
 *
 * Hasta ese día hubo cuatro (`nuevo`, `en_curso`, `esperando`, `resuelto`):
 * los dos del medio decían en qué tejado estaba la pelota —eso ya lo dicen
 * las parejas de fechas (`tienePendienteNuestro`, `tieneRespuestaSinVer`)— y
 * `resuelto` decía dos veces lo que ya dice la tarea del Registro («el
 * Resuelto no tiene sentido en el buzón», esa misma noche). Las filas viejas
 * se leen con `estadoActual()`: `en_curso` y `resuelto` cuentan como enviado,
 * `esperando` como nuevo; `scripts/migrate-buzon-estados.js` las reescribe
 * para que no queden.
 */
export const ESTADOS = [
  // Rotulado «Activo» unas horas el 15/09/2026; esa tarde Rodrigo lo llamó
  // «Nuevo» y reservó «Activo» para lo de abajo.
  { key: "nuevo", label: "Nuevo", nivel: "amber" },
  // Ya lo habíamos mandado al Registro o a Resuelto y la persona ha vuelto a
  // escribir (Rodrigo, 15/09/2026: «una pestaña donde vemos las contestaciones
  // de la gente a los tickets»). Lo pone `estadoTrasMensaje`; de aquí se sale
  // con los mismos dos botones.
  { key: "activo", label: "Activo", nivel: "violet" },
  { key: "enviado", label: "Enviado al registro", nivel: "blue" },
  // Directo a Resuelto (Rodrigo, 15/09/2026): lo que no necesita tarea —una
  // duda contestada, un mensaje nuestro— se apunta ya cerrado. Clave `cerrado`
  // y no `resuelto`, porque `resuelto` es un nombre VIEJO que se lee como
  // enviado y una fila de antes del 02/09 cambiaría de sitio sin tocarla.
  // Lo pone el botón (`POST /api/admin/buzon/[id]/resuelto`), no la mano.
  { key: "cerrado", label: "Resuelto", nivel: "green" },
];

/** Los tres estados que hubo hasta el 02/09/2026, y como qué se leen ahora. */
export const ESTADOS_ANTIGUOS = Object.freeze({ en_curso: "enviado", esperando: "nuevo", resuelto: "enviado" });
// «respondido» existió unas horas el 15/09/2026; si quedara alguna fila, se lee
// como activo. Va aparte para no tocar la tabla de los tres de siempre.
const RETIRADOS = Object.freeze({ respondido: "nuevo" });

/** El estado de una fila en el vocabulario de hoy (los viejos se traducen). */
export function estadoActual(estado) {
  return ESTADOS_ANTIGUOS[estado] ?? RETIRADOS[estado] ?? estado;
}
...
export function tienePendienteNuestro(fila) {
  ...
  const nuestro = ultimoMovimientoNuestro(fila);
  if (!nuestro) return true;
  return nuestro < new Date(fila.clienteEscribioAt);
}

/**
 * La última vez que HICIMOS algo con este aviso: abrirlo o contestarlo, lo que
 * sea más reciente.
 *
 * ── POR QUÉ NO BASTA CON `leidoAt` (09/09/2026, Rodrigo) ───────────────────
 * «Hay conversaciones que han continuado y no lo registras.» Y al medirlo
 * salieron 45 avisos marcados como «nos esperan» de 100, cuando los que de
 * verdad habían seguido eran NUEVE. La diferencia era esta función: se puede
 * contestar un aviso sin abrirlo —el triaje del buzón lo hace, y también
 * cualquier respuesta escrita desde fuera de la ficha—, y entonces `leidoAt` se
 * queda antiguo para siempre y el aviso no deja de «esperarnos» nunca.
 *
 * Un contador que siempre marca 45 es un contador apagado: se mira dos veces y
 * se deja de mirar. Es el mismo razonamiento que ya está escrito arriba para el
 * punto del cliente, del otro lado del hilo.
 *
 * Contestar cuenta como atender. Si después de contestar el cliente vuelve a
 * escribir, entonces sí vuelve a ser nuestro, que es justo lo que se quería
 * ver.
 */
export function ultimoMovimientoNuestro(fila) {
  const fechas = [fila?.leidoAt, fila?.respondidoAt]
    .filter(Boolean)
    .map((f) => new Date(f))
...
export function validarCambio(entrada = {}) {
  ...
  if (entrada.estado !== undefined) {
    ...
    // Como «enviado», «Resuelto» es una acción: apunta la entrada en el
    // documento. Ponerlo a mano dejaría un aviso cerrado sin nada escrito.
    if (entrada.estado === "cerrado") {
      return { ok: false, status: 422, error: "A Resuelto se manda con su botón, que lo apunta en el Registro." };
    }
...
/**
 * ── LOS AVISOS QUE ABRIMOS NOSOTROS (15/09/2026, Rodrigo) ──────────────────
 * «Quiero poder escribir a cada empleado de cada tenant desde el buzón sin
 * necesidad de que me hayan abierto un ticket.» Es el mismo aviso de siempre
 * —asunto, texto, capturas, hilo— pero lo abre Salamandra y el destinatario
 * es la persona elegida: `usuario_*` es A QUIÉN va, no quién lo escribió.
 *
 * Se marca en `contexto.origen` y no en una columna nueva a propósito: una
 * columna nueva en el modelo haría que TODAS las consultas del buzón la
 * pidieran, y hasta correr la migración a mano `/ayuda` se quedaría vacía
 * para todo el mundo (ver `faltaLaTabla` en `buzonStore.js`). `contexto` ya
 * es JSONB, y lo que manda el navegador del cliente pasa por
 * `limpiarContexto`, que no deja entrar `origen`: el cliente no puede
 * hacerse pasar por nosotros.
 */
export const ORIGEN_SALAMANDRA = "salamandra";

/** ¿Este aviso lo abrimos nosotros? */
export function esDeSalamandra(fila) {
  return fila?.contexto?.origen === ORIGEN_SALAMANDRA;
}

/**
 * ¿Es un mensaje nuestro que todavía no ha contestado? (Rodrigo, 15/09/2026)
 *
 * «Los mensajes que enviamos a alguien no deben aparecer como activos»: viven
 * en la pestaña «Mensajes» hasta que la persona escribe algo en el hilo, y
 * entonces pasan a Activos como cualquier aviso. Solo cuenta mientras está en
 * `nuevo`: si ya lo mandamos al Registro o a Resuelto, está en esa pestaña.
 *
 * Gemela en SQL: `buzonStore.whereMensajeSinRespuesta()`. Las dos a la vez.
 */
export function esperaSuRespuesta(fila) {
  return esDeSalamandra(fila) && !fila?.clienteEscribioAt && estadoActual(fila?.estado) === "nuevo";
}

/**
 * ¿Se puede mandar este aviso nuestro? Mismos topes que el del cliente, sin
 * el mínimo de «cuéntanos un poco más» (eso es para quien reporta un fallo),
 * y con destinatario obligatorio.
 */
export function validarAvisoDeSalamandra(entrada = {}) {
  const asunto = texto(entrada.asunto, LIMITES.asunto);
  const cuerpo = texto(entrada.cuerpo, LIMITES.cuerpo);
  const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

  if (!uuid.test(String(entrada.tenantId ?? ""))) {
    return { ok: false, status: 422, error: "Elige a qué cliente va." };
  }
  if (!uuid.test(String(entrada.usuarioId ?? ""))) {
    return { ok: false, status: 422, error: "Elige a qué persona va." };
  }
  if (asunto.length < LIMITES.asuntoMinimo) {
    return {
      ok: false,
...
/**
 * En qué estado queda un aviso cuando alguien escribe en su hilo.
 *
 * Desde el 02/09/2026, en el mismo: un mensaje, de quien sea, no mueve nada.
 * El estado dice si está en el Registro, no en qué tejado está la pelota (eso
 * lo dicen las fechas: `tienePendienteNuestro`, `tieneRespuestaSinVer`). Un
 * «sigue pasando» sobre un aviso enviado enciende la campana del panel igual
 * que cualquier mensaje, y el trabajo sigue en su tarea del tablero. Lo que sí
 * hace es traducir los nombres viejos y devolver al principio un estado que
 * no exista (una fila tocada a mano): es lo único que no esconde nada.
 *
 * La excepción (Rodrigo, 15/09/2026): si la persona vuelve a escribir en uno
 * que ya estaba En el registro o en Resuelto, pasa a `activo`, la pestaña de
 * sus contestaciones. Conserva la ficha: «Enviar al registro» no duplica una
 * tarea que siga abierta y «Enviar a Resuelto» no reescribe una ya cerrada.
 * Nuestro mensaje (contestar el «gracias») no lo mueve.
 */
export function estadoTrasMensaje(estado, autorTipo) {
  const de = estadoActual(estado);
  if ((de === "cerrado" || de === "enviado") && autorTipo !== "salamandra") return "activo";
  return CLAVES_ESTADO.has(de) ? de : "nuevo";
}

function metaEstado(key) {
  return ESTADOS.find((e) => e.key === estadoActual(key)) ?? ESTADOS[0];
}
...
export function serializarAviso(fila, { para = "salamandra", quienMira = null } = {}) {
  ...
  const base = {
    ...
    // Lo abrimos nosotros (15/09/2026): el primer texto es NUESTRO y las dos
    // pantallas lo firman como Salamandra, no como quien lo recibe.
    deSalamandra: esDeSalamandra(fila),
    firmante: esDeSalamandra(fila) ? fila.contexto?.firmante || "Salamandra" : null,
    // Siempre en el vocabulario de hoy: una fila vieja con `en_curso` sale como
    // «enviado», y las pantallas no tienen que conocer los nombres de ayer.
    estado: estadoActual(fila.estado),
  ...
  return {
    ...
    esperaSuRespuesta: esperaSuRespuesta(fila),
    ...
    // La tarea del Registro que salió de este aviso (02/09/2026), por su ficha.
    registroFicha: fila.registroFicha ?? null,
    registroEnviadoAt: fila.registroEnviadoAt ?? null,
  };
}
