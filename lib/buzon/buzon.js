 ...
 * `estadoTrasMensaje`.
 *
 * ── TRES ESTADOS, Y EL DEL MEDIO ES UNA ACCIÓN (Rodrigo, 02/09/2026) ────────
 * Un aviso del Buzón existe para acabar en el Registro, que es donde se
 * arregla. Así que el camino es `nuevo` → `enviado` (al Registro) → `resuelto`,
 * y «enviado» no es una etiqueta que se ponga a mano: la pone el botón «Enviar
 * al registro» de `/admin/buzon`, que apunta la tarea de verdad
 * (`POST /api/admin/buzon/[id]/registro`, `lib/buzon/alRegistro.js`).
 *
 * Hasta ese día hubo cuatro (`nuevo`, `en_curso`, `esperando`, `resuelto`) y
 * los dos del medio decían en qué tejado estaba la pelota. Eso ya lo dicen las
 * parejas de fechas (`tienePendienteNuestro`, `tieneRespuestaSinVer`) sin que
 * nadie tenga que mover nada. Las filas viejas se leen con `estadoActual()`:
 * `en_curso` cuenta como enviado (era lo que marcaba /mailbox al apuntar la
 * tarea) y `esperando` como nuevo; `scripts/migrate-buzon-estados.js` las
 * reescribe para que no queden.
 */
...
/** Los dos estados que hubo hasta el 02/09/2026, y como qué se leen ahora. */
export const ESTADOS_ANTIGUOS = Object.freeze({ en_curso: "enviado", esperando: "nuevo" });
...
export function estadoTrasMensaje(estado, autorTipo) {
  // Un estado que no es de hoy ni de ayer (una fila tocada a mano) vuelve al
  // principio: es lo único que no esconde nada.
  const de = estadoActual(estado);
  const actual = CLAVES_ESTADO.has(de) ? de : "nuevo";
  // Contestar nosotros no lo mueve: desde el 02/09/2026 el estado dice si está
  // en el Registro, no en qué tejado está la pelota (eso lo dicen las fechas).
  if (autorTipo === "salamandra") return actual;
  // Escribe el cliente: un resuelto se REABRE («sigue pasando» no puede quedar
  // enterrado en un hilo cerrado); lo demás se queda donde está.
  return actual === "resuelto" ? "nuevo" : actual;
}
