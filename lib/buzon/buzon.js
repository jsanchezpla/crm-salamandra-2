...
export const ESTADOS = [
  // Se rotula «Activo» (Jorge, 15/09/2026): la clave sigue siendo `nuevo`.
 ...
 *
 * La excepción es Resuelto (Rodrigo, 15/09/2026): «si vuelven a escribir,
 * vuelve a aparecer como Activo aunque estuviera ya resuelta». Lo resuelto no
 * tiene tarea viva que siga el trabajo, así que un mensaje del cliente lo
 * reabre; `anadirMensaje` suelta además la ficha vieja para que se pueda
 * volver a mandar al Registro o a Resuelto como uno nuevo. Nuestro mensaje
 * (contestar el «gracias») no lo reabre.
 */
export function estadoTrasMensaje(estado, autorTipo) {
  const de = estadoActual(estado);
  if (de === "cerrado" && autorTipo !== "salamandra") return "nuevo";
