...
export async function abrirFichero(ruta) {
  ...
  // `stream` es un ReadableStream web, no el de Node: el porqué, en lib/utils/cuerpoDeFichero.js (11/09/2026).
  return { stream: cuerpoDeFichero(abs), size: stat.size };
}

/**
 * El fichero entero en memoria. Solo para COPIARLO a otro sitio (las capturas
 * que viajan al Registro con «Enviar al registro», 03/09/2026): son 10 MB como
 * mucho y se escriben de una vez. Para servirlo por HTTP sigue siendo
 * `abrirFichero`, que no dobla la memoria.
 */
export async function leerFichero(ruta) {
  return fs.readFile(absoluta(ruta));
}
