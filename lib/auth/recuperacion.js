  ...
  try {
    ...
    try {
      ...
      await notifyAdmins({
        ...
        body: "Restablécesela desde Equipo, en su ficha. Le llegará una nueva y podrá cambiársela al entrar.",
        entityType: "User",
        entityId: user.id,
        dedupe: true,
      });
 ...
 */
export async function incidenciaUsuarioOlvidado({ empresa, cargo, nombre, correo, usuario, ip }) {
  try {
    const limpio = {
    ...
    // Si venía de la pantalla del correo, su usuario ya lo sabemos: se apunta
    // y el triaje empieza con media identificación hecha.
    const elUsuario = limpio.texto(usuario, 255);
    ...
    const aviso = await crearAviso({
      ...
      limpio: {
        ...
        cuerpo:
          `Desde la pantalla de recuperación, sin poder entrar` +
          `${elUsuario ? " y sin poder abrir el correo de su cuenta" : " y sin recordar su usuario"}.\n\n` +
          `Empresa: ${laEmpresa}\nNombre: ${elNombre}\nCargo: ${elCargo || "no lo dijo"}\n` +
          `${elUsuario ? `Usuario que dice tener: ${elUsuario}\n` : ""}` +
