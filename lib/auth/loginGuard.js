...
/**
 * Los cerrojos DE LA CUENTA para un identificador que ya se ha resuelto contra
 * la base de datos. Sin `request` y sin mirar el barrido por IP.
 *
 * ── PARA QUÉ HACE FALTA (26/08/2026) ───────────────────────────────────────
 * Desde hoy una cuenta tiene DOS identificadores válidos: su usuario
 * (`nombre_aumenta`) y su correo. `comprobarIntentoLogin` cuenta por lo que se
 * TECLEA, y tiene que seguir haciéndolo porque corre antes de tocar la base de
 * datos. Pero si solo contara eso, alternar entre los dos daría 12 intentos por
 * cuenta en vez de 6: el doble de presupuesto por la cara.
 *
 * Así que el login, cuando ya sabe a quién señalaba lo tecleado y resulta que
 * era el correo, vuelve a preguntar por el identificador de verdad. Los fallos
 * se apuntan en los dos cubos, así que da igual por cuál de los dos nombres se
 * pruebe: los seis son seis.
 */
export function cerrojoDeCuenta(ip, identificador) {
  const ahora = Date.now();
  const clave = String(identificador || "").slice(0, 120);

  const porCuentaIp = leer(`cuenta-ip:${clave}|${ip}`, ahora);
  if (porCuentaIp.fallos >= MAX_FALLOS_CUENTA) {
    return { bloqueado: true, motivo: "cuenta", retryAfter: Math.max(1, Math.ceil((porCuentaIp.hasta - ahora) / 1000)), ip };
  }

  const porCuenta = leer(`cuenta:${clave}`, ahora);
  if (porCuenta.fallos >= MAX_FALLOS_CUENTA_GLOBAL) {
    return { bloqueado: true, motivo: "cuenta-global", retryAfter: Math.max(1, Math.ceil((porCuenta.hasta - ahora) / 1000)), ip };
  }
...
export async function tenantDeEmail(email) {
  try {
    ...
    // Por los DOS identificadores: si el aviso solo mirara `email`, un bloqueo
    // sufrido tecleando el correo saldría sin cliente y no aparecería en la
    // pantalla de Actividad de nadie.
    const u = await User.findOne({ where: whereDelLogin(email), attributes: ["id", "tenantId"] });
