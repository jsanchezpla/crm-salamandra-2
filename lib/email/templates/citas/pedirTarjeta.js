...
<p><strong>Tu cita sigue en pie</strong>, no la hemos cancelado. ${escapeHtml(porque)}</p>`;
  ...
  // El `href` va escapado (21/08/2026). El repaso del 10/08 escapó todos los
  // enlaces del módulo de citas MENOS este, que se quedó crudo dentro de un
  // atributo: era el único `href="${…}"` sin escapar de todo lib/email, y ahí
  // un enlace con una comilla no rompe el marcado, lo AMPLÍA (`" onmouseover="…`)
  // dentro de un correo que sale del dominio verificado del CRM.
...
<p style="margin:0 0 16px;"><a href="${escapeHtml(ctx.enlace)}" style="display:inline-block;padding:11px 20px;border-radius:6px;background:#111827;color:#ffffff;text-decoration:none;font-weight:500;">Introducir mi tarjeta</a></p>
