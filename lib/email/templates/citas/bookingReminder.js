...
<p>Te recordamos que <strong>mañana</strong> tienes tu cita de <strong>${ctx.eventTypeName}</strong>.</p>`;
  ...
  if (ctx.modality === "online" && ctx.meetUrl) {
    ...
    lines.push(`<p style="margin:0 0 12px;"><strong>Dónde:</strong> ${ctx.location}</p>`);
  ...
  if (ctx.cancelUrl) {
    lines.push(
      `<p style="margin:18px 0 0;font-size:13px;color:#6B7280;">¿No puedes venir? <a href="${ctx.cancelUrl}" style="color:inherit;">Avísanos aquí</a> cuanto antes y así podemos ofrecer tu hueco a otra persona.</p>`
    );
  }
