...
<p>Te recordamos que <strong>mañana</strong> tienes tu cita de <strong>${escapeHtml(ctx.eventTypeName)}</strong>.</p>`;
  ...
  if (ctx.modality === "online" && ctx.meetUrl) {
    ...
    // Presencial sin dirección: la misma red que en el correo de confirmación.
    // Aquí duele más —esto sale de madrugada, sin nadie delante— porque la cita
    // es mañana y la paciente todavía no sabe adónde ir.
    lines.push(`<p style="margin:0 0 12px;">La cita es presencial. Si no conoces la dirección, respóndenos a este correo y te la confirmamos.</p>`);
