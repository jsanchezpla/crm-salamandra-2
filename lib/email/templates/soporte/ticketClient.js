 ...
 *
 * `canReplyByEmail`: hay reply-to útil (correo de soporte del tenant o captura
 * del CRM) → se invita a responder al correo directamente; si no, el único
 * camino de vuelta es el enlace del portal.
 */
export function ticketClientTemplate({ tenantName, brand, kind, ticketRef, title, replyBody, portalUrl, canReplyByEmail = false }) {
  const copy = {
    created: {
  ...
  if (canReplyByEmail) {
    pieces.push(
      `<p style="margin:18px 0 0;font-size:14px;">Puedes <strong>responder directamente a este correo</strong> y tu mensaje quedará en la solicitud.</p>`
    );
  }
  if (portalUrl) {
    pieces.push(
      `<p style="margin:${canReplyByEmail ? "10px" : "18px"} 0 0;"><a href="${escapeHtml(portalUrl)}" style="display:inline-block;background:#1B3A2D;color:#FFFFFF;text-decoration:none;font-weight:600;font-size:14px;padding:10px 18px;border-radius:8px;">${canReplyByEmail ? "O verla en el portal" : "Ver mi solicitud y responder"}</a></p>`
    );
  }
  ...
  if (canReplyByEmail) textLines.push("", "Puedes responder directamente a este correo.");
  if (portalUrl) textLines.push("", `Ver en el portal: ${portalUrl}`);
