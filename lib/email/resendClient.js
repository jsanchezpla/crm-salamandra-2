 ...
 *     `{ ok: true, dryRun: true, id: null }`. Se activa cuando la clave que
 *     se va a usar —`params.apiKey` (la del cliente, BYOK) o, si no llega,
 *     `process.env.RESEND_API_KEY`— está ausente o vale exactamente
 *     "dry-run" (ver `isDryRun`). Nada más: una clave `re_test_…` se usa tal
 *     cual. En producción `RESEND_API_KEY` va vacía a propósito, así que un
 *     cliente sin clave propia está en dry-run.
