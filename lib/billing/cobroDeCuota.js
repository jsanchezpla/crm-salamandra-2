...
export async function sincronizarCobroDelMes({ tenantModels, cuotaId, mes = null }) {
  ...
  try {
    ...
    // La cuota no toca este mes (alta futura, baja anterior o en pausa).
    if (!fila && !fallida) return salida("fuera-del-mes");

    // Toca, pero no vale nada: conceptos borrados del catálogo o importe 0. Es
    // el agujero silencioso de AV-0048 — se dice, no se calla.
    if (!fila) return salida("sin-importe", { motivo: fallida.motivo, cobroId: cobro?.id ?? null });
