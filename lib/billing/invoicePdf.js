...
function renderInvoice(doc, { invoice, client, settings, partnerName, logo, patientName, stamp, spec }) {
  ...
  if (footer) {
    ...
    const alto = doc.heightOfString(footer, { width: RIGHT - LEFT, align: "center" });
    const abajo = doc.page.height - 40 - alto;
    if (abajo < y + 10) { doc.addPage(); }
    doc.text(footer, LEFT, Math.max(50, doc.page.height - 40 - alto), { width: RIGHT - LEFT, align: "center" });
  }
}
