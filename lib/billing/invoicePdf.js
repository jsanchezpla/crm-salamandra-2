...
export function buildInvoicePdfBuffer({ invoice, client, settings, partnerName, logo, patientName, stamp }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.on("error", reject);
      renderInvoice(doc, { invoice, client, settings: settings || {}, partnerName, logo, patientName, stamp });
      doc.end();
    } catch (e) {
      reject(e);
    }
...
export function buildQuotePdfBuffer({ quote, client, settings, logo }) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ size: "A4", margin: 50 });
      const chunks = [];
 ...
 */
export function buildInvoicePreviewPdfBuffer({ invoice, client, settings, partnerName, logo, patientName, stamp }) {
  return new Promise((resolve, reject) => {
    try {
...
function renderInvoice(doc, { invoice, client, settings, partnerName, logo, patientName, stamp, spec }) {
  const S = {
    titulo: "FACTURA",
  ...
  doc.font("Helvetica-Bold").fontSize(20).fillColor(INK).text(S.titulo, 320, topY, { width: RIGHT - 320, align: "right" });
  ...
  if (footer) {
    ...
    doc.text(footer, LEFT, Math.max(doc.page.margins.top, arriba()), { width: ancho, align: "center" });
  }
}
