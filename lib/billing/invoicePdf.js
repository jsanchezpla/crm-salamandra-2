...
import { nifDeCliente } from "./nifCliente.js";
...
function renderInvoice(doc, { invoice, client, settings, partnerName }) {
  ...
  doc.font("Helvetica-Bold").fontSize(11).fillColor(INK).text(client?.fiscalName || client?.name || "—", LEFT, blockY + 14, { width: 260 });
  doc.font("Helvetica").fontSize(9).fillColor(MUTED);
  const clientLines = [
    nifDeCliente(client) ? `NIF/CIF: ${nifDeCliente(client)}` : null,
    client?.fiscalAddress || null,
    [client?.fiscalZip, client?.fiscalCity].filter(Boolean).join(" ") || null,
    client?.email || null,
  ].filter(Boolean);
  doc.text(clientLines.join("\n") || "—", LEFT, blockY + 30, { width: 260 });
  ...
  let y = Math.max(blockY + 78, my + 14);
  ...
  doc.text("TOTAL", totX, y, { width: labelW, align: "left" });
  doc.text(money(invoice.total), totX + labelW, y, { width: RIGHT - (totX + labelW), align: "right" });
  y += 24;
