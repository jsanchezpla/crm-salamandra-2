...
/**
 * ¿Se imprime la línea «Vencimiento» en el papel? (18/09/2026, AV-0176)
 *
 * Isabel, de Aumenta: «y quitar lo de Vencimiento». No es un capricho de
 * maquetación: sus facturas nacen COBRADAS —la Facturación del mes las emite
 * contra cobros que ya están hechos— y les ponía de vencimiento el mismo día de
 * la emisión, así que el papel le decía a una familia que le vence hoy algo que
 * ya ha pagado. Y cuando no había fecha, la fila salía igual con una raya.
 *
 * La regla, entonces: el vencimiento se imprime cuando SIGNIFICA algo — hay
 * fecha, y queda dinero por cobrar. Ni se toca el dato ni se toca ninguna
 * factura ya emitida: es lo que se pinta.
 */
export function vencimientoALaVista(invoice) {
  if (!invoice?.dueDate) return false;
  if (invoice.status === "paid") return false;
  const total = Number(invoice.total ?? 0);
  const paidAmount = Number(invoice.paidAmount ?? 0);
  if (total > 0 && paidAmount >= total - 0.0049) return false;
  return true;
}
