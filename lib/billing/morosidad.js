...
import { mesLegible, mesValido } from "./cuotas.js";
...
/*
 * ── QUIÉN ENTRA EN LA LISTA (15/09/2026, Rodrigo) ──────────────────────────
 * «La gente que esté de baja tiene que desaparecer de morosidad.» Entra el
 * paciente ACTIVO (En pausa y Baja, fuera) de una familia que no esté ni de
 * Baja ni En pausa. Los dos estados los pone `lib/clients/estadoPorActividad.js`
 * y se deshacen solos con una cuota o un cobro, así que quien vuelve, vuelve a
 * salir aquí. Sus facturas y cobros de antes siguen en su ficha: nada se borra.
 */
export function entraEnMorosidad(paciente, familia) {
  if (paciente?.status && paciente.status !== "active") return false;
  const estadoFamilia = familia?.status ?? "active";
  return estadoFamilia !== "inactive" && estadoFamilia !== "paused";
}
...
/*
 * ── EL IMPORTE Y EL MES (18/09/2026, Rosa, AV: «Importe y mes en morosidad») ─
 * «Sale el número de meses que debe el paciente pero quieren que salga también
 * el importe que debe y al mes que corresponde.»
 *
 * El mes ya se sabía y no se decía: la fila contaba «3 meses» sin nombrar
 * ninguno. Eso se arregla escribiéndolos (`mesesDebe`, que la ruta rellena con
 * los MISMOS meses que cuenta `mesesSinPagarSeguidos`).
 *
 * El importe de cada mes sale de UNA sola fuente y no de un cálculo:
 * **los cobros PENDIENTES de ese mes**. Un pendiente es lo que el centro pidió
 * y no ha cobrado — el mismo criterio que ya usaba el mes pagado a medias
 * desde el 07/09. ⚠️ Lo que NO se hace, y por eso está escrito aquí: sumar los
 * conceptos de la cuota para inventar el importe. Se probó el 07/09 y acusó a
 * ~95 familias de deber 30 € que no debían (el «Descuento reserva ya abonada»
 * volvía a pedirse), y además en Aumenta 290 de las 291 cuotas activas tienen
 * el importe a NULL: no hay nada pactado que leer. Un mes sin pendiente sale
 * con su nombre y SIN importe, que es la verdad.
 */

/** Los meses de `mesesDebe` en cristiano: «septiembre, agosto». */
export function textoDeMeses(mesesDebe = []) {
  const lista = (mesesDebe ?? []).map((x) => (typeof x === "string" ? x : x?.mes)).filter(mesValido);
  if (!lista.length) return "";
  // El año solo cuando hay más de uno: «septiembre, agosto» se lee, «septiembre
  // 2026, agosto 2026» no, y en diciembre→enero el año sí hace falta.
  const anios = new Set(lista.map((m) => m.slice(0, 4)));
  return lista.map((m) => (anios.size > 1 ? mesLegible(m) : mesLegible(m).split(" ")[0])).join(", ");
}
 ...
 * vez de contar meses de una deuda que nadie ha escrito. `meses` va aparte
 * porque la pantalla lo pinta como texto y no dentro de la pastilla.
 */
export function etiquetaDeMoroso(m = {}) {
  const meses = textoDeMeses(m.mesesDebe);
  if (m.debe != null) return { texto: `debe ${importe(m.debe)}`, tono: "importe", meses };
  if (!m.tieneCuota) return { texto: "sin cuota escrita", tono: "sinCuota", meses };
  const n = Number(m.mesesSeguidos ?? 0);
  if (n <= 0) return { texto: "este mes", tono: "leve", meses };
  return { texto: n === 1 ? "1 mes" : `${n} meses`, tono: n >= 3 ? "grave" : n === 2 ? "medio" : "leve", meses };
}

/**
 * Lo que suma una lista de morosos, y de cuántos se sabe.
 *
 * No se suma lo que no se sabe: quien no trae importe no cuenta ni como cero.
 * Por eso vuelve `sinImporte`, para que la pantalla pueda decir «1.480,50 € en
 * 8 de 12» en vez de dar un total que parece el de todos.
 */
export function totalDeMorosidad(lista = []) {
  let total = 0;
  let conImporte = 0;
  for (const m of lista ?? []) {
    const v = Number(m?.debe);
  ...
  return {
    ...
    conImporte,
    sinImporte: (lista ?? []).length - conImporte,
  };
}

/**
 * 60 → «60,00 €»; 1480.5 → «1.480,50 €». Sin Intl para que la prueba no
 * dependa del locale del sistema (y el total de la cabecera son miles: sin el
 * punto, «22256,97 €» hay que contarlo con el dedo).
 */
function importe(n) {
  const v = Number(n ?? 0);
  const [ent, dec] = Math.abs(v).toFixed(2).split(".");
  const miles = ent.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
  return `${v < 0 ? "-" : ""}${miles},${dec} €`;
}
...
export function resumenDeMorosidad({ conCuota = [], sinCuota = [], alDia = 0, pacientes = 0 } = {}) {
  return {
    ...
    // Lo que suma la lista, arriba del todo: era la otra mitad de lo que se
    // pedía —«que en la pantalla inicial de morosidad salga también el
    // importe»— y hasta hoy había que ir fila a fila sumando a mano.
    totalConCuota: fraseDeTotal(conCuota),
    sinCuota: `${sinCuota.length} ${sinCuota.length === 1 ? "paciente activo" : "pacientes activos"} sin cuota escrita`,
    totalSinCuota: fraseDeTotal(sinCuota),
    alDia: `${alDia} al día · ${pacientes} ${pacientes === 1 ? "paciente activo" : "pacientes activos"}`,
  };
}

/** «1.480,50 € en 8 de 12» — o null si de ninguno se sabe el importe. */
function fraseDeTotal(lista = []) {
  const { total, conImporte } = totalDeMorosidad(lista);
  if (!conImporte) return null;
  const n = (lista ?? []).length;
  return conImporte === n ? `${importe(total)} en total` : `${importe(total)} en ${conImporte} de ${n}`;
}
