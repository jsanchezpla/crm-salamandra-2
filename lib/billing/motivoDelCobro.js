...
/** El mismo separador con el que junta `notaDeCobro`: si se separan, esto deja de leer. */
...
export function explicaCobro({ notes, concepto = null } = {}) {
  ...
  let deLaNota = null;
  if (trozos.length && !esMotivo(trozos[0])) deLaNota = trozos.shift();

  return {
    concepto: delCatalogo ?? deLaNota,
    motivos: trozos.map(acortaMotivo).filter(Boolean),
  };
}
