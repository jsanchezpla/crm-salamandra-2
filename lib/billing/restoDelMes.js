 ...
 */
const esSuyo = (c, patientId) =>
  !patientId ||
  String(c?.patientId ?? "") === String(patientId) ||
  c?.patientId == null ||
  c?.patientId === "";

export function cobrosQueCuentan(cobros, patientId = null, conceptIds = null) {
  return (Array.isArray(cobros) ? cobros : []).filter(
    (c) => esSuyo(c, patientId) && !esDeOtroServicio(c, conceptIds)
  );
}

/**
 * ── LO DE OTRO SERVICIO NO PAGA ESTA CUOTA (10/09/2026, Rodrigo) ───────────
 *
 * «Los 50 € se le han cobrado de la entrevista inicial y ahora queríamos
 * cobrar la cuota de logopedia», y el cajón ofrecía 95 € de 145 €. La
 * entrevista inicial de Aumenta no está en ninguna cuota: es un servicio
 * suelto, se cobra aparte y no es media mensualidad adelantada.
 *
 * Se descarta solo cuando el cobro dice de qué concepto es y ese concepto no
 * es de los que paga esta cuota. Lo tecleado a mano —que nace sin concepto—
 * sigue contando, que es el encargo del 04/09: quien deja 50 € a cuenta ve
 * salir el resto solo. En septiembre de 2026 esto son 15 cobros de Aumenta y
 * 1.295 €, 12 de ellos entrevistas iniciales.
 */
export function esDeOtroServicio(cobro, conceptIds = null) {
  const suyos = new Set((Array.isArray(conceptIds) ? conceptIds : []).filter(Boolean).map(String));
  if (!suyos.size) return false;
  const cid = cobro?.conceptId;
  if (!cid) return false;
  return !suyos.has(String(cid));
}

/**
 * Los que se quedan fuera por eso, para poder NOMBRARLOS: un cobro que
 * desaparece de la cuenta sin decir nada se lee como un cobro perdido.
 */
export function cobrosDeOtroServicio(cobros, patientId = null, conceptIds = null) {
  return (Array.isArray(cobros) ? cobros : []).filter(
    (c) => esSuyo(c, patientId) && esDeOtroServicio(c, conceptIds)
  );
}
...
export function yaCobradoDelMes(cobros, patientId = null, conceptIds = null) {
  return round2(
    cobrosQueCuentan(cobros, patientId, conceptIds).reduce((s, c) => s + (Number(c?.amount) || 0), 0)
  );
}
 ...
 * @param {?Array} args.conceptIds            los conceptos que paga esta cuota: lo
 *        cobrado de OTRO servicio (una entrevista inicial) no la va saldando.
 *
 ...
 */
export function restoDelMes({ esperado, cobros, patientId = null, conceptIds = null } = {}) {
  const yaCobrado = yaCobradoDelMes(cobros, patientId, conceptIds);
