...
export async function crearBonoConSuCobro({
  ...
  patientId = null,
  /*
   * Quién da las sesiones (18/09/2026, AV-0183). Opcional: un centro sin equipo
   * —o un bono que todavía no tiene profesional asignado— se da igual, y la
   * columna se queda a NULL como los 243 que ya estaban dados.
   */
  teamMemberId = null,
  eventTypeId,
  nombreDelTipo = null,
  ...
  creador = null,
  /*
   * El bono de un DIAGNÓSTICO (12/09/2026): `totalSessions` llega a null (sin
   * tope), `diagnosticoId` ata el bono a su expediente y `cobro` —
   * `{ conceptId, texto, invoiceText }`— hace que el pendiente se llame como
   * el producto contratado y no «Bono «DIAGNÓSTICO»». Los tres son opcionales;
   * sin ellos, esto es el alta de bono de siempre.
   */
  diagnosticoId = null,
  cobro: cobroPropio = null,
}) {
  ...
  const bono = await SessionPack.sequelize.transaction(async (t) => {
    ...
    const cobro = cobroPendienteDeBono({
      ...
      conceptId: cobroPropio?.conceptId ?? null,
      texto: cobroPropio?.texto ?? null,
      invoiceText: cobroPropio?.invoiceText ?? null,
    });
    if (cobro && Payment) {
