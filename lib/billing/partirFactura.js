...
export function planDePartir({ factura, cobros = [], ficha = null, por = "paciente", conceptos = [], pacientes = [] } = {}) {
  ...
  const { facturables, sinNif } = agruparLoteCuotas({
    ...
    conceptos,
    pacientes,
    // Una factura que ya es de un tutor no se vuelve a partir por tutor: aquí
    // se parte por paciente o por terapia, y a nombre de quien ya iba.
    repartoTutores: false,
  });
