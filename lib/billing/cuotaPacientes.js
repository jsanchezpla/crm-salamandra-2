...
/** Los campos contra los que busca la pantalla: quién paga y a quién cubre. */
export function partesBuscablesDeCuota(cuota) {
  return [
...
/**
 * El mismo texto en una sola cadena y en minúsculas. Se mantiene porque es lo
 * que se enseña al depurar una fila; buscar ya no pasa por aquí.
 */
export function textoBuscableDeCuota(cuota) {
  return partesBuscablesDeCuota(cuota).join(" ").toLowerCase();
}

/**
 * ¿Casa esta cuota con lo tecleado? Vacío = casan todas.
 *
 * 10/09/2026, Rodrigo: «las cuotas distinguen por tildes en el buscador».
 * Era verdad: esto hacía un `includes` a pelo, así que «muñoz» encontraba a
 * los Muñoz y «munoz» no encontraba a nadie. Ahora usa `coincidePorNombre`,
 * la regla del resto del CRM: sin tildes y por palabras sueltas, así que
 * «castro hugo» también vale y no hay que acertar con el orden.
 */
export function cuotaCasaCon(cuota, texto) {
  return coincidePorNombre(texto, partesBuscablesDeCuota(cuota));
}
