...
export async function assignInvoiceNumber({ sequelize, models, seriesCode = "F", date, t }) {
  ...
  if (lastEmitted && String(lastEmitted.issueDate).slice(0, 10) > newDate) {
    const err = new Error(
      `No se puede emitir con fecha ${fmtDate(newDate)}: es anterior a la última factura de la serie (${lastEmitted.number}, del ${fmtDate(String(lastEmitted.issueDate).slice(0, 10))}). La numeración debe ir en orden de fecha.`
    );
