...
export function mesesSeguidosSinPagar({ meses, pagados, primerMes }) {
  return mesesSinPagarSeguidos({ meses, pagados, primerMes }).length;
}

/**
 * CUÁLES son esos meses, del más reciente al más antiguo (18/09/2026, Rosa:
 * «que salga también el importe que debe y al mes que corresponde»).
 *
 * Misma regla exacta que el contador de arriba —que ahora no es más que su
 * longitud—: contar y nombrar no pueden discrepar nunca.
 */
export function mesesSinPagarSeguidos({ meses, pagados, primerMes }) {
