...
/*
 * EL MÉTODO SE ELIGE AL COBRARLO (10/09/2026, Rodrigo).
 *
 * Dar un bono no es cobrarlo: el pendiente nace sin decir por dónde va a
 * entrar el dinero. Hasta hoy salía como «Transferencia» porque la columna era
 * obligatoria, y se acababa pagando en efectivo delante del mostrador.
 */
...
export function cobroPendienteDeBono({
  ...
  metodo = null,
  /*
   * ── EL COBRO DE UN DIAGNÓSTICO SE LLAMA COMO SU PRODUCTO (12/09/2026) ─────
   * El bono de un diagnóstico es SIN TOPE (`total_sessions` a NULL), así que
   * «Bono «DIAGNÓSTICO» · N sesiones» no dice nada: lo que la familia contrató
   * es «Diagnóstico Simple» (350 €) o «Diagnóstico Completo» (650 €), un
   * concepto del catálogo con su «Texto en la factura». Estos tres son
   * opcionales y, sin ellos, el cobro sale exactamente como hasta hoy: los
   * bonos de siempre no cambian de nombre.
   */
  conceptId = null,
  texto = null,
  invoiceText = null,
  ...
  const textoPropio = typeof texto === "string" ? texto.trim().slice(0, 200) : "";
  const impreso = typeof invoiceText === "string" ? invoiceText.trim() : "";
  ...
  return {
    ...
    status: "pending",
    notes: textoPropio || textoDelCobroDeBono({ nombre, sesiones }),
    // La línea impresa en la factura, como foto (`payments.invoice_text`);
    // null = se factura por la nota, que es lo de siempre.
    invoiceText: impreso || null,
  };
}
