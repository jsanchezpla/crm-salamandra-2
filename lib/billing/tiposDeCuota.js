...
/**
 * LOS NOMBRES de quienes lleva un grupo de cuotas, sin repetir y ordenados
 * (18/09/2026, AV-0192 de Isabel: «vendría bien un buscador de pacientes en
 * los tipos de cuotas»).
 *
 * `contarPacientes` dice CUÁNTOS son, que es lo que pinta la tabla; para poder
 * buscar a uno hace falta saber CÓMO SE LLAMAN. Misma regla que en todo este
 * fichero: con paciente en la cuota, ese; sin él, los de su familia. No se
 * inventa ningún reparto — quien sale por la cuota de su casa sale igual que
 * sus hermanos, porque esa cuota es de los dos.
 */
export function nombresDePacientes(cuotas = []) {
  const nombres = new Set();
  for (const cuota of Array.isArray(cuotas) ? cuotas : []) {
    for (const n of pacientesDeCuota(cuota).nombres) if (n) nombres.add(n);
  }
...
export function resumenPorTipo({ conceptos = [], cuotas = [], hoy = null } = {}) {
  ...
  const tipos = [...cubos.entries()].map(([id, cubo]) => {
    ...
    return {
      ...
      // Para buscar por el niño desde la lista de tipos (AV-0192). Solo los de
      // las cuotas VIVAS: buscar a alguien y que salga un tipo que dejó de
      // pagar hace un curso es encontrarlo donde ya no está.
      nombres: nombresDePacientes(cubo.vivas),
    };
  });
...
/**
 * EL LISTADO DE CUOTAS, POR SERVICIO (18/09/2026, AV-0194 de Isabel: «que
 * salgan ordenadas, todas las de TO juntas, todas las de logo juntas»).
 *
 * La pantalla de Cuotas pintaba las 291 filas en el orden en que las devolvía
 * Postgres, o sea en ninguno: para ver quién va a logopedia había que leerlas
 * todas. Se ordenan por el NOMBRE de sus conceptos —que es como el centro
 * llama al servicio— y, dentro de cada uno, por el paciente (o la familia,
 * cuando la cuota es de toda la casa).
 *
 * Las cuotas de varios conceptos hacen grupo propio («Logopedia 45x1 + T.O.
 * 45x1»), que es la verdad: no son de logopedia ni de T.O., son de las dos, y
 * meterlas en uno de los dos montones sería elegir por el centro. Las de
 * importe suelto, sin concepto que las nombre, van al final.
 */
export function ordenarCuotasPorServicio(cuotas = [], conceptosPorId = new Map()) {
  const servicio = (cuota) =>
    conceptosDeCuota(cuota)
      .map((id) => conceptosPorId.get(String(id))?.name ?? "")
      .filter(Boolean)
      .sort((a, b) => a.localeCompare(b, "es"))
      .join(" + ");
  const quien = (cuota) => {
    const { nombres } = pacientesDeCuota(cuota);
    return (nombres[0] || cuota?.client?.fiscalName || cuota?.client?.name || "").toLowerCase();
  };
  return [...(Array.isArray(cuotas) ? cuotas : [])].sort((a, b) => {
    const sa = servicio(a);
    const sb = servicio(b);
    if (Boolean(sa) !== Boolean(sb)) return sa ? -1 : 1;
    const porServicio = sa.localeCompare(sb, "es");
    if (porServicio !== 0) return porServicio;
    return quien(a).localeCompare(quien(b), "es");
  });
}
