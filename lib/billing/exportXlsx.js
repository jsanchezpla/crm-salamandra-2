 ...
 *   hojasExtra: [{ name, columns, rows }] hojas ADICIONALES con el mismo trato
 *            que la principal (cabecera en negrita y fila congelada). Existe
 *            desde el 04/09/2026 para el Excel del resumen de caja, que son dos
 *            miradas del mismo dato —el día y el cobro— y en dos ficheros no se
 *            pueden cruzar. Opcional: quien no la pasa, igual que siempre.
 */
export async function xlsxResponse({ filename, sheetName = "Datos", columns, rows, filters, hojasExtra }) {
  ...
  for (const hoja of Array.isArray(hojasExtra) ? hojasExtra : []) {
    const extra = wb.addWorksheet(hoja.name);
    extra.columns = (hoja.columns ?? []).map((c) => ({
      header: c.header,
      key: c.key,
      width: c.width ?? 18,
      style: c.numFmt ? { numFmt: c.numFmt } : undefined,
    }));
    for (const fila of hoja.rows ?? []) extra.addRow(fila);
    extra.getRow(1).font = { bold: true };
    extra.views = [{ state: "frozen", ySplit: 1 }];
  }
