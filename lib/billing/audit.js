...
export function resumenImporte(fila) {
  ...
  // Solo el gasto tiene proveedor (Cost.supplierId); en un cobro o una tarifa la
  // clave sobraría, así que se añade únicamente si el modelo lo expone. Sin
  // esto, cambiar el proveedor de un gasto dejaba `before` y `after` idénticos.
  if (fila.supplierId !== undefined) resumen.proveedorId = fila.supplierId ?? null;
  // Y solo el cobro tiene factura (Payment.invoiceId). Mismo motivo, desde el
  // 31/08/2026: asociar un cobro suelto a su factura dejaba un rastro con
  // `before` y `after` idénticos, como si no hubiera cambiado nada.
  if (fila.invoiceId !== undefined) resumen.facturaId = fila.invoiceId ?? null;
  // Y el mes, el paciente, el estado y la fecha de la devolución (07/09/2026):
  // desde hoy «Editar cobro» corrige el mes y el hijo, y «Devuelto» apunta
  // cuándo salió el dinero. Sin esto, cambiarlos dejaba un rastro vacío.
  if (fila.periodMonth !== undefined) resumen.mes = fila.periodMonth ? String(fila.periodMonth).slice(0, 7) : null;
  if (fila.patientId !== undefined) resumen.pacienteId = fila.patientId ?? null;
  if (fila.paidAt !== undefined && fila.status !== undefined) resumen.estado = fila.status ?? null;
  if (fila.refundedAt !== undefined) resumen.devueltoEl = fila.refundedAt ?? null;
  return resumen;
}
