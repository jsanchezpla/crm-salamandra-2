 ...
 */
export function esConteoDeVerdad(cierre) {
  if (!cierre) return false;
  const contado = Number(cierre.countedAmount ?? cierre.counted_amount);
  if (!Number.isFinite(contado) || contado < 0) return false;
  const loCerroAlguien = cierre.hechoPorUnaPersona !== false;
  return loCerroAlguien || contado > 0;
}

export function fondoSugerido(ultimoCierre) {
  if (!esConteoDeVerdad(ultimoCierre)) return null;
...
/**
 * ── LA CAJA QUE NUNCA SE HA CONTADO (18/09/2026, Registro) ─────────────────
 *
 * Mientras no haya un solo cierre hecho contando el dinero, el cajón se
 * arrastra desde el primer día que pasó efectivo (AV-0157, `tramoDeArrastre`),
 * que es lo correcto pero no es lo que el centro espera ver: el punto de
 * partida bueno lo fija el primer arqueo de verdad. Aumenta lleva 828 cierres
 * importados de Organízate, todos a cero y sin autor, y ha escrito TRES veces
 * que la caja no le cuadra sin que la pantalla dijera nunca esto.
 *
 * No bloquea nada: es una línea, y se va sola en cuanto alguien cierre un día
 * contando.
 *
 * @param {{ total: number, deVerdad: number }|null} conteos cierres de esa caja;
 *   sin el recuento del servidor no se dice nada (no se avisa a ciegas)
 * @returns {{ texto: string, importados: number }|null}
 */
export function avisoDeCajaSinContar(conteos) {
  if (!conteos) return null;
  const { total = 0, deVerdad = 0 } = conteos;
  if (deVerdad > 0) return null;
  return {
    texto:
      total > 0
        ? "Esta caja no se ha cerrado nunca contando el dinero: los cierres que hay vinieron importados, a cero y sin autor. El primer cierre fija de dónde parte el cajón."
        : "Esta caja no se ha cerrado nunca contando el dinero. El primer cierre fija de dónde parte el cajón.",
    importados: total,
  };
}
...
/**
 * ¿ESE ESTADO OBLIGA A DECIR POR DÓNDE ENTRÓ EL DINERO? (10/09/2026)
 *
 * `payments.method` puede estar vacío mientras el cobro es un PENDIENTE: la
 * cuota del mes y el bono generan la deuda, no el cobro, y por dónde se va a
 * pagar se decide el día que se paga. En cuanto el cobro se da por cobrado —o
 * por devuelto, que es dinero que entró y volvió a salir— el hueco deja de
 * valer: un importe sin método sumaría al total del día y no estaría ni en
 * efectivo, ni en tarjeta, ni en banco, así que el arqueo cuadraría de menos
 * sin que nada lo explicara.
 */
export function exigeMetodo(status) {
...
/**
 * ── LO QUE QUEDA EN EL CAJÓN, DÍA A DÍA (09/09/2026, petición de Aumenta) ────
 *
 * «Solo un apartado de efectivo donde se vea cada día lo que queda en caja a
 * golpe de vista, una vez restadas las salidas de caja.»
 *
 * El resumen por día contesta «¿cuánto entró?» —efectivo, tarjeta y banco, cada
 * uno por su lado— y esa pregunta se queda como está. Esta contesta otra: «¿y
 * cuánto hay ahora en el cajón?». Son distintas porque el cajón ARRASTRA: lo
 * que quedó el lunes sigue ahí el martes, y las salidas de la semana lo van
 * bajando. Sumar la columna «Efectivo» de siete días no da eso.
 *
 * La tarjeta y el banco NO entran: no pasan por el cajón. Lo que suma es el
 * efectivo cobrado (ya sin lo devuelto en efectivo, que `resumenDelDia` restó
 * de su cesta) más las entradas apuntadas, menos las salidas.
 *
 * ── Y EL DÍA QUE SE ARQUEA, MANDA LO CONTADO (10/09/2026, Aumenta) ─────────
 *
 * El saldo arrastrado es lo que DEBERÍA haber. El día que alguien cuenta el
 * cajón y lo cierra, se sabe lo que HAY, y a partir de ahí el arrastre sigue
 * desde ahí: si faltaban 20 €, siguen faltando mañana y pasado. Sin esto, la
 * columna de la pantalla y el número que propone «Cerrar caja» —que parte de
 * lo contado en el último cierre— se separaban justo por el descuadre, y no
 * hay nada peor que dos cifras de la misma caja que no coinciden.
 *
 * Qué cierre cuenta: el mismo de `fondoSugerido` (lo decide quien llama, que
 * es el que sabe si lo cerró alguien). Un cascarón importado no manda nada.
 *
 * @param {Array}  dias         filas de `resumenDelDia`, EN ORDEN ASCENDENTE
 *                              (del día más antiguo al más reciente: el saldo
 *                              se arrastra hacia delante, no hacia atrás)
 * @param {number} saldoInicial lo que había en el cajón antes del primer día
 * @param {Map|object} cierres  fecha → { contado } de los arqueos que valen
 * @returns las mismas filas con `efectivoDelDia`:
 *   { cobrado, entradas, salidas, movimiento, esperado, contado, descuadre, queda }
 */
export function saldoDiarioEfectivo(dias = [], saldoInicial = 0, cierres = null) {
  const contadoDe = (fecha) => {
    if (!cierres || !fecha) return null;
    const c = typeof cierres.get === "function" ? cierres.get(fecha) : cierres[fecha];
    if (!c) return null;
    const n = Number(c.contado ?? c.countedAmount);
    return Number.isFinite(n) ? round2(n) : null;
  };

  let saldo = round2(saldoInicial);
  return dias.map((d) => {
    const cobrado = round2(d?.efectivo?.importe);
    const entradas = round2(d?.movimientos?.entradas);
    const salidas = round2(d?.movimientos?.salidas);
    const movimiento = round2(cobrado + entradas - salidas);
    const esperado = round2(saldo + movimiento);
    const contado = contadoDe(d?.fecha);
    const queda = contado === null ? esperado : contado;
    saldo = queda;
    return {
      ...d,
      efectivoDelDia: {
        ...
        salidas,
        movimiento,
        // Lo que debería haber al cerrar ese día…
        esperado,
        // …y lo que se contó de verdad, si ese día se arqueó (si no, null).
        contado,
        descuadre: contado === null ? null : round2(contado - esperado),
        queda,
      },
    };
  });
...
/**
 * ── LO QUE DEBERÍA QUEDAR AL CERRAR (10/09/2026, Aumenta) ──────────────────
 *
 * Rodrigo: «en el botón de cerrar caja Rosa escribe lo que quiere y lo correcto
 * es que lo escriba el sistema; eso se calcula con las entradas y salidas».
 *
 * La cuenta es la del cajón de toda la vida, y es UNA: el fondo del que se
 * parte, más lo que haya entrado desde el último arqueo (el `arrastre`: los
 * días que nadie cerró siguen moviendo el cajón), más los cobros en efectivo
 * del día, menos lo devuelto en efectivo, más las entradas y menos las salidas
 * apuntadas. La comparten la vista previa del cierre, el cierre que se guarda
 * y la columna «Queda en caja» del resumen por día: si cada una hiciera la
 * suya, tres pantallas dirían tres números.
 */
export function esperadoAlCerrar({
  fondo = 0,
  arrastre = 0,
  efectivo = 0,
  devuelto = 0,
...
/**
 * ── DE CUÁNDO SE ARRASTRA EL EFECTIVO (16/09/2026, AV-0157 de Aumenta) ─────
 *
 * Rosa: «hago salida de caja y tengo un saldo que corresponde con el dinero en
 * efectivo (162,42) y al cerrar caja me pone que hay 145,20». Medido en
 * producción el 16/09: la pestaña Efectivo decía 162,42 € y el cierre del
 * 15/09 proponía −145,50 €. Las dos cifras estaban bien calculadas y eran
 * distintas, que es lo peor que puede pasarle a un arqueo.
 *
 * Por qué: el cierre solo arrastraba los días que van DESDE el último arqueo
 * válido, y Aumenta no tiene ninguno — sus 828 cierres vienen importados de
 * Organízate, a cero y sin autor, y `fondoSugerido` los descarta con razón.
 * Sin arqueo del que partir, el cierre se olvidaba de todo lo anterior y solo
 * miraba el día: 354,50 cobrados menos 500 de dos salidas, −145,50. En un
 * cajón no puede haber menos de cero euros; era la prueba de que faltaba algo
 * delante.
 *
 * La regla, ahora, es una y la misma para las dos pantallas: se arrastra desde
 * el último arqueo válido y, si no hay ninguno, DESDE EL PRINCIPIO —el primer
 * día que pasó dinero por esa caja—. El día que alguien cierre de verdad, ese
 * conteo manda y el arrastre vuelve a ser corto.
 *
 * @param {object|null} fondo    lo que devuelve `fondoSugerido` (o null)
 * @param {string} fecha         el día que se cierra, 'AAAA-MM-DD'
 * @param {string|null} primerDia el primer día con cobro o apunte en esa caja
 * @returns {{desde: string, hasta: string}|null} null = no hay nada que arrastrar
 */
export function tramoDeArrastre({ fondo = null, fecha, primerDia = null } = {}) {
  if (!FECHA_RE.test(String(fecha ?? ""))) return null;
  const dia = (f, n) => {
    const [a, m, d] = String(f).slice(0, 10).split("-").map(Number);
    return new Date(Date.UTC(a, m - 1, d + n)).toISOString().slice(0, 10);
  };
  const hasta = dia(fecha, -1);
  const desde = fondo?.fecha ? dia(String(fondo.fecha).slice(0, 10), 1) : primerDia ? String(primerDia).slice(0, 10) : null;
  if (!desde || desde > hasta) return null;
  return { desde, hasta };
}

/**
 * ── DE DÓNDE SALE EL «SE PARTÍA DE» (18/09/2026, cuadre de caja de Aumenta) ──
 *
 * La pestaña «Efectivo en caja» arranca de un saldo, y decía SIEMPRE que era
 * «lo contado al cerrar el …». Desde el 16/09/2026 (AV-0157) puede no venir de
 * ningún cierre: cuando no hay arqueo válido anterior, `saldoAntesDe` devuelve
 * todo lo que pasó por el cajón desde el principio, con `fecha` a null y
 * `desdeElPrincipio`. La pantalla seguía diciendo «contado al cerrar el » —con
 * el hueco de la fecha vacío— y eso es lo peor que puede leer quien cuadra una
 * caja: un número presentado como un conteo que nadie hizo. En Aumenta es el
 * caso NORMAL (sus 828 cierres vienen importados de Organízate, a cero y sin
 * autor), así que lo veía cada vez que miraba media semana.
 *
 * Aquí se decide qué se dice, en un `if` con nombre y con prueba, porque lo
 * comparten la tarjeta de arriba y el pie de la pestaña: si cada uno lo
 * decidiera por su lado, la misma cifra tendría dos explicaciones.
 *
 * El dinero NO entra: el importe lo pinta la pantalla con su `fmtMoney`.
 *
 * @param {{importe:number, fecha:string|null, desdeElPrincipio?:boolean}|null} saldoInicial
 * @returns {{tipo:"cero"|"acumulado"|"cierre", fecha:string|null, breve:string, frase:string}}
 */
export function origenDelSaldoInicial(saldoInicial) {
  const importe = Number(saldoInicial?.importe);
  if (!saldoInicial || !Number.isFinite(importe)) {
    return {
      tipo: "cero",
      fecha: null,
      breve: "sin nada anterior: se empieza en cero",
      frase:
        "No hay nada anterior a estas fechas —ni un cierre con dinero contado ni movimientos del cajón—, así que se empieza en cero: cierra la caja un día y a partir de ahí el saldo se arrastra solo.",
    };
  }

  const iso = saldoInicial.fecha ? String(saldoInicial.fecha).slice(0, 10) : null;
  const fecha = iso && FECHA_RE.test(iso) ? `${iso.slice(8, 10)}/${iso.slice(5, 7)}/${iso.slice(0, 4)}` : null;

  if (saldoInicial.desdeElPrincipio || !fecha) {
    return {
      tipo: "acumulado",
      fecha: null,
      breve: "todo lo que pasó por el cajón antes de estas fechas",
      frase:
        "Nadie ha cerrado la caja antes de estas fechas, así que se parte de todo lo que pasó por el cajón desde el principio: nadie ha contado ese dinero. Cierra la caja un día y a partir de ahí se parte de lo contado.",
    };
  }
  ...
  return {
    tipo: "cierre",
    fecha,
    breve: `contado al cerrar el ${fecha}`,
    frase: `Se parte de lo que se contó al cerrar el ${fecha}.`,
  };
}
