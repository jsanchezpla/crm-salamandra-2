...
async function saldoAntesDe({ tenantModels, desde, cajaId }) {
  ...
  /*
   * SIN ARQUEO VÁLIDO, DESDE EL PRINCIPIO (16/09/2026, AV-0157). Aumenta no
   * tiene ninguno —sus 828 cierres vienen importados a cero y sin autor— y el
   * periodo arrancaba en cero: mirando una semana, la pestaña decía que en el
   * cajón quedaban −145,50 €. Lo que hay en el cajón es TODO lo que ha pasado
   * por él desde el primer día, así que eso es de lo que se parte. Es la misma
   * regla que el arrastre del cierre, para que las dos pantallas no puedan
   * decir dos números distintos.
   */
  const importe = await acumuladoAntesDe({ tenantModels, desde, cajaId });
  return importe === null ? null : { importe, fecha: null, desdeElPrincipio: true };
}

/** Todo lo que ha movido el cajón antes de `desde`: cobros en efectivo − devoluciones + entradas − salidas. */
async function acumuladoAntesDe({ tenantModels, desde, cajaId }) {
  const { Payment, CashMovement } = tenantModels;
  if (!Payment) return null;
  const { start } = madridDayRange(new Date(`${desde}T12:00:00Z`));
  const [cobros, devoluciones, movimientos] = await Promise.all([
    Payment.findAll({
      where: {
        method: "cash",
        paidAt: { [Op.lt]: start },
    ...
    Payment.findAll({
      where: { method: "cash", status: "refunded", refundedAt: { [Op.lt]: start } },
      attributes: ["amount"],
    }),
    CashMovement
      ? CashMovement.findAll({
          where: { date: { [Op.lt]: desde }, ...(cajaId ? { cashPointId: cajaId } : {}) },
          attributes: ["direction", "amount"],
        })
      : [],
  ]);
  if (!cobros.length && !movimientos.length) return null;
  const suma = (filas) => filas.reduce((s, p) => s + Math.abs(Number(p.amount) || 0), 0);
  const caja = saldoDeMovimientos(movimientos.map((m) => m.toJSON()));
  return +(suma(cobros) - suma(devoluciones) + caja.entradas - caja.salidas).toFixed(2);
}
...
export async function construirResumenCaja({ tenantModels, hasModule, desde, hasta, cajaId = null }) {
  ...
  const movimientos = await CashMovement.findAll({
    ...
    // La observación va con el apunte: es donde se explica un apunte raro —el
    // saldo que se trajo de Organízate, corregido en 69 €— y sin ella el número
    // aparece sin motivo justo en la pantalla donde se cuadra (18/09/2026).
    attributes: ["id", "date", "direction", "amount", "concept", "notes"],
  });
  ...
  const filas = [...dias.entries()].map(([fecha, { cobros: c, devoluciones: dv, movimientos: m }]) => {
    ...
    return {
      ...
      // Cada entrada y salida del cajón, suelta (15/09/2026, AV-0131 de Aumenta):
      // la fila solo daba el neto y «−957,19» se leía como UNA salida que nadie
      // apuntó, cuando eran 900 y 150 fuera y 92,81 dentro.
      apuntes: m
        .map((x) => ({ id: x.id, direction: x.direction, amount: Math.abs(Number(x.amount) || 0), concept: x.concept, notes: x.notes ?? null }))
