...
export function limpiarBono(body, { parcial = false } = {}) {
  ...
  /*
   * QUIÉN LO DA (18/09/2026, AV-0183 de Aumenta: «no aparece el terapeuta al
   * que se le asigna»).
   *
   * Opcional a propósito, y en los dos sentidos: el alta no corta por no
   * ponerlo —hay centros sin equipo y bonos que da la casa— y el vacío es un
   * valor que se puede escribir, para poder QUITAR el que se puso mal. Por eso
   * '' y null pasan como null en vez de rechazarse.
   *
   * Que la persona exista de verdad lo comprueba la API, que es la que tiene la
   * plantilla delante; aquí solo se mira la forma.
   */
  if (!parcial || "teamMemberId" in b) {
    const id = b.teamMemberId ? String(b.teamMemberId) : null;
    if (id && !UUID_RE.test(id)) return { valores: null, problema: "El terapeuta del bono no es válido" };
    valores.teamMemberId = id;
  }
...
export function renovacionDe(bono, cambios = {}) {
  const base = {
    ...
    // Quien lo llevaba lo sigue llevando (18/09/2026): renovar es seguir con la
    // misma terapia, y volver a teclear el nombre cada vez es como se acaba
    // quedando en blanco. Se puede cambiar en el mismo botón.
    teamMemberId: bono?.teamMemberId ?? null,
...
/**
 * ¿ESTE BONO ES EL DE UN DIAGNÓSTICO? (18/09/2026, AV de Aumenta: «a veces
 * salen en bonos sesiones que no tienen bonos como la de diagnóstico»)
 *
 * Lo dice `session_packs.diagnostico_id`, que es quien lo ata al expediente, y
 * NO «no tiene tope»: sin tope es hoy la forma de ese bono, pero mañana puede
 * haber otra cosa sin tope y seguiría sin ser un diagnóstico.
 *
 * Hace falta porque este bono no lo compró nadie: lo crea el CRM al pulsar
 * «Seguir con el diagnóstico» (`/api/clinica/diagnosticos/[id]/seguir`), y en
 * la lista de Bonos sale como una fila más. El 17/09/2026 en Aumenta se
 * anularon los dos que había —con sus cobros de 650 € y 350 €— tomándolos por
 * filas sueltas que sobraban, y los dos expedientes se quedaron en curso
 * apuntando a un bono muerto: sin poder apuntarles más horas y sin poder
 * volver a pulsar «Seguir».
 */
export function esBonoDeDiagnostico(bono) {
  return Boolean(bono?.diagnosticoId);
}

/**
 * LO QUE HAY QUE AVISAR ANTES DE ANULAR UN BONO, espejo de
 * `avisosDeRenovacion`.
 *
 * Hoy solo tiene la frase del diagnóstico, y vive aquí y no en el hook por lo
 * mismo que aquélla: lo avisan DOS pantallas —la lista y la ficha de un
 * grupo— y una copia en cada una acabaría avisando distinto del mismo botón.
 *
 * NO prohíbe nada. Anular por error el bono de un diagnóstico abierto es
 * legítimo —quien pulsó «Seguir» sin querer tiene que poder deshacerlo— y la
 * salida es «Reactivar», que devuelve el bono y su cobro. Lo que faltaba no
 * era una valla: era decir qué se lleva por delante.
 */
export function avisosDeAnulacion(bono) {
  const avisos = [];
  if (esBonoDeDiagnostico(bono)) {
    avisos.push(
      "Este bono es el del diagnóstico de esta persona: mientras esté anulado no se le pueden apuntar más horas, y «Seguir con el diagnóstico» tampoco vuelve a estar disponible."
    );
    avisos.push("Si te has equivocado, «Reactivar» lo devuelve con su cobro.");
  }
  return avisos;
}
