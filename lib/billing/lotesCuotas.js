...
export function lineasDeCuota({ cobros = [], mes, vatRate = 0, textosPorConcepto = null } = {}) {
  ...
  for (const cobro of cobros) {
    ...
    if (impreso) {
       ...
       * traen), se imprime el «Texto en la factura» de ese concepto y la nota
       * se queda dentro. La nota solo se usa cuando no hay ni texto ni
       * concepto: una cuota de importe pactado a mano, donde no hay nada más
       * que imprimir.
       */
      ...
      description = !nota ? etiqueta : yaRotulada ? nota : `${etiqueta} — ${nota}`;
    }
    if (tipo === 0) {
