 ...
 *                            { id, clientId, amount, method, paidAt, notes, invoiceText }
...
export function agruparLoteCuotas({ cobros = [], clientes = [], clientesConFacturaDelMes = [], agrupacion = "pagador", conceptos = [], pacientes = [], repartoTutores = true } = {}) {
  ...
  for (const [clave, { cid, conceptId, patientId, cobros: suyos }] of grupos) {
     ...
     *
     * Desde el 08/09/2026 ese defecto ya no hay que elegirlo: sin tutor puesto
     * a mano, `razonSocialPorDefecto` devuelve el TUTOR PRINCIPAL. Solo entran
     * tutores con DNI, así que ningún grupo que hoy se factura se queda ahora
     * apartado por esto; al revés, las familias sin NIF en la ficha pero con un
     * tutor con DNI dejan de caer en «sin NIF».
     */
    /*
    ...
    if (reparto) {
      ...
      reparto.forEach((r, i) => {
        ...
        const cobrosParte = suyos.map((c) => {
          ...
          // Lo IMPRESO no cambia: el concepto es el mismo servicio, y lo que
          // esta factura cobra de él ya lo dice su importe. El «(50 %)» es de
          // puertas adentro y se queda en la nota (09/09/2026, Rodrigo:
          // «simplemente y llanamente el concepto»).
          return {
            ...c,
...
/**
 * El catálogo, venga como venga (Map, array de filas o objeto plano), reducido
 * a `id → texto que se imprime`. El texto es la `description` del concepto —el
 * «Texto en la factura»—; el `name` es el rótulo interno («Cuota T.O. 45x1») y
 * NO se imprime: si el concepto no tiene texto, no hay nada que poner y manda
 * el camino de siempre (la nota).
 */
function mapaDeTextos(origen) {
  const mapa = new Map();
  if (!origen) return mapa;
  const filas = origen instanceof Map
    ? [...origen.entries()].map(([id, v]) => (typeof v === "string" ? { id, description: v } : { ...v, id }))
    : Array.isArray(origen)
      ? origen
      : Object.entries(origen).map(([id, v]) => (typeof v === "string" ? { id, description: v } : { ...v, id }));
  for (const fila of filas) {
    if (!fila || fila.id == null) continue;
    const texto = String(fila.description ?? "").trim();
    if (texto) mapa.set(String(fila.id), texto);
  }
  return mapa;
}
 ...
 *
 * `textosPorConcepto` es el catálogo reducido a lo que se IMPRIME: un Map (o un
 * array de `{ id, description, name }`, o un objeto) de id de concepto a su
 * «Texto en la factura». Lo usa el cobro que no trae `invoiceText`; sin él,
 * todo sigue como antes.
 */
export function lineasDeCuota({ cobros = [], mes, vatRate = 0, textosPorConcepto = null } = {}) {
  const tipo = round2(Number(vatRate) || 0);
  const etiqueta = `Cuota ${mesLegible(mes)}`;
  const textos = mapaDeTextos(textosPorConcepto);
  const lines = [];
  let resto = 0;
  ...
  for (const cobro of cobros) {
    ...
    /*
     * ── LO QUE SE IMPRIME (09/09/2026, Rodrigo) ────────────────────────────
     * «Los conceptos de las facturas no deben ir aparejados a un texto aparte
     * tipo "(viene de organizate)" o "(30 euros menos de reserva)": simplemente
     * y llanamente el concepto.»
     *
     * Así que si el cobro trae su TEXTO DE FACTURA, esa es la línea entera y
     * tal cual: «Terapia 45 min semanales». Ni el mes delante, ni el rótulo del
     * prorrateo, ni de dónde salió el dato — todo eso vive en la NOTA del
     * cobro, que es de puertas adentro y sigue intacta en la pantalla de
     * Cobros.
     *
     * Un cobro SIN texto de factura —apuntado a mano, o anterior a la
     * columna— se factura por su nota como se ha hecho siempre: con «Cuota
     * septiembre 2026» delante si la nota no lo trae ya (`notaDeCobro` sí lo
     * trae, y anteponerlo otra vez daba «Cuota septiembre 2026 — Cuota
     * septiembre 2026 — …»; revisión del 06/09/2026).
     */
    const impreso = String(cobro.invoiceText ?? "").trim();
    const delCatalogo = textos.get(String(cobro.conceptId ?? "")) ?? "";
    let description;
    if (impreso) {
      description = impreso;
    } else if (delCatalogo) {
      /*
       * ── EL CONCEPTO ANTES QUE LA NOTA (17/09/2026, Aumenta) ──────────────
       * Un cobro apuntado a mano desde Cobros nace SIN `invoiceText` —ese
       * campo solo lo rellena el plan de cuotas—, y entonces la factura del
       * mes caía a la nota, que es de puertas adentro: en la emisión del
       * 17/09 salieron impresas «Reserva de plaza ya abonada: −30 €» y
       * «Pendiente según Organízate: 151,25 € (Organízate #20281); el CRM
       * tenía 260,00 €». Es justo lo que Rodrigo prohibió el 09/09.
       *
       * Si el cobro sabe de qué CONCEPTO es (`conceptId`, que esos cobros sí
       * traen), se imprime el «Texto en la factura» de ese concepto.
       */
      description = delCatalogo;
    } else {
      /*
       * ── Y SIN CONCEPTO, LA ETIQUETA A SECAS (18/09/2026, Jorge) ─────────
       * Hasta hoy, un cobro sin texto y sin concepto se facturaba POR SU
       * NOTA. Eso tapaba el agujero en el 79 % de los casos y lo dejaba
       * abierto en el resto: al comprobar el arreglo del 17/09 en producción
       * quedaban CUATRO cobros que habrían impreso en la factura de una
       * familia «Cuota septiembre 2026 — Cuota cobrada en Organízate el
       * 01/09/2026 (Organízate #20138, pago 16493, tarjeta)».
       *
       * La nota es de puertas adentro y ya no sale al papel NUNCA: sin nada
       * que imprimir se imprime «Cuota <mes>» y se acabó. Lo que la nota
       * explica sigue entero en la pantalla de Cobros, que es su sitio.
       */
