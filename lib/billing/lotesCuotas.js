 ...
 *   Cada grupo: { clientId, nombre, nif, cobros, importe, facturaPrevia, motivo? }.
 *   `sinNif` es la lista de trabajo para recepción: se salta, nunca revienta.
 */
export function agruparLoteCuotas({ cobros = [], clientes = [], clientesConFacturaDelMes = [] } = {}) {
  ...
  for (const cobro of cobros) {
    ...
    porCliente.get(cid).push(cobro);
  }

  const facturables = [];
  const sinNif = [];
  for (const [cid, suyos] of porCliente) {
  ...
  facturables.sort(porNombre);
  sinNif.sort(porNombre);
