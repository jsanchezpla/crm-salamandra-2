    ...
    if (reparto) {
      ...
      reparto.forEach((r, i) => {
        ...
        const cobrosParte = suyos.map((c) => {
          ...
          // El porcentaje va también en lo que se imprime: la factura de este
          // tutor es de media cuota y tiene que decirlo (si no, `lineasDeCuota`
          // usaría el texto entero y la línea diría 145 € donde pone 72,50).
          const impreso = String(c.invoiceText ?? "").trim();
          return {
            ...c,
            amount: partes[i].importe,
            notes: `${nota}${nota ? " " : ""}(${r.pct} %)`,
            invoiceText: impreso ? `${impreso} (${r.pct} %)` : null,
            parteDe: c.id,
...
export function lineasDeCuota({ cobros = [], mes, vatRate = 0 } = {}) {
  ...
  for (const cobro of cobros) {
    ...
    /*
     * Lo que se imprime es el TEXTO DE FACTURA del cobro, no su nota
     * (09/09/2026, Aumenta). La nota es de puertas adentro y lleva los nombres
     * internos de los conceptos —«Cuota Logopedia 45x1 + Cuota T.O. 45x1»—, así
     * que la factura de la familia venía diciendo qué terapias hace su hijo. El
     * cobro guarda desde hoy la misma frase con el «Texto en la factura» de cada
     * concepto («Terapia 45 min semanales»), congelada al generarlo.
     *
     * Sin ese texto —un cobro apuntado a mano, o de antes de la columna— se
     * factura por la nota, que es lo que se hacía siempre.
     */
    const nota = String(cobro.invoiceText ?? "").trim() || String(cobro.notes ?? "").trim();
