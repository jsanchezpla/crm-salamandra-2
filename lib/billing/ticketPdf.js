...
export function buildTicketPdfBuffer({
  ...
  return new Promise((resolve, reject) => {
    try {
      // El alto se estima generoso y al final se recorta: pdfkit exige el
      // tamaño al crear el documento, así que se calcula antes con lo que se
      // va a escribir. Sobrar en blanco es feo pero inofensivo; quedarse corto
      // parte el ticket en dos páginas.
      const alto = altoEstimado({ logo, settings, clientName, patientName, notes: payment?.notes });
      ...
      renderTicket(doc, F, { payment, clientName, patientName, invoiceNumber, settings, logo });
...
/**
 * Cuánto papel hace falta. Cada bloque que puede faltar suma solo si está.
 *
 * Se estima al ALZA a propósito, y poco: pdfkit exige el tamaño al crear el
 * documento, así que hay que calcularlo antes de escribir. Pasarse deja un
 * dedo de papel en blanco al final —feo en un rollo, inofensivo—; quedarse
 * corto parte el justificante en dos páginas y entonces no se entrega.
 */
function altoEstimado({ logo, settings, clientName, patientName, notes }) {
  let alto = 175; // cabecera fija + reglas + importe + pie legal
  if (logo) alto += 46;
  if (settings?.fiscalName || settings?.name) alto += 16;
  if (settings?.taxId) alto += 11;
  if (settings?.address) alto += 11 + Math.floor(String(settings.address).length / 38) * 11;
  if (clientName) alto += 11;
  if (patientName) alto += 11;
  // «Cuota de septiembre de 2026» ocupa dos líneas en 80 mm: el concepto y el
  // método se cuentan con holgura de una línea de más cada uno.
  alto += 33;
  if (notes) alto += 6 + Math.ceil(String(notes).length / 44) * 11;
  if (settings?.invoiceFooterText) alto += 6 + Math.ceil(String(settings.invoiceFooterText).length / 46) * 10;
  return alto;
}
...
function renderTicket(doc, F, { payment, clientName, patientName, invoiceNumber, settings, logo }) {
  ...
  if (settings?.address) centro(String(settings.address));
