...
/**
 * Las citas de UNA terapia dentro de las del paciente (08/09/2026, Rosa).
 *
 ...
 * que había. Si las hay atadas pero ninguna es de este concepto, la lista sale
 * vacía y el tramo se prorratea por días: mejor eso que ponerle a la logopedia
 * el ritmo de la psicología.
 */
export function citasDelConcepto(citas, conceptId) {
  if (!Array.isArray(citas) || !citas.length || !conceptId) return citas;
  const atadas = citas.filter((c) => c?.conceptId);
  if (!atadas.length) return citas;
  return atadas.filter((c) => String(c.conceptId) === String(conceptId));
}
...
export function tramoDelMes(mes, { startDate, endDate } = {}, { citas = null } = {}) {
  ...
  if (!tramo.completo && Array.isArray(citas) && citas.length) {
    ...
    if (s && s.enElMes > 0 && s.enElTramo > 0) {
      tramo.sesiones = s;
...
export function sesionesDelTramo(mes, tramo, citas = []) {
  ...
  const diasSemana = new Set(enTramo.map((f) => new Date(`${f}T12:00:00Z`).getUTCDay()));
  let enElMes = 0;
  for (let d = 1; d <= dias; d++) {
    const f = `${mes}-${String(d).padStart(2, "0")}`;
    if (diasSemana.has(new Date(`${f}T12:00:00Z`).getUTCDay())) enElMes++;
  }
  return { enElTramo: new Set(enTramo).size, enElMes };
}
...
export function repartoPorConcepto(mes, cuota, { conceptosPorId = new Map(), citas = null } = {}) {
  ...
  return {
    ...
    tramo: { ...base, factor: mensual > 0 ? importe / mensual : base.factor, sesiones: { enElTramo, enElMes } },
  };
}
...
export function planDeCuotasDelMes({ mes, cuotas = [], conceptos = [], yaGenerados = [], metodos = null, citasPorClave = null } = {}) {
  ...
  for (const cuota of cuotas) {
    ...
    const citasCuota = citasDeLaCuota(citasPorClave, cuota);
    ...
    const fila = {
       ...
       */
      importe: round2((reparto ? reparto.importe : mensualProrrateable * tramo.factor) + fijo),
