...
/** El mes ('AAAA-MM') de una fecha; null si no hay fecha. */
...
/**
 * EL TRAMO DICHO POR MESES: «de septiembre a junio» (18/09/2026, Aumenta:
 * «poner número de meses o poner los meses para crear cuotas nuevas»).
 *
 * `bajaTrasMeses` contesta a «¿cuántos meses?»; esto contesta a la otra mitad,
 * «¿QUÉ meses?», que es como el centro firma el curso. Devuelve las DOS fechas
 * —no solo la baja— porque elegir el mes del alta es parte de la pregunta:
 *
 *   tramoDeMeses("2026-09", "2027-06") → { startDate: "2026-09-01", endDate: "2027-06-30" }
 *
 * **El día del alta se respeta si ya estaba en ese mismo mes**: quien puso el
 * 15/09 porque el niño empezó a mitad de mes y luego dice «de septiembre a
 * junio» no quiere que le regalen quince días — ese trozo es el prorrateo del
 * mes de alta, y es dinero. En cualquier otro mes se arranca el día 1.
 *
 * Sin mes de fin sale `endDate: null` (indefinida), que es un tramo legítimo:
 * «desde enero y hasta que se dé de baja». Un mes de fin ANTERIOR al de alta
 * no es un tramo: devuelve null y la pantalla no escribe nada.
 */
export function tramoDeMeses(mesDesde, mesHasta, { altaActual = null } = {}) {
  if (!mesValido(mesDesde)) return null;
  const hasta = mesValido(mesHasta) ? mesHasta : null;
  if (mesHasta && !hasta) return null;
  if (hasta && hasta < mesDesde) return null;
  const dia = mesDeFecha(altaActual) === mesDesde ? soloFecha(altaActual) : `${mesDesde}-01`;
  return { startDate: dia, endDate: hasta ? ultimoDiaDe(hasta) : null };
}
 ...
 *
 * ── DENTRO DE UNA CUOTA, UNA SOLA VARA DE MEDIR (18/09/2026, Rodrigo) ──────
 *
 * AV-0170, Rosa: da de alta el 21/09 una cuota con Logopedia 45x1 (145 €) y
 * T.O. 45x1 (145 €) y espera 72,50 + 72,50 = 145 €. Salían 120,83 €, y el cobro
 * lo corrigieron a mano cinco minutos después. El motivo: T.O. tenía dos citas
 * apuntadas en el tramo —dibuja patrón, 2 de 4, 72,50 €— y logopedia solo una,
 * así que caía a días (10/30 = 48,33 €). Dos varas de medir dentro de la MISMA
 * cuota, y la que caía a días lo hacía justo cuando la agenda de ese servicio
 * está a medias, que es lo normal el día que se da de alta a alguien. No era un
 * caso suelto: de las 16 cuotas dadas de alta a mitad de septiembre, 10 tenían
 * el importe corregido a mano.
 *
 * Decisión de Rodrigo el 18/09/2026: «todas tienen que utilizar el tema de las
 * sesiones, no los días». Así que **si algún concepto de la cuota dibuja
 * patrón, los demás usan ESE factor** en vez de caer a días. La regla de
 * AV-0082 (una sola sesión no dibuja un patrón) sigue intacta donde importa: lo
 * que no puede hacer un servicio es inventarse SU ritmo con una cita suelta;
 * usar el ritmo que ya ha demostrado su propia cuota no es inventar nada.
 *
 * Si NINGUNO dibuja patrón no hay nada que heredar, se devuelve null y la cuota
 * entera va por días, igual que hasta hoy.
 */
export function repartoPorConcepto(mes, cuota, { conceptosPorId = new Map(), citas = null } = {}) {
  ...
  // 1.ª pasada: el tramo de cada servicio con SUS citas, sin decidir nada.
  const partes = [];
  for (const id of ids) {
    const c = conceptosPorId.get(String(id));
    const precio = Number(c?.unitPrice) || 0;
    ...
    if (!t) continue;
    partes.push({ precio, tramo: t });
  }
  const base = partes[0]?.tramo ?? null;
  const conSesiones = partes.filter((p) => p.tramo.sesiones);
  if (!base || !conSesiones.length) return null;

  /*
   * El ritmo de la CUOTA: las sesiones de los servicios que sí lo dibujaron.
   * Con varios se suman los dos lados de la fracción —no se hace media de
   * factores—, que es la misma cuenta que ya hacía el rótulo y la única que no
   * le da el mismo peso a un servicio de una sesión que a uno de cuatro.
   */
  const refTramo = conSesiones.reduce((n, p) => n + p.tramo.sesiones.enElTramo, 0);
  const refMes = conSesiones.reduce((n, p) => n + p.tramo.sesiones.enElMes, 0);
  const factorDeLaCuota = refMes > 0 ? Math.min(1, refTramo / refMes) : null;

  // 2.ª pasada: el que no dibujó patrón cobra al ritmo de su cuota.
  let importe = 0;
  let mensual = 0;
  let enElTramo = 0;
  let enElMes = 0;
  let heredado = false;
  for (const { precio, tramo } of partes) {
    const suyo = tramo.sesiones !== null;
    const factor = suyo || factorDeLaCuota === null ? tramo.factor : factorDeLaCuota;
    if (!suyo && factorDeLaCuota !== null) heredado = true;
    importe += precio * factor;
    mensual += precio;
    if (suyo) {
      enElTramo += tramo.sesiones.enElTramo;
      enElMes += tramo.sesiones.enElMes;
    }
  }
  ...
  return {
    ...
    tramo: {
      ...
      /*
       * Ya no hay cuotas «mixtas»: o todos los servicios van al ritmo de la
       * cuota, o ninguno cuenta sesiones y el reparto ni se hace. Por eso
       * `mixto` se queda en false y el rótulo vuelve a decir la fracción, que
       * es lo que el centro quiere leer; `heredado` dice si algún servicio
       * cobró al ritmo de otro, por si la pantalla quiere contarlo algún día.
       */
      mixto: false,
      heredado,
    },
