...
export function planDeCuotasDelMes({ mes, cuotas = [], conceptos = [], yaGenerados = [], metodos = null, citasPorClave = null } = {}) {
  ...
  for (const cuota of cuotas) {
    ...
    const fila = {
      ...
      // La misma frase, con los textos de factura en vez de los nombres
      // internos: es la línea que verá la familia (09/09/2026). Se guarda
      // congelada en el cobro; ver `Payment.invoiceText`.
      invoiceText: notaDeCobro({ mes, conceptos: textosFactura, rotulo }),
      conceptosPerdidos,
    };
