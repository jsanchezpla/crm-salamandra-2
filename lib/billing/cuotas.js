...
export function sesionesDelTramo(mes, tramo, citas = []) {
  ...
  return { enElTramo: enTramo.length, enElMes };
}
...
export function planDeCuotasDelMes({ mes, cuotas = [], conceptos = [], yaGenerados = [], metodos = null, citasPorClave = null } = {}) {
  ...
  for (const cuota of cuotas) {
    ...
    const tramo = tramoDelMes(mes, cuota, { citas: citasDeLaCuota(citasPorClave, cuota) });
    if (!tramo) continue;
    ...
    const fila = {
       ...
       */
      importe: round2(mensualProrrateable * tramo.factor + fijo),
