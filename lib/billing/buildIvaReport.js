...
export async function buildIvaReport({ tenantModels, from, to }) {
  ...
  for (const inv of invoices) {
    ...
    // El libro de IVA y el PDF tienen que decir lo MISMO de la misma factura,
    // así que los dos preguntan aquí: la foto congelada al emitir, o el cliente
    // vivo si esa factura no la tiene (las anteriores al 26/08/2026).
    const fiscales = datosFiscalesDe(inv, inv.client);
    outputInvoices.push({
      id: inv.id,
      number: inv.number,
      issueDate: inv.issueDate,
      clientName: fiscales.nombre,
      clientTaxId: fiscales.nif,
      base: invBase,
      vat: invVat,
