...
import { ATRIBUTOS_CLIENTE_FACTURA } from "./nifCliente.js";
import { invoicePatientInclude } from "./patientLink.js";
/**
...
export async function getKpisForPeriod({ tenantModels, from, to }) {
  ...
  const periodWhere = { issueDate: { [Op.between]: [from, to] }, ...activeInvoiceScope(Invoice) };
...
/*
 * ── EL HISTÓRICO ENTERO, NO LAS ÚLTIMAS 50 (07/09/2026, AV-0066 de Aumenta) ──
 * `limite` decide cuántas facturas se traen. Sigue habiendo tope —una familia
 * con mil facturas no se pinta de una— pero ya no es 50 a secas: la ficha pide
 * las que hagan falta para poder ofrecer los ejercicios anteriores, que es lo
 * que Rosa echaba en falta. El tope duro está en `TOPE_FACTURAS` para que no
 * se pueda pedir la tabla entera desde la barra de direcciones.
 */
export const FACTURAS_POR_DEFECTO = 50;
export const TOPE_FACTURAS = 500;

export function limiteDeFacturas(pedido) {
  const n = Number(pedido);
  if (!Number.isFinite(n) || n <= 0) return FACTURAS_POR_DEFECTO;
  return Math.min(Math.trunc(n), TOPE_FACTURAS);
}

export async function getClientBillingSummary({ tenantModels, clientId, from, to, limite, hasModule = null }) {
  const { Invoice, Cost, Client } = tenantModels;
  const incluirPaciente = hasModule ? invoicePatientInclude(tenantModels, hasModule) : [];
  const where = { clientId, ...activeInvoiceScope(Invoice) };
  ...
  const client = await Client.findByPk(clientId, { attributes: ATRIBUTOS_CLIENTE_FACTURA });
  ...
  const tope = limiteDeFacturas(limite);
  const invoices = await Invoice.findAll({
    where,
    attributes: ["id", "number", "issueDate", "dueDate", "status", "taxBase", "total", "paidAmount", "patientId"],
    include: incluirPaciente,
    // Con desempate: hay miles de facturas que comparten fecha de emisión, y
    // sin un segundo criterio dos recargas pueden enseñar filas distintas.
    order: [["issueDate", "DESC"], ["number", "DESC"]],
    limit: tope,
  });

  /*
   * ── LAS DE SUS HIJOS QUE PAGA OTRO (14/09/2026, Rodrigo) ────────────────
   * «Que funcione bien meter a una empresa (Fundación Adecco) a pagar parte o
   * la factura completa… y que llegue todo correcto a la ficha de cliente». La
   * factura que paga la fundación va a nombre de la FUNDACIÓN (`clientId`), así
   * que en la ficha de la familia no salía: la familia no sabía que su hijo
   * tenía facturas pagadas por otro, y su parte parecía la terapia entera.
   *
   * Van APARTE y FUERA de los totales: el dinero no es de la familia —se lo
   * debe la fundación— y sumarlo le pondría un pendiente que no es suyo. Se
   * encuentran por el paciente, que es lo único que las une a esta casa.
   */
  let pagadasPorOtros = [];
  if (incluirPaciente.length) {
    const hijos = await tenantModels.Patient.findAll({ where: { clientId }, attributes: ["id"], raw: true });
    if (hijos.length) {
      const whereOtros = {
        patientId: { [Op.in]: hijos.map((h) => h.id) },
        clientId: { [Op.ne]: clientId },
        ...activeInvoiceScope(Invoice),
      };
      if (where.issueDate) whereOtros.issueDate = where.issueDate;
      pagadasPorOtros = await Invoice.findAll({
        where: whereOtros,
        attributes: ["id", "number", "issueDate", "dueDate", "status", "taxBase", "total", "paidAmount", "patientId", "clientId"],
        include: [...incluirPaciente, { model: Client, as: "client", attributes: ["id", "name", "fiscalName"] }],
        order: [["issueDate", "DESC"], ["number", "DESC"]],
        limit: tope,
      });
    }
  ...
  return {
    ...
    // Si se ha llegado al tope hay más detrás, y la pantalla tiene que decirlo
    // en vez de enseñar una lista corta con cara de estar completa.
    invoicesTruncadas: invoices.length >= tope,
    pagadasPorOtros: withEffectiveStatusList(pagadasPorOtros),
  };
}
...
export async function getEmployeeBillingSummary({ tenantModels, employeeId, from, to }) {
  ...
  const invWhere = { employeeId, ...activeInvoiceScope(Invoice) };
