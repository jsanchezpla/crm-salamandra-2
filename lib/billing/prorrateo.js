 ...
 */
import { tramoDelMes, rotuloDeTramo, citasDelConcepto } from "./cuotas.js";
...
export function partesConProrrateo(partidas, { mes = null, citas = null } = {}) {
  ...
  const partes = (partidas ?? []).map(({ importe, inicio, conceptId = null }) => {
    if (inicio && porSesiones) {
      const tramo = tramoDelMes(mes, { startDate: inicio }, { citas: citasDelConcepto(citas, conceptId) });
      if (tramo) {
        return {
