...
function renderMenu(doc, { plan, client, tenantName, brand, photoBuffers }) {
  ...
  if (hasWeek) {
    // Bloques por día en orden Lunes→Domingo; las comidas sin día (raras en un
    // plan semanal, posibles tras editar) van al final bajo "Otras comidas".
    const groups = [];
    for (let d = 1; d <= 7; d++) {
      const dayMeals = meals.filter((m) => m.weekday === d);
      if (dayMeals.length) groups.push({ title: WEEKDAY_NAMES[d], meals: dayMeals });
    }
    const noDay = meals.filter((m) => m.weekday == null);
    if (noDay.length) groups.push({ title: "Otras comidas", meals: noDay });
