 ...
 *             photoPath/steps del SNAPSHOT congelado en la pauta — no de la
 *             receta viva (13/08/2026). Antes se leían en vivo, así que el PDF
 *             de una pauta de hace meses cambiaba al reescribir la receta.
...
export async function buildMenuPdfBuffer({ plan, client, tenantName, brand, tenantSlug }) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Se le pasa una COPIA, nunca la constante: pdfkit guarda la referencia
      // tal cual en `doc.options` y `switchToPortrait` la MUTA. Sin la copia,
      // el primer menú semanal del proceso dejaba `LANDSCAPE` puesto en
      // vertical y todos los siguientes salían con la portada-calendario en A4
      // vertical, en un servidor que vive semanas (21/08/2026).
      const hoja = hasWeek ? LANDSCAPE : PORTRAIT;
      const doc = new PDFDocument({ ...hoja, margins: { ...hoja.margins } });
      const chunks = [];
