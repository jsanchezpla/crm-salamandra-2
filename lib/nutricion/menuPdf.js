...
function renderMenu(doc, { plan, client, tenantName, brand }) {
  ...
  // ── Comentarios / planificación semanal ────────────────────────────────────
  // Los comentarios del plan llevan la planificación por días (WeekdayStrip,
  // Nutrinotas): es contenido de cara al paciente y DEBE llegar al PDF.
  const description = (plan.description || "").trim();
  if (description) {
    ensureSpace(doc, 70);
    doc.font("Helvetica-Bold").fontSize(11.5).fillColor(primary).text("Planificación y comentarios");
    doc.moveDown(0.3);
