  ...
  if (!hasWeek) {
    ...
    if (meals.length === 0) {
      doc.font(ctx.F.regular).fontSize(11).fillColor(MUTED)
        .text("Este plan aún no tiene comidas.");
    }
...
function renderCoverCalendar(doc, ctx) {
  ...
  doc.moveDown(0.35);
  doc.font(F.bold).fontSize(19).fillColor(primary).text(plan.name || "Menú semanal");
...
function renderPlainHeader(doc, ctx) {
  ...
  doc.moveDown(0.4);
  doc.font(F.bold).fontSize(20).fillColor(primary).text(plan.name || "Plan nutricional");
