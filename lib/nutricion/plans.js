...
export async function deepCopyPlanTree({ models, srcMeals, destPlanId, transaction }) {
  ...
  for (const m of srcMeals) {
    const newMeal = await PlanMeal.create(
      { planId: destPlanId, name: m.name, description: m.description, order: m.order },
      { transaction }
    );
...
/** Ordena meals → options → foods por `order` ASC. Las recetas de la opción se
 * cargan/ordenan aparte con `attachRecipesToTree`. */
