...
// Desde errorTypes.js y no desde errors.js: este módulo lo importan scripts de
// línea de comandos, y errors.js arrastra `next/server`, que Node no resuelve
// fuera del empaquetador de Next (ver lib/utils/errorTypes.js).
...
/**
 * El contexto de un tenant POR SU SLUG, sin petición y sin usuario
 * (03/09/2026, para el calendario global).
 *
 * ── POR QUÉ EXISTE Y POR QUÉ SIN USUARIO ────────────────────────────────────
 * El calendario global lee y mueve eventos de VARIOS tenants en una misma
 * petición, y `getTenantContext` resuelve uno solo: el del JWT. Esto da el
 * mismo contexto —misma caché, mismo `buildContext`, mismos `hasModule` y
 * modelos— para un slug dado, que es lo que necesitan `leerEventos` y
 * `moverEvento` para escribir por el MISMO camino que /api/calendar/tasks
 * (espejo de Google incluido).
 *
 * Va sin usuario a propósito, en «modo infraestructura» (como los webhooks):
 * `hasModule` mira solo si el TENANT tiene el módulo. La autorización de la
 * persona ya la ha dado la fila de `calendario_global_vinculos` antes de
 * llegar aquí; volver a cruzarla con el `moduleAccess` de una cuenta que es de
 * OTRO tenant no tendría sentido. Quien llame a esto sin haber comprobado el
 * vínculo se está saltando la puerta: por eso solo lo importa
 * lib/calendario-global/.
 */
export async function getTenantContextPorSlug(slug) {
  if (!slug || !/^[a-z0-9_]+$/.test(slug)) {
  ...
  const { sequelize: tenantSequelize, models: tenantModels } = getTenantDb(slug);
  return buildContext(slug, config.tenant, config.modules, tenantModels, tenantSequelize, null);
}
