...
import { datosDelContexto } from "../ai/avisoDeCuentaIa.js";
 ...
 *
 * **Rate limiting**: por defecto aplica 30 req/min por IP, key por route.
 * Se ejecuta ANTES de resolver el tenant para no dar pistas al atacante
 * con timing y para no pagar la query a `master` por cada request bloqueada.
 * Para personalizar (o desactivar) pasa `opts.rateLimit`:
 *   withPublicTenant(handler, { rateLimit: { limit: 10, windowMs: 60_000 } })
 *   withPublicTenant(handler, { rateLimit: false })  // desactiva
 *
 * Uso:
 ...
 *   });
 */
export function withPublicTenant(handler, opts = {}) {
  const rateLimitOpts = opts.rateLimit === false ? null : { limit: 30, windowMs: 60_000, ...(opts.rateLimit || {}) };
  return async function (request, routeContext) {
    try {
      if (rateLimitOpts) {
        const params = await routeContext?.params;
        const slug = params?.tenantSlug || "unknown";
        const limited = enforceRateLimit(request, {
          ...rateLimitOpts,
          key: rateLimitOpts.key || `public-tenant:${slug}`,
        });
      ...
      // (13/09/2026, regla #2) El handler corre dentro del contexto de la
      // petición (`lib/ai/usoDeIA.js`), como en `withTenant`. La clasificación
      // automática del portal de soporte gasta la IA del centro: sin contexto,
      // su fallo de cuenta no avisaba a nadie y su coste se guardaba sin
      // tenant. Efectos en los 48 handlers de los 38 ficheros que usan este
      // envoltorio, comprobados: la caché de respuestas de la IA pasa a ir por
      // tenant, y AuditLog sigue leyendo `impersonadorId` null (aquí no hay
      // sesión). Nada más lee ese contexto.
      return await conContextoDeUso(datosDelContexto(tenantContext), () => handler(request, routeContext, tenantContext));
    } catch (err) {
