 ...
 *
 *   { razonSocial, cif, telefonos: [], proteccionDatos,
 *     proteccionDatosAdultos, sedes: [ … ] }
 *
 ...
 * se atendió.
 *
 * ── POR QUÉ DOS AVISOS DE PROTECCIÓN DE DATOS (29/08/2026) ─────────────────
 * Al pedirle a Aumenta su texto legal resultó que no tienen uno, sino DOS, y
 * son distintos de verdad: el de un menor habla de «los datos del menor», de la
 * autorización del padre/madre/tutor y dice que el informe queda «en poder de
 * los padres/tutor legal»; el de un adulto habla de sus propios datos y de su
 * propio consentimiento. Un centro que atiende a niños y a adultos —el caso de
 * Aumenta— no puede imprimir el mismo párrafo en los dos informes: al adulto le
 * estaría diciendo que su informe lo guardan sus padres.
 *
 * Por eso son dos campos y no uno más largo: no es que no quepan (cada uno ronda
 * los 1.700 caracteres y el tope es 2.000), es que **hay que elegir uno según a
 * quién se le hace el informe**, y eso lo decide el generador con la fecha de
 * nacimiento del paciente.
 *
 * `proteccionDatos` sigue siendo el de siempre y el que se usa por defecto —así
 * quien ya lo tenía escrito no nota nada—; `proteccionDatosAdultos` solo entra
 * cuando el paciente es mayor de edad EN LA FECHA DEL INFORME y está relleno.
 *
...
export function normalizarCentro(bruto) {
  ...
  return {
    ...
    proteccionDatosAdultos: texto(c.proteccionDatosAdultos, LIMITES.proteccionDatosAdultos),
    sedes,
  };
}
