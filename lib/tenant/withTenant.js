...
export function withTenant(handler) {
  return async function (request, routeContext) {
    try {
      ...
      // (13/09/2026, regla #2) Y con qué avisar a los admins del tenant cuando
      // falla su cuenta de IA (`avisarFalloDeCuenta`): lo usa el cliente
      // central (`lib/ai/trasFalloDeIa.js`), así ninguna ruta tiene que
      // acordarse de avisar. Antes lo hacían 7 de 20.
      return await conContextoDeUso(
        datosDelContexto(tenantContext, { userId: tenantContext?.user?.id ?? null, impersonadorId }),
