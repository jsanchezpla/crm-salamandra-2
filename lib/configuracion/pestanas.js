...
export const PESTANAS = Object.freeze([
  ...
  // `etiquetarModulo`: cada tarjeta lleva encima de qué módulo es. Solo aquí,
  // porque solo aquí hace falta — en «Agenda» las cinco son de Citas y repetir
  // «CITAS» cinco veces es ruido; en «Módulos» conviven Clínica, Clientes y una
  // que vale para todos, y sin decirlo no se entiende qué pinta cada una junto
  // a la otra (Rodrigo, 23/08/2026).
  { clave: "modulos", titulo: "Módulos", resumen: "Ajustes que solo aplican a ciertos módulos", etiquetarModulo: true },
]);
...
/**
 * De qué módulo es una tarjeta, para poder rotularla.
 *
 ...
 * que se esconden solas —cuando se ven, se ven— y las universales, que dicen
 * «Todo el CRM» en vez de callarse: un hueco sin etiqueta en una lista donde
 * las demás la llevan se lee como un olvido, no como «esta vale para todo».
 */
export function etiquetaDeModulo(clave) {
  const tarjeta = TARJETAS[clave];
  if (!tarjeta) return null;
  if (!tarjeta.requiere) return "Todo el CRM";
