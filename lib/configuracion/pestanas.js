 ...
 *   · `openai` → hasta el 12/09/2026 solo Whisper (`sessions/transcribe`), y
 *     por eso pedía Clínica. Desde ese día es también el proveedor con el que
 *     se puede REDACTAR todo lo anterior (`lib/ai/proveedorIa.js`): universal,
 *     como Anthropic.
 *   · `proveedorIa` → el interruptor entre los dos. Universal por definición.
...
export const TARJETAS = Object.freeze({
  ...
  // ── Conexiones ────────────────────────────────────────────────────────────
  // Con qué IA se redacta (Claude o ChatGPT), delante de las dos claves que
  // elige entre sí.
  proveedorIa: { pestana: "conexiones", requiere: null },
  anthropic: { pestana: "conexiones", requiere: null },
  openai: { pestana: "conexiones", requiere: null },
  ...
  // Las credenciales de Amazon SES del módulo Mailing (06/09/2026). Quien las
  // LEE es solo ese módulo (lib/mailing/ses.js → app/api/mailing/**). Aparte
  // de Resend a propósito: el marketing no comparte cuenta con los recordatorios.
  amazonSes: { pestana: "conexiones", requiere: ["mailing"] },

  // ── Agenda ────────────────────────────────────────────────────────────────
  recordatorios: { pestana: "agenda", requiere: ["citas"] },
  agendaCompartida: { pestana: "agenda", requiere: ["citas"] },
  semanaLaboral: { pestana: "agenda", requiere: ["citas"] },
  colorBloqueos: { pestana: "agenda", requiere: ["citas"] },
  colorCitas: { pestana: "agenda", requiere: ["citas"] },
  ...
  /*
   * Lo que el centro le cuenta de sí mismo a la IA (09/09/2026). Va aquí y no
   * en la pestaña «IA» a propósito: en «IA» está la CLAVE de Anthropic —una
   * credencial, con su tarjeta de integración—, y esto es material clínico del
   * centro que solo existe si tiene Clínica. Misma puerta que las plantillas,
   * con las que se lee junto: aquí se dice cómo se escribe, allí qué apartados
   * hay. `lib/clinica/perfilDelCentro.js` pregunta igual.
   */
  perfilCentro: { pestana: "modulos", requiere: ["clinica", "pacientes"], seEsconde: true },
  ...
  // y concepto de cobro del simple y el completo. Misma puerta que las
  // coordinadoras: es del módulo asistencial (`lib/clinica/diagnostico.js`).
  diagnosticos: { pestana: "modulos", requiere: ["clinica", "pacientes"] },
  consultasExternas: { pestana: "modulos", requiere: ["clients"] },
  /*
   * Toda cita atada a un dinero (04/09/2026, Aumenta). Va en «Módulos» y no en
   * «Agenda» porque pide DOS módulos —Citas y Facturación— y «Agenda» entera
   * cuelga solo de Citas: esa homogeneidad es la que deja resumir su aviso una
   * vez arriba en vez de repetirlo tarjeta a tarjeta, y `_smoke-config-pestanas`
   * la vigila. Aquí encaja además por lo que dice la pestaña: ajustes que solo
   * aplican a ciertos módulos.
   */
  cobroCita: { pestana: "modulos", requiere: ["citas", "billing"] },
