...
/**
 * El ALTA espera más que una consulta (03/09/2026, AV-0031 de tunutrilaura).
 *
 * Crear la cuenta no es solo un INSERT: WordPress arranca, inserta el usuario,
 * genera el enlace para elegir contraseña y MANDA el correo antes de contestar,
 * y en el Hostinger de tunutrilaura eso ha tardado más de 8 s dos veces (31/08
 * y 03/09). El CRM se cansaba a los 8 s, decía «Acceso a la web NO creado» y
 * tres segundos después la web terminaba de crear la cuenta y de enviar el
 * correo. Laura leía «NO creado» y escribía al Buzón preguntando si le había
 * llegado. Se espera hasta 25 s: una aceptación es un clic que ya tarda, y es
 * peor una alerta falsa que dos segundos más de espera.
 */
const TIMEOUT_ALTA_MS = 25000;
/** Cuánto se espera antes de volver a preguntar si, aun así, la web se durmió. */
const ESPERA_RECOMPROBAR_MS = 3000;
...
/**
 * ¿Existe cuenta en la web con ESTE correo? (05/08/2026)
 *
 * Es la respuesta a «¿cómo forzamos que sea el mismo correo?»: forzarlo no se
 * puede —son dos sistemas con su propio campo—, pero sí se puede COMPROBAR y
 * enseñarlo en la ficha. Sin esto, un correo con una letra cambiada no se nota:
 * la ficha parece correcta, y lo que falla en silencio es que su bono no le
 * funciona y que sus citas no se enlazan con nadie.
 *
 * Va por POST aunque solo lea, para compartir firma y antirreplay con el alta:
 * dos formas de firmar es como acaban divergiendo dos comprobaciones de
 * seguridad.
 *
 * Nunca lanza. Devuelve `{ ok:false, motivo }` cuando no se ha podido
 * preguntar, que NO es lo mismo que «no existe» — quien lo pinta tiene que
 * distinguir «no tiene cuenta» de «no he podido comprobarlo», o acabará
 * enseñando una cruz roja porque la web tardó en responder.
 */
export async function consultarUsuarioPortal({ tenantSlug, wordpressUrl, email }) {
  if (!wordpressUrl) return { ok: false, motivo: "sin_url" };
  ...
  const destino = `${String(wordpressUrl).replace(/\/+$/, "")}/wp-json/crm/v1/portal-user/existe`;
  ...
  try {
    ...
    if (!res.ok) {
      // 404 = la web todavía tiene un theme sin esta consulta. Es el caso
      // normal hasta que se suba la versión nueva, y no es un error que deba
      // pintarse en rojo en la ficha.
      return { ok: false, motivo: res.status === 404 ? "sin_soporte" : `http_${res.status}` };
    }
    const json = await res.json().catch(() => ({}));
    return { ok: true, existe: json?.existe === true, altaEn: json?.alta_en ?? null };
  } catch (err) {
    return { ok: false, motivo: err?.name === "AbortError" ? "timeout" : "red" };
  } finally {
    clearTimeout(reloj);
  }
}
...
export async function crearUsuarioPortal({ tenantSlug, wordpressUrl, email, nombre }) {
  ...
  const controlador = new AbortController();
  const reloj = setTimeout(() => controlador.abort(), TIMEOUT_ALTA_MS);
  ...
  try {
    ...
    if (abortado) {
      // Que no contestara a tiempo NO quiere decir que no lo hiciera: la web
      // sigue trabajando después de que aquí se cuelgue. Antes de dar la
      // alarma se le pregunta si la cuenta ya está; si está, el correo para
      // elegir contraseña salió con ella y el resultado es un alta normal.
      const yaEsta = await esperarYComprobar({ tenantSlug, wordpressUrl, email });
      if (yaEsta) {
        return {
          ok: true,
          creado: true,
          tardo: true,
          mensaje: "La web tardó en responder, pero la cuenta está creada y se le ha enviado el correo para elegir su contraseña.",
        };
      }
    }
    return {
      ...
      mensaje: abortado
        ? "WordPress no respondió a tiempo y la cuenta no aparece: vuelve a intentarlo desde la ficha, en «Acceso a la web»."
...
/** Tras un timeout del alta: un respiro y una consulta. `true` solo si la web dice que existe. */
async function esperarYComprobar({ tenantSlug, wordpressUrl, email }) {
  await new Promise((r) => setTimeout(r, ESPERA_RECOMPROBAR_MS));
  const r = await consultarUsuarioPortal({ tenantSlug, wordpressUrl, email });
  return r.ok === true && r.existe === true;
}
