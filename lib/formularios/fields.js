...
/**
 * Destinos que caen en la FICHA de cliente. Son los que la ficha ya pinta, así
 * que aceptar no obliga a tocar la UI.
 */
export const DESTINOS_FICHA = Object.freeze(["name", "email", "phone", "age", "reason", "taxId"]);

/**
 * Destinos que caen en el PACIENTE o en el TUTOR (08/08/2026). Solo se usan en
 * clientes con el módulo `pacientes`: donde no lo hay, la respuesta se queda
 * únicamente en «lo que nos contó» y no se pierde.
 */
export const DESTINOS_FAMILIA = Object.freeze(["patientName", "patientAge", "relationship"]);

export const DESTINOS = Object.freeze([...DESTINOS_FICHA, ...DESTINOS_FAMILIA]);

/**
 * Valor EXACTO de la opción que significa «no hay ningún menor: vengo yo».
 * Vivía como literal suelto en el seed del formulario de Aumenta, y de él
 * depende que el aceptar cree o no la ficha de un paciente. Si alguien
 * reformula la pregunta sin tocar esta constante, el CRM empezaría a crear
 * pacientes con el nombre del adulto sin que nadie relacione una cosa con la
 * otra — por eso sube aquí (justificación de la regla #2).
 */
export const RELACION_ES_EL_PACIENTE = "Soy yo quien necesita ayuda";

/** De lo que contesta la familia a la relación que guarda `Client.guardians`. */
export const RELACION_A_TUTOR = Object.freeze({
  Madre: "madre",
  Padre: "padre",
 ...
 *
 * El prefijo internacional se escribe de las DOS maneras que existen: `+34` y
 * `0034` (el `00` es el prefijo de salida de toda la vida; es lo que llevan
 * impreso las tarjetas de visita y lo que sale al copiar un contacto de la
 * agenda del móvil). Hasta el 21/08/2026 aquí solo se reconocía `+34` o `34` a
 * secas, así que «0034 600 112 233» se quedaba en 13 dígitos, no pasaba el
 * `\d{9}` y tumbaba el ENVÍO ENTERO del formulario público con «El teléfono
 * debe tener 9 dígitos.». La misma persona, con el mismo número, entraba
 * escribiéndolo con «+34» y se perdía escribiéndolo con «0034»: no se guardaba
 * nada, no quedaba rastro y el lead se perdía.
 *
 * El `34` sin nada delante se sigue admitiendo (ningún número español de 9
 * dígitos empieza por 34: empiezan por 6, 7, 8 o 9) y un prefijo de otro país
 * sigue sin colarse como español: `+33 6 12 34 56 78` no empieza por 34 y sus
 * 11 dígitos no son 9.
 */
...
export function validarRespuestas(form, cuerpo) {
  ...
  for (const campo of campos) {
    ...
    // La coma es como se escribe un decimal en España: un peso, una altura o
    // unas horas de sueño las teclea aquí una persona, en la web del cliente, y
    // el cuerpo llega crudo de `request.json()` desde un WordPress que no es
    // nuestro. Nadie lo normaliza antes (el único llamante es
    // `app/api/public/c/[tenantSlug]/formularios/[formSlug]/route.js`, que pasa
    // el JSON tal cual), así que la tolerancia tiene que estar AQUÍ y no en
    // quien llama. Se hace igual que en el validador hermano de las preguntas
    // de la cita (`lib/citas/preguntasCita.js`), que ya la admitía: son dos
    // puertas del mismo cuerpo público, y tener una que acepta lo que la otra
    // rechaza es exactamente lo que se evitó con el DNI unos párrafos más
    // arriba. Un punto de miles («1.000,5») sigue sin ser un número: es
    // ambiguo, y ahí es mejor preguntar que adivinar.
 ...
 *
 * ⚠️ Se excluyen los destinos de FICHA, no «tener mapTo» (08/08/2026). Desde
 * que existen destinos de paciente y de tutor, filtrar por «tiene mapTo» haría
 * desaparecer las respuestas sobre el peque en cualquier cliente que tenga
 * `formularios` y NO tenga `pacientes`: no habría paciente donde guardarlas y
 * tampoco estarían aquí. Y donde SÍ hay paciente tampoco sobra que se lean
 * literales: esto es lo que escribió la familia, palabra por palabra, y la
 * ficha del paciente es un dato ya interpretado.
 */
export function infoAdicional(form, answers) {
  const conDestino = new Set(
    camposDe(form).filter((c) => c.mapTo && DESTINOS_FICHA.includes(c.mapTo)).map((c) => c.key)
  );
  return (answers || [])
