...
/**
 * Lo mismo, pero diciendo los ATRIBUTOS del modelo en vez de las columnas de la
 * base (10/09/2026, Rodrigo: «ni un solo buscador del CRM debería obligar a
 * poner tildes»).
 *
 * Ese repaso tocaba veinte endpoints, y el aviso de arriba —columna de BASE,
 * cualificada con el alias— es justo donde se cae uno: escribir `fileName` en
 * vez de `file_name` no da un resultado raro, da un 500. Aquí el nombre real
 * lo pregunta el propio modelo, que es quien lo sabe:
 *
 *   const filtro = await filtroPorAtributos(Document, q, ["fileName"]);
 *   if (filtro) (where[Op.and] ||= []).push(filtro);
 *
 * `alias` solo hace falta cuando la cláusula viaja dentro del `where` de un
 * include: ahí manda el nombre de la ASOCIACIÓN, no el del modelo.
 */
/**
 * Y el remate: montar el filtro y COLGARLO del `where`, que es lo que hace
 * cada endpoint en la línea siguiente. Va a `Op.and` respetando lo que ya
 * hubiera, porque asignar `where[Op.or]` dos veces pisa el primero en silencio
 * (era el fallo del Buzón: con la pestaña «Nuevos» puesta, escribir en el
 * buscador se llevaba por delante el filtro de estado).
 *
 *   await filtrarPorTexto(where, Document, q, ["fileName"]);
 *
 * Sin texto que buscar no toca nada.
 */
export async function filtrarPorTexto(where, Model, texto, atributos, opciones) {
  const filtro = await filtroPorAtributos(Model, texto, atributos, opciones);
  if (!filtro || !where) return where;
  const previos = where[Op.and];
  where[Op.and] = Array.isArray(previos) ? [...previos, filtro] : previos ? [previos, filtro] : [filtro];
  return where;
}

export async function filtroPorAtributos(Model, texto, atributos, { alias } = {}) {
  if (!Model || palabrasDe(texto).length === 0) return null;
  const defs = (typeof Model.getAttributes === "function" ? Model.getAttributes() : Model.rawAttributes) || {};
  const columnas = (Array.isArray(atributos) ? atributos : [atributos])
    .filter(Boolean)
    .map((a) => `${alias || Model.name}.${defs[a]?.field || a}`);
  return filtroPorNombre(Model.sequelize, texto, columnas);
}
