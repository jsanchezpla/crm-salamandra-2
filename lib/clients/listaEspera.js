...
export async function propagarTerapeutaAlAceptar({ ctx, clientId, terapeutaId, transaction = null }) {
  ...
  // El modelo SIEMPRE está registrado: la puerta es el módulo, y la tabla puede
  // faltar igualmente en un schema a medias (el precedente de urgentes).
  const conClinica = Boolean(ctx?.hasModule?.("pacientes") || ctx?.hasModule?.("clinica"));
  if (conClinica && Patient) {
    try {
      // El espejo es el invariante: columna vacía = lista vacía (lib/clinica/terapeutas.js).
      const sinTerapeuta = await Patient.findAll({ where: { clientId, mainTherapistId: null }, ...conTx });
      for (const paciente of sinTerapeuta) {
        ...
        // `despues` queda vacío: ese paciente no cuenta como propagado.
        if (movimiento.despues.length) salida.pacientes += 1;
      }
    } catch (err) {
