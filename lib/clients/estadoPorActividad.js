 ...
 *
 * Lo que sí reactiva solo es entrar en una cuota, pagar un cobro NUEVO o que
 * se le dé una cita nueva (`reactivarPorActividad.js`): eso ya no es la cola de
 * un mes pasado.
 *
 ...
 * @param {boolean} [a.esperandoPlaza]          está en la cola de admisión, esperando terapeuta
 * @param {string|Date|null} [a.altaEn]          cuándo se abrió la ficha
 * @param {string} baja                         el valor de «Baja» en esa tabla ('inactive' en
 *                                              clientes, 'discharged' en pacientes)
 */
export function estadoPorActividad(
  { ultimaCita = null, ultimoDinero = null, cuotaVigente = false, esperandoPlaza = false, altaEn = null },
  { baja, hoy = new Date() }
) {
  ...
  /*
   * ── QUIEN ESPERA PLAZA NO ES UNA BAJA (18/09/2026, AV-0177 de Aumenta) ───
   * Isabel: «los pacientes de lista de espera, hasta que sean asignados a una
   * terapeuta deberían aparecer en pausa y no baja».
   *
   * Y es literal: una familia que se apunta hoy a la cola no tiene ninguna
   * cita ni ha pagado nada —todavía no le hemos dado hueco—, así que por la
   * regla de arriba nace de Baja. De las 5 que esperaban plaza en Aumenta el
   * 18/09/2026, tres salían «Baja» y sus dos pacientes también. Es lo contrario
   * de lo que dice esa palabra: no se han ido, están esperando a que les demos
   * hora. En cuanto la primera cita entre en la agenda, la regla de arriba los
   * pone Activos sola.
   */
  if (esperandoPlaza) return PAUSA;
  /*
   * ── NI LA FICHA QUE SE ABRIÓ ESTE CURSO (18/09/2026) ─────────────────────
   * Salió al medir lo de arriba: el pase de estados iba a dar de Baja a dos
   * familias abiertas el 17/09 y a una del 11/09 —sin cita todavía y sin estar
   * en la cola—. Es el mismo error con otra cara: «Baja» dice que se fueron, y
   * lo que pasa es que aún no han empezado. Quien entró dentro del curso en
   * marcha no puede ser Baja: como mucho, En pausa hasta que tenga su primera
   * cita, y entonces la regla de arriba lo pone Activo solo.
   */
  if (altaEn && dia(altaEn) >= inicioDelCurso(hoy)) return PAUSA;
  return baja;
}

/**
 * Los estados de los que se VUELVE a activo solo, con una cuota, un cobro o una
 * cita nueva. `prospect` no está: es el que pone la tienda a quien compra una
 * vez (laura_ubeda, 183 fichas) y no se le cambia el sentido por esto. En salud
 * ya no se ofrece («No vino» se quitó el 15/09/2026) y no queda ninguna ficha.
 */
...
/**
 * ¿Ese cambio SUBE de estado? (Baja → En pausa → Activo.)
 *
 * Lo usa el pase con `--solo-subir`, para arreglar a quien está peor de lo que
 * le toca sin bajar de Activo a nadie de paso: son dos decisiones distintas y
 * la segunda mueve a gente que nadie ha mirado (18/09/2026, AV-0177).
 */
export function sube(de, a) {
  const grado = (e) => (e === ACTIVO ? 2 : e === PAUSA ? 1 : 0);
  return grado(a) > grado(de);
}
