...
import { edadDe } from "../clinica/edad.js";

/*
 * `conMotivo` (15/09/2026, AV-0135 de Aumenta): añade el motivo de consulta de
 * cada paciente. Lo piden SOLO la ficha y la lista de espera, donde recepción
 * lo escribió en el alta y después no lo encontraba («¿desaparece?»). Va detrás
 * de una opción y no siempre porque este helper también alimenta Citas y
 * Facturación, que no tienen por qué recibir texto clínico.
 */
/*
 * `conEdad` (18/09/2026, AV-0210 de Aumenta): añade la fecha de nacimiento y la
 * edad de cada paciente. La pide SOLO la ficha, que las escribe al lado del
 * nombre porque es con lo que se decide qué terapeuta lleva a un niño. Va
 * detrás de una opción por lo mismo que `conMotivo`: este helper también
 * alimenta Citas y Facturación, y allí no hacen falta.
 *
 * La edad se calcula con `lib/clinica/edad.js`, que ya sabe que la fecha manda
 * sobre la casilla «Edad» escrita a mano (la escrita se queda vieja cada
 * cumpleaños).
 */
export async function pacientesPorFamilia({ clientIds, Patient, hasModule, conMotivo = false, conEdad = false }) {
  ...
  try {
    const filas = await Patient.findAll({
      ...
      attributes: [
        "id", "clientId", "firstName", "lastName", "status",
        ...(conMotivo ? ["referralReason"] : []),
        ...(conEdad ? ["birthDate", "age"] : []),
      ],
    ...
    for (const p of filas) {
      ...
      const fila = { id: p.id, nombre, estado: p.status };
      if (conMotivo) fila.motivo = p.referralReason?.trim() || null;
      if (conEdad) {
        fila.birthDate = p.birthDate ?? null;
        fila.edad = edadDe(p);
      }
