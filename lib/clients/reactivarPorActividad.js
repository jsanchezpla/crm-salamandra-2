 ...
 * a Activo solo en cuanto entra en una cuota o recibe un cobro (15/09/2026,
 * Rodrigo: «en el momento en que un cliente en pausa o de baja entre en una
 * cuota o reciba un cobro por alguno de sus pacientes, pasará a estar activo»).
 *
 * (Fichero nuevo en /lib, regla #2: va como HOOK de los modelos `Payment` y
 * `Cuota` y no en cada endpoint porque un cobro completado nace por al menos
 * seis caminos —el cajón, el PATCH, «a cuenta», el bono, el webhook de Stripe,
 * la conciliación del banco— y el séptimo que alguien escriba se olvidaría.)
 *
 * Solo sube estados, nunca baja ninguno, y no toca «No vino» (prospect). Va en
