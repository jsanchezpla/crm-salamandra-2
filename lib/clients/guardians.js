 ...
 *
 * Forma: { id, name, relationship, dni, phone, email, domicilio, signer }
 *   - relationship ∈ madre | padre | tutor | otro
 *   - signer: debe firmar el Contrato del Centro en el portal.
 *   - domicilio: lo que el tutor escribió al firmar el contrato en el portal
 *     (`tutorDeclarado`, lib/clients/datosFicha.js). Es un dato DECLARADO, no
 *     uno que teclee el centro: por eso ninguna pantalla lo deja editar y por
 *     eso hay que conservarlo cuando se vuelve a guardar la lista.
 */
...
//
// ⚠️ Esta función RECONSTRUYE cada tutor clave a clave, así que lo que no esté
// aquí se pierde al guardar. El `domicilio` estuvo fuera desde el 28/07/2026 y
// nadie lo notó porque no hay pantalla que lo enseñe: la ficha de la familia
// manda de vuelta el tutor entero tal cual se lo dio el GET, y al normalizarlo
// el domicilio desaparecía sin aviso y sin poder recuperarlo (el contrato
// firmado guarda su copia, pero no vuelve a la ficha).
export function normalizeGuardians(input) {
  ...
  return input
    ...
    .map((g) => ({
      ...
      domicilio: cleanStr(g.domicilio),
...
export function tutoresDeLaFamilia(client) {
  ...
  const i = crudos.findIndex((g) => {
    ...
    // El DNI CONFIRMA, pero no VETA (18/09/2026, comprobado sobre las 1.107
    // familias de aumenta). La regla anterior decía que dos DNI puestos y
    // distintos eran dos personas aunque se llamaran igual, y sobre los datos
    // de verdad acertaba 0 de 18: en las 18 familias donde eso pasaba, el
    // titular y el tutor son la MISMA persona con un DNI mal tecleado o
    // prestado (a veces el de la propia niña), y la ficha la enseñaba dos
    // veces. Dos padres de una misma familia con el nombre y los apellidos
    // idénticos no existen; un DNI mal copiado, 18 veces.
    if (dniTitular && dni && dni === dniTitular) return true;
 ...
 *   · `dni`, `domicilio` y `signer` se heredan de la entrada que ya existía;
...
export function fusionarTutoresDeFicha(actuales, entrantes) {
  ...
  for (const [i, e] of entrantes.entries()) {
    ...
    guardians.push({
      ...
      // El domicilio va aquí y no arriba porque no es un campo del formulario:
      // lo escribió el tutor al firmar y solo puede cambiarlo él, firmando otra
      // vez. Un tutor nuevo nace sin domicilio, como nace sin DNI.
      dni: previo?.dni ?? null,
      domicilio: previo?.domicilio ?? null,
      phone,
      email,
