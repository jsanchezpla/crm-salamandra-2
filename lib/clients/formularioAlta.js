 ...
 *
 * Y encima del perfil, un campo suelto que depende de OTRO módulo: el «Tipo»
 * del contratante (`booking`). No es un tercer perfil —una agencia de
 * management pregunta lo mismo que cualquier otro cliente comercial—, es una
 * pregunta más, así que entra como opción y no partiendo la lista en tres.
 *
...
import { opcionesCategoria } from "../booking/categorias.js";
 ...
 *
 * `conCategoria` = el cliente tiene el módulo `booking`, o sea que sus fichas
 * son contratantes y lo primero que hay que saber de uno es QUÉ ES. Se pasa
 * igual que la anterior: quien llama a esta función ya sabe los módulos, y así
 * el fichero no tiene que preguntarlos ni desde el navegador ni desde el
 * servidor.
 */
export function camposCliente(perfil, { conPacientes = false, conCategoria = false, titularEsProgenitor = false, titularEsElPaciente = false } = {}) {
  const salud = perfil === PERFIL_SALUD;
  return [
    // Con el alta que empieza por el paciente (`lib/clients/altaPorPaciente.js`,
    // 07/09/2026) el «Nombre» de la ficha se pregunta DESPUÉS del niño, y hay
    // que decir de quién es: es el padre, la madre o el tutor que la abre. Sin
    // eso, el primer nombre que se teclea es el del hijo (AV-0051 de Aumenta).
    // Y si han marcado que el paciente abre su propia ficha (AV-0201), ese
    // rótulo mentiría: no hay padre ni tutor, es él mismo. El nombre viene
    // copiado de arriba y se puede corregir aquí.
    titularEsElPaciente
      ? {
          label: "Nombre y apellidos *",
          key: "name",
          type: "text",
          placeholder: "Ana Ruiz Pérez",
          ayuda: "El paciente es el titular de esta ficha: quien paga y quien recibe los avisos.",
        }
      : titularEsProgenitor
      ? {
          label: "Padre, madre o tutor que abre la ficha *",
          key: "name",
          type: "text",
          placeholder: "Ana Ruiz Pérez",
          ayuda: "Su nombre y apellidos. Es el titular de la ficha de la familia: quien paga y quien recibe los avisos.",
        }
    ...
    /*
     * De qué tipo es el contratante (01/09/2026, Rodrigo: «no puedo añadir Tipo
     * a los contratantes, debería dejarme en Editar ficha»).
     *
     * La lista sale de `lib/booking/categorias.js`, la MISMA con la que la
     * pantalla filtra y pinta la columna «Tipo». Hasta hoy ese campo solo lo
     * escribía la importación de los 210 contactos: un contratante dado de alta
     * a mano nacía sin tipo, salía con un guión en la columna, se caía de todos
     * los filtros y no había ninguna pantalla donde ponérselo.
     *
     * Justo después del nombre y la empresa porque es el orden de la lista
     * (Contratante · Tipo · Email…) y porque es lo segundo que se sabe de
     * alguien: el Teatro Circo es una sala antes de que nadie tenga su correo.
     */
    ...(conCategoria
      ? [{
          label: "Tipo",
          ...
          ayuda: "Decide cómo se le escribe: a un ayuntamiento se le manda un presupuesto; a una revista, una nota de prensa.",
        }]
      : []),
    ...
    // ficha) y solo hace falta teclear al segundo. Si el titular ES el paciente
    // no se pregunta: «qué eres del paciente» no significa nada (AV-0201).
    ...(conPacientes && !titularEsElPaciente
      ? [{
...
/**
 * ¿Alguno de los pacientes del alta es el propio titular de la ficha?
 * (18/09/2026, AV-0201 de Aumenta.)
 *
 * Es la pregunta que decide cómo se rotula el bloque de la familia y si el
 * nombre del titular se copia solo. Vive aquí, al lado del valor que mira, y
 * no en la pantalla: la hacen el alta y los textos de `altaPorPaciente.js`, y
 * comparada a mano en dos sitios se separa a la primera.
 */
export function hayPacienteQueEsElTitular(pacientes) {
  return (Array.isArray(pacientes) ? pacientes : []).some(
    (p) => p?.relationship === PARENTESCO_ES_EL_CLIENTE
  );
}

/**
 * ¿Ese nombre es el de alguno de los pacientes que se están dando de alta?
 *
 * La usa el alta para saber si el nombre del titular lo copió la casilla «es
 * mayor de edad» o lo tecleó una persona: al desmarcarla se borra lo copiado y
 * se respeta lo escrito. Compara sin mayúsculas ni espacios de más, que es
 * como se teclea de verdad; con el nombre vacío, `false` (no hay nada que
 * proteger).
 */
export function esNombreDeAlgunPaciente(pacientes, nombre) {
  const igual = (s) => String(s ?? "").trim().replace(/\s+/g, " ").toLocaleLowerCase("es");
  const buscado = igual(nombre);
  if (!buscado) return false;
  return (Array.isArray(pacientes) ? pacientes : []).some(
    (p) => igual(`${p?.firstName ?? ""} ${p?.lastName ?? ""}`) === buscado
  );
}
...
export const CAMPOS_PACIENTE = [
  ...
  { label: "Fecha de nacimiento", key: "birthDate", type: "date" },
  // El DNI del PACIENTE (18/09/2026, AV-0179 de Aumenta). La columna existe
  // desde el principio y la ficha lo enseña; lo que faltaba era dónde
  // escribirlo: la importación de Organízate rellenó 502 de 1.201 y los otros
  // 699 no se podían completar por ninguna pantalla. No es obligatorio: la
  // mayoría de los niños pequeños no tienen.
  { label: "DNI / NIE", key: "dni", type: "text", placeholder: "12345678Z" },
  // Y el domicilio, por lo mismo (ficha «Datos tutor», AV-181): 922 de 1.201
  // pacientes lo tienen puesto por la importación y ninguna pantalla del alta
  // lo preguntaba. No se hereda del titular a propósito: el domicilio del menor
  // no siempre es el del progenitor que abre la ficha (padres separados).
  { label: "Domicilio", key: "address", type: "text", placeholder: "C/ Mallorca 210, 3º 2ª" },
...
export function normalizarPacientes(lista) {
  ...
  for (const [i, p] of lista.entries()) {
    ...
    const resto = [p.birthDate, p.dni, p.address, p.educationCenter, p.educationLevel, p.relationship, p.referralReason].some(
      (v) => limpio(v, 300)
    );
    ...
    pacientes.push({
      ...
      // El DNI y el domicilio del niño (AV-0179 y AV-181): sin estas líneas se
      // teclean, se mandan y no se guardan, igual que le pasó al motivo de
      // consulta. Mismos topes que la columna y que el PATCH de la ficha.
      dni: limpio(p.dni, 20),
      address: limpio(p.address, 255),
