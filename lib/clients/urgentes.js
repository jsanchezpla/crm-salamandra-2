...
import { ESTADOS_FICHA, NO_VINO, dejaDeReclamar } from "./estados.js";
...
export const CARPETAS = [
  ...
  // sin esa marca no ve la carpeta, ni a cero.
  {
    ...
    entidad: "client",
    opcional: true,
    // Las archivadas SÍ salen (Rodrigo, 03/09/2026: «pero salen en la lista,
    // ¿no?»): es una lista de revisión, no un hueco, y una familia archivada
    // que pagó la reserva y tiene cuota de septiembre es justo lo que hay que
    // ver. Salen con su chip «Archivada», como las de hora cogida.
    incluyeBajas: true,
  },
  // ── Las cuotas y las citas (06/09/2026, Rodrigo: «pon todos los problemas
  // en Fichas a completar») ─────────────────────────────────────────────────
  //
  // Al atar cada cita importada de Organízate a la cuota de su familia
  // (`scripts/atar-citas-organizate-a-cuota.js`) quedaron a la vista cinco
  // clases de problema: familias con citas y sin ninguna cuota, citas que no
  // dicen de qué se cobran aunque la familia tenga cuota, cuotas que no
  // cuadran con las citas (45 frente a 60 minutos), pacientes sin familia y
  // fichas de paciente duplicadas. Ninguna es un dato que se escriba: se
  // CALCULAN en cada carga, así que cuando el centro da de alta la cuota o
  // funde las fichas, la fila desaparece sola. Las tres de cuotas llevan
  // `requiere: "cuotas"`: un centro sin la tabla `billing_cuotas` (sin
  // Facturación) no las ve, ni a cero.
  {
    key: "citas_sin_cuota",
    label: "Con citas y sin ninguna cuota: no se les puede cobrar el mes",
    ayuda: "Tienen citas de este mes o del pasado que no dicen de qué se cobran, y la familia no tiene ninguna cuota viva en Facturación → Cuotas. Dar de alta la cuota que toque ata las citas solas; si el niño ya no viene, dar de baja sus citas. Un paciente en pausa sale con su chip.",
    bloquea: true,
    entidad: "client",
    requiere: "cuotas",
  },
  {
    key: "citas_sin_cobro",
    label: "Con citas que no dicen de qué se cobran (la familia sí tiene cuota)",
    ayuda: "La cuota de la familia no cubre ese servicio (la cita es de logopedia y la familia paga psicología), o la cita es de un bono, un informe o un programa. Se arregla en la propia cita (su cobro) o dando de alta la cuota que falta.",
    bloquea: false,
    entidad: "client",
    requiere: "cuotas",
  },
  {
    key: "cuota_no_cuadra",
    label: "La cuota que pagan no cuadra con sus citas (otra terapia, o 45 frente a 60 minutos)",
    ayuda: "El curso de Organízate de la cita es de una terapia o de unos minutos y la cuota a la que quedó atada es de otra: una cita de logopedia cobrada como psicología, o 45 minutos con una cuota de 60. O la cuota está mal, o lo está la cita. Se enseña qué cita va a qué cuota.",
    bloquea: false,
    entidad: "client",
    requiere: "cuotas",
  },
  // ── La factura sale a nombre de la ficha porque al tutor le falta el DNI
  //    (09/09/2026, Rodrigo) ────────────────────────────────────────────────
  //
  // Desde el 08/09 la factura sale sola a nombre del TUTOR PRINCIPAL y con su
  // DNI (`lib/billing/razonSocial.js`): 859 de las 1.098 familias de Aumenta.
  // Las que no tienen ni un tutor con nombre y DNI siguen facturando a nombre
  // de la ficha —que muchas veces es el nombre del niño—, y se arreglan solas
  // en cuanto alguien escriba ese DNI en «Padres y tutores», sin elegir nada
  // más. Pero para eso hay que saber cuáles son, y no salían en ninguna
  // pantalla: 142 familias invisibles, contadas en producción el 09/09/2026.
  // Las otras 95 que facturan a la ficha no tienen ningún tutor, y esas sí se
  // veían ya, en las dos carpetas de «sin tutor».
  //
  // NO se calla por las carpetas de arriba, como tampoco lo hace la de reservas
  // de plaza: es OTRO hueco. 90 de las 142 están además mudas y 17 sin correo,
  // y rellenarles el teléfono no les pone el DNI — callándose, esas 107
  // volverían a ser invisibles justo para lo que se hizo esta carpeta.
  {
    key: "tutor_sin_dni",
    label: "Familias con tutor apuntado pero sin su DNI: la factura sale a nombre de la ficha",
    ayuda: "La factura va sola a nombre del tutor principal, y para eso hace falta su DNI. Sin él sale a nombre de la ficha, que muchas veces es el nombre del niño. Se arregla escribiendo el DNI en «Padres y tutores»: no hay que elegir nada más. Las facturas ya emitidas no cambian.",
    bloquea: false,
    entidad: "client",
    // No porque la consulta necesite la tabla de cuotas, sino porque sin
    // Facturación no hay factura y el DNI del tutor no le hace falta a nadie.
    // Es el mismo interruptor con el que se esconden las tres de cuotas.
    requiere: "cuotas",
  },
  {
    key: "paciente_sin_familia",
    label: "Pacientes con citas del curso y sin familia",
    ayuda: "La ficha del paciente no cuelga de ninguna familia: no hay a quién cobrarle ni a quién avisar. Suele ser una ficha duplicada (mira la carpeta de abajo).",
    bloquea: true,
    entidad: "patient",
  },
  {
    key: "ficha_duplicada",
    label: "Pacientes con dos fichas con el mismo nombre",
    ayuda: "Dos fichas de paciente que se llaman igual: una tendrá las citas y la otra la familia o la cuota. Hay que quedarse con una y mover lo de la otra.",
    bloquea: false,
    entidad: "patient",
  },
];
...
/**
 * Un tutor al que SÍ se le puede facturar: con id, con nombre y con DNI.
 *
 * Es la misma condición que `tutorPrincipalDe` (lib/billing/razonSocial.js),
 * dicha en SQL porque aquí se pregunta por las 1.098 fichas de golpe y traerse
 * los tutores de todas para mirarlos en JavaScript no cabe en una pantalla que
 * ya tarda cuatro segundos. Las dos tienen que decir lo mismo: si allí cambia
 * qué hace falta para facturar a un tutor, esta carpeta se queda vieja y deja
 * de enseñar el hueco. Lo empareja `_smoke-clients-urgentes-tutor-sin-dni.mjs`.
 */
const SQL_TUTOR_FACTURABLE = `EXISTS (
    SELECT 1 FROM jsonb_array_elements(
      CASE WHEN jsonb_typeof(c.guardians) = 'array' THEN c.guardians ELSE '[]'::jsonb END
    ) g
    WHERE g->>'id' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
      AND coalesce(g->>'name','') <> '' AND coalesce(g->>'dni','') <> ''
  )`;

/**
 * La razón social escrita A MANO en la ficha: una empresa, una fundación, el
 * abuelo que paga. Manda sobre el cálculo (`razonSocialAutomatica`), así que a
 * esa familia no hay ningún DNI que pedirle aunque ninguno de sus tutores lo
 * tenga: ya está dicho a nombre de quién se factura.
 */
const SQL_RAZON_SOCIAL_A_MANO = `(coalesce(c.fiscal_name,'') <> '' OR coalesce(c.fiscal_tax_id,'') <> '')`;

/**
 * A quién hay que pedirle el DNI: los nombres de los tutores de la ficha.
 *
 * Solo los NOMBRES. Ni el DNI de quien lo tenga, ni el teléfono: es la misma
 * regla que `opcionesDeRazonSocial`, que tampoco deja bajar los documentos al
 * navegador para elegir un nombre.
 */
const SQL_NOMBRES_DE_TUTORES = `(SELECT string_agg(coalesce(nullif(g->>'name',''), '(sin nombre)'), ', ')
 ...
 */
// `NO_VINO` aparte: desde el 15/09/2026 no se ofrece, pero la ficha que lo
// tenga sigue sin reclamar (se lee como Baja).
const ESTADOS_QUE_NO_RECLAMAN = [...ESTADOS_FICHA, NO_VINO].filter(dejaDeReclamar)
  .map((e) => `'${e}'`)
  .join(", ");
 ...
 */
function cuerpoBase(esquema, carpeta, { conPacientes = true, conCitas = true, conCuotas = true } = {}) {
  ...
  // habla por sí misma.
  // ── Piezas de las carpetas de cuotas y fichas (06/09/2026) ─────────────
  const CITA_COBRABLE = `b.status <> 'cancelled' AND b.taller_grupo_id IS NULL
                AND b.scheduled_at >= date_trunc('month', now()) - interval '1 month'`;
  const CITA_SIN_COBRO = `${CITA_COBRABLE} AND b.cobro_modo IS NULL`;
  // El curso de Organízate que la cita lleva escrito o, si no, su tipo.
  const CURSO_DE_CITA = `coalesce(substring(b.additional_data from '· (.*)$'), et.name, 'sin tipo')`;
  const FAMILIA_CON_CUOTA = `EXISTS (SELECT 1 FROM ${esquema}.billing_cuotas q WHERE q.client_id = c.id AND q.active)`;
  const HAY_CITA_SIN_COBRO = `EXISTS (SELECT 1 FROM ${esquema}.bookings b WHERE b.client_id = c.id AND ${CITA_SIN_COBRO})`;
  const DETALLE_SIN_COBRO = `(SELECT count(*)::text || ' cita(s) sin cobro · ' || string_agg(DISTINCT ${CURSO_DE_CITA}, ', ')
                FROM ${esquema}.bookings b LEFT JOIN ${esquema}.event_types et ON et.id = b.event_type_id
               WHERE b.client_id = c.id AND ${CITA_SIN_COBRO})`;
  const PACIENTE_EN_PAUSA = conPacientes
    ? `CASE WHEN EXISTS (SELECT 1 FROM ${esquema}.patients hijo WHERE hijo.client_id = c.id AND hijo.status = 'paused') THEN 'paciente en pausa' END`
    : `NULL::text`;
  // «45 frente a 60»: los minutos del curso de la cita contra los del concepto
  // al que quedó atada. Solo citas importadas (las que llevan curso).
  const CUOTA_NO_CUADRA_JOIN = `${esquema}.bookings b JOIN ${esquema}.billing_concepts bc ON bc.id = b.cobro_concept_id`;
  // Los minutos se leen del CURSO (lo que va detrás de «· »), nunca de la
  // cadena entera: el id de Organízate («#126600») ya lleva un 60 dentro.
  const MINUTOS_DEL_CURSO = `substring(substring(b.additional_data from '· (.*)$') from '(30|45|60)')`;
  // Y la TERAPIA: una cita de logopedia atada a la cuota de psicología también
  // es «no cuadra» (pasó con una familia que solo paga psicología y trae al
  // niño a logopedia). La palabra clave del curso y la del concepto se
  // traducen a la misma etiqueta; lo que no se reconoce no se compara.
  const TERAPIA_DE = (expr) => `CASE WHEN ${expr} ~* 'LOGOP' THEN 'logopedia' WHEN ${expr} ~* 'PSICO|PISCO' THEN 'psicologia'
                WHEN ${expr} ~* 'PEDAG' THEN 'pedagogia' WHEN ${expr} ~* 'OCUPACIONAL|T\.O\.' THEN 'to' END`;
  const CUOTA_NO_CUADRA_WHERE = `b.client_id = c.id AND ${CITA_COBRABLE} AND b.additional_data ~ '· C_'
                AND (${MINUTOS_DEL_CURSO} IS DISTINCT FROM substring(bc.name from '(30|45|60)')
                     OR ${TERAPIA_DE("b.additional_data")} IS DISTINCT FROM ${TERAPIA_DE("bc.name")})`;
  const MISMO_NOMBRE = `lower(trim(regexp_replace(p2.first_name || ' ' || p2.last_name, '\\s+', ' ', 'g')))
                = lower(trim(regexp_replace(p.first_name || ' ' || p.last_name, '\\s+', ' ', 'g')))`;
  ...
  switch (carpeta) {
    ...
    case "reserva_plaza":
      return {
        ...
        // `orden` lo escribe el cotejo (0 = lo que más urge mirar … 9 = cuadra),
        // para que el SQL no tenga que conocer los textos de los avisos.
        order: "coalesce((c.custom_fields->'reservaPlaza'->>'orden')::int, 9), c.name",
      };

    /*
     * ── Las cinco de las cuotas y las fichas (06/09/2026) ──────────────────
     * «Cita cobrable»: no anulada, no de taller, de este mes o del anterior
     * (lo que se cobra ahora y lo que aún se puede estar cobrando). El
     * detalle enseña cuántas y de qué curso de Organízate o de qué tipo son.
     */
    case "citas_sin_cuota":
      if (!conCitas || !conCuotas) return null;
      return {
        id: "c.id",
        select: `${SOLO_FAMILIA}, ${DETALLE_SIN_COBRO} AS dato, ${PACIENTE_EN_PAUSA} AS aviso`,
        from: `${esquema}.clients c`,
        where: `${HAY_CITA_SIN_COBRO} AND NOT ${FAMILIA_CON_CUOTA}`,
        order: "c.name",
      };
    case "citas_sin_cobro":
      if (!conCitas || !conCuotas) return null;
      return {
        id: "c.id",
        select: `${SOLO_FAMILIA}, ${DETALLE_SIN_COBRO} AS dato, NULL::text AS aviso`,
        from: `${esquema}.clients c`,
        where: `${HAY_CITA_SIN_COBRO} AND ${FAMILIA_CON_CUOTA}`,
        order: "c.name",
      };
    case "cuota_no_cuadra":
      if (!conCitas || !conCuotas) return null;
      return {
        id: "c.id",
        select: `${SOLO_FAMILIA}, (SELECT count(*)::text || ' cita(s): ' || string_agg(DISTINCT substring(b.additional_data from '· (.*)$') || ' → ' || bc.name, ', ')
                    FROM ${CUOTA_NO_CUADRA_JOIN} WHERE ${CUOTA_NO_CUADRA_WHERE}) AS dato`,
        from: `${esquema}.clients c`,
        where: `EXISTS (SELECT 1 FROM ${CUOTA_NO_CUADRA_JOIN} WHERE ${CUOTA_NO_CUADRA_WHERE})`,
        order: "c.name",
      };
    /*
     * Tiene tutor apuntado, ninguno con nombre y DNI, y nadie ha escrito la
     * razón social a mano: la factura de esta familia sale a nombre de la
     * ficha. Es lo que decide `razonSocialAutomatica()`, dicho en SQL.
     *
     * El detalle enseña A QUIÉN pedirle el DNI, y solo el nombre: de esta
     * pantalla no baja ningún documento al navegador.
     */
    case "tutor_sin_dni":
      if (!conCuotas) return null;
      return {
        id: "c.id",
        select: `${SOLO_FAMILIA}, ${SQL_NOMBRES_DE_TUTORES} AS dato`,
        from: `${esquema}.clients c`,
        where: `NOT (${SQL_SIN_TUTOR}) AND NOT ${SQL_TUTOR_FACTURABLE} AND NOT ${SQL_RAZON_SOCIAL_A_MANO}`,
        order: "c.name",
      };
    case "paciente_sin_familia":
      return {
        id: "p.id",
        select: `${PACIENTE_Y_FAMILIA}, ${PROXIMA_CITA}`,
        from: `${esquema}.patients p LEFT JOIN ${esquema}.clients c ON c.id = p.client_id`,
        where: `p.client_id IS NULL AND ${TIENE_CITA_FUTURA}`,
        order: "dato",
      };
    case "ficha_duplicada":
      return {
        id: "p.id",
        select: `${PACIENTE_Y_FAMILIA}, (SELECT count(*)::text FROM ${esquema}.patients p2 WHERE ${MISMO_NOMBRE}) || ' fichas con este nombre' AS dato,
                 CASE WHEN p.client_id IS NULL THEN 'sin familia' END AS aviso`,
        from: `${esquema}.patients p LEFT JOIN ${esquema}.clients c ON c.id = p.client_id`,
        where: `EXISTS (SELECT 1 FROM ${esquema}.patients p2 WHERE p2.id <> p.id AND ${MISMO_NOMBRE})`,
        order: "lower(p.first_name || ' ' || p.last_name), p.created_at",
      };
 ...
 */
function cuerpoDe(esquema, carpeta, { conPacientes = true, conCitas = true, conCuotas = true } = {}) {
  const base = cuerpoBase(esquema, carpeta, { conPacientes, conCitas, conCuotas });
  if (!base) return null;
...
export async function filasDe(
  ...
  esquema,
  carpeta,
  { limite = 5000, conPacientes = true, conCitas = true, conCuotas = true, incluirBajas = false } = {}
) {
  const c = cuerpoDe(esquema, carpeta, { conPacientes, conCitas, conCuotas });
  if (!c) return [];
  // Una carpeta `incluyeBajas` (la lista de reservas de plaza) enseña siempre
  // a las archivadas, con su chip; en las demás manda la casilla.
  const conBajas = incluirBajas || !!CARPETAS.find((x) => x.key === carpeta)?.incluyeBajas;
  ...
  const [filas] = await sequelize.query(
    `SELECT ${c.select}, (${c.baja}) AS de_baja
       FROM ${c.from}
      WHERE ${c.where}${conBajas ? "" : ` AND ${c.sinBajas}`}
...
export async function cuentasDe(
  sequelize,
  esquema,
  { conPacientes, conCitas, conCuotas, conRevisiones = true, incluirBajas = false } = {}
) {
  ...
  const tablas = conPacientes == null || conCitas == null || conCuotas == null ? await tablasDe(sequelize, esquema) : null;
  const hayPac = conPacientes ?? tablas.conPacientes;
  const hayCitas = conCitas ?? tablas.conCitas;
  const hayCuotas = conCuotas ?? tablas.conCuotas;
  const porCarpeta = {};
  let bloquea = 0;
  let completar = 0;
  ...
  for (const c of CARPETAS) {
    ...
    if (c.requiere === "cuotas" && !hayCuotas) continue;
    const cuerpo = cuerpoDe(esquema, c.key, { conPacientes: hayPac, conCitas: hayCitas, conCuotas: hayCuotas });
    if (!cuerpo) continue;
    ...
    const sinBajas = incluirBajas || c.incluyeBajas ? "" : ` AND ${cuerpo.sinBajas}`;
...
async function tablasDe(sequelize, esquema) {
  ...
  const [filas] = await sequelize.query(
    `SELECT to_regclass(:pacientes) IS NOT NULL AS con_pacientes,
            ...
            to_regclass(:cuotas)    IS NOT NULL AS con_cuotas`,
    { replacements: { pacientes: `"${esquema}"."patients"`, citas: `"${esquema}"."bookings"`, cuotas: `"${esquema}"."billing_cuotas"` } }
  );
  return {
    conPacientes: Boolean(filas?.[0]?.con_pacientes),
    conCitas: Boolean(filas?.[0]?.con_citas),
    // Las carpetas de cuotas (06/09/2026) preguntan por `billing_cuotas`: sin
    // Facturación no existe y no se enseñan, ni a cero.
    conCuotas: Boolean(filas?.[0]?.con_cuotas),
  };
}

export async function carpetasCon(sequelize, esquema, DataReview, { incluirBajas = false } = {}) {
  const { conPacientes, conCitas, conCuotas } = await tablasDe(sequelize, esquema);
  ...
  for (const c of CARPETAS) {
    ...
    if (c.requiere === "cuotas" && !conCuotas) continue;

    // Una carpeta que no se pueda consultar (una columna que ese schema aún
    // no tiene) no puede tumbar la pantalla entera: se calla y se apunta.
    let filas;
    try {
      filas = (await filasDe(sequelize, esquema, c.key, { conPacientes, conCitas, conCuotas, incluirBajas }))
        .filter((f) => !revisadas.has(`${c.key}|${String(f.id).toLowerCase()}`));
    } catch (err) {
      process.stderr.write(`[urgentes] carpeta ${c.key} de ${esquema}: ${err.message}\n`);
      continue;
    }
