 ...
 */
export async function filasDe(sequelize, esquema, carpeta, { limite = 5000 } = {}) {
    ...
    case "sin_tutor_ni_contacto":
      return q(`
        SELECT c.id, c.name AS nombre, c.id AS client_id, NULL AS familia,
               (SELECT count(*)::text FROM ${esquema}.patients p WHERE p.client_id = c.id) || ' paciente(s)' AS dato
...
export async function carpetasCon(sequelize, esquema, DataReview) {
  ...
  for (const c of CARPETAS) {
    const filas = (await filasDe(sequelize, esquema, c.key))
