 ...
 */
function cuerpoDe(esquema, carpeta, { conPacientes = true } = {}) {
  ...
  const PROXIMA_CITA = `(SELECT min(b.scheduled_at) FROM ${esquema}.bookings b
                 WHERE b.patient_id = p.id AND b.scheduled_at > now())::date::text AS dato`;
  const TIENE_CITA_FUTURA = `EXISTS (SELECT 1 FROM ${esquema}.bookings b WHERE b.patient_id = p.id AND b.scheduled_at > now())`;
  ...
  switch (carpeta) {
    case "citas_sin_terapeuta":
      return {
        ...
        where: `p.main_therapist_id IS NULL AND ${TIENE_CITA_FUTURA}`,
    ...
    case "citas_sin_contacto":
      return {
        ...
        where: `${SQL_MUDA} AND ${TIENE_CITA_FUTURA}`,
        order: "dato NULLS LAST",
      };
    ...
    case "sin_contacto":
      return {
        ...
        select: `${SOLO_FAMILIA}, ${CUENTA_PACIENTES} AS dato`,
        from: `${esquema}.clients c`,
        where: `${SQL_MUDA} AND NOT (${SQL_SIN_TUTOR})`,
        order: "c.name",
      };
...
export async function filasDe(sequelize, esquema, carpeta, { limite = 5000, conPacientes = true } = {}) {
  const c = cuerpoDe(esquema, carpeta, { conPacientes });
  if (!c) return [];
  const [filas] = await sequelize.query(
    `SELECT ${c.select} FROM ${c.from} WHERE ${c.where} ORDER BY ${c.order} LIMIT ${limite}`
  );
  return filas;
}
 ...
 */
export async function cuentasDe(sequelize, esquema, { conPacientes, conRevisiones = true } = {}) {
  ...
  const hayPac = conPacientes ?? (await hayPacientes(sequelize, esquema));
  const porCarpeta = {};
  let bloquea = 0;
  let completar = 0;
  ...
  for (const c of CARPETAS) {
    ...
    const cuerpo = cuerpoDe(esquema, c.key, { conPacientes: hayPac });
    if (!cuerpo) continue;
    ...
    try {
      const [filas] = await sequelize.query(
        `SELECT count(*)::int AS n FROM ${cuerpo.from} WHERE ${cuerpo.where}${sinArchivar}`
      );
      n = filas[0]?.n ?? 0;
    } catch {
