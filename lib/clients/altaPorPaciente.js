 ...
 *
 * `pacienteEsElTitular` = han marcado que el paciente es mayor de edad y abre
 * su propia ficha (18/09/2026, AV-0201 de Aumenta). Entonces no hay familia a
 * la que preguntar y los textos dejan de hablar de padres: lo que falta por
 * teclear son los datos de contacto de esa misma persona.
 */
export function textosDelAlta({ porPaciente = false, singular = "cliente", pacienteEsElTitular = false } = {}) {
  if (!porPaciente) {
    return {
  ...
  if (pacienteEsElTitular) {
    return {
      titulo: "Nuevo paciente",
      boton: "Crear paciente",
      cabeceraFamilia: {
        titulo: "Sus datos de contacto",
        ayuda: "Es su propia ficha: el nombre ya está copiado de arriba. Falta su teléfono, su correo y su domicilio.",
      },
      sinNombreTitular: "Falta el nombre y los apellidos del paciente",
