...
/**
 * Módulos que PUEDEN asignarse solos al dar de alta un cliente. Ser candidato
 * no basta: hace falta además que el tenant tenga el módulo activo Y que lo
 * haya pedido con el flag de abajo.
 *
...
/**
 * El flag, en `featureFlags` de la fila del módulo en `master.tenant_modules`.
 *
 * ── POR QUÉ DEJÓ DE SER GLOBAL (13/08/2026) ────────────────────────────────
 * Esto se escribió para una consulta de nutrición de una persona, donde «todo
 * cliente nuevo es paciente» es verdad. Pero la regla estaba atada solo a TENER
 * el módulo, así que el día que Nutrición se vendiera a un centro grande —un
 * centro de psicología con 1.083 familias, por ejemplo— toda ficha nueva que
 * entrase por la puerta quedaría marcada como paciente de nutrición, incluidas
 * las que van a terapia y no pisan la consulta de dietas. Y sin nada que lo
 * dijera: se descubre semanas después, mirando por qué el buscador de nutrición
 * lista a medio centro.
 *
 * Es la misma lección que «Fichas a completar»: lo que resuelve el problema de
 * una consulta de una persona no es el default de un centro de veinte.
 *
 * Apagado por defecto, entonces, y encendido a mano para quien lo pida. Laura
 * lo tiene encendido desde la migración que introdujo el flag, para que su
 * comportamiento no cambiara.
 */
export const AUTO_ASSIGN_FLAG = "autoAsignarEnAlta";
...
/**
 * Marca los módulos de AUTO_ASSIGN_MODULE_KEYS en un cliente recién creado, en
 * los tenants que lo hayan pedido (ver AUTO_ASSIGN_FLAG).
 *
 ...
 *
 * `hasFeatureFlag` viene del mismo contexto de tenant. Si no llega —una ruta
 * que aún no lo pase— NO se marca nada: ante la duda, no tocar la ficha de
 * nadie. Marcar de menos se ve y se arregla con un clic; marcar de más son
 * mil fichas mal etiquetadas que nadie mira hasta que molestan.
 */
export async function applyAutoAssignments({
  ...
  if (typeof hasFeatureFlag !== "function") return [];

  const marcados = [];
  for (const moduleKey of AUTO_ASSIGN_MODULE_KEYS) {
    if (!tenantHasModule(moduleKey)) continue;
    if (!hasFeatureFlag(moduleKey, AUTO_ASSIGN_FLAG)) continue;
    try {
