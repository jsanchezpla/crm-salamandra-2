...
/**
 * «No vino» YA NO SE OFRECE (15/09/2026, Rodrigo: «no tiene mucho sentido,
 * déjalo en Baja; son fichas con baja de larga duración»). En Aumenta no
 * quedaba ni una. El valor sigue en el ENUM —la tienda marca `prospect` a
 * quien compra una vez, y esas fichas no enseñan este estado— y una ficha de
 * salud que lo tuviera de antes se LEE como Baja (`etiquetaDeEstado`,
 * `tonoDeEstado`, `dejaDeReclamar`). No se reescribe ninguna fila.
 */
export const NO_VINO = "prospect";
export const BAJA = "inactive";
/**
 * En pausa (15/09/2026, Rodrigo): vino el curso pasado y en este todavía no.
 * La regla que pone Activo / En pausa / Baja por lo que ha pasado con cada
 * familia vive en `estadoPorActividad.js`.
 */
export const PAUSA = "paused";

/** Los tres valores que se ofrecen, en el orden en que se enseñan. */
export const ESTADOS_FICHA = [ACTIVO, PAUSA, BAJA];
...
const CATALOGO = [
  ...
  },
  {
    ...
    label: "En pausa",
    ayuda:
      "Vino el curso pasado y en este todavía no. Vuelve a Activo sola en cuanto entra en una cuota o paga un cobro.",
    dot: "bg-sky-400",
  ...
  },
  {
    ...
    label: "Baja",
    ayuda:
      "No viene desde hace más de un curso, o nunca llegó a empezar. Se conserva entera, con sus facturas y cobros; vuelve a Activo sola con una cita, una cuota o un cobro.",
    dot: "bg-neutral-400",
...
/** El «No vino» de antes se lee como Baja (ver `NO_VINO`). */
const comoSeLee = (estado) => (estado === NO_VINO ? BAJA : estado);

/** Lo que se lee en el chip. Un valor desconocido se dice tal cual, no se traga. */
export function etiquetaDeEstado(estado) {
  return CATALOGO.find((e) => e.key === comoSeLee(estado))?.label ?? String(estado ?? "");
}

/** El color del chip. Sin estado conocido, el de «Activo», que es el defecto de la columna. */
export function tonoDeEstado(estado) {
  const e = CATALOGO.find((x) => x.key === comoSeLee(estado)) ?? CATALOGO[0];
  return { dot: e.dot, bg: e.bg };
}

/**
 * Qué valores de la columna trae la pestaña de un estado. «Baja» trae también
 * el «No vino» de antes, que se lee igual (ver `NO_VINO`).
 */
export function valoresDelFiltroDeEstado(estado) {
  const e = String(estado ?? "").trim();
  return e === BAJA ? [BAJA, NO_VINO] : [e];
}
...
export function dejaDeReclamar(estado) {
  // En pausa tampoco (15/09/2026): el paciente en pausa ya no reclamaba, y la
  // familia en pausa es la misma gente.
  return estado === NO_VINO || estado === BAJA || estado === PAUSA;
}
