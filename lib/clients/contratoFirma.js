...
import { leerDeFicha, campoEsObligatorio, destinoDe } from "./datosFicha.js";
...
/**
 * Edad máxima que se considera una vida humana. Fuera de 0..120 no hay una
 * edad: hay una fecha mal tecleada. Es el mismo tope que usa `edadDesde` en
 * `formularioAlta.js`, y se repite aquí a propósito —aquel cuenta contra el
 * reloj de ahora y este contra una referencia que le dan— en vez de importarlo.
 */
...
export function edadEn(fechaNacimiento, referencia = new Date()) {
  ...
  /**
   * Una edad NEGATIVA no es una edad (21/08/2026).
   *
   * Una fecha de nacimiento en el futuro —un dedo en el año al teclearla en el
   * portal, 19→20— daba años en negativo, y `esMenor` los leía como «menor de
   * 18»: a una adulta se le pedía el consentimiento de su tutor y se quedaba
   * sin poder terminar de firmar. Justo lo que el comentario de `esMenor` dice
   * que quiere evitar.
   *
   * Fuera del rango se devuelve `null` = «no lo sé», que es lo que ya se hacía
   * con una fecha ilegible y lo que `esMenor` —su ÚNICO lector— cuenta como
   * MAYOR: el lado seguro. La otra decisión que depende de la edad, si el DNI
   * es obligatorio, no pasa por aquí: `campoEsObligatorio` llama a `edadDesde`,
   * que ya acotaba 0..120 por su cuenta y ante «no lo sé» pide el DNI. Los dos
   * lados seguros, pero por dos caminos: tocar este no arregla aquel.
   */
  return edad >= 0 && edad <= EDAD_MAXIMA ? edad : null;
}
 ...
 *
 * Una fecha FUTURA entra por esa misma puerta desde el 21/08/2026: `edadEn` la
 * devuelve como `null`, no como una edad negativa (ver allí).
 */
...
export function bloquesDe(plantilla) {
  ...
  return raw
    ...
    .map((b) => ({
      ...
      body: texto(b.body),
      acceptLabel:
        texto(b.acceptLabel) || `He leído y acepto ${texto(b.title) || "este documento"}.`,
...
/**
 * ¿Este campo de la plantilla es una fecha de NACIMIENTO?
 *
 * Por su destino en la ficha (`cliente.birthDate`, `tutor.birthDate`) o por su
 * clave, que es exactamente lo que ya mira `nacimientoDeclarado` para decidir
 * quién es menor: si esas claves valen para tomar la decisión, valen para
 * validar el dato. Un campo `date` sin ninguna de las dos cosas —la fecha de la
 * firma— no es un nacimiento y no se juzga.
 */
function esFechaDeNacimiento(campo) {
  const d = destinoDe(campo);
  if (d && !d.custom && d.clave === "birthDate") return true;
  return CLAVES_NACIMIENTO.includes(texto(campo?.key));
}

/** El día de hoy en fecha UTC (YYYY-MM-DD), para comparar con lo tecleado. */
function hoyUTC(momento) {
  const m = momento instanceof Date && !Number.isNaN(momento.getTime()) ? momento : new Date();
  return m.toISOString().slice(0, 10);
}

function validarUno(campo, valor, fechaNacimiento = null, hoy = hoyUTC(), enFicha = "") {
  ...
  switch (campo.type) {
    ...
    case "date": {
      ...
      // `new Date` corrige en silencio los días que no existen («2026-02-31» es
      // el 3 de marzo, no NaN) y la fecha llegaba hasta Postgres, que la tumbaba
      // con un 500 en vez de este error. Se vuelve a formatear y se compara,
      // como `fechaONull` en formularioAlta.js (19/08/2026, lo sacó la prueba
      // _smoke-clients-contrato-firma.mjs).
      if (Number.isNaN(d.getTime()) || d.toISOString().slice(0, 10) !== v) {
        return { error: `«${campo.label}» no es una fecha real` };
      }
      /**
       * Nadie ha nacido mañana (21/08/2026). Un año tecleado de más («2086» por
       * «1986») pasaba el filtro de arriba —el día existe— y se guardaba en
       * `signerData` y en la ficha, donde la nutricionista ve una paciente
       * nacida dentro de 60 años. `edadEn` ya no la cuenta como menor, pero
       * eso tapa la consecuencia; esto es la puerta, y aquí se puede DECIR lo
       * que pasa en vez de tragar el dato en silencio.
       *
       * Solo las fechas de nacimiento: la de la firma se compara igual contra
       * hoy y un desfase de reloj entre navegador y servidor la dejaría fuera
       * por unas horas, que es peor que el fallo que arregla.
       *
       * Y solo lo que se está DECLARANDO ahora, no lo que ya está en la ficha
       * (`v !== enFicha`, 21/08/2026). El endpoint de firma vuelve a meter por
       * aquí los datos de la ficha —`{ ...body.datos, ...datosDeFicha(...) }`,
       * para que el DNI del contrato sea el del CRM y no el que teclee el
       * navegador—, así que una ficha con la fecha futura YA guardada (las que
       * dejó este mismo fallo) se quedaba sin poder firmar NADA: 422 en el
       * primer documento, nombrando un campo que el portal ni le enseña
       * (`camposQueFaltan` lo da por lleno) ni la dejaría sobrescribir
       * (`actualizacionDeFicha` solo tapa huecos). Eso no lo arregla quien
       * firma: lo arregla el centro en la ficha. Lo que sí se rechaza es lo que
       * se teclea, que es lo único que llegaría a escribirse.
       */
      if (v !== enFicha && v > hoy && esFechaDeNacimiento(campo)) {
        return { error: `«${campo.label}» no puede ser una fecha futura` };
      }
      return { valor: v };
    }
    ...
    case "dni": {
      ...
      if (ok === false)
        return { error: `La letra de «${campo.label}» no corresponde. Revísalo, por favor.` };
 ...
 *
 * `momento` es el «hoy» contra el que se mide si una fecha de nacimiento está
 * en el futuro. Solo lo pasan las pruebas; los tres endpoints que llaman aquí
 * lo omiten y vale el reloj.
 */
export function validarDatos(plantilla, entrada, client = null, momento = new Date()) {
  const campos = camposDe(plantilla);
  const fuente = entrada && typeof entrada === "object" ? entrada : {};
  const hoy = hoyUTC(momento);
  const datos = {};
  ...
  for (const campo of campos) {
    // Lo que la ficha YA tiene viaja también: hay comprobaciones que solo
    // valen para lo que se declara ahora (ver la fecha futura en `validarUno`).
    const { valor, error } = validarUno(
      campo,
      fuente[campo.key],
      fechaNacimiento,
      hoy,
      leerDeFicha(client, campo)
    );
 ...
 * Se busca por `CLAVES_NACIMIENTO` (arriba, junto a `validarUno`, que valida
 * esos mismos campos).
 */
...
export function situacionDocumentos({ plantillas, firmas, firmantes, firmante, client = null }) {
  ...
  const suyas = (id) =>
    new Set(
      (firmas ?? [])
        .filter(
          (s) => String(s.guardianId ?? s.guardian_id).toLowerCase() === String(id).toLowerCase()
        )
  ...
  const misFirmas = firmante
    ? (firmas ?? []).filter(
        (f) =>
          String(f.guardianId ?? f.guardian_id).toLowerCase() === String(firmante.id).toLowerCase()
      )
    : [];
...
export function validarAceptaciones(plantilla, entrada, momento = new Date()) {
  ...
  const marcados = new Set(
    Array.isArray(entrada)
      ? entrada.map((x) => texto(x))
      : Object.keys(entrada ?? {}).filter((k) => entrada[k])
  );
