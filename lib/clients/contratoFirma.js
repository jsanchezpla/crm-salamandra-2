...
export function serializarPlantilla(plantilla) {
...
function validarUno(campo, valor) {
 ...
 */
export function validarDatos(plantilla, entrada) {
  const campos = camposDe(plantilla);
  const fuente = entrada && typeof entrada === "object" ? entrada : {};
  const datos = {};

  for (const campo of campos) {
    const { valor, error } = validarUno(campo, fuente[campo.key]);
 ...
 *
 * El consentimiento parental (`onlyMinors`) solo sale si lo declarado dice que
 * la destinataria es menor. Mientras no haya firmado el contrato principal no
 * lo sabemos, así que no se pide: es el segundo paso, no el primero.
 */
export function documentosQueAplican(plantillas, firmas) {
  const nacimiento = nacimientoDeclarado(firmas);
 ...
 */
export function situacionDocumentos({ plantillas, firmas, firmantes, firmante }) {
  const aplican = documentosQueAplican(plantillas, firmas);
  const suyas = (id) =>
    new Set(
      (firmas ?? [])
