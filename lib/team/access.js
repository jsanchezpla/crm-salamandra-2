...
/**
 * Roles que este flujo puede EDITAR después (módulos, contraseña, borrado).
 *
 * ⚠️ Los admin siguen fuera, y eso NO cambió el 27/08/2026 cuando se permitió
 * CREARLOS: son dos permisos distintos a propósito. Crear una cuenta de
 * dirección es un alta que el centro decide; poder entrar en la cuenta de OTRA
 * directora —cambiarle la contraseña desde una pantalla de RRHH y entrar como
 * ella— es un secuestro, y esa puerta se queda cerrada. Quien pierda su
 * contraseña la recupera por su correo (`lib/auth/recuperacion.js`), que es
 * justo lo que quita la necesidad de esa puerta.
 * (12/09/2026: el calendario global entra como admin de un cliente por OTRA puerta, auditada y sin contraseñas: docs/decisions/2026-09-12-el-calendario-global-ve-todos-y-los-proyectos.md.)
 */
export const MANAGEABLE_ROLES = new Set(["user", "manager"]);

/**
 * Roles que se pueden CREAR desde Equipo (27/08/2026, Rodrigo).
 *
 * Hasta hoy una cuenta de dirección solo nacía por SSH: el alta de las dos
 * administradoras de Aumenta (26/08) fue un script contra producción, con las
 * contraseñas en un fichero de /root y la entrega en persona. Con 11 clientes
 * de UN solo admin, el caso se repite; y cada vez nos convierte en el
 * departamento de informática de su centro.
 *
 * `superadmin` no está y no va a estar: es el rol del back-office, con alcance
 * sobre TODOS los clientes. Eso no lo reparte un cliente desde su Equipo.
 */
export const ROLES_CREABLES = new Set(["user", "manager", "admin"]);
...
/**
 * Qué módulos le tocan a una cuenta según su rol.
 *
 * Un administrador nace con `["all"]`, el mismo valor que le pone el alta de
 * cliente del back-office (`lib/provisioning/altaTenant.js`): dirige el centro,
 * así que marcarle módulos uno a uno no significa nada — y dejarle alguno sin
 * marcar sería una trampa silenciosa el día que lo necesite. Para los demás
 * manda lo que haya marcado quien da el alta.
 */
export function modulosSegunRol(rol, modulosPedidos) {
  return rol === "admin" ? ["all"] : modulosPedidos;
}
...
/**
 * Quita el login del CRM cuando un empleado pasa a INACTIVO.
 *
 * (Añadido 2026-07-28 tras la auditoría del sprint.) Antes esto lo decidía el
 * NAVEGADOR: la pantalla de Equipo miraba `member.userId` de su copia en
 * memoria y solo entonces llamaba al DELETE de /access. Dos caminos dejaban al
 * empleado dado de baja pero con el login VIVO y sus módulos intactos:
 *   1. crear el usuario y desactivar sin recargar (el padre no se enteraba del
 *      alta, así que `userId` seguía vacío);
 *   2. Editar → Estado → «Inactivo» → Guardar, que nunca revocó nada.
 * Dar de baja a alguien tiene que cerrarle la puerta desde el SERVIDOR.
 *
 * Respeta las mismas guardas que el borrado manual: no toca cuentas
 * admin/superadmin (se gestionan aparte) ni la del propio solicitante. Si no se
 * puede revocar, se devuelve el motivo para que el handler lo cuente en vez de
 * fallar en silencio.
 *
 * @returns {{revocado: boolean, username?: string, motivo?: string}}
 */
export async function revocarAccesoPorBaja({ member, tenantId, requesterUserId }) {
  if (!member?.userId) return { revocado: false };

  const managed = await loadManagedUser({ userId: member.userId, tenantId, requesterUserId });
  if (managed.error) {
    // El enlace roto (usuario ya borrado a mano) se limpia igualmente.
    if (managed.status === 404) {
      try { await member.update({ userId: null }); } catch { /* da igual: ya no hay login */ }
      return { revocado: false, motivo: "enlace roto" };
    }
    return { revocado: false, motivo: managed.error };
  }

  const { user } = managed;
  const username = user.email;
  // Mismo orden que el DELETE manual: primero se suelta el enlace en el schema
  // del tenant y después se borra el login en master. Si lo segundo fallase,
  // queda un User suelto pero la ficha ya no apunta a él.
  await member.update({ userId: null });
  await user.destroy();
  return { revocado: true, username };
}
