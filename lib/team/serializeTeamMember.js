...
export function serializeTeamMember(member, { isAdmin = false } = {}) {
  ...
  const result = {
    ...
    notes: m.notes ?? null,
    /*
     * Con lo que firma un informe clínico (28/08/2026). Los dos pueden faltar
     * —hoy no los tiene nadie— y faltar tiene que ser UNA sola cosa: `null`.
     *
     * Por eso `|| null` y no `?? null` como sus vecinos: una cadena vacía que se
     * hubiera colado en la base (una fila vieja, una importación) llegaría al
     * generador del PDF como un dato puesto, y saldría una línea en blanco justo
     * debajo de una firma profesional. Ausente, null y "" son el mismo hueco.
     */
    collegiateNumber: m.collegiateNumber || null,
...
export const CAMPOS_FUERA_DE_LA_LISTA = [
  ...
  "startDate",
  /*
   * Nº de colegiada y titulación (28/08/2026). Fuera de la lista recortada a
   * propósito, aunque no sean dinero: un desplegable de profesionales elige a
   * una persona por su nombre y su color, y estos dos no ayudan a elegir a
   * nadie. Quien los necesita de verdad es el PDF del informe, que se genera en
   * el servidor leyendo la ficha entera, no esta lista.
   *
   * Regla de la casa: en el recorte entra lo que hace falta para PINTAR y
   * ELEGIR; todo lo demás pasa por la pantalla de Equipo, que pide el módulo en
   * los accesos.
   */
  "collegiateNumber",
  "qualification",
