# Módulo Banco (`banco`) — el extracto real y la conciliación
...
| | |
| --- | --- |
| **moduleKey** | `banco` · requiere `billing` (`lib/provisioning/dependencias.js`: la conciliación casa movimientos con `payments`/`costs` y la pantalla vive en la pestaña de Facturación). |
...
| **Endpoints** | `app/api/banco/**` (9, todos `hasModule("banco")`): `estado` (GET), `bancos` (GET, lista de entidades), `conectar` (POST, admin, crea acuerdo+requisición y devuelve el enlace AL banco), `confirmar` (POST, admin, la vuelta con `?ref=`; comprueba que la referencia empiece por `<slug>-`), `sincronizar` (POST, freno de 15 min por cuenta), `movimientos` (GET, con `?estado=`, `?q=`, `?id=`), `sugerencias` (GET), `casar`/`descasar` (POST), `cuentas/[id]` (DELETE, admin). `conectar`/`confirmar`/`cuentas` rechazan las demos (`isDemoTenant`): son públicas y esto toca un banco real. |
...
| **Scripts** | `migrate-banco-conciliacion.js` (CORE en `_module-migrations.js`: columnas de `payments`/`costs` + tablas del banco en todos los schemas; **VA ANTES del despliegue**, los modelos piden las columnas por nombre). Activación: `node scripts/enable-module.js <slug> banco`. |
