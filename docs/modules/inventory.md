...
| | |
| --- | --- |
| **moduleKey** | `inventory` · **cuelga de Productos desde el 03/09/2026**: requiere `productos` + `productos_avanzado` (`lib/provisioning/catalogo.js`, grupo «Productos»; `lib/provisioning/dependencias.js`: su lista es el endpoint del catálogo y su entrada de menú exige el avanzado; sin `team` las entradas y ajustes no quedan firmados, sin `orders` el stock solo baja por ajuste manual). Es Pedidos quien descuenta al vender; Facturación solo avisa. Ver `docs/modules/productos.md`. |
| **Reina** | — (ni el doc ni el código nombran una; en producción ningún cliente real tiene todavía un producto dado de alta) |
| **Pantallas** | `app/(dashboard)/inventario/page.jsx` (`/inventario`: lista con stock y unidad, filtro por categoría y bajo mínimo, ficha con el histórico de movimientos, alta de entrada con proveedor de desplegable, ajuste con motivo). Toda la pantalla vive en esa página. **Desde el 03/09/2026 NO da de alta ni edita productos**: eso es de `/productos` (nombre, unidad, precios, mínimo); aquí solo se mueve stock. |
...
| **Endpoints** | `app/api/inventory/**` — 5 `route.js`. `entries`, `stock-movements` y `products/[id]/variantes` con `hasModule("inventory")`; **`products` y `products/[id]` con `hasModule("productos")`** desde el 03/09/2026 (la ruta no se movió: el catálogo es del módulo Productos y esta pantalla lo lee). |
