...
la ficha (`clients.cuota_concept_ids`). Con paciente elegido gana la cuota DE
ESE paciente — dos hermanos pagan cosas distintas.

Solo se rellena cuando las líneas siguen EN BLANCO (una sola, sin texto y a
cero): pisar lo que alguien acaba de escribir sería peor que no rellenar nada.
Para lo demás está el botón «Poner su cuota (n)», que dice de dónde salen los
conceptos.
