...
| Método y ruta | Propósito | Restricciones |
| --- | --- | --- |
| `GET /morosidad?mes=AAAA-MM` | Quién no ha pagado el mes, **una fila por paciente activo** (desde el 15/09/2026; antes por familia, y un hermano pagado dejaba al otro al día) con su familia al lado, y cuántos meses seguidos lleva. Sus cuotas son las que llevan su `patientId` más las de la familia sin paciente; sus cobros, los que llevan su `patientId` más los de la familia (o de quien paga sus cuotas) sin paciente, que cuentan para todos los hermanos y marcan la fila `compartido` (`lib/billing/morosidad.js`). El bloqueo del portal sigue siendo por familia. Lo pinta `/facturacion/cobros`. | `422` si el mes no es `AAAA-MM`. |
