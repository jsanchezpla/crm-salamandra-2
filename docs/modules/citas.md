...
| **Endpoints** | `app/api/citas/**` — 24 `route.js`: `event-types` (su GET trae el `concepto` de cada tipo cuando el centro factura, para que el alta enseñe la cuota sin pedirle nada a Facturación), `cobro-del-mes` (04/09/2026: qué hay que cobrarle a una familia ese mes según SUS citas; solo dirección. Con `&booking=<id>` devuelve además `deLaCita: { conceptId, fuente }` —qué cuota cubre ESA cita, la suya o, si nació sin ella, la de su tipo—: es lo que hace que «Cobrar mes» desde una cita de DIAGNÓSTICO rellene los 650 € y no la cuota mensual de la familia, 10/09/2026), `availability` (+`bulk`), `bookings` (+`[id]`, `confirm`, `reject`, `pedir-tarjeta`, `reschedule-request`, `suggest-slots`, `calendar`; **12/09/2026**: el POST acepta `diagnosticoId` + `diagnosticoTramo` (`entrevista`|`horas`) y, SOLO con ellos, `duration` en múltiplos de 30 entre 30 y 480 —la cita de un DIAGNÓSTICO: se comprueba que es del paciente del expediente y que cabe en sus horas, y el dinero lo pone el servidor; con `diagnosticoId` se ignoran `packId` y `cobro` del cuerpo; ver «La cita de un diagnóstico»—, y el GET de `[id]` añade `diagnostico: { id, status, estado, productoNombre, horas, rotulo }` o null), `reschedule-requests`, `packs`, `blocked-days`, `bloqueos`, `avisos`, `clientes`, `sin-profesional`, `informe-ocupacion`, `bloqueos/[id]/documents` (+`[docId]`, `[docId]/download` — 01/09/2026: lo que cuelga de un tramo de la agenda; el documento va al archivo central con `source='bloqueo'`), `bloqueos/[id]/acta` (GET lee el acta y con qué apartados se escribe · PUT la guarda, y **guardarla en blanco la BORRA**; solo en bloqueos `reunion_equipo` y con un 409 —no un 404— si el tramo no es una reunión: el bloqueo existe, lo que no procede es el acta) y `bloqueos/[id]/acta/redactar` (POST multipart `file` y/o `texto`: audio → Whisper → Claude → acta propuesta por los apartados del centro. **NO guarda nada**, propone; confirma una persona con el PUT, igual que en el registro de sesión. Gate `citas` y no `clinica` — un acta de equipo no tiene paciente. 01/09/2026. Si el reparto con IA falla, devuelve la transcripción con el MOTIVO dentro de `avisoIA` y `avisoTipo: "fallo"`, como el registro de sesión, y la cuota de Whisper responde con su frase legible —sin saldo ≠ límite—, 13/09/2026; hoy ninguna pantalla consume este endpoint, así que es contrato) · Configuración → Citas se guarda en `app/api/tenant/settings/route.js`. **Públicos** (`app/api/public/c/[tenantSlug]/`, 21 de citas): agenda anónima `info`, `event-types`, `availability` (+`month`), `book`, `booking/[token]`, `cancel/[token]`, `pagar/[token]`; portal con sesión SSO bajo `citas-portal/` (13: `session`, `bookings`, `cancel/[id]`, `admision`, `avisos`, `comunicaciones`, `consentimiento-imagen`, `mis-datos`, `documents` (+`[id]`), `contract` (+`sign`, `documento`)). `formularios/[formSlug]`, `registro-web` y `soporte/*` NO son de citas. **Webhook**: `app/api/webhooks/stripe/[tenantSlug]/route.js` (cobros y devoluciones; `lib/payments/entityHooks.js` → `citaPagada`) |
| **Lógica** | `lib/citas/` (78 ficheros, contados el 13/09/2026). Huecos: `booking.js` (qué cita OCUPA su hueco, única fuente), `slots.js`, `ausencias.js`, `horarioProfesional.js`, `festivos.js`, `validation.js`. Puertas de la agenda pública: `puertaIdentidad.js`, `puertaFormulario.js` (+`avisoAdmisionRota.js`), `puertaContrato.js`, `puertaValoracion.js`, `puertaReserva.js`, `tiposVisibles.js`, `quienPregunta.js`. Dinero: `cobroCita.js`, `reembolsoCita.js`, `caducidadRetencion.js`, `packs.js` (bonos; desde el 12/09/2026 `estadoPack` entiende `totalSessions` a null = bono SIN TOPE, el de un diagnóstico: nunca se agota, `total`/`restantes` null, `sinTope: true` y resumen «N usadas · sin tope»), `gastaSesion.js` (**12/09/2026**: la regla de qué cita GASTA su sesión y cuál la tiene RESERVADA, sacada de `packs.js` sin ORM porque la barra de horas de un diagnóstico la pinta también el navegador; `packs.js` la re-exporta y nadie notó el traslado), `altaDesdeDiagnostico.js` (**12/09/2026: el CONTRATO entre Diagnósticos y la agenda** — `urlDeAltaDesdeDiagnostico` escribe `/citas?nueva=1&diagnostico=<id>&tramo=entrevista|horas&paciente=<id>&duracion=60` y `leerAltaDesdeDiagnostico` la lee en `CitasModule`; `TRAMO_ENTREVISTA`/`TRAMO_HORAS` son lo que guarda `bookings.diagnostico_tramo` y `duracionLimpia` los minutos válidos; prueba `_smoke-alta-desde-diagnostico.mjs`. Lo que el servidor hace con la cita vive en `lib/clinica/citaDeDiagnostico.js`), `politicaReembolso.js`, `dinero.js`, `tokenPago.js`, `consentimientoRetencion.js` (con `lib/payments/`: `autorizacion.js`, `fraccionado.js`, `entityHooks.js`). Portal: `ssoToken.js`, `portalSession.js`, `portalRateLimit.js`, `portalClient.js`, `portalContract.js`, `portalDocumentos.js`, `portalMeses.js`, `bienvenida.js`. Avisos: `recordatorios.js`, `notificarCancelacion.js`, `avisosWhatsapp.js` (+`lib/whatsapp/whatsappConfig.js`), plantillas en `lib/email/templates/citas/` (10). Otros: `visibilidad.js` (agenda compartida), `cancelBooking.js`/`cancelacion.js`, `rescheduleRequests.js`, `suggestSlots.js` (IA), `videollamada.js`, `preguntasCita.js`, `valoracionInicial.js`, `coloresBloqueo.js`, `categoriasBloqueo.js` (**01/09/2026: DE QUÉ es un bloqueo** — la lista del centro en `settings.citas.categoriasBloqueo`, su color y la clave que guarda cada bloqueo; nace vacía y el color de la categoría MANDA sobre el de la persona, que es lo que se pidió: «que a todo el equipo le salga igual»), `googleCalendar.js`, `audit.js` (con red desde el 13/09/2026: con `entity "Booking"` quita lo privado aunque se lo pasen) + `resumenDeCita.js` (**13/09/2026: lo ÚNICO de una cita que llega a `master.audit_logs`** — `resumenDeCita` (lista blanca: ids, hora, estado y dinero), `cambiosDeCita` (qué cambió al editar; lo privado solo por NOMBRE en `cambiadosSinValor`), `fijosDeCita` (lo que una edición lleva SIEMPRE en los dos lados, cambie o no: hora, duración, profesional, estado, tipo y paciente, con null explícito; lo leen las comprobaciones sobre movimientos), `huellaDeCita` (la del borrado, por `clientId`/`patientId`/`tallerGrupoId`, sin nombre; `borrarCita.js` la re-exporta), `sinDatosPrivadosDeCita` y `ladosParaAuditar` (la red); prueba `_smoke-citas-auditoria.mjs`), `pegarCita.js` (dónde cae una cita cortada/copiada del menú contextual: con hora se pega ahí, solo-fecha conserva la hora original — 31/08/2026), `fichaDeLaCita.js` (a qué ficha salta una cita; lo usan el modal Y el menú contextual), `recurrencia.js` (las fechas de una cita que se repite semanal/quincenal/mensual hasta un día: citas INDEPENDIENTES sin concepto de serie, aritmética de pared, mensual salta los meses sin ese día, tope 60; el drawer las crea una a una por el POST de siempre — la que choca no se crea y se avisa — y con `omitirCorreo` solo la primera manda correo — 31/08/2026; desde el 09/09/2026 también los BLOQUEOS, con `repeticionDeBloqueo` — misma regla, salvo que un bloqueo tiene fin y el fin se desplaza los mismos días que el inicio), `recuperacionFalta.js` (la falta justificada ES la recuperable: el rótulo con esa palabra delante y qué citas pueden recuperarla — mismo cliente, vivas, posteriores o hasta `DIAS_ANTES_RECUPERACION` (7) días antes, por cercanía (15/09/2026, AV-0139: la sesión se ADELANTA cuando avisan con tiempo; el modal las pide con `?from=…&orden=asc` porque en DESC con `limit=100` llegaban las más lejanas de la serie); el enlace vive en `bookings.recovered_by_booking_id` sin FK dura y lo valida el PATCH — 31/08/2026), `asistencia.js` (**01/09/2026: una cita confirmada que ya TERMINÓ se da por asistida** aunque nadie la marcara — `estadoEfectivo`, `esPresunta`, `cuentaComoAtendida`. Es una presunción AL LEER: no escribe en `bookings.status`, no hay migración ni tarea nocturna, y marcar «No asistió» sigue funcionando igual el día 400. Solo toca a `confirmed`: una `pending` es una petición que el centro nunca aceptó)., `resultadoCita.js` (**01/09/2026: CÓMO ACABÓ una cita**, en un solo sitio para las dos pantallas que lo ponen —la ficha de la cita y las citas de la ficha del paciente—: los cuatro resultados (completada · falta justificada · falta injustificada · cancelada), qué se pregunta antes de cada uno y **el cuerpo del PATCH**, que es lo que garantiza que una falta nunca viaje sin `noShowJustified` —el endpoint lo lee como `=== true`, así que callarlo es decir «sin justificar»—), `incidenciaPorFalta.js` (**01/09/2026: marcar una falta ABRE UNA INCIDENCIA** y avisa a quien lleve la administración — **desde el 18/09/2026 avisa pero NO se la asigna** (AV-0197): nace sin responsable y se queda en la pestaña «Faltas», porque asignárselas sola llenaba un tercio de la bandeja de administración con trabajo que nadie se había repartido. Las DOS faltas la abren —la justificada hay que recuperarla y la otra, reclamarla—, con prioridad media y alta; entra por Administrativa · Citas. A quién se le manda es un DATO del cliente, `settings.citas.incidenciaPorFalta` (Configuración → Módulos), **nunca una persona a fuego**; lista vacía = apagado, que es como nace cualquier centro. Best-effort: si falla, la falta se marca igual), `cambioDeTipo.js` (**03/09/2026: CAMBIAR DE TIPO una cita que ya existe**, desde su ficha y solo dirección — `puedeCambiarTipo` es la valla del PATCH (`eventTypeId`) y `tiposParaCambiar` la lista del desplegable. Nunca desde o hacia un taller (es otra estructura), nunca sobre la sesión de un bono (el bono es de su tipo), y la duración NO se toca: es de la cita, así que no hay solape nuevo. Nació de entrevistas iniciales de Aumenta apuntadas con el tipo de la cuota), `dineroDeLaCita.js` (**04/09/2026, Aumenta por Rodrigo: toda cita nace atada a un dinero** — los tres modos (`cuota` del catálogo · `libre` de texto · `sin_coste` CON motivo), de dónde baja por defecto (el concepto del TIPO), y `loQueSeCobraDe`, que arma el cobro del mes desde las citas. Ojo a su aritmética: una cuota es MENSUAL y no se multiplica por las citas del mes —ocho sesiones de logopedia son 190 €, no 1.520— y un cobro suelto sí se suma. Lo exige `settings.citas.cobroObligatorio`, apagado de fábrica), `cobrarMes.js` (**03/09/2026: el botón «COBRAR MES» de la ficha de la cita**, Aumenta por Rodrigo. Solo sale con módulo de Facturación (`enabledModules` de `/api/auth/me`, que ya cruza centro y acceso de la persona) y con la familia enlazada; lleva a `/facturacion/cobros?abrir=cuota&cliente=…&paciente=…&mes=AAAA-MM` —el «Registrar cobro» se abre relleno con familia, paciente y el MES DE LA CITA— en pestaña nueva. **Se apaga en cuanto la familia tiene un cobro COMPLETADO con `periodMonth` de ese mes** (`GET /api/billing/payments/mes`), lo apuntara la cita o alguien a mano desde Cobros, y vuelve solo con la primera cita del mes siguiente. Se mira en COBROS, nunca en facturas: Aumenta cobra primero y factura al cierre. Un cobro de UN hijo no apaga el botón en las citas de su hermano; uno de toda la familia, sí. El modal vuelve a preguntar al volver a la pestaña. El «Cobrar» del menú contextual pasa a ser el mismo enlace, con paciente y mes, y se desactiva sin el módulo) **Fuera de `lib/citas/`**: `lib/reuniones/acta.js` + `redactarActa.js` (01/09/2026, EL ACTA de una reunión de equipo — la escribe el CRM del audio o de las notas, como un registro de sesión. `puedeTenerActa` (solo `category_key = 'reunion_equipo'`), `bloquesDelActa` (los apartados del centro + las notas internas SIEMPRE al final y marcadas `interno`), el prompt propio y `limpiarActa`/`actaVacia`. **Sin tabla nueva**: un acta es de UNA reunión y la reunión ya es una fila, así que son tres columnas de `team_blocks`; `TallerSesion` sí necesitó tabla porque aquella se COPIA a la ficha de cada asistente y un acta no baja a ninguna ficha) |
| **UI** | `modules/default/CitasModule.jsx` (869 líneas: el armazón — calendario FullCalendar, pestañas, filtros) con sus piezas en `modules/default/citas/` (`CitaDetalleModal` —desde el 03/09/2026 con la fila «Tipo», solo para dirección: un desplegable con el catálogo sin talleres que cambia el tipo por el PATCH de siempre—, `NuevaCitaDrawer`, `Waitlist`, `chips`, `CitaMenuContextual` —clic derecho en la cita: información del paciente, cortar, copiar y cobrar (salta a Cobros con el drawer de cuota abierto y el cliente puesto), 31/08/2026—, `BloqueoModal` —pulsar un bloqueo en el calendario: concepto, fecha y duración por el PATCH de bloqueos, que pone las vallas; arrastrarlo lo reprograma igual que una cita, 31/08/2026; y desde el 01/09/2026 sus DOCUMENTOS: subir el acta de la reunión eligiendo a la vez a quién se le pide leerla (`components/documents/LectoresPicker.jsx`)—; partido el 27/08/2026, tenía 2.672) · `components/citas/` (`BuscadorPaciente`, `ModalFestivos`, `PanelVacaciones`, `CitasDelPaciente` —01/09/2026: las citas de un paciente CON su resultado y los cuatro botones para cambiarlo; la usa la ficha del paciente y habla por `lib/citas/resultadoCita.js`, no por su propio `fetch`—) · en la ficha de cliente, `components/clients/` (`ClientCitasSection`, `ClientBookingsPanel`, `ClientBonosSection`) · `components/team/TeamHoursEditor.jsx` (Mi horario) · `components/ui/Dialogo.jsx` (sustituyó a los diálogos del navegador). No hay `modules/citas/` |
| **Modelos** | `models/tenant/`: `EventType`→`event_types` (con `taller_grupo_id` desde el 01/09/2026: el tipo de cita de un grupo de taller, creado y mantenido desde Clínica → Talleres, siempre `is_hidden`), `Availability`→`availabilities`, `Booking`→`bookings` (con `client_id`, `patient_id`, `team_member_id`, `pack_id`, las cuatro de cobro del 04/09/2026 —`cobro_modo`, `cobro_concept_id`, `cobro_texto`, `cobro_importe`: de qué se cobra ESTA cita, NULL en las 13.408 de antes— y, desde el 01/09/2026, `taller_grupo_id`: la cita ES un taller de grupo —`patient_id` a NULL, los asistentes en `taller_asistencias`—, ver «Los talleres son citas» en `clinica.md`; y desde el 12/09/2026 `diagnostico_id` + `diagnostico_tramo` —`entrevista`|`horas`—: de qué expediente de diagnóstico es la cita, NULL en las de siempre y sin FK dura, ver «La cita de un diagnóstico»), `BookingChangeRequest`→`booking_change_requests`, `SessionPack`→`session_packs` (desde el 12/09/2026 con `diagnostico_id` y `total_sessions` NULLABLE: NULL = bono SIN TOPE, el de un diagnóstico; las dos por `migrate-diagnosticos`, en el bloque `citas` y ANTES del despliegue), `BlockedDay`→`blocked_days`, `TeamBlock`→`team_blocks` (desde el 01/09/2026 con `category_key` —su categoría— y `taller_id` —qué taller se da en ese tramo—, las dos nullable y sin FK dura; los documentos aparejados NO viven aquí sino en `documents.team_block_id`, con FK ON DELETE SET NULL: borrar el bloqueo no se lleva el acta. Y desde el 01/09/2026 el ACTA de la reunión sí vive aquí, en tres columnas nullable: `acta_sections` (JSONB, misma forma que el `content_sections` de una sesión clínica: la foto de apartados + un valor por clave), `acta_transcript` (de qué texto salió — el acta es un resumen y tres semanas después la pregunta es «¿eso se dijo?») y `acta_updated_at`. **No es una tabla porque es uno a uno con la reunión**; no confundir con los documentos aparejados, que son ficheros que se cuelgan del tramo), `ClientNotice`→`client_notices`, `ContractTemplate`→`contract_templates`, `ContractSignature`→`contract_signatures`; compartidos con Pagos: `PaymentSession`→`payment_sessions`, `StripeWebhookEvent`→`stripe_webhook_events`. Se registran y asocian en `lib/db/tenantDb.js` |
| **Interruptores y parámetros** | `featureFlags`: `autoConfirmPublicBookings` (lo lee `app/api/public/c/[tenantSlug]/book/route.js`; ausente = `true`) y `sinHorarioPropio` (03/09/2026, Aumenta: el equipo no tiene hora fija y las citas se las coloca administración; lo lee `lib/citas/horarioPropio.js` y con él el menú esconde «Mi horario», la ficha de Equipo no pinta «Horario de trabajo», `/api/team` deja `tieneHorario` a `null` y devuelve `horarioPropio: false`, y la agenda pública no recorta al horario de la profesional; se quita/devuelve con `scripts/horario-propio.js <slug> --quitar|--devolver`; prueba `_smoke-citas-horario-propio.mjs`). `logicOverrides`: ninguno que lea el código. ⚠️ Los interruptores por cliente de este módulo NO van en `tenant_modules` sino en `tenant.settings.citas.*` (Configuración → Citas): `identidadObligatoria`, `formularioObligatorio` (+`formularioUrl`), `contratoObligatorio`, `soloConPago`, `cobroObligatorio` (04/09/2026: no se guarda una cita sin decir de qué se cobra; se enciende en Configuración → Módulos y solo manda si el centro tiene `billing`. No frena la agenda pública ni los talleres), `recordatorios`, `avisosWhatsapp`, `agendaCompartida`, `portalBloqueoImpago`, `cancelacionBloqueada`, `reservaOnlineCerrada`, `meetModo`, `colorBloqueos`, `categoriasBloqueo` (la LISTA de categorías, 01/09/2026: `[{ key, label, color }]`, solo admin desde Configuración → Agenda), `avisoFaltas`, `incidenciaPorFalta` (01/09/2026: **ids de `team_members`** a quienes se AVISA de cada falta; vacío = no se abre ninguna incidencia. Desde el 18/09/2026 solo eso: avisar. La incidencia no se les asigna. Se elige en Configuración → Módulos y lo lee `lib/citas/incidenciaPorFalta.js`), `portalUrl`/`reservaUrl`… (los leen `lib/citas/puerta*.js`, `visibilidad.js`, `recordatorios.js`, `portalMeses.js`, `avisosWhatsapp.js`…); y `settings.widget.sso.enabled` / `settings.widget.auth.*` (portal SSO). Entorno: `WIDGET_SSO_SECRETS`, `CITAS_PORTAL_SESSION_SECRET`, `WIDGET_FRAME_ANCESTORS` (widget y portal), `APP_PUBLIC_URL` (enlaces del recordatorio) |
...
## Toda cita con su cobro (04/09/2026, Aumenta por Rodrigo)
...
**Lo que había, medido en producción ese día**: 13.408 citas y ninguna con
importe; 63 tipos de cita y ninguno con precio; 46 conceptos en el catálogo
—lo que Aumenta llama «cuotas»— sin ninguna relación con la agenda. El dinero
de una cita vivía en la cabeza de quien la apuntaba, y al cobrar el mes había
que reconstruirlo mirando la agenda.

### El dinero baja del TIPO de cita

El concepto se pone UNA vez en Citas → Tipos («LOGOPEDIA 45» → «Logopedia 45x1
· 50 €», `event_types.concept_id`) y de ahí baja solo a cada cita nueva de ese
tipo. Es lo que hace que la regla no cueste un clic más: con 63 tipos y ~250
citas al día, elegir el concepto en cada una sería elegir siempre el mismo.

En el alta se puede cambiar, y solo hay tres salidas (`lib/citas/dineroDeLaCita.js`):

| Modo | Qué es | Qué se guarda |
| --- | --- | --- |
| `cuota` | La cubre un concepto del catálogo | `cobro_concept_id` + nombre e importe COPIADOS |
| `libre` | Un cobro suelto de esa cita | el texto y su importe |
| `sin_coste` | No se cobra, **y se dice por qué** | el motivo, importe 0 |

`sin_coste` es la mitad del encargo: «nunca sin quererlo» no dice que no haya
citas gratuitas, dice que sean una decisión. Por eso el motivo es obligatorio —
dentro de tres meses, «sin coste» a secas no distingue una cortesía de un
despiste.

### Quién lo exige, y a quién NO frena

`settings.citas.cobroObligatorio` (Configuración → Módulos), apagado de fábrica
y solo efectivo si el centro tiene `billing`. Aunque esté encendido, no frena:

- **la agenda pública** — quien reserva es la familia y no hay a quién pedirle
  que elija una cuota; la cita hereda la del tipo si la tiene y ya;
- **los talleres** — no tienen UNA familia a la que cobrar: su dinero va por la
  inscripción de cada niño al grupo.

### Cobrar el mes: lo dicen sus citas

`GET /api/citas/cobro-del-mes?clientId=…&mes=AAAA-MM` devuelve lo que hay que
cobrarle a una familia ese mes sacado de sus citas, y el «Registrar cobro» de
Cobros se rellena con eso cuando la familia no tiene cuota asignada (va después
de la cuota de verdad y antes de la «aprendida» del último cobro, que es una
suposición sobre el pasado).

⚠️ **Una cuota NO se multiplica por las citas del mes.** Un concepto de Aumenta
es la cuota MENSUAL: ocho sesiones de logopedia en octubre son 190 €, no 1.520.
Las citas cubiertas por la misma cuota suman una vez y se cuenta cuántas fueron;
un cobro suelto es de ESA cita y sí se suma. Está en `loQueSeCobraDe` con su
prueba (`_smoke-cita-con-dinero.mjs`), porque equivocarlo es cobrarle de más a
una familia.

### Lo que no cambia

Las 13.408 citas de antes se quedan con las cuatro columnas a NULL y se leen,
se pintan y se cobran igual. `bookings.amount` sigue siendo lo suyo —el precio
que se cobra CON TARJETA en el widget, con sus retenciones—: son dos dineros
distintos y por eso son dos columnas. Y el IMPORTE del cobro no sale del CRM
para quien no es dirección (`lib/citas/dinero.js`), aunque el concepto sí: quien
apunta la cita tiene que ver de qué se cobra.

### La cita de un diagnóstico (12/09/2026, Rodrigo con Isa, Aumenta)

Un DIAGNÓSTICO (Clínica → Diagnósticos, `docs/modules/clinica.md`) es un
producto de horas —simple 10 h, completo 20 h— que se apunta en la agenda
**desde la lista de Diagnósticos**, con la cita ya preparada. El contrato
entre las dos pantallas es una URL, en `lib/citas/altaDesdeDiagnostico.js`
(prueba `_smoke-alta-desde-diagnostico.mjs`):

```
/citas?nueva=1&diagnostico=<id>&tramo=entrevista|horas&paciente=<id>&duracion=60
```

`CitasModule` la lee al montarse (`leerAltaDesdeDiagnostico`), abre
`NuevaCitaDrawer` con `inicial.diagnostico = { id, tramo, patientId, duracion }`
y limpia la URL con `router.replace` para que recargar no abra otra alta. El
cajón trae al paciente por su id (nombre en la franja, familia y terapeuta por
`datosAlElegirFicha`, los mismos que al elegirlo a mano), pone el tipo
DIAGNÓSTICO del centro (`tipoDiagnosticoDe`: `informe_tipo = 'diagnostico'` y
nombre que empieza por DIAGN), esconde los selectores de familia y paciente,
el bono, el bloque de cobro y la repetición, y en el tramo `horas` pregunta la
**duración** (30/60/90/120/180/240 min). Manda al `POST /api/citas/bookings`:

| Campo | Qué es |
| --- | --- |
| `diagnosticoId` | uuid del expediente. Sin él, la cita es de siempre y `duration` se ignora |
| `diagnosticoTramo` | `entrevista` (la entrevista inicial) u `horas` (una sesión más) |
| `duration` | minutos, múltiplo de 30 entre 30 y 480; solo se escucha con `diagnosticoId` |

Lo que el servidor hace con eso vive en `lib/clinica/citaDeDiagnostico.js`
(prueba `_smoke-cita-de-diagnostico.mjs`), en este orden: carga el expediente
(422 «Este centro no tiene diagnósticos» / «El diagnóstico indicado no
existe»), comprueba que sigue abierto (422 «Este diagnóstico está en «No
continúa»: no admite más citas») y que para `horas` ya tiene bono (422 «…pulsa
«Seguir con el diagnóstico»…»), que la cita lleva **el paciente del
expediente** (422 «Esta cita no es del paciente del diagnóstico…») y que con
esta cita **no se pasa de las horas del producto** —hechas Y reservadas,
`cabeHora` de `lib/clinica/diagnostico.js`— (422 «El diagnóstico ya tiene sus
N h: desbloquéalas desde Diagnósticos», la misma frase que la lista). Un
taller no puede serlo (422). Se resuelve antes del solape y de los bloqueos,
porque la duración entra en los dos.

**El dinero lo pone el servidor**, no el cajón: la entrevista nace `sin_coste`
con el texto «Diagnóstico: el cobro nace al decidir si sigue» (el cobro de
verdad nace al parar, 50 €, o al seguir, el producto); las horas salen del
**bono sin tope** del expediente (`session_packs.total_sessions` a NULL) por
`elegirPack`, la misma puerta que un bono elegido a mano —de esa familia, de
ese paciente y de ese tipo—, así que un `packId` del cuerpo se ignora y el
`cobroTexto` dice «Bono «DIAGNÓSTICO» · sesión N» sin «de N». La cita guarda
`bookings.diagnostico_id` y `diagnostico_tramo`; en la entrevista el
expediente apunta a ella (`diagnosticos.entrevista_booking_id`, best-effort).
La respuesta añade `diagnostico: { id, tramo, ocupadas, max }`.
...
`resumenDelDiagnostico` —consulta aparte, null si algo no está, como `bono`— y
el modal pinta el chip «Diagnóstico · 3 de 10 h» con enlace a
`/clinica/diagnosticos` (tras un PATCH, que no lo trae, dice el tramo).
...
| `/api/citas/bookings/[id]/suggest-slots` | POST | IA: propone 3 horarios para reprogramar. Admin con ámbito `professional` o `company`; la profesional de la cita, solo para las suyas y siempre en `professional`. Candidatos = horario − citas que ocupan hueco (`ocupaHuecoWhere`, hasta el horizonte) − festivos − **bloqueos de `team_blocks`** (los del centro y los de cada persona, tratados como una cita más para no correr la rejilla: `lib/citas/choqueConBloqueos.js`; `cargarAusencias` acepta `profesionalIds`). La rejilla es la del TIPO y la longitud la de la CITA (`generateSlotsForDay` con `duracionCita`, opcional: el widget público no lo pasa y no cambia); la re-validación y el contexto de la IA miden con `booking.duration`. Mover una cita sigue sin avisar de bloqueos (13/09/2026; prueba `_smoke-sugerir-huecos.mjs`). **Si la IA falla** (sin saldo, clave, 5xx…), `chooseSlots` devuelve igual tres huecos repartidos sin IA, pero con `avisoIA` —el motivo legible, `avisoSinIa`— y `CitaDetalleModal` lo pinta en una caja ámbar encima de los huecos; sin clave no hay aviso. Si es la cuenta, la campana `ai_cuenta` a dirección sale del cliente central (13/09/2026; prueba `_smoke-ia-aviso-central.mjs`). **El candado y el tope** (`vetoAi`, 403/429) solo se miran si de verdad se va a llamar a la IA —con clave, fuera de las cuatro demos (que simulan) y con algún hueco que elegir—; sin clave no queda `ai.uso` (`lib/ai/modoDeIa.js`, 14/09/2026; prueba `_smoke-salamandrobot-turno.mjs`) |
...
- **Puede borrar quien puede cancelar** (`noPuedeTocarla`), no solo admin. Quien
  ...
  (`citas.booking_deleted`) con quién, cuándo, de quién era la cita —por
  `clientId`/`patientId`; desde el 13/09/2026 sin el nombre, que no se duplica
  en master: la ficha sigue en el tenant, y una reserva pública sin ficha queda
  sin ese dato— y qué se llevó por delante: es el ÚNICO rastro que queda,
  porque la fila ya no está.
...
### Repetir un bloqueo el resto de semanas (09/09/2026, AV-0090 de Aumenta)

Olga: «he intentado crear una nueva cita de bloqueo (libre pacientes) y, al
intentar programar para el resto de semanas, no me sale opción para poder
hacerlo». No salía porque no estaba: el formulario del bloqueo solo tenía
«Termina», que es **cuánto dura el tramo**, no hasta cuándo se repite. Y esa
confusión ya había hecho daño: el 08/09/2026 dos personas cerraron su agenda
entera hasta el 30/06/2027 metiendo en «Termina» la fecha de fin de curso, que
es de donde salió el aviso de bloqueo largo del servidor.

`BloqueoRapido` (dentro de `NuevaCitaDrawer`) tiene ahora «Repetir» y «Hasta el
día (incluido)», con las mismas tres cadencias que una cita. La regla vive en
`lib/citas/recurrencia.js` —`repeticionDeBloqueo`, junto a la de las citas, con
`diasEntre` y `comoDia`—, así que es la misma aritmética de pared: la hora no
baila con el cambio de hora y el mensual salta los meses sin ese día.

Lo que añade sobre la de las citas es que **un bloqueo tiene fin**: el fin se
desplaza los mismos días que el inicio, de modo que un tramo que cruza la
medianoche sigue cruzándola en cada repetición.

Se crean bloqueos SUELTOS, uno por POST al endpoint de siempre, como las citas
repetidas: sin concepto de serie, cada uno se mueve o se quita solo. Van con
`confirmarLargo` porque el aviso de tramo largo ya se contestó con el primero y
...
## La falta se avisa, pero no es de nadie (18/09/2026, AV-0197 de Aumenta)

> Olga: «Rodri, soy Olga. Sigo viendo incidencias en las que no estoy
> etiquetada».

No era el filtro: la bandeja abre en «las mías» desde el 01/09. Era que ella es
la destinataria de las faltas del centro (`settings.citas.incidenciaPorFalta`) y
el CRM se las asignaba solas. Medido el 18/09 en producción: de las **128**
incidencias en las que figuraba como responsable, **41 se las había puesto el
propio CRM** — un tercio de su bandeja era trabajo que ella no se había
repartido. (Y de paso: **50** incidencias que ella registró y en las que no es
responsable NO le salen al abrir, porque para dirección `mine=1` mira solo la
pivote de responsables.)

Decidido por Rodrigo, opción 2 de tres: **la incidencia de una falta nace sin
responsable** y se queda en la pestaña «Faltas», que existe desde el 03/09
(AV-0038) y ya las separa de las incidencias de siempre. Lo que NO cambia es el
aviso por campana a la lista del centro: quitarlo sería que una falta no llegue
a nadie. La lista sigue siendo el interruptor (vacía = no se abre ninguna).

**Quien quiera hacerse cargo de una se pone de responsable a mano** desde su
ficha, y entonces sí sale en su bandeja — porque lo ha decidido una persona.

⚠️ Esto da por hecho que quien está en la lista VE las faltas sin ser
responsable: hoy se cumple porque las ve dirección y porque quien marcó la falta
la registró. Comprobado el 18/09/2026: el único centro con la función encendida
es Aumenta y su destinataria es `admin`. Si algún día se pone ahí a alguien que
no es dirección, habrá que darle alcance por la lista en
`lib/clinica/alcanceIncidencias.js`, o dejará de ver lo que se le avisa.

Las **41** que ya estaban asignadas se soltaron con
