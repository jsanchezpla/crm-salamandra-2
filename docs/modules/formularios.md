...
| **Pruebas** | En `npm test`: `scripts/_smoke-formularios-fields-preguntas.mjs` (`node:test`, 19/08/2026: lo que devuelven `lib/formularios/fields.js` —`camposDe` ordena por `order` y tira la basura de un JSONB roto; `normalizarTelefono` deja 9 dígitos se escriba como se escriba (fuera el prefijo, y desde el 21/08/2026 también escrito **`0034`**, no solo `+34`: el «00» solo abre paso al 34, ni otro país ni un 00 suelto se cuelan como españoles); `validarRespuestas`: el consentimiento es obligatorio aunque no lo declare y cuenta como aceptado con `true`/`"true"`/`"on"`/`1` (y nunca sube a la ficha), las casillas son opcionales, un obligatorio en blanco —o solo HTML— frena con su frase, el texto se guarda sin etiquetas y con el tope del tipo o del campo, email en minúsculas, teléfono a 9 dígitos, DNI/NIE con su letra y un pasaporte pasa tal cual, el número sube como TEXTO («06» → «6») con min/max —y desde el 21/08/2026 la **coma decimal española** («6,5» → «6.5»), traducida ANTES de mirar `min`/`max`, mientras el punto de miles («1.000,5») sigue sin ser un número—, desplegable solo de la lista, lo que llega de más se tira porque el endpoint es público, todos los errores juntos y en orden, y qué sube a la ficha por `mapTo` (gana el último); `infoAdicional` («lo que nos contó») excluye lo que ya sube a la ficha pero INCLUYE lo del peque donde no hay `pacientes`; `formPublico` no lleva `settings` ni `mapTo`; y el formulario de familias de Aumenta de punta a punta— y `lib/citas/preguntasCita.js` —cuatro tipos de pregunta y no más, `normalizarPreguntas` desde el panel o JSONB sin guardar nada roto (ids únicos, escala de 2 a 10, tope de preguntas), `validarRespuestas` de quien reserva se para en el primer error y guarda el número como NÚMERO, `paquetePreguntas` a `bookings.form_answers` o `null` si no hay nada—; y desde el 21/08/2026 un bloque que compara **caso a caso** lo que los DOS validadores dicen de cada número, para que ninguno acepte lo que el otro rechaza: son dos puertas del mismo cuerpo público), `scripts/_smoke-aceptar-solicitud.mjs` (`accept.js` y `fields.js`, sin base) y `_smoke-menor-firma.mjs` (`edadDeclarada.js`); más las de Citas que leen la bandeja con un `FormSubmission` falso (`_smoke-puerta-descartada`, `_smoke-puerta-profesional`, `_smoke-aviso-admision`, `_smoke-paciente-borrado`). Con base de datos: `_smoke-formulario-cita.mjs` (`fields.js` desde un tipo de cita). Con servidor y base: `_smoke-dni-formulario.mjs` (formulario público → aceptar → `Client.taxId`, por HTTP) y `_smoke-puerta-formulario.mjs` (sin solicitud aceptada no hay cita). |
...
- `type`: `text | textarea | tel | dni | email | number | select | checkbox |
  ...
  Dos tipos tienen reglas que conviene saber, porque las teclea alguien de fuera
  en la web del cliente y un rechazo tumba el **envío entero** (no se guarda
  nada, y la solicitud se pierde sin dejar rastro):

  - **Teléfono (`tel`)** — se normaliza a 9 dígitos quitando espacios, puntos,
    guiones y paréntesis, y el prefijo internacional escrito de cualquiera de
    las dos formas: `+34` o `0034` (también vale `34` a secas). Un prefijo de
    otro país no se cuela: lo que no queden 9 dígitos se rechaza. El `0034`
    —el prefijo de salida de toda la vida, el que llevan impreso las tarjetas
    de visita y el que sale al copiar un contacto del móvil— **se admite desde
    el 21/08/2026**; antes tumbaba el envío entero.
  - **Número (`number`)** — se admite la coma decimal española («6,5» se guarda
    como «6.5»), igual que en las preguntas propias de un tipo de cita
    (`lib/citas/preguntasCita.js`), que ya la admitía: son dos puertas del mismo
    cuerpo público y tienen que decir lo mismo. El punto de miles («1.000,5»)
    **no** es un número en ninguna de las dos: es ambiguo, y ahí es mejor
    preguntar que adivinar. El `min`/`max` se comprueba DESPUÉS de traducir la
    coma («99,5» no cuela en un `max` de 99).
