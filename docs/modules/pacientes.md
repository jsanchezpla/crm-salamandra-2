...
| **Pantallas** | `app/(dashboard)/pacientes/` (3): `/pacientes` (listado paginado con KPIs, búsqueda por nombre y filtros por terapeuta y estado; alta con especialidades), `/pacientes/[id]` (ficha: datos, consentimientos, contactos externos, plan de intervención, sesiones, informes, coordinaciones, documentos, facturación; desde el 02/09/2026 también «Padres y tutores» de la familia —nombre, parentesco, teléfono y correo, sin DNI— porque las terapeutas no entran en Clientes, AV-0023/0024 de Aumenta: `tutoresParaFicha` en `lib/clients/guardians.js`, fijado en `scripts/_smoke-tutores-ficha-paciente.mjs`. Desde el 18/09/2026 (ficha «Datos tutor» y AV-181 de Aumenta) ese bloque **ESCRIBE** —`components/clinica/TutoresDeLaFamilia.jsx`— y la lista la arma `tutoresDeLaFamilia`, que mete dentro al TITULAR de la ficha: era el único progenitor que no salía y 104 familias de Aumenta aparecían «sin padres ni tutores» teniendo el teléfono de la madre delante. El DNI de los tutores y quién firma **siguen sin verse y sin poder tocarse desde aquí**: los conserva el servidor al fusionar. En la MISMA vuelta, «Editar ficha» pasa a preguntar el DNI, el domicilio y el parentesco del paciente, que la ficha pintaba y ningún formulario de esta pantalla dejaba rellenar. Y esa tarjeta pasa a llamarse **«La familia»**: además del contacto del pagador enseña el domicilio y el motivo de consulta de la familia —lo que había que pedirle a recepción—, y el nombre solo enlaza a `/clientes/[id]` para quien tenga Clientes; ver «La ficha del paciente enseña a la familia» abajo), `/pacientes/[id]/sesiones/nueva` (subir audio → transcribir → revisar → guardar). En el menú es el primer hijo del grupo «Clínica» (`components/layout/Sidebar.jsx`), no una entrada raíz. Ya no hay `pacientes/_components/`: su único fichero, el `dummyData.js` de la maqueta, se borró el 20/08/2026 por no importarlo ninguna página (ver «Coherencia con sprint Clínica»). |
| **Endpoints** | `app/api/pacientes/**` (11 `route.js`): `pacientes` (listar/crear), `[id]` (ficha/editar; no hay DELETE), `[id]/plan` (plan de intervención) + `[id]/plan/objetivos-ia` ⚡ **Claude** (02/09/2026, AV-0019 de Aumenta: de las ideas clave de la terapeuta a objetivos de intervención adaptados al paciente; NO guarda nada, la propuesta se marca y se añade al plan desde `components/clinica/InterventionPlanSection.jsx`; clave BYOK + `vetoAi` + demo simulada; al modelo no viaja el nombre del paciente — `lib/clinica/objetivosIa.js`, fijado en `scripts/_smoke-objetivos-ia.mjs`), `[id]/tutores` (PUT, 18/09/2026: padres y tutores de la familia escritos desde la ficha del paciente. Puerta propia y no `/api/clients/[id]/guardians`, cuyo gate es el módulo del USUARIO y devuelve 403 a las 13 terapeutas de Aumenta —la piedra del 05/09—; entra quien puede abrir la ficha, y solo los cuatro campos que esa ficha enseña), `[id]/contactos/**` (2, agenda de profesionales externos), `[id]/documents/**` (3, adjuntos del paciente), `[id]/contract` (solo descarga los PDF legado que la migración no pudo mover), `contract-template/**` (2, contrato estándar del centro). Solo `[id]/plan/objetivos-ia` gasta IA. El registro clínico del paciente (sesiones, informes, coordinaciones) va por `app/api/clinica/**` (ver `clinica.md`). Desde el 01/09/2026 la ficha **pone el resultado de sus citas** sin salir de ella (completada · falta justificada · falta injustificada · cancelada) por el PATCH de siempre `app/api/citas/bookings/[id]`: la pieza es `components/citas/CitasDelPaciente.jsx` y lo que se manda lo arma `lib/citas/resultadoCita.js`, el mismo que usa la ficha de la cita en la Agenda. También crean o leen pacientes: `app/api/clients/route.js` (el alta de la familia crea a los suyos en la misma transacción), `app/api/clients/[id]/module-assignments/route.js`, `app/api/formularios/[id]/accept/route.js` y `app/api/citas/bookings/**` (una cita apunta a `patient_id`). |
| **Lógica** | No hay `lib/pacientes/`: vive en `lib/clinica/` — `serialize.js` (`serializePatient`, etiquetas de estado y `care_type`), `specialties.js` (taxonomía compartida con Equipo), `consents.js` (consentimientos RGPD con traza legal), `contractStorage.js` (PDF legado por paciente), `patientClient.js` (de qué cliente es un paciente), `audit.js` — y en `lib/clients/`: `formularioAlta.js` (`normalizarPacientes` —con `dni` y `address` desde el 18/09/2026—, perfil `salud`), `guardians.js` (`tutoresDeLaFamilia`: la lista con el titular dentro, deduplicado por DNI o por nombre; `fusionarTutoresDeFicha`: lo que la ficha del paciente puede escribir, conservando `dni` y `signer`), `moduleAssignments.js` (`syncClinicPatient`), `familiaEnLaFicha.js` (18/09/2026: qué datos de la familia entran en la ficha del paciente —domicilio, localidad y motivo— y por qué el dinero no). |
| **UI** | Sin `modules/pacientes/` ni `components/pacientes/`. Las piezas están en `components/clinica/` (`SpecialtyPicker.jsx`, `PatientDocumentsSection.jsx`, `PatientExternalContactsSection.jsx`, `InterventionPlanSection.jsx`, `NuevaCoordinacionModal.jsx`, `TutoresDeLaFamilia.jsx`) y en `components/clients/` (`PacientesDelAlta.jsx`, `ClientPatientsSection.jsx`: los pacientes vistos desde la ficha de la familia). |
...
| **Interruptores y parámetros** | `featureFlags.altaPorPaciente` (07/09/2026, AV-0051 de Aumenta por Rodrigo, «solo para Aumenta»): el alta de Clientes pregunta PRIMERO por el paciente y después por «Padre, madre o tutor que abre la ficha». Lo lee `app/(dashboard)/clientes/page.jsx` en servidor (`lib/clients/altaPorPaciente.js`); se pone con `scripts/alta-por-paciente.js <slug> --poner`, sin despliegue. Lo que se guarda es lo mismo (familia + paciente en una transacción). **Si el paciente es mayor de edad** (18/09/2026, AV-0201): la casilla «Es mayor de edad y abre su propia ficha» copia su nombre al titular y lo sigue mientras se teclea, el rótulo deja de decir «padre, madre o tutor», desaparece «Parentesco con el paciente» y el aviso de lo que falta habla del paciente. Es la MISMA marca de siempre (`relationship = «El propio cliente»`, la que ya existía en el alta normal copiando al revés), así que el servidor no cambia. Sin ella, un adulto que viene solo obligaba a inventarse un familiar para poder darle de alta. Prueba: `_smoke-alta-por-paciente.mjs`. |
...
| **Pruebas** | `scripts/_smoke-busqueda-nombre.mjs` (`node:test`, 28/08/2026, en `npm test`, 49 casos): la regla del buscador —todas las palabras, cada una en cualquier columna, sin importar orden ni tildes— y que el endpoint sigue llamándola con las columnas cualificadas. · `scripts/_smoke-alta-progenitores.mjs` — entra en `npm test`, sin base de datos: `normalizarPacientes` (el motivo llega hasta lo que se guarda) y el alta con dos progenitores. · `scripts/_smoke-tutores-de-la-familia.mjs` (`node:test`, 18/09/2026, en `npm test`, 19 casos): que el titular entra en la lista y no se pinta dos veces, y que escribir desde la ficha del paciente NO borra el DNI de los tutores ni cambia quién firma. · `scripts/_smoke-familia-en-la-ficha.mjs` (`node:test`, 18/09/2026, en `npm test`): los datos de trabajo de la familia salen y el dinero no, y el enlace a su ficha depende del módulo del USUARIO. Ninguna otra toca `patients`: `_smoke-borrar-paciente.mjs` y `_smoke-paciente-borrado.mjs` son de `clients` (la «paciente» de una consulta de nutrición es un `Client`). |
...
## La ficha del paciente enseña a la familia (18/09/2026, AV-181)

Isabel, de Aumenta: «nos resulta lioso ir a una ficha u otra… que si me meto en
el paciente, pueda acceder a los datos del cliente con facilidad, completos».
Las 13 terapeutas NO tienen Clientes en su menú —es lo suyo, lo dicen ellas—,
así que la ficha del paciente es TODO lo que ven de la familia: hasta esta
vuelta, el contacto del pagador y los padres y tutores. El domicilio había que
pedírselo a recepción.

**Lo que se hizo, y lo que NO.** No era «enseñar más campos»: era decidir
cuáles. Lo de trabajo sí —el domicilio, la localidad y el motivo de consulta de
la familia—; el dinero no, que es la línea que ya traza
`lib/clients/quienVeElDinero.js` y que aquí no se cruza. Las **notas** de la
ficha se quedan fuera aunque el aviso las citara: son texto libre donde
...
domicilio de la familia es `customFields.domicilio` (una línea de texto), NO
`Client.address`, que es JSONB. Lo pinta la tarjeta «La familia» de
`app/(dashboard)/pacientes/[id]/page.jsx` y lo sirve `serializePatient`.

**Las dos puertas, distintas a propósito.** El bloque VIAJA siempre: lo gatea el
CENTRO (`tenantHasModule("clients")` en el include del pagador, la piedra del
05/09). Lo que gatea el USUARIO es poder ABRIR la ficha del cliente: con
`clients` en su `moduleAccess` el nombre es un enlace, y sin él se pinta en
plano en vez de llevar a un 403. Lo fija
`scripts/_smoke-familia-en-la-ficha.mjs`.
