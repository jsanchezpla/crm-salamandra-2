...
| **Interruptores y parámetros** | `featureFlags.altaPorPaciente` (07/09/2026, AV-0051 de Aumenta por Rodrigo, «solo para Aumenta»): el alta de Clientes pregunta PRIMERO por el paciente y después por «Padre, madre o tutor que abre la ficha». Lo lee `app/(dashboard)/clientes/page.jsx` en servidor (`lib/clients/altaPorPaciente.js`); se pone con `scripts/alta-por-paciente.js <slug> --poner`, sin despliegue. Lo que se guarda es lo mismo (familia + paciente en una transacción). Prueba: `_smoke-alta-por-paciente.mjs`. |
