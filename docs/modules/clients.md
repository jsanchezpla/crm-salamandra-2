...
| `/api/clients/waitlist` | GET/POST/PATCH | Lista de espera de ADMISIÓN (`lib/clients/listaEspera.js`; no es la de Citas). Desde el 15/09/2026 (AV-0133) el POST acepta `{ clientId }` para meter una ficha YA creada, enlazada; la ficha lo ofrece con «Pasar a lista de espera» / «Sacar de la lista de espera» (esto último es `status: "removed"`) | JWT + `hasModule(clients_avanzado)` |
