...
| **Scripts** | `configure-portal-citas.js <slug>` (enciende `widget.sso`; sustituye a `configure-nutri-laura-citas-portal.js`, que sigue en disco), `_hechos/configure-nutri-laura-widget-auth.js` (el gate `?wpa=1`, legado), `dev-mint-wpsso.js <slug> <email>` (fabrica un `wpsso` de prueba sin WordPress), `comprobar-citas.js` (solo lectura: Stripe, Resend, precios y salas; NO mira los secretos del SSO) |
