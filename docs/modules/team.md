...
| **Pantallas** | `team`: `/equipo` → `app/(dashboard)/equipo/page.jsx` (plantilla, drawer de alta/edición, horario, «Acceso al CRM»; al no admin le pinta `MiEquipo`) · `team_avanzado`: `/equipo/mi-desempeno`, `/equipo/desempeno-config`, `/equipo/direccion`, `/equipo/productividad`, `/equipo/incidencias`, `/equipo/bandeja` (con `clinica`), `/equipo/ocupacion` (con `citas`), `/equipo/actividad` → `app/(dashboard)/equipo/<carpeta>/page.jsx`, con sus piezas en `app/(dashboard)/equipo/_components/` (el paciente de `IncidenciaModal` se elige con `SelectorPaciente` —busca en el servidor, 31/08/2026: el desplegable cortaba en 1.000 y Aumenta tiene 1.174—, vigilado en `_smoke-selector-fichas`; y sus cajas van en el orden en que pasan las cosas —Observaciones, Acción realizada, Verificación— desde el 18/09/2026, AV-0182: estaban al revés y la ficha pedía el remedio antes que el problema, `_smoke-ficha-incidencia-orden.mjs`) · `fichaje`: `/equipo/fichaje` · el grupo del menú se ve con `team` O `clinica` (`visibleModules`) |
| **Endpoints** | `app/api/team/**` — 13 `route.js`: `route.js` (listado/alta), `[id]` (detalle, edición, baja lógica), `[id]/borrar` (GET la radiografía, DELETE el borrado definitivo), `modules` y `[id]/modules`, `[id]/access` (+ `password`: el login del miembro, escribe en `master.users`), `[id]/hours` (horario), `me` y `me/documents` (+ `[id]`) (autoservicio; gate `team` O `clinica` a nivel de tenant), `[id]/billing-summary` (gatea `billing`), `[id]/projects` (gatea `projects`) · `team_avanzado` — 18 `route.js` con `hasModule("team_avanzado")`: `app/api/actividad`, `app/api/clinica/performance/**`, `app/api/clinica/productividad/**`, `app/api/clinica/incidencias/**` (01/09/2026: `/equipo/incidencias` se abre filtrada por **las que me atañen a mí**; quién soy lo resuelve el servidor y vuelve en `yoSoy`, porque `/api/auth/me` da el usuario y no su ficha de equipo. **18/09/2026, AV-198**: los filtros admiten VARIOS valores —`status`, `category`, `reportedById` y `assignedToId` por comas o repitiendo el parámetro, unidos con O—, `faltas=1` puede venir CON estados (dos ramas en O), y `mine=1` pasa a ser «la registré yo O soy responsable», la misma regla del alcance; todo en `lib/clinica/filtroIncidencias.js`, con `whereSinPestana` para los recuentos, y la pantalla recoge los desplegables en un panel «Filtros» plegado), `app/api/clinica/incentive-items/**`, `app/api/clinica/dashboard`, `app/api/clinica/bandeja` y `app/api/citas/informe-ocupacion` (desde el 31/08/2026 la bandeja, la campana y el bloque Mi trabajo miran las incidencias por la tabla PIVOTE —todos los responsables, no solo el principal— con `lib/clinica/incidenciasDe.js` y su prueba; y la portada gana la tarjeta «Incidencias abiertas» en Pendiente, mismos gates que el endpoint) · `app/api/auth/me` (`enabledModules`) · Públicos: ninguno |
| **Lógica** | `lib/team/`: `serializeTeamMember.js` (BD → API; oculta coste y salario a quien no es admin) · `access.js` (crear, cambiar y quitar el login y su `moduleAccess`) · `currentTeamMember.js` (qué `TeamMember` es el usuario logueado) · `rastro.js` (qué queda de una persona en el schema y si su ficha se puede borrar) · `departamentos.js` (01/09/2026: quién es «Administración» a partir de `TeamMember.department`, normalizando el texto libre — lo usa el botón «Todos menos Administración» de los selectores de equipo; son DOS departamentos, «Administración» y «Contabilidad», porque en producción Olga y Rosa no comparten el suyo) · `lib/auth/correoCuenta.js` (el correo de una cuenta, que NO es su usuario; puro, sin imports, lo usa también el navegador) + `correoCuentaDb.js` (sus consultas) · Actividad: `lib/actividad/etiquetas.js` (acción de auditoría → frase) · las pantallas avanzadas tiran de `lib/clinica/` | · **Quién coordina NO es un departamento** (09/09/2026, AV-0078): la lista está en `settings.clinica.coordinadoras` y la lee `lib/clinica/coordinadoras.js` (`veTodoElEquipo`), se edita en Configuración → Módulos y en Aumenta ya lleva a las dos coordinadoras desde el 02/09/2026. `departamentos.js` es otra cosa: el departamento REAL de cada ficha (Logopedia, Pedagogía, Administración), que usan los selectores de equipo. Ponerle «Coordinación» a alguien la sacaría de su departamento de verdad. La Bandeja de trabajo (`/equipo/bandeja`) es también la vista del equipo para quien coordina: `?vista=equipo` da los cuatro recuentos de cada persona, y ni un euro ni una hora. **Coordinar es MIRAR, no que te avisen** (18/09/2026, AV-0191): la tarjeta «Informes vencidos» de Inicio → Pendiente cuenta solo los propios a todo el que no sea dirección —coordinadoras incluidas—, con `avisaDelPendienteDeTodos` del mismo fichero; `veTodoElEquipo` se queda para la Bandeja, que es donde se va a mirar el trabajo de otra persona. Las incidencias de esa misma caja ya lo hacían así desde el 01/09/2026. |
...
| `GET/PUT /api/team/[id]/hours` | Horario semanal PROPIO del miembro (`team_member_hours`: `dayOfWeek` 0–6 + franjas). El PUT es un reemplazo atómico. Lo usa `TeamHoursEditor` en la ficha y en `/mi-horario` (Citas). Con `sinHorarioPropio` de Citas encendido (Aumenta, 03/09/2026) ni la ficha ni el menú lo ofrecen y `/api/team` devuelve `horarioPropio: false` (`lib/citas/horarioPropio.js`); el endpoint sigue ahí. | Gate de tenant `team` O `clinica`. El admin ve/edita cualquiera; un profesional solo el SUYO. |
...
## Los filtros de incidencias admiten varios valores (18/09/2026, AV-198 de Aumenta)

Rosa, por el Buzón: «nos vendría bien poder tener la opción de filtrar más de un
tipo de incidencia (ej: faltas, pendientes y en proceso). Y luego si pudiera ser
que aparecieran por defecto en cada usuario las que ha registrado y debe
resolver ese usuario, aunque luego puedas filtrar otras». Y por el Registro, la
queja de fondo: «hay mucho filtro y es muy confuso».

Tres cosas, todas en el módulo base —cualquier centro con varias personas
querría las tres—:

**1. Los filtros se suman.** Los tipos (Pendientes · En proceso · Resueltas ·
Faltas) dejan de ser pestañas excluyentes y pasan a ser marcas; «Todas» es la
que las quita. Igual los tres desplegables: Registrada por, Responsable y
Categoría son `MultiSelect` (`null` = todos, `[a,b]` = solo esos, `[]` no
existe). Por la URL viajan repetidos o con comas, y `listaDeParametro`
(`lib/clinica/filtroIncidencias.js`) los lee en UN sitio para la lista y para su
Excel. Un valor suelto sigue filtrando exactamente igual, así que la campana y
los enlaces guardados no se enteran. Las faltas viven en su propia rama —`falta
IS NULL` frente a `falta IS NOT NULL`—, así que pedir faltas Y pendientes es un
O de dos ramas y no una condición imposible; por eso los recuentos de las
pestañas se cuentan con `whereSinPestana` y ya no borrando claves a mano.

**2. Menos cosas a la vista.** Arriba se quedan los tipos y el buscador, que son
los de todos los días. Los tres desplegables, «Solo las mías» y el interruptor
de las dadas por vistas se recogen en un panel «Filtros» plegado, que lleva en
el botón cuántos hay puestos y dentro un «Quitar filtros». Va en el flujo de la
página y no flotando: en móvil empuja la lista hacia abajo en vez de taparla.

**3. «Las mías» son las dos cosas.** `mine=1` filtraba solo por responsable, así
que a dirección no le salían las que ella misma había apuntado para otra
persona —justo las que quiere seguir—. Ahora usa `whereIncidenciasVisibles`, la
MISMA regla que el alcance de quien no es dirección (la registré yo O soy
responsable), escrita una vez para que no puedan divergir. Es un interruptor
aparte del filtro de responsable y entra como AND, así que «las mías» + «de
Olga» son las que me atañen Y lleva Olga. Viene puesto de fábrica y la pantalla
lo DICE bajo el título, con el remedio al lado («Quita “Solo las mías” en
Filtros para verlas todas»): antes se abría acotada sin explicar por qué.

Y en el formulario de una nueva, quien la escribe sale ya relleno en «Quién la
registra» y marcado como responsable (se quita con un clic). Sale de su ficha de
