...
| `invoice.paid` | cuotas 2ª en adelante del fraccionado: se apuntan (una de 0 € —prorrateo o abono— no cuenta), se borra el rastro de un rechazo anterior y, si el tope no llegó a ponerse, `frenarSiYaEstaPagado` cancela al completar el total (cuenta las pagadas con `limit: 100`, la página entera; ver «Tope de cuotas») |
| `invoice.payment_failed` | el banco rechazó una cuota (07/09/2026): rastro completo en `PaymentSession.metadata` (factura, intento, reintento prometido, enlace de pago), motivo REAL leído del PaymentIntent fuera de la transacción, campana a los admins (`cuota_rechazada`); se enseña en la ficha y en «Mis pagos». **No** se reintenta ni se toca el bono (`lib/payments/cuotaRechazada.js`, sección «Cuando el banco rechaza una cuota» de `citas.md`) |
| `customer.subscription.deleted` | el plan a plazos terminó (07/09/2026): si estaba completo, `planCompletadoAt`; si Stripe se rindió con cuotas sin cobrar, `planInterrumpidoAt` + campana (`plan_interrumpido`), contrastando el recuento con Stripe. El bono no se toca: decide la profesional |
