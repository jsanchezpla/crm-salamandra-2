...
- **Por qué el multipart de Whisper se arma antes (08/09/2026).** Los logs de producción escupían
  `TypeError: Invalid state: ReadableStream is already closed` como `uncaughtException` de Next mientras el
  centro transcribía: 4 apariciones sobre 21 transcripciones en tres horas, sin perder ni una. La sospecha
  razonable —`await request.formData()` cortado por un navegador que se va— es FALSA, y lo delata el propio
  mensaje: ese texto solo lo lanza un `ReadableByteStreamController`, y el cuerpo entrante que monta Next usa un
  controlador por defecto, que diría «Controller is already closed». Además las seis rutas que reciben audio ya
  envuelven esa línea en try/catch. El stream de bytes es el SALIENTE: undici serializa un `FormData` con un
...
- **El informe se puede DICTAR**: `POST /api/clinica/reports/[id]/desde-material`
  ...
  dictar un informe a partir de notas. Si el informe tiene tabla de pruebas
  (el de diagnóstico), viaja con el material en su propio bloque, leída de lo
  guardado, y la respuesta dice cuántas en `pruebas` (13/09/2026, ver «El
  informe de valoración diagnóstica»).
...
Isabel Alberca mandó el guion entero del informe que hacen y el listado de
pruebas por áreas (un .docx de 13 áreas y 75 entradas). Rodrigo: «haz ambos».

**Un tipo de informe más** (`diagnostico`, `migrate-informe-diagnostico`,
antes del despliegue como beca y asesoramiento): «Diagnóstico» en el chip,
«Informe de valoración diagnóstica» en la portada, color índigo en la lista.
Nace con SU plantilla puesta (`contentSections.plantilla = "diagnostico"` lo
escribe el POST si no viene otra), así que no hay que ir a buscarla.

**Su plantilla es de fábrica** (`PLANTILLA_DIAGNOSTICO`,
`lib/clinica/pruebasDiagnosticas.js`): 25 apartados en el orden del centro
—datos escolares, profesionales, periodo, motivo, antecedentes, historia
evolutiva, contextos, área emocional, intervenciones previas, medicación,
observación clínica, pruebas administradas, integración clínica, la
conclusión diagnóstica en siete apartados (principal, DSM-5-TR, CIE-11,
comórbidos, diferenciales, otras consideraciones, sospecha clínica), el plan
en tres (terapéutico, escolar, familiar), derivaciones, orientaciones—. Se
ofrece como `PLANTILLAS_EXTRA.informe`, igual que la entrevista lo es del
registro: detrás de las del centro, y si el centro guarda una con esa clave,
manda la suya. No repite lo que ya imprime la portada.

**Y el catálogo de pruebas con sus puntuaciones**, que es lo que no era una
plantilla:

- `CATALOGO_PRUEBAS`: las 13 áreas del listado, 69 pruebas únicas (BRIEF-2, la
  Figura de Rey, PROLEXIA, TALE, RIAS-2 y PEDI-CAT están en dos áreas y salen
  una vez con las dos). El centro añade las suyas en Configuración → Módulos →
  «Pruebas diagnósticas» (`settings.clinica.pruebasDiagnosticas`, JSONB en
  master; `GET/PUT /api/clinica/pruebas`, PUT solo admin); nunca pisan las de
  fábrica.
- En el informe, el bloque **«Resultados de la evaluación»**
  (`components/clinica/PruebasDiagnosticas.jsx`): se elige el área, la prueba,
  y se abre su tarjeta con los tres subapartados que pidió Isabel —qué evalúa
  (rellena desde el catálogo), la tabla de puntuaciones y la interpretación—.
  Se guarda en `contentSections.pruebas`, clave RESERVADA, limpiada por
  `normalizarPruebas` (sin nombre no entra, las filas vacías no se guardan).
- **La tabla es la misma para todas** a propósito: escala/índice · P. directa
  · P. típica/escalar · percentil · clasificación. Un baremo por prueba serían
  69 tablas que nadie mantiene; las columnas que ninguna fila rellena no se
  imprimen (`columnasConDatos`).
- **En el PDF** salen como el apartado «Resultados de la evaluación», numerado
  y en el índice, justo detrás de «Pruebas administradas» (o al final si ese
  no se escribió): `apartadosDelInforme` mete el bloque y `documentoPdf.js`
  (`bloqueDePruebas`, `tablaDePuntuaciones`) lo dibuja con cabecera tintada y
  salto de página que repite la cabecera. En la beca no sale nunca.
- **A la IA (13/09/2026).** Hasta ese día ni «Unir» ni el dictado mandaban la
  tabla —`pruebas` es clave reservada, no apartado— y la regla dura de
  `saberClinico.js` prohíbe citar una puntuación que no conste: la integración
  y la conclusión salían sin resultados. Y la caché de respuestas (24 h por el
  SHA de lo que viaja) devolvía la misma propuesta aunque se hubiera retocado
  la tabla. Ahora `pruebasParaLaIA` la pasa a texto (`### Prueba: <nombre>`,
  área —«Otras pruebas» para las propias—, «Qué evalúa», una línea por fila
  solo con las celdas escritas, «Puntuaciones anotadas: ninguna.» si no hay, y
  la interpretación marcada como escrita por la profesional) y
  `mensajeDeInforme` la pone detrás del material, bajo `PUNTUACIONES DE LAS
  PRUEBAS (TABLA DEL INFORME)`, con la orden de citarla tal cual, darla por
  ADMINISTRADA y no copiarla en ningún apartado. Va en el mensaje de usuario,
  nunca en el system (su núcleo se cachea como prefijo). Se lee de lo
  GUARDADO (las dos pantallas guardan antes de llamar); en la beca no viaja
  (`tablaDePruebasParaLaIA`); más de `MAX_PRUEBAS_PARA_IA` (120.000
  caracteres) es un 413 que dice qué acortar, nunca un recorte; con tabla, un
  apartado nuevo titulado «Resultados de la evaluación» se descarta
  (`bloquesOcupados`). `pulir` no cambia: no redacta esos apartados.

Prueba: `scripts/_smoke-informe-diagnostico.mjs` (19 casos: el tipo, la
plantilla, el catálogo, la limpieza y el PDF generado de verdad) y
`scripts/_smoke-informe-pruebas-ia.mjs` (16: lo que ve la IA de la tabla).
...
#### El acta se pinta en un sitio, y se puede corregir (18/09/2026, AV-0102)

**Qué pasaba.** «Acabo de registrar una coordinación y no se visualiza», y antes
«en un paciente las veo enteras y en otro no» (Silvia, Aumenta). El acta estaba
guardada entera: lo que pasaba es que la pestaña Coordinaciones de la FICHA del
paciente pintaba **una tarjeta distinta y recortada** —tipo, fecha,
participantes y temas— mientras el listado general pintaba el acta completa. La
que ella escribió tenía **once acuerdos y un solo tema**: en la ficha se veía
una línea. No era un caso raro: de las **701 actas de Aumenta, 593 tienen
contenido que la ficha no pintaba** (acuerdos, próximos pasos, con quién,
el reparto de asistentes o la firma). Y explica también la queja anterior: que
un acta pareciera completa dependía de en qué campo hubiera caído el texto al
importarla de Organízate, no del paciente.

**El arreglo.** La tarjeta se escribe UNA vez, en
`components/clinica/ActaCoordinacion.jsx`, y de ahí tiran el listado general y
la ficha. `mostrarPaciente` solo lo pide el listado: dentro de la ficha, el
enlace al paciente es el paciente que ya estás mirando. Dos copias de un acta
acaban enseñando cosas distintas, que es exactamente lo que pasó; la prueba
`_smoke-acta-coordinacion.mjs` comprueba ahora que ninguna de las dos pantallas
se vuelva a pintar la suya.

**Y se corrige.** `/api/clinica/coordinations` tenía GET y POST y ningún PATCH:
una fecha mal puesta, un acuerdo a medio escribir o el paciente equivocado se
quedaban escritos, y la única salida era registrar el acta otra vez y dejar la
mala al lado. Ahora hay `PATCH /api/clinica/coordinations/[id]`, y el formulario
del alta se reutiliza en modo corrección (`coordinacion` puesta → se rellena con
lo que hay y manda PATCH): dos formularios para la misma acta acaban admitiendo
cosas distintas.

- **Quién puede**, con su prueba en `lib/clinica/alcanceCoordinaciones.js`
  (`_smoke-coordinaciones-alcance.mjs`, 12 casos): **quien la registró**
  (`createdById`) o **dirección**. Un acta sin ficha de equipo detrás —las 171
  traídas de Organízate que firma gente que ya no está— **solo dirección**: no
  hay autoría que reconocer y el resto del equipo no estuvo en esa reunión. Es
  la misma frontera que en los informes. Esconder el botón no es un permiso: lo
  decide el endpoint.
- **A diferencia de los informes, no mira ningún estado**: un acta no se entrega
  a una familia ni pasa por revisión.
- **Lo que arrastra cambiar el paciente**: el pagador (`clientId`, que es una
  foto) se recalcula y el contacto de referencia **se revalida** contra la
  agenda del paciente nuevo. Sin eso, el acta quedaría enlazada con la
  orientadora de otra familia. Ese guard vivía dentro del POST y se sacó a
  `lib/clinica/contactoDelPaciente.js` para que corran los dos por el mismo
  sitio.
- El troceo al corregir es el mismo del alta (`actaCoordinacion.js`): por
  líneas, la coma no parte. Solo se toca lo que viene en el cuerpo.
- **No hay DELETE**: nadie lo ha pedido, y un acta borrada es una reunión que
  deja de constar en el expediente. Si un día hace falta, la regla va en el
  mismo fichero de `lib/`.
- Auditoría `clinica.coordination.updated`, con su frase en
  `lib/actividad/etiquetas.js` y el resumen de siempre (identificadores y
  fechas, nunca el texto del acta).

Sin migración: no cambia ninguna tabla.
...
- **El aviso del registro va también a los tutores cuando la ficha no tiene correo** (06/09/2026, Rodrigo): `correosParaAvisar(client)` (`lib/clients/contactoDeFicha.js`) devuelve el correo de la ficha o, si no lo tiene, todos los de los tutores con uno válido; `motivoParaNoAvisar({ correos })`; el envío es uno por destinatario. El GET de `/enviar` devuelve `destinatario.correos` y `deTutores`; el modal lo enseña.

## Diagnóstico: el expediente con su barra de horas (12/09/2026, Aumenta por Rodrigo e Isa)

**Por qué.** Aumenta vende el diagnóstico como un producto cerrado de horas:
«simple» (10 h = 1 de entrevista inicial + 9) y «completo» (20 h = 1 + 19), a
350 y 650 €. Desde el 05/09 (AV-0045) el CRM tenía el tipo de cita
DIAGNÓSTICO, los dos conceptos del catálogo y el informe de valoración
diagnóstica, pero **no sabía por dónde iba cada niño**: cuántas horas se han
dado, cuántas están puestas en la agenda, cuántas quedan. Isa lo llevaba a
mano. El porqué de cada decisión —y lo que se descartó— está en
`docs/decisions/2026-09-12-el-expediente-de-diagnostico.md`; aquí va cómo
funciona.

**Qué es.** Dentro de Clínica (sin módulo nuevo: `moduleKey` `clinica`),
`/clinica/diagnosticos` es la lista de **expedientes** (`diagnosticos`, modelo
`Diagnostico`). Un expediente es de UN paciente, de UN producto, con un
terapeuta asignado en general (cada cita lleva además el suyo) y una barra
0→10 o 0→20 que se **CALCULA desde las citas** (`lib/clinica/diagnostico.js`,
`horasDe`), nunca un contador guardado —la misma lección que los bonos—. Lo
único que se guarda es el TOPE (`horas_max`), porque es una decisión de
alguien.

**El flujo, en estados** (`ESTADOS`, `puedePasarA`; el estado va en
`diagnosticos.status`, VARCHAR con CHECK y no enum, para no repetir el ALTER
TYPE del informe de beca):

1. **«Nuevo diagnóstico»** (cualquiera del equipo): paciente + producto +
   terapeuta → `entrevista`. Sin dinero. `POST /api/clinica/diagnosticos`
   copia `client_id` del paciente, apunta el tipo DIAGNÓSTICO del centro y el
   concepto del producto, y contesta 409 si el paciente ya tiene uno abierto
   (se reenvía con `permitirOtro` si de verdad se quiere otro).
2. **«Abrir entrevista inicial»** → la agenda con la cita preparada
   (`/citas?nueva=1&diagnostico=…&tramo=entrevista&paciente=…&duracion=60`):
   tipo DIAGNÓSTICO —el que tiene `informe_tipo = 'diagnostico'` y nombre que
   empieza por DIAGN, `esTipoDiagnostico`; NO el tipo ENTREVISTA INICIAL de
   terapia—, nota «Entrevista inicial», 60 min, `sin_coste` con «Diagnóstico:
   el cobro nace al decidir si sigue». Su registro se escribe con la
   plantilla `entrevista_inicial` aunque el tipo sea DIAGNÓSTICO
   (`lib/clinica/plantillaDeLaCita.js`).
3. **Después de la entrevista**, dos botones, solo para quien da bonos
   (`puedeDarBonos`: dirección o quien lleve Facturación):
   - **«Parar el diagnóstico»** → `no_continua`. Nace un cobro PENDIENTE de la
     entrevista: el concepto que lleva el tipo marcado como valoración
     inicial (`isInitialAssessment`) si tiene precio; si no, 50 € con la nota
     «Entrevista inicial de diagnóstico» (`cobroDeLaEntrevista`). Cobro y
     estado en la MISMA transacción. Sin Facturación o sin ficha de familia
     se para igual, sin cobro, y `avisos` lo dice.
   - **«Seguir con el diagnóstico»** → `en_curso`. Nace un **bono del tipo
     DIAGNÓSTICO SIN TOPE** (`session_packs.total_sessions` a NULL,
     `diagnostico_id` apuntando al expediente) con su cobro PENDIENTE de
     350/650 € —el concepto del producto o, sin él, su precio de caída
     (`cobroDelProducto`)—, por `crearBonoConSuCobro` (la regla de AV-0070:
     bono y deuda en una transacción). El cobro se llama como el producto
     («Diagnóstico Simple»), no «Bono «DIAGNÓSTICO»», y lleva el «Texto en la
     factura» del concepto. La entrevista va DENTRO del precio. Si un intento
     anterior dejó el bono creado y el expediente sin apuntar, se ADOPTA.
     `no_continua` **no vuelve** a `en_curso`: ya nació un cobro.
4. **«Añadir horas»** (en `en_curso`) → la agenda con el tramo `horas`: tipo
   DIAGNÓSTICO, modo de cobro `bono` con el bono del expediente (por
   `elegirPack`, la misma puerta que un bono elegido a mano) y la **duración
   se elige** al abrir (múltiplos de 30 min, hasta 480: una tarde de pruebas
   dura 3 h). El servidor comprueba `cabeHora` y, si con esta cita se pasa del
   tope, 422 «El diagnóstico ya tiene sus N h: desbloquéalas desde
   Diagnósticos» (`mensajeTope`, la MISMA frase que apaga el botón en la
   lista). Todo eso en `lib/clinica/citaDeDiagnostico.js` y en `citas.md`.
5. **«Desbloquear horas»** (quien da bonos): sube `horas_max` solo hacia
   arriba, de media en media y hasta 200 (`puedeDesbloquear`), y queda quién
   y cuándo (`horas_desbloqueadas_por/_at`) más su línea de auditoría.
6. **«Cerrar»** (cualquiera con `clinica`): a mano, desde cualquier estado
   que pueda pasar a `cerrado`. No toca el bono ni el cobro pendiente; es el
   expediente cerrado el que corta las citas nuevas. La respuesta dice las
   horas sin dar y si había citas futuras, para que la pantalla avise.

**Qué cuenta como hora.** La regla de los bonos, sin inventar otra
(`lib/citas/gastaSesion.js`, sacada de `packs.js` sin ORM para que la barra la
pinte también el navegador): completada, falta injustificada y cancelación
tardía CUENTAN; cancelada a tiempo o falta justificada NO; una cita futura
RESERVA (suma a la barra y bloquea el hueco). Cada cita vale `duration / 60`;
la entrevista vale 1 h dure lo que dure, y si se canceló tarde y se repitió, la
primera cuesta 1 h de las hechas. La barra son cuatro tramos que suman el tope,
[entrevista | hechas | reservadas | libres], con el rótulo «3 de 10 h · 2
reservadas»; `cabeHora` compara con las OCUPADAS (entrevista + hechas +
reservadas): una hora reservada ya no está libre.

**El bono sin tope.** `total_sessions` NULL es lo que dice que un bono no
tiene contador: `estadoPack` devuelve `sinTope: true`, `total` y `restantes` a
null, nunca `agotado`, y el resumen «N usadas · sin tope»; `elegirPack` lo
deja pasar; `cobroDeBono` escribe «Bono «DIAGNÓSTICO» · sesión N» sin «de N»;
`bonoSinTope`/`rotuloDelBono` en `lib/billing/bonos.js` y la pantalla de
Bonos lo rotulan «sin tope · diagnóstico» con enlace a la lista, y sin
«Renovar» ni «Volver a darlo». Lo que acota las horas es el expediente.

**El dinero, en la fila.** Los cobros son `payments` pendientes (`method` y
`period_month` a null: no entran en «Facturar el mes» ni en el bloqueo del
portal). El del bono se encuentra por `pack_id`; el de la entrevista no tiene
columna que lo ate —`payments` no sabe de diagnósticos— y la lista lo reconoce
por paciente + la nota con la que nació + haber nacido después que el
expediente (`cobrosDeExpedientes`). La fila suma `dinero: { pendiente,
cobrado, devuelto }` y `cobroPendiente` con el TOTAL que queda aunque el cajón
de Cobros lo haya partido en dos filas.

**Los productos.** `settings.clinica.diagnosticos`, lista `[{ key, nombre,
horas, conceptId, precioEuros }]`; de fábrica `simple` (10 h, 350 €) y
`completo` (20 h, 650 €) con `conceptId` null, que se resuelve por nombre
(/diagn.*simple/i, /diagn.*completo/i) entre los conceptos ACTIVOS del
catálogo — en Aumenta existen «Diagnóstico Simple» y «Diagnóstico Completo»,
así que sin tocar nada el cobro sale del catálogo, y ningún id suyo va a fuego
en el código. Un producto propio del centro se casa por nombre exacto. Se
editan en Configuración → Módulos → «Productos de diagnóstico» (ver la
sección siguiente); el expediente copia nombre y horas al nacer, así que
cambiar un producto no toca los abiertos.

**La fila** (`lib/clinica/diagnosticoFila.js`, `filaDeExpediente`) es la
misma en los seis endpoints: paciente, producto, terapeuta, `horas` con sus
tramos y su rótulo, `entrevista` (la columna `entrevista_booking_id` o, si
Citas no la escribió, la cita de tramo entrevista deducida), el dinero, las
`urls` a la agenda y al informe (`/pacientes/<id>?informe=diagnostico`, que ya
existía) y `acciones` —qué botones salen— ya cruzadas con el estado y con
`permisos.puedeDecidir`. La pantalla no decide, pinta. Las horas de toda la
lista se cuentan con UNA consulta de citas y otra de cobros (`filasDe`), no
con una por fila.

**Datos y migración.** Tabla `diagnosticos` (índices por paciente, estado y
terapeuta; sin FK duras, patrón `taller_grupo_id`) más `bookings.diagnostico_id`
+ `diagnostico_tramo`, `clinic_sessions.diagnostico_id`,
`session_packs.diagnostico_id` y `session_packs.total_sessions` DROP NOT NULL:
`scripts/migrate-diagnosticos.js`, por TABLA (`byTable`, cuatro conjuntos),
idempotente, en los bloques `clinica`, `citas` y `pacientes` de
`_module-migrations.js` porque los tres modelos declaran esas columnas para
todos los tenants. **VA ANTES del despliegue.** El modelo `Diagnostico` se
registra para todos (`lib/db/tenantDb.js`, asociaciones `constraints: false`)
y solo lo consultan los endpoints con `clinica` y el alta de citas cuando
llega `diagnosticoId`; un centro sin la tabla ve la lista vacía con
`sinMigrar` y un 503 al abrir.

**Auditoría.** Una línea por acción, con resumen y sin datos clínicos
(`lib/utils/auditoria.js`): `diagnostico.abierto`, `.parado` (con el id y el
importe del cobro), `.seguido` (con el `packId` y si hubo cobro),
`.horas_desbloqueadas` (antes y después), `.cerrado` (horas hechas y tope),
`.terapeuta_cambiado` y `.editado`; el prefijo `diagnostico` se lee bajo
«Clínica» en `lib/actividad/etiquetas.js`.

**La segunda entrega está hecha** (misma tarde, sección siguiente): los
**registros de diagnóstico por fecha y título** (`clinic_sessions.diagnostico_id`
ya la escribe el servidor, y `titulo` es columna nueva), **«Unir en informe»**
(`informe_id` se rellena al crear el informe desde el expediente y `/unir` lo
redacta con la IA a partir de los registros), `entrevista_payment_id` (el
cobro de la entrevista atado por id; la búsqueda por nota + fecha queda solo
para los parados de antes) y «Empezar desde lo que ya hay». Lo único que
sigue fuera, y a propósito: volver de `no_continua` a `en_curso` (si alguien
lo pide, con una regla para el cobro ya nacido).

**Pruebas** (todas `node:test`, ligeras, en `npm test`): `_smoke-diagnostico.mjs`
(la barra, los estados, los productos y los cobros), `_smoke-alta-desde-diagnostico.mjs`
(el contrato de la URL), `_smoke-diagnostico-fila.mjs` (la fila y el cobro del
bono con nombre propio), `_smoke-cita-de-diagnostico.mjs` (qué rechaza el alta
de citas y la plantilla del registro) y `_smoke-diagnosticos-ajustes.mjs` (lo
que guarda Configuración). Detalle de cada una en la fila «Pruebas» del Mapa.

## Diagnóstico, segunda entrega: los registros por fecha, «Unir en informe» y empezar desde lo que ya hay (12/09/2026, Aumenta por Rodrigo e Isa)

**Por qué.** Lo que pidió Rodrigo, literal: «Informe de diagnóstico por
paciente: entradas por fecha y título (registros de diagnóstico, con IA como un
registro de sesión) que al final se unen con IA en el informe completo.
Terapeuta asignado general por paciente; en cada registro/informe se puede
firmar con otro». Y tres respuestas de Aumenta a las preguntas del PDF que
cambiaron el encargo la misma tarde: **(A)** los apartados del registro de
diagnóstico son los cinco que se les propuso MENOS «notas internas» («ese
informe es el que se entrega al paciente; todo lo demás genial»); **(B)** las
citas de los tipos «INFORME PARA DIAGNOSTICO» (45 min) SON horas del
diagnóstico («se cuentan dentro de las 19 h»), y una entrevista inicial de
terapia hecha antes de decidir el diagnóstico cuenta como la primera hora y
**sus 50 € ya cobrados se descuentan del producto**; **(C)** los seis
pacientes que ya están en diagnóstico los dan de alta ELLOS («lo único que hay
que saber cómo hacerlo»), así que la pantalla tiene que ofrecerles, con
buscador, a quien ya tiene citas de diagnóstico o una entrevista hecha y sin
expediente, y abrirle el expediente METIENDO dentro lo que ya hay. El porqué
de cada decisión, en `docs/decisions/2026-09-12-el-expediente-de-diagnostico.md`
(«Segunda entrega»). Nada de Aumenta a fuego: los tipos se reconocen por
`event_types.informe_tipo = 'diagnostico'` y `is_initial_assessment`, no por
nombre ni por id.

**Sin módulo ni tabla nuevos.** Un registro de diagnóstico ES una fila de
`clinic_sessions` con `diagnostico_id` (la columna existía desde la primera
entrega y nadie la escribía); el informe ES `clinical_reports` de tipo
`diagnostico`. Solo dos columnas: `clinic_sessions.titulo VARCHAR(160)` —el
título de la entrada: «Sesión de diagnóstico 3», «Pruebas WISC-V»— y
`diagnosticos.entrevista_payment_id UUID` —el cobro de la entrevista atado por
id—. Las declaran los modelos (`ClinicSession.titulo`,
`Diagnostico.entrevistaPaymentId`), así que `scripts/migrate-diagnosticos-2.js`
**VA ANTES del despliegue**: por tabla (`byTable`, dos conjuntos: donde hay
`clinic_sessions` y donde hay `diagnosticos`), idempotente (`IF NOT EXISTS` y
`exige` comprueba que la columna está), sin FK (hay Clínica sin Facturación, y
anular un cobro no puede llevarse el expediente) y sin backfill. Registrada en
los bloques `clinica` y `pacientes` de `_module-migrations.js` detrás de
`migrate-diagnosticos` (NO en `citas`: `bookings` no cambia).

**El registro de diagnóstico.** Es la misma pantalla del registro de sesión
(`RegistroSesionEditor.jsx`, rutas `/pacientes/[id]/sesiones/nueva` y
`/pacientes/[id]/sesiones/[sesionId]`), con dos cosas más cuando el registro
es de un expediente:

- **La plantilla `sesion_diagnostico`** («Sesión de diagnóstico»,
  `PLANTILLA_SESION_DIAGNOSTICO` en `lib/clinica/plantillas.js`, ofrecida en
  `PLANTILLAS_EXTRA.registro` detrás del taller, el centro la sustituye
  guardando una con esa clave): exactamente los cinco apartados de (A), en su
  orden y todos de texto — `pruebaAdministrada` «Prueba administrada» ·
  `queEvalua` «Qué evalúa» · `observacionesDurantePrueba` «Observaciones
  durante la prueba» · `puntuaciones` «Puntuaciones» · `interpretacion`
  «Interpretación». Claves nuevas en camelCase a propósito: ninguna es columna
  de fábrica ni reservada, todo cae en el JSONB `contentSections`. Las notas
  internas siguen siendo el bloque aparte del registro y nunca van al informe
  ni a la IA. La entrevista del expediente sigue con `entrevista_inicial`.
  `plantillaDeLaCita` (`lib/clinica/plantillaDeLaCita.js`) gana la regla 2:
  el tramo `horas` de un diagnóstico abre `sesion_diagnostico` (entre «tramo
  entrevista → entrevista_inicial» y «tipo con `isInitialAssessment`»).
- **Cómo se ata al expediente: lo decide el SERVIDOR.** `POST
  /api/clinica/sessions` hereda `diagnosticoId` de la CITA
  (`bookings.diagnostico_id`) cuando llega `bookingId`, por encima de lo que
  diga el cuerpo; sin cita (o con una cita que no es de ningún diagnóstico),
  acepta el `diagnosticoId` del cuerpo SOLO si tiene forma de UUID, el
  expediente existe y es del MISMO paciente (422 «diagnosticoId inválido» /
  «Ese diagnóstico no existe» / «Ese diagnóstico no es de este paciente»;
  `diagnosticoDelRegistro`). `PATCH /api/clinica/sessions/[id]` lo admite con
  CANDADO: solo si la sesión no tenía ninguno, comprobado igual contra el
  paciente; ya atada, se ignora en silencio, como `bookingId`. `GET
  /api/clinica/sessions?diagnosticoId=` filtra (400 si no es UUID) y
  `serializeSession` devuelve `diagnosticoId` y `titulo`.
- **El título** (`titulo`) se escribe y se corrige libremente (≤ 160,
  `limpiarTitulo` colapsa espacios; vacío se guarda como `null` y lo quita).
  El editor enseña el input «Título» solo cuando hay expediente (el
  `?diagnostico=` de la URL al estrenar, o `diagnosticoId` de la sesión al
  seguir), con el título propuesto por la URL como placeholder; lo manda en el
  POST y en los dos PATCH solo cuando hay expediente, y `diagnosticoId` SOLO
  al crear (también en `payloadDePreparacion` al preparar desde una cita).
  Sin título, el expediente enseña el de por defecto con su número correlativo
  entre las de horas (`tituloDe`, `registrosPorFecha`): «Entrevista inicial»
  para la que se escribió con esa plantilla, «Sesión de diagnóstico 1, 2, 3…»
  para el resto por orden de fecha; un título propio conserva su número de
  sitio.
- **La cola de la URL** la escribe la fila y la lee el editor por el mismo
  fichero (`lib/clinica/registroDeDiagnostico.js`, puro): `urlDeNuevoRegistro`
  = `/pacientes/<pid>/sesiones/nueva` + la cola de `colaDePreparacion`
  (`preparar=1`, `fecha`, `cita`, `prof`, `plantilla`) + `&diagnostico=<id>`
  y `&titulo=<enc>` (`colaDeRegistroDeDiagnostico`); `leerDiagnosticoDeLaUrl`
  valida el UUID (sin forma de id, el registro nace suelto) y recorta el
  título. `urlDeRegistro` = `/pacientes/<pid>/sesiones/<sid>` para seguir el
  que hay. El chip «Registro de diagnóstico» arriba enlaza «Volver al
  expediente», y Cancelar y la vuelta tras guardar van a
  `/clinica/diagnosticos/<id>` en vez de a la ficha.
- **La firma** no cambia: el desplegable del registro ya estaba y
  `terapeutaPorDefecto` sigue leyendo `?prof=`, que la fila rellena con el
  terapeuta asignado del expediente (o el de la cita).

**El expediente tiene ficha propia**: `/clinica/diagnosticos/[id]`
(`app/(dashboard)/clinica/diagnosticos/[id]/page.jsx` monta
`components/clinica/ExpedienteDiagnostico.jsx`). Cabecera con lo de la fila
(paciente con enlace, chip de estado, producto y horas, terapeuta asignado
como desplegable que hace `PATCH`, barra con leyenda, dinero —cobrado, sin
cobrar, devuelto, la entrevista cobrada o pendiente, y «Al seguir: X
(descontada la entrevista de N €)» solo si `acciones.seguir`—, notas y los
MISMOS botones que la lista) y tres bloques: **Registros de diagnóstico**
(fecha · título · quién firma · estado · «Abrir»; «Nuevo registro» →
`urls.nuevoRegistro`, sin cita, tramo horas, título siguiente), **Citas**
(fecha · duración · tramo · estado · terapeuta · «Escribir registro» si la
cita no tiene sesión o «Seguir registro» si la tiene, por `citas[].sessionId`
y `urls.registro`; nada en las canceladas) e **Informe** («Unir en informe»
si no existe; si existe, su estado, su fecha y «Abrir el informe»). Las
acciones se comparten con la lista por el hook
`components/clinica/useAccionesDeDiagnostico.js` (parar, seguir, desbloquear,
cerrar y cambiar de terapeuta con sus confirmaciones por `useDialogo`), no
copiadas; los botones, por `BotonesDeDiagnostico` de `DiagnosticoFila.jsx`.
La ficha pide aparte `GET /api/clinica/diagnosticos?estado=todos&paciente=`
para el equipo, el tipo DIAGNÓSTICO y el cobro de la entrevista al parar, que
la fila no trae; sin ese GET funciona igual.

**«Unir en informe» son dos pasos, y ninguno inventa.**

1. `POST /api/clinica/diagnosticos/[id]/informe` (cualquiera con `clinica`;
   body opcional `{ therapistId }`) CREA el informe de valoración diagnóstica
   del expediente UNA sola vez: `clinical_reports` con `reportType:
   "diagnostico"`, `status: "draft"`, `reportDate` hoy, `contentSections = {
   plantilla: "diagnostico", sourceSessionIds: [los registros terminados],
   diagnosticoId }` y el `clientId` del expediente; y escribe
   `diagnosticos.informe_id` en la MISMA transacción con `SELECT … FOR
   UPDATE` (dos pestañas a la vez: la segunda adopta el de la primera). Firma
   = el `therapistId` del cuerpo (TeamMember del centro, 422 si no) ??
   `expediente.therapistId` ?? quien pide; sin ninguno, 422 «Hace falta quien
   firme el informe…». Respuesta 201 `{ informe: serializeReport, expediente:
   la ficha entera, creado: true }`; si ya existía, 200 con `creado: false` y
   el que hay. 404 sin expediente; 503 sin `ClinicalReport`. Desde la ficha,
   «Unir en informe» confirma quién firma y cuántos registros terminados y
   borradores hay, crea y salta a `/clinica/informes/<id>?unir=1`.
2. `POST /api/clinica/diagnosticos/[id]/unir` (gate `clinica`;
   `assertNotDemoPaidCall` fuera del try y `vetoAi`, como `desde-material`,
   que es su molde) junta el TEXTO de los registros TERMINADOS —`status`
   `registered` o `published` (`ESTADOS_TERMINADOS`); un borrador no es
   material, la misma regla que `desde-sesiones`— con `materialDeLosRegistros`
   y se lo da a `structureInforme` con los apartados de ESE informe (los que
   manda la pantalla en el body JSON opcional `{ apartados, escrito }`, o
   `apartadosPara` del informe guardado), el perfil del centro, el paciente
   sin nombre y la clave/modelo del tenant, y —desde el 13/09/2026— la tabla
   de puntuaciones del propio informe (`tablaDePruebasParaLaIA`, de lo
   guardado; nunca en la beca). Devuelve `{ propuesta, nuevos,
   bloques, material, registros: <nº unidos>, pruebas: <nº que viajaron>,
   incidencia: null|"cortada"|"ilegible"|"vacia", avisoIA? }` (`pruebas`
   también en el `after` de `diagnostico.informe_unido`) **SIN guardar nada del
   texto**: en `InformeEditor` entra por `recibirPropuesta` —el MISMO camino
   que el dictado— y la profesional acepta apartado por apartado en
   `PropuestaIA`. Lo único que escribe es `contentSections.sourceSessionIds`
   con los registros unidos (metadatos: el anexo del PDF,
   `sesionesDelInforme.js`). Errores: 409 «Crea primero el informe desde el
   expediente» (sin `informeId` válido; si apuntaba a uno borrado, se limpia)
   y 409 si el informe ya está `delivered`; 422 «No hay registros terminados
   que unir…» / «…no tienen nada escrito que unir»; 413 si el material pasa
   de `MAX_TRANSCRIPCION` o la tabla de pruebas de `MAX_PRUEBAS_PARA_IA`; 503 sin clave de IA (la del proveedor que tenga
   elegido el centro: `getTenantIaKey` / `sinClaveDeIa`); 502 si la IA falla
   (`mensajeDeErrorIa` y aviso a los admins).

**Lo que viaja a la IA, y lo que no** (`materialDeLosRegistros`,
`lib/clinica/registroDeDiagnostico.js`): por registro, `### <dd/mm/yyyy> ·
<título>` y debajo cada apartado con su rótulo (`apartadosConPlantillas` con
las plantillas del centro por encima de las de fábrica + `valoresDeSesion`;
las listas como «- ítem»; los vacíos no salen; un registro sin nada escrito no
aparece), más «Devolución de la familia» si hay `parentFeedback`. NUNCA
`prepText`, `prepFiles`, `internalNotes` ni `aiTranscription`: `registrosDe`
ni siquiera pide esas columnas, la misma frontera que `sesionesDelInforme.js`
pone al PDF. El NOMBRE del paciente no viaja (lo sabe `estiloClinico.js`).
Del informe, además, su tabla de puntuaciones (`contentSections.pruebas`) en
su propio bloque del mensaje, nunca en la beca (13/09/2026).

**La firma del informe.** La cabecera de `InformeEditor.jsx` gana el
desplegable «Firma» (equipo activo por `GET /api/team?status=active&limit=200`;
resiliente al 403: sin equipo no hay desplegable; quien firma y ya no está
sale fijado como «Nombre (ya no está)») que hace `PATCH
/api/clinica/reports/[id]` con `therapistId` en el acto. El PATCH lo admite
desde hoy: UUID de un `TeamMember` de este schema (activo o de baja), 422
«therapistId inválido» / «Ese profesional no es del centro»; igual al actual,
sin cambios. Cuando `report.contentSections.diagnosticoId` existe, el editor
enseña además la tarjeta «Informe de un diagnóstico» (pide `GET
/api/clinica/diagnosticos/<id>` y cuenta los `registros[]` terminados, con sus
chips fecha · título y cuántos hay en borrador), el botón «Unir los registros
con la IA» (guarda callado y llama a `/unir`) y «Ver el expediente»; con
`?unir=1` hace `scrollIntoView` a la tarjeta al cargar, y NO pulsa el botón
solo: la IA cuesta dinero.

**La fila y la ficha del expediente** (`GET/PATCH /api/clinica/diagnosticos/[id]`
→ `{ expediente: fila }`, montada por `fichaDeExpediente` de
`lib/clinica/registrosDelExpediente.js` sobre `filasDe` con `extras`):
además de lo de la primera entrega, `registros[]` (`registrosPorFecha` de las
sesiones con `diagnosticoId = id` **Y** `patientId` del expediente —candado,
como `sesionesDelInforme`—, con `terapeuta: { id, nombre }` resuelto y `url`),
`siguienteTitulo`, `informe` (`{ id, status, statusLabel, reportDate, url }` o
`null`; si `informeId` apunta a un informe borrado o de otro paciente,
`informeDe` limpia la columna en la base —sí, un GET que escribe— y devuelve
`null`), `citas[]` con `sessionId` y `urls.registro` (una consulta
`sesionesDeLasCitas`; si dos sesiones apuntan a la misma cita gana la tocada
más recientemente, como `sesionDeLaCita`), `urls.expediente`
(`/clinica/diagnosticos/<id>`), `urls.nuevoRegistro` y `urls.informe` =
`/clinica/informes/<informeId>` cuando existe y `null` si no (el enlace de
antes a la ficha con `?informe=diagnostico` DESAPARECE: el informe nace desde
el expediente; «Informe» en la lista solo sale cuando existe). Y el dinero de
ESE expediente: `cobroAlSeguir: { importe, descuento, texto }` (lo calcula
`filasDe` con el producto del expediente, su concepto y su cobro de
entrevista) y `dinero.entrevista: { id, importe, status }`. La confirmación de
«Seguir» en pantalla usa `e.cobroAlSeguir`, no `productos[].cobro` (que se
queda para el panel de alta).

**El descuento de la entrevista** (respuesta B). `cobroDelProducto(producto,
concepto, { descuentoEuros })` → `importeEuros = max(0, precio − descuento)` y
el texto «Diagnóstico Completo (descontada la entrevista inicial de 50 €)»;
sin tercer argumento, exactamente lo de antes más `descuentoEuros: 0`.
`descuentoDeLaEntrevista(pago)` (`lib/clinica/adoptarEnDiagnostico.js`) es el
`amount` del cobro si existe, está `completed` o `pending` y no se ha devuelto;
si no, 0 (un pendiente también descuenta: la familia debe la entrevista, no la
entrevista más el producto entero). `seguir` lo aplica con el cobro atado por
`entrevistaPaymentId` (o, para expedientes anteriores a la columna, el que
encuentra `cobrosDeExpedientes`), lo dice en la nota del cobro y avisa aparte
si la entrevista cubre el precio entero (el bono nace sin cobro); la respuesta
gana `cobro.descuento` y `entrevista`. `parar`: si el expediente YA tiene
`entrevistaPaymentId` no crea otro cobro —solo cambia el estado y avisa «La
entrevista ya estaba cobrada (50 €)» (o «…ya tenía su cobro apuntado (N €,
pendiente): no se crea otro.»), `cobro.yaExistia: true`—; cuando lo crea,
escribe `entrevistaPaymentId` en la misma transacción. `cobrosDeExpedientes`
busca PRIMERO por `entrevista_payment_id` y deja la búsqueda por paciente +
nota + fecha SOLO para los parados sin columna.

**«Empezar desde lo que ya hay»** (respuesta C). `GET
/api/clinica/diagnosticos/candidatos?q=&meses=12` (gate `clinica`) devuelve
los pacientes SIN expediente abierto que en los últimos `meses` (12 por
defecto, 36 como mucho, `mesesDeAdopcion`) tienen (a) citas de tipos con
`informe_tipo = 'diagnostico'` sin `diagnosticoId`, y/o (b) una cita de un
tipo con `is_initial_assessment` sin `diagnosticoId`, y/o (c) un registro con
`contentSections.plantilla = 'entrevista_inicial'` sin `diagnosticoId`. UNA
consulta por tabla —tipos (`tiposDeAdopcion`, activos e inactivos), citas
(`citasAdoptablesDe`), registros (`registrosDeEntrevistaDe`), expedientes,
pacientes (con `q` por `filtroPorNombre`: palabras, sin tildes), cobros
(`cobrosDeEntrevistaDe`: concepto de la entrevista o nota ILIKE «entrevista
inicial», `completed`/`pending`, sin devolver) y equipo—, nunca una por
paciente; tope 300 con `truncado`. La fila (`filaDeCandidato`, puro):
`{ paciente: { id, nombre }, clientId, terapeutaSugerido: { id, nombre } |
null (el team_member más repetido en esas citas, empate al de la cita más
reciente), citasDiagnostico: { total, hechas, horasHechas, futuras,
horasReservadas, primera, ultima } | null, entrevista: { bookingId, fecha,
status, sessionId, cobro: { id, importe, status } | null } | null
(`bookingId` null = solo hay registro), expedientesCerrados, resumen: la
frase de la casilla (`resumenDeAdopcion`: «2 citas ya dadas (1,75 h), 1
futura (1,5 h) y la entrevista del 02/09/2026 (cobrada, 50 €)»; «nada que
meter»), adoptar: { citas: true, entrevistaBookingId, meses } }`: la pantalla
no recalcula nada.

En la lista, el bloque plegado `components/clinica/CandidatosDiagnostico.jsx`
(«N pacientes con diagnóstico sin expediente»; buscador al servidor con 300 ms
de retardo y descarte de respuestas tardías; desplegable de ventana 6/12/24/36
meses) enseña paciente · entrevista (fecha, cobrada/pendiente/sin cobro, «solo
el registro») · citas de diagnóstico (n, h dadas, h reservadas) · terapeuta
sugerido y «Abrir expediente», que abre `NuevoDiagnosticoPanel` con el
paciente FIJO (`pacienteFijo`, sin buscador y con «Ver ficha»), el terapeuta
sugerido puesto, el producto a elegir y la casilla marcada «Meter en el
expediente lo que ya hay: …» con esa frase; el 409 «ya tiene uno abierto» pasa
a preguntarse con `useDialogo`. Al crear desde ahí, va a
`/clinica/diagnosticos/<id>`.

**El alta adopta** (`POST /api/clinica/diagnosticos` con `adoptar: { citas:
true|false, entrevistaBookingId: uuid|null, meses }`; `citas: false` con
`entrevistaBookingId` = solo la entrevista; sin nada, el alta de siempre).
El servidor **RECALCULA** las citas adoptables (`citasAdoptables` de
`lib/clinica/adoptarEnDiagnostico.js`: sin `diagnosticoId`, no `cancelled`,
de la ventana; `tramoDeAdopcion` = `entrevista` si el tipo es de entrevista o
si es de diagnóstico y la nota dice «entrevista inicial», `horas` si es de
diagnóstico) y **nunca acepta ids de citas del navegador salvo
`entrevistaBookingId`**, que tiene que ser del paciente, de un tipo de
entrevista o de diagnóstico, sin expediente y no cancelada (422 «Esa cita no
es de este paciente o no es una entrevista inicial que se pueda meter en el
expediente»; sin forma de UUID, 422 «Esa cita de entrevista no es válida»);
sin ella, la entrevista es la ÚLTIMA de tramo entrevista. En la MISMA
transacción que el `create` (`adoptarEnExpediente`, cada UPDATE con
`diagnostico_id IS NULL` y el `patient_id` del expediente: no se roba una
cita a otro expediente ni entra una fila de otro niño): `bookings.diagnostico_id`
+ `diagnostico_tramo` en las citas, `clinic_sessions.diagnostico_id` en los
registros de esas citas y en el último registro `entrevista_inicial` del
paciente sin expediente, y `diagnosticos.entrevista_booking_id` +
`entrevista_payment_id` (el cobro del paciente con el concepto de la
entrevista o nota ILIKE, `completed`/`pending`, sin `refundedAt`, el más
reciente y no usado ya por otro expediente del paciente; SOLO si hay
entrevista que cobrar). La respuesta añade `adoptado: { citas, hechas,
horasHechas, futuras, horasReservadas, entrevista, entrevistaBookingId,
registros, cobroEntrevista, resumen } | null` y `avisos` («No había citas ni
entrevista que meter…», «La entrevista señalada ya era de otro expediente…»,
«…no tiene cobro apuntado: al seguir no se descontará nada…», «Al seguir se
descontarán los N € de la entrevista…»); la auditoría `diagnostico.abierto`
lleva `adoptadas: { citas, entrevista, registros, cobroEntrevistaId }`.

⚠️ **Las citas pasadas en `confirmed`.** Dato real de Aumenta: las citas
pasadas se quedan en `confirmed` (nadie las marca «hecha»), y `gastaSesion`
solo cuenta `completed`, falta injustificada y cancelación tardía; una pasada
en `confirmed` la barra la pinta como RESERVADA, también en los candidatos y
en el expediente recién abierto. No se cambia la regla (es la de los bonos):
**marcar la cita como «hecha» es lo que la mueve de tramo**, de reservada a
hecha, y hasta que alguien lo haga la barra dirá «reservadas» aunque la cita
sea de hace un mes. Se dice aquí y en la guía.

**Auditoría nueva** (sin datos clínicos en `after`):
`diagnostico.informe_creado` («Creó el informe de valoración diagnóstica de un
diagnóstico»; `informeId`, `therapistId`, nº de `registros`, `patientId`) y
`diagnostico.informe_unido` («Unió con la IA los registros de un diagnóstico
en su informe»; `informeId`, nº de `registros`, `apartados`, `nuevos`,
`incidencia`). `diagnostico.abierto` gana `adoptadas`; `.seguido`, `descuento`
y `entrevistaPaymentId`; `.parado`, `yaExistia`. Frases en
`lib/actividad/etiquetas.js`. La ficha del paciente enseña un chip
«Diagnóstico» en las sesiones con `diagnosticoId` que lleva al expediente.

**Permisos**: escribir registros, crear el informe y unirlo lo hace cualquiera
con `clinica` (es clínico, no dinero); parar, seguir y desbloquear siguen con
`puedeDarBonos`.

**Pruebas** (todas `node:test`, ligeras, en `npm test`):
`_smoke-registro-de-diagnostico.mjs` (títulos, cola de la URL en los dos
sentidos, `registrosPorFecha` con la numeración que salta la entrevista, el
material exacto que viaja a la IA y lo que NO viaja),
`_smoke-adoptar-en-diagnostico.mjs` (tipos por columnas, tramo, qué citas
entran y cuáles no, el descuento, la frase de la casilla, el terapeuta
sugerido), `_smoke-diagnostico-db.mjs` (ventana de meses, cobro por columna
frente a nota, las condiciones exactas de `cobrosDeExpedientes`, `adopcionDe`,
la forma de `filaDeCandidato`, `filasDe` con `cobroAlSeguir` 350 − 50),
`_smoke-registros-del-expediente.mjs` (modelos falsos: candado de paciente en
`registrosDe`, `sesionesDeLasCitas`, `informeDe` limpiando la columna, y las
frases de las rutas por texto), y las actualizadas `_smoke-diagnostico.mjs`
(`cobroDelProducto` con descuento), `_smoke-diagnostico-fila.mjs` (los campos
nuevos de la fila y las URLs), `_smoke-cita-de-diagnostico.mjs` (la regla 2
de `plantillaDeLaCita`), `_smoke-clinica-preparar.mjs` (`payloadDePreparacion`
con `diagnosticoId` y `titulo`) y `_smoke-plantilla-entrevista.mjs` (una
plantilla extra más). Nada probado en navegador en esta entrega (`next dev`
prohibido a los agentes): lo primero que mirar con base es la respuesta del
`POST /informe`, `citas[].urls.registro` y `registros[].url`.

## Diagnósticos: la lista y lo visible (12/09/2026, Rodrigo con Isa)

La pantalla `/clinica/diagnosticos` (`app/(dashboard)/clinica/diagnosticos/page.jsx`,
cliente) es la lista de expedientes que sirve `GET /api/clinica/diagnosticos`:
una fila por expediente (`components/clinica/DiagnosticoFila.jsx`) con el
paciente (enlace a su ficha), el producto, el terapeuta asignado (desplegable
que hace `PATCH` al cambiar), la barra de horas con sus cuatro tramos
(`components/clinica/BarraDeHoras.jsx`: entrevista · hechas · reservadas ·
libres, ya calculados por la API) y los botones que trae `acciones` — la
pantalla NO decide qué se puede hacer, solo lo pinta y lo confirma:

- «Abrir entrevista inicial» y «Añadir horas» son ENLACES a la agenda
  (`urls.entrevista` / `urls.horas`, `lib/citas/altaDesdeDiagnostico.js`);
  salen apagados con su motivo si quien mira no tiene `citas` o el centro no
  tiene tipo de cita DIAGNÓSTICO, y «Añadir horas» también cuando la barra
  está llena (`mensajeTope`, la misma frase que daría el 422).
- «Parar» y «Seguir» confirman con el IMPORTE que va a nacer: lo calcula el
  servidor en la misma respuesta (`cobroEntrevista` para parar y, desde la
  segunda entrega, `e.cobroAlSeguir` de CADA fila para seguir —con la
  entrevista ya cobrada descontada—; `productos[].cobro` se queda para el
  panel de alta), para que la confirmación y el cobro apuntado no puedan decir
  cifras distintas. «Parar» dice «la entrevista ya tiene su cobro apuntado»
  cuando `dinero.entrevista` existe, y no anuncia cobro nuevo si el servidor
  contesta `cobro.yaExistia`.
- «Desbloquear horas» pide el tope nuevo (`pedirTexto`) y lo valida con
  `puedeDesbloquear` antes de mandarlo; «Cerrar» avisa de las citas futuras,
  las horas sin dar y el cobro pendiente que no retira.
- «Expediente» abre la ficha del expediente (`urls.expediente`,
  `/clinica/diagnosticos/[id]`: registros por fecha, citas e informe; ver la
  sección anterior) e «Informe» solo sale cuando el informe ya existe
  (`urls.informe`). Los botones viven en `BotonesDeDiagnostico` (export de
  `DiagnosticoFila.jsx`) y las confirmaciones en
  `components/clinica/useAccionesDeDiagnostico.js`: la lista y la ficha usan
  los MISMOS.

Filtros: En curso / Cerrados / Todos (`?estado=`), «Mis pacientes» (`?mios=1`,
con aviso si no se tiene ficha de equipo) y un buscador en cliente por
paciente, producto o terapeuta. «Nuevo diagnóstico» es un panel lateral
(`components/clinica/NuevoDiagnosticoPanel.jsx`, regla #13) con
`SelectorPaciente`, producto y terapeuta; el 409 «ya tiene uno abierto» se
contesta preguntando si se abre otro (`permitirOtro`, por `useDialogo`).
Debajo de la tabla, plegado, el bloque **«Empezar desde lo que ya hay»**
(`components/clinica/CandidatosDiagnostico.jsx`, solo si no `sinMigrar`): los
pacientes con citas de diagnóstico o entrevista inicial hecha y sin expediente,
con buscador y ventana de meses, y «Abrir expediente», que abre el mismo panel
con el paciente fijo, el terapeuta sugerido y la casilla de meter dentro lo
que ya hay (detalle en la sección anterior). Menú: hijo «Diagnósticos» del
grupo Clínica en `components/layout/Sidebar.jsx`.

**Los productos se editan en Configuración → Módulos → «Productos de
diagnóstico»** (`DiagnosticosCard` en `modules/config/tarjetas/Modulos.jsx`,
clave `diagnosticos` en `lib/configuracion/pestanas.js`, solo admin): nombre,
horas, concepto del catálogo y precio de caída de cada producto, con «Otro
producto» y «Volver a los de fábrica». `GET/PATCH /api/tenant/settings` los
devuelven en `diagnosticos` (con caída a los de fábrica) y el PATCH guarda
`settings.clinica.diagnosticos` por `productosParaGuardar`
(`lib/clinica/diagnosticosAjustes.js`, prueba `_smoke-diagnosticos-ajustes.mjs`):
lista vacía = quitar lo guardado; sin nada válido = 400 con la frase. El
cambio se audita resumido («simple: 10 h · 350 € · completo: 20 h · 650 €»).
Los expedientes ya abiertos no cambian: llevan copiados nombre y horas.

En `/facturacion/bonos`, un bono con `total` a null (el del diagnóstico) se
...
## El diagnóstico se cobra por fases, y la cita de la agenda cuenta sus horas (18/09/2026, Rodrigo)

Rodrigo, por Aumenta: «hay que arreglar una cosa de Diagnósticos: poder
facturar por fases el diagnóstico (50 € EI y 600 o 300 del diagnóstico) y
poder hacerlo desde la pestaña de diagnóstico, mandarlo a cobro y generar un
cobro desde ahí. Y poder unirlo a las citas correctamente: cuando hago una
cita de diagnóstico de una persona debería poderle poner el tiempo que
quisiera y que ese tiempo se descontara del que le queda».

Dos cosas distintas con la misma raíz: **el expediente solo sabía del dinero y
de las horas cuando se entraba por sus botones**.

### 1. Las dos fases del cobro
...
entero (350 € / 650 €). Es decir: la familia que sigue —la mayoría— nunca
pagaba la entrevista aparte, y no había forma de pedirle los 50 € el día que
vino.

Ahora el expediente (`/clinica/diagnosticos/[id]`) tiene un bloque **«Cobro
por fases»** con las dos, cada una con su importe, en qué anda su cobro y el
botón **«Mandar a cobro»**:

| Fase | Cuánto | Cuándo se puede |
| --- | --- | --- |
| Entrevista inicial | el concepto «Entrevista Inicial» del centro, o 50 € | en `entrevista`, `en_curso` y `no_continua`, mientras no tenga cobro |
| El producto | el precio del producto **menos la entrevista** (300 € / 600 €) | en `en_curso` (o sea: con su bono), mientras no tenga cobro |

`POST /api/clinica/diagnosticos/[id]/cobrar` con `{ fase: "entrevista" |
"producto" }`, gate `clinica` + `puedeDarBonos` (la misma regla que parar,
seguir y desbloquear: apuntar una deuda es dar un bono). Auditado como
`diagnostico.cobrado`.

- **La entrevista no cambia el estado.** Cobrarla no es decidir si la familia
  sigue —que es lo que hacía «Parar», la única forma que había de apuntarla—.
  El cobro queda atado en `diagnosticos.entrevista_payment_id`, que es
  exactamente lo que hace que el producto valga después 300/600: el descuento
  es el de la segunda entrega (`cobroDelProducto` +
  `descuentoDeLaEntrevista`), no uno nuevo.
- **El producto sale por la misma puerta que al «Seguir»**
  (`cobroPendienteDeBono`, con `pack_id`, el concepto y el «Texto en la
  factura»): se llama y se factura igual. Este camino es para el expediente
  que no lo tiene —los de antes del submódulo, los abiertos sin Facturación, o
  aquel al que le borraron el cobro—; en el flujo normal sigue naciendo solo
  al seguir.
- **Nunca nacen dos.** Una fase con cobro —cobrado o pendiente— ya no se
  ofrece, y el endpoint devuelve 409 con el que hay. Quién puede y por qué no
  lo decide `fasesDeCobro` (`lib/clinica/cobrosDelDiagnostico.js`, puro), la
  MISMA función con la que la pantalla pinta o apaga el botón: el motivo que
  se lee apagado es el que contestaría la API.
- **El orden importa, y se dice antes.** Si se pulsa «Seguir» sin haber
  mandado a cobro la entrevista, el producto nace entero (350/650, con la
  entrevista dentro), que es lo correcto; la confirmación lo avisa
  (`avisoDeOrden`, en la fila como `avisoAlSeguir`). Y al revés —cobrar la
  entrevista DESPUÉS de seguir— no se toca el cobro ya apuntado: nace la fila
  de 50 € y la respuesta avisa de que hay dos en Cobros y que la del producto
  hay que ajustarla a mano.

La fila del expediente gana `cobros: { entrevista, producto }` (cada una
`{ clave, titulo, importe, estadoCobro, rotuloCobro, cobroId, puedeGenerar,
motivo }`), `avisoAlSeguir` y `dinero.producto`. Prueba:
`_smoke-diagnostico-cobros.mjs`.

### 2. La cita de diagnóstico apuntada desde la agenda

La duración libre que descuenta de la barra existía desde el 12/09… pero solo
entrando por «Añadir horas» del expediente. La misma cita apuntada desde la
agenda —con el tipo DIAGNÓSTICO elegido a mano y el paciente puesto— nacía
suelta: sin `diagnostico_id`, sin descontar horas y sin que nadie lo dijera.

Ahora, en el cajón de alta de citas, **en cuanto el tipo elegido es de
diagnóstico y el paciente tiene expediente EN CURSO, la cita se engancha
sola**:

- el tipo se reconoce por `event_types.informe_tipo = 'diagnostico'`
  (`esTipoDeHorasDeDiagnostico`), no por el nombre: en Aumenta también lo es
  «INFORME PARA DIAGNOSTICO», de 45 min;
- sale el selector de **Duración** (15 min → 4 h) con la del tipo puesta, y
  debajo cuántas horas cuenta contra el diagnóstico;
- sale una franja con el expediente, el producto y su barra («3 de 20 h · 2
  reservadas»), y una **casilla para desengancharla**: hay citas de ese tipo
  que no son del expediente, y el centro tiene que poder decirlo;
- enganchada, la cita se comporta como las de «Añadir horas»: sin bloque de
  cobro ni bono a mano (el dinero lo pone el servidor con el bono sin tope del
  expediente) y sin repetición (las horas se apuntan de una en una);
- si el paciente tiene el expediente en «Entrevista inicial» (todavía sin
  bono), no se engancha nada y la franja dice por qué: hay que pulsar antes
  «Seguir con el diagnóstico». Si no le quedan horas, lo dice y manda a
  desbloquearlas.

Una consulta al elegir (`GET /api/clinica/diagnosticos?paciente=&estado=en_curso`)
y solo cuando el tipo es de diagnóstico: la agenda no pregunta por
diagnósticos en las otras 11.000 citas. Sin módulo de Clínica o sin permiso no
pasa nada: la cita es la de siempre.

**Los minutos van ahora de cuarto en cuarto** (`DURACION_PASO_MIN = 15`, antes
30). Las de 45 min —que son un tipo de verdad— se rechazaban con «minutos en
múltiplos de 30» al apuntarlas contra un expediente. El TOPE del expediente
sigue subiéndose de media en media (`puedeDesbloquear`): una cosa es lo que
dura una sesión y otra cuántas horas se contratan.

**Sin migración**: las dos fases viven en columnas que ya existían
(`diagnosticos.entrevista_payment_id` y `payments.pack_id`).
