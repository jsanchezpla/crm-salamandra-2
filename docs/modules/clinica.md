...
plantilla, el catálogo, la limpieza y el PDF generado de verdad).
...
2. `POST /api/clinica/diagnosticos/[id]/unir` (gate `clinica`;
   ...
   sin nombre y la clave/modelo del tenant. Devuelve `{ propuesta, nuevos,
   bloques, material, registros: <nº unidos>, incidencia:
   null|"cortada"|"ilegible"|"vacia", avisoIA? }` **SIN guardar nada del
   ...
   de `MAX_TRANSCRIPCION`; 503 sin clave de IA (la del proveedor que tenga
