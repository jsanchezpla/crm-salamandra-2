...
**Lo que queda para la segunda entrega**: los **registros de diagnóstico por
fecha** (la columna `clinic_sessions.diagnostico_id` ya existe y nadie la
escribe aún: hoy la entrevista y las sesiones se registran como registros de
sesión de siempre desde su cita), **«Unir en informe»** (`informe_id` existe;
el botón de la lista lleva de momento a la ficha con el informe de valoración
diagnóstica preparado), `entrevista_payment_id` para no reconocer el cobro de
la entrevista por su nota, y —si alguien lo pide— volver de `no_continua` a
`en_curso` con una regla para el cobro ya nacido.
...
- «Parar» y «Seguir» confirman con el IMPORTE que va a nacer: lo calcula el
  servidor en la misma respuesta (`cobroEntrevista` y `productos[].cobro`,
  con `cobroDeLaEntrevista` / `cobroDelProducto`), para que la confirmación y
  el cobro apuntado no puedan decir cifras distintas.
...
contesta preguntando si se abre otro (`permitirOtro`). Menú: hijo
«Diagnósticos» del grupo Clínica en `components/layout/Sidebar.jsx`.
