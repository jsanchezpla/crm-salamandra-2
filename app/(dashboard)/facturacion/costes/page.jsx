    ...
    <div className={anchoPantalla("listado")}>
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
        <div>
          ...
          <h1 className="font-display text-2xl text-[var(--ink-900)] mt-1">Gastos</h1>
        ...
        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
          ...
          {puedeFacturar && (
            <button onClick={openCreate} className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide text-white"
              style={{ background: "var(--color-primary, #1B3A2D)" }}>+ Nuevo gasto</button>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <input type="date" value={filterFrom} onChange={(e) => setFilterFrom(e.target.value)} title="Gasto desde" aria-label="Gasto desde" className="rounded-lg px-3 py-1.5 text-xs text-neutral-700 bg-white border border-neutral-200 focus:outline-none focus:border-neutral-400" />
        <input type="date" value={filterTo} onChange={(e) => setFilterTo(e.target.value)} title="Gasto hasta" aria-label="Gasto hasta" className="rounded-lg px-3 py-1.5 text-xs text-neutral-700 bg-white border border-neutral-200 focus:outline-none focus:border-neutral-400" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[940px]">
            ...
            <tbody>
              ...
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={puedeFacturar ? 10 : 9} className="text-center py-12 text-xs text-neutral-400">{search || hayFiltros ? "Sin gastos que coincidan con los filtros" : "Sin gastos registrados"}</td></tr>
              )}
              {filtered.map((c) => (
          ...
          <aside className="fixed top-14 lg:top-0 right-0 bottom-0 w-full sm:w-[480px] bg-white z-50 shadow-pop overflow-y-auto ink-scroll slide-right">
            <div className="px-6 pt-6 pb-4 border-b border-neutral-100 flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="font-display text-xl text-neutral-900 mt-1">{editingId ? "Editar gasto" : "Nuevo gasto"}</h2>
              </div>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-3">
              ...
              <div className="flex gap-2 justify-end pt-3 border-t border-neutral-100">
                ...
                <button type="submit" disabled={saving}
                  ...
                  style={{ background: "var(--color-primary, #1B3A2D)" }}>{saving ? "Guardando..." : (editingId ? "Guardar cambios" : "Crear gasto")}</button>
              </div>
            </form>
          </aside>
        </>
