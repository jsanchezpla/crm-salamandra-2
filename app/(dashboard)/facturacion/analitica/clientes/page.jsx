...
export default function AnaliticaClientesPage() {
  ...
  // casar a caballo entre dos campos y marcar a quien no era.
    return rows.filter((r) => coincidePorNombre(search, [r.clientName, r.taxId]));
  }, [rows, search]);
    ...
    <div className={`${anchoPantalla("listado")} space-y-5`}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-400 mt-1">{fmtDate(from)} → {fmtDate(to)} · {rows.length} clientes</p>
        </div>
