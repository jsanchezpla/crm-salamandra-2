...
function formatMonthLabels(byMonth) {
  ...
  return byMonth.map((m, i) => {
    ...
    if (i === 0 || (prevYear && y !== prevYear)) return `${name} ${String(y).slice(2)}`;
    return name;
  });
}
...
export default function FacturacionResumen() {
  ...
  const load = useCallback(async () => {
    ...
    setErrorMsg(null);
    try {
      const res = await fetch(`/api/billing/analytics?from=${from}&to=${to}`, { cache: "no-store" });
      ...
      {data && (
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-3">
          <div className="flex flex-col gap-3">
            <div className="rounded-xl p-4 lg:p-5 flex flex-col gap-3 min-h-[220px]" style={{ background: "var(--color-primary, #1B3A2D)" }}>
          ...
          <div className="bg-white border border-neutral-100 rounded-xl p-4 lg:p-5 flex flex-col">
            ...
            <div className="flex-1 space-y-2">
              {[
                { label: "Variables", value: costs.byCategory.variable, color: "bg-amber-400" },
                { label: "Fijos", value: costs.byCategory.fixed, color: "bg-neutral-400" },
                { label: "OPEX", value: costs.byCategory.opex, color: "bg-sky-400" },
              ].map((row) => {
