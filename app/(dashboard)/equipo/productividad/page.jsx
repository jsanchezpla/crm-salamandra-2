      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="bg-white border border-neutral-100 rounded-xl p-4">
          ...
          <div className="text-[11px] text-neutral-500 mt-0.5">
            Citas atendidas del mes
            {!loading && totals.totalInternalHours != null && (
              <span className="text-neutral-400"> · internas {totals.totalInternalHours} h</span>
            )}
          </div>
        </div>
        <div className="bg-white border border-neutral-100 rounded-xl p-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-neutral-50/50">
              <tr className="text-left text-[10px] uppercase tracking-wider text-neutral-400">
                ...
                <th className="px-4 py-2 font-medium tabular text-right">
                  Internas
                  <HelpTooltip title="Trabajo interno" className="ml-1">
                    Los bloqueos «Reservado T.I.» y «Reunión equipo» de su agenda (en cualquiera
                    de sus grafías), más las valoraciones a pacientes que no tiene asignados.
                  </HelpTooltip>
                </th>
                <th className="px-4 py-2 font-medium text-right">Bono · Taller · Normal</th>
            ...
            <tbody>
              ...
              {!loading && rows.length === 0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-neutral-400">Sin profesionales activos.</td></tr>}
              {!loading && rows.map((r) => {
                ...
                return (
                  <tr key={r.therapistId} className="border-t border-neutral-100 hover:bg-neutral-50/50">
                    ...
                    <td className="px-4 py-3 text-right tabular text-neutral-700">
                      {r.internalHours != null ? (
                        <span title={`T.I. ${r.tiHours} h · equipo ${r.equipoHours} h · valoraciones sin asignar ${r.valoracionesInternasHours} h`}>
                          {r.internalHours} h
                          {(r.tiHours > 0 || r.equipoHours > 0) && (
                            <span className="block text-[10px] text-neutral-400">T.I. {r.tiHours} · eq. {r.equipoHours}</span>
                          )}
                        </span>
                      ) : "—"}
                    </td>
                    <td className="px-4 py-3 text-right tabular text-neutral-500">
                      {r.desglose ? `${r.desglose.bono} · ${r.desglose.taller} · ${r.desglose.normal}` : "—"}
                    </td>
