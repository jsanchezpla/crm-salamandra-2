...
/**
 * Borrado físico de una cita. La regla (que el dinero frena el borrado) y el
 * barrido de lo que cuelga de ella viven en `lib/citas/borrarCita.js` desde el
 * 12/09/2026: se borra también EN BLOQUE desde `.../[id]/siguientes`, y eso
 * escrito dos veces acaba con una de las dos copias olvidándose del dinero.
 *
 * Aquí queda lo que es de esta ruta: traducirlo a HTTP y dejar la línea de
 * auditoría, que es el único rastro que sobrevive a la cita.
 */
async function borrarDeVerdad({ ctx, row, userId, ip }) {
  const { tenant, tenantModels } = ctx;
  const res = await borrarCitaDeVerdad({ tenantModels, row });
  if (!res.ok) {
    return error(
      `Esta cita ${res.freno}: no se puede borrar, porque el registro del dinero tiene que quedar. Puedes cancelarla.`,
      409
    );
  }
  ...
  await logCitasAudit({
    ...
    entityId: row.id,
    before: res.huella,
    after: { borrada: true, ...res.colgantes },
    ip,
  });
