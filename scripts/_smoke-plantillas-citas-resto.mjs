 ...
 *
 * Las otras 9 son TOLERANCIA, y el porqué de cada una está junto a su `it`.
 * En una frase, por qué ninguna tiene camino de entrada:
 *    320  la duración solo sale en el html, no en el texto plano
 *         → La entrada es la normal (duration siempre viene), pero lo que…
 *    330  sin nombre saluda «Hola Hola,» (bookingMeetLink)
 *         → El resultado sí es feo de verdad, pero la entrada que lo prod…
 *    338  un nombre con espacios de sobra sale sin recortar
 *         → Mismo motivo que el saludo vacío: el nombre llega ya recortad…
 *    443  en modalidad teléfono, «Te llamaremos» solo sale en e…
 *         → La entrada es real (modality 'phone' es un valor del ENUM), p…
 *    451  sin nombre saluda «Hola Hola,» (bookingReminder)
 *         → Idéntico al de bookingMeetLink y con el mismo origen de dato:…
 *    457  se despide con el nombre del centro a pelo, sin la ra…
 *         → Es una incoherencia de estilo entre dos plantillas de la mism…
 *    568  importe null se imprime como 0,00 €
 *         → El resultado sería grave de verdad («se reservan 0,00 € en tu…
 *    573  sin importe imprime «NaN €»
 *         → Doblemente inalcanzable: el único que llama pasa siempre la c…
 *    748  sin enlace, «Entra en la web» solo sale en el html
 *         → La entrada sí es real (hay tenants con formularios y sin URL …
 */
