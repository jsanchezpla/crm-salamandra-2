...
export const ONE_OFF = {
  ...
  "migrate-ai-uso": "MASTER, no toca schemas de tenant: crea `master.ai_uso`, la contabilidad de la IA (una fila por llamada a Claude o Whisper con tokens y coste estimado, 11/09/2026). Se corre a mano, idempotente y aditiva; el código escribe best-effort. Desde el 14/09/2026 además AÑADE `error` (la causa de una llamada fallida, `causaDelFallo`) y `error_http` a la tabla existente: las llamadas que fallan dejan fila a coste 0. Nacen NULL, sin backfill ni UPDATE. VA ANTES del despliegue (por stdin, ver su cabecera): `/api/tenant/ia/consumo` filtra por `error`",
  ...
  "migrate-calendario-global": "MASTER, no toca schemas de tenant: crea `master.calendario_global_vinculos`, la tabla que dice qué calendarios de cliente ve cada cuenta de Salamandra desde calendar.salamandrasolutions.com (03/09/2026). Solo tabla e índice, no escribe filas: los vínculos se ponen con `scripts/calendario-global-vincular.js` o desde /admin/calendario. Se corre a mano con `npm run db:migrate:calendario-global`; idempotente, y VA ANTES del despliegue: el modelo pide la tabla por nombre",
