...
export default function MiEquipo({ modulos = null }) {
  ...
  return (
    <div className={anchoPantalla("listado")}>
      ...
      {/* Accesos ARRIBA (15/09/2026, AV-0078 de Aumenta): estaban debajo de la
          documentación y Daniela, coordinadora, entraba en Equipo y decía «solo
          me sale mi perfil». Lo del día a día va primero; el porqué de cuándo
          salen, justo encima del `return`. */}
      {verAccesosDeEquipoAvanzado && <AccesosDelDia />}
...
/**
 * Las dos tarjetas del día a día, con lo que hay dentro dicho en la propia
 * tarjeta (15/09/2026, AV-0078): «Bandeja de trabajo» a secas no decía que ahí
 * están los registros sin escribir de la semana, y quien coordina no sabía que
 * desde ahí ve al equipo entero. Los números salen del MISMO endpoint que la
 * Bandeja, así que no pueden discrepar; si falla, la tarjeta sale sin números.
 */
function AccesosDelDia() {
  const [bandeja, setBandeja] = useState(null);
  useEffect(() => {
    let vivo = true;
    fetch("/api/clinica/bandeja", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => { if (vivo && j?.ok) setBandeja(j.data); })
      .catch(() => {});
    return () => { vivo = false; };
  }, []);

  const c = bandeja?.counts;
  const registros = c ? (c.registrosSinEmpezar ?? 0) + (c.registrosAMedias ?? 0) : null;
  const resumen = c
    ? [
    ...
    : "Registros sin escribir de la semana, informes, incidencias y citas de hoy.";
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
      <Link href="/equipo/bandeja" className="group bg-white border border-neutral-100 rounded-xl p-5 hover:border-[var(--color-primary,#1B3A2D)] hover:shadow-sm transition-all">
        <div className="flex items-start justify-between gap-3">
          ...
          {registros > 0 && (
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 tabular-nums">{registros}</span>
          )}
        </div>
        <div className="font-display text-base text-neutral-900">Mi bandeja de trabajo</div>
        <div className="text-xs text-neutral-500 mt-1 leading-relaxed">{resumen}</div>
        {bandeja?.coordina && (
          <div className="text-[11px] font-medium mt-2" style={{ color: "var(--color-primary,#1B3A2D)" }}>
            Como coordinadora, desde aquí ves también la bandeja de cada compañera y la del equipo entero →
          </div>
        )}
      </Link>
